#!/usr/bin/env node
/**
 * Verify pipeline_leads backfill; run backfill when counts diverge.
 *
 * Usage:
 *   npm run pipeline:sync
 *   npm run pipeline:sync -- --org=ORG_ID
 *   npm run pipeline:sync -- --dry-run
 */
...
if (!isSupabaseEnabled()) {
  console.error('Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY before running pipeline sync.')
  process.exit(1)
}

console.log('Connect Intel — pipeline_leads verify + backfill\n')

const verifyBefore = await verifyPipelineLeadsBackfill({ orgId: orgArg || null })
...
const backfill = orgArg
  ? await backfillOrganization(orgArg, { batchSize: 50 })
  : await backfillAllPipelineShards({ batchSize: 50 })

console.log('\nBackfill:', backfill)

const verifyAfter = await verifyPipelineLeadsBackfill({ orgId: orgArg || null })

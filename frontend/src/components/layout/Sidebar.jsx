...
export default function Sidebar({ active, panelOptions, onNavigate, mobileOpen, onMobileClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:static z-50 md:z-auto w-[260px] md:w-[240px] shrink-0 h-full bg-white border-r border-gray-200 flex flex-col transition-transform duration-200 ${
...
function NavGroup({ group, icons, expanded, onToggle, isTargetActive, onGo, resolveBadge, muted = false }) {
  const Icon = icons[group.icon] || HomeIcon
  const hasChildren = group.children?.length > 0
  const groupActive =
    hasChildren && group.children.some((child) => isTargetActive(child))
  ...
  return (
    <div className="mb-0.5">
      ...
      >
        ...
        <ChevronIcon className={`w-3.5 h-3.5 shrink-0 transition-transform ${expanded ? 'rotate-90' : ''}`} />
      </button>
      {expanded && (

...
describe('leadLastOrder', () => {
  ...
  it('clears crm_payload.erp_tags when ERP sends an empty array', () => {
    const payload = buildCrmPayload({
      crm: { tagIds: ['crm-only'], status: 'new' },
      crm_payload: {
        lastOrderCreatedAt: '2026-01-01T00:00:00.000Z',
        erp_tags: [],
      },
    })
    assert.deepEqual(payload.tagIds, ['crm-only'])
    assert.deepEqual(payload.erp_tags, [])
  })

  it('preserves erp_tags in crm_payload without touching tagIds', () => {
    const payload = buildCrmPayload({
      crm: { tagIds: ['crm-only'], status: 'new' },
      crm_payload: {

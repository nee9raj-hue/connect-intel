...
describe('freight volumetric weight', () => {
  ...
  it('estimated revenue uses higher of gross and volumetric times freight rate', () => {
    const deal = {
      amount: 10,
      freight: {
        transportMode: 'air',
    ...
    assert.equal(estimatedFreightRevenue(deal), 120)
    assert.equal(
      sumEstimatedFreightRevenue([{ deal }, { deal: { amount: 5, freight: { transportMode: 'air', grossWeightKg: 10 } } }]),
      170
    )
  })

  it('estimated revenue uses CBM for ocean when volume is higher than declared', () => {
    const deal = {
      amount: 200,
      freight: {
        transportMode: 'ocean',
    ...
    assert.equal(estimatedFreightRevenue(deal), 200)
    assert.equal(estimatedFreightRevenueInr(deal, 88), 17600)
  })

  it('keeps air revenue in INR', () => {
    const deal = {
      amount: 10,
      freight: { transportMode: 'air', grossWeightKg: 8 },
    }
    assert.equal(estimatedFreightRevenueInr(deal, 88), 80)
  })

...
describe('xindusCustomerErp', () => {
  ...
  it('reads last shipment from camelCase fields and day-month timestamps', () => {
    const overlay = erpFromXindusCustomerRow({
      Company: 'West Express',
      lastShipmentDate: '22-07-2026 10:55:43',
      shipmentCount: 10,
    })
    assert.equal(overlay.revenue.lastShipmentDate, '2026-07-22')
    const named = erpFromXindusCustomerRow({
      Company: 'SPR TRADERS',
      'Last Transacted Date': '22 Jul 2025',
    })
    assert.equal(named.revenue.lastShipmentDate, '2025-07-22')
  })

  it('maps Last Transacted without the Date suffix', () => {
    const overlay = erpFromXindusCustomerRow({
      Company: 'XLP ENGINEERS PVT LTD',
      'Shipment Count': 10,
      'Last Transacted': '2026-07-11T06:26:03',
      Gstn: '06AAACX0076C1ZR',
      'Crm ID': '426904000018309832',
    })
    assert.equal(overlay.revenue.lastShipmentDate, '2026-07-11')
    assert.equal(overlay.revenue.shipmentCount, 10)
    const keys = matchKeysFromXindusRow({
      Gstn: '06AAACX0076C1ZR',
      'Crm ID': '426904000018309832',
      ID: 1443,
    })
    assert.equal(keys.gst, '06AAACX0076C1ZR')
    assert.equal(keys.crmId, '426904000018309832')
    assert.equal(keys.xindusId, '1443')
  })
  ...
  it('maps ops-list JSON sales owner and ignores empty KAM', () => {
    const overlay = erpFromXindusCustomerRow({
      Company: 'Chuninda',
      'Sales Owner': '{"id":0,"name":"Lokesh Joshi","email":"lokesh.joshi@xindus.net"}',
      'Kam Info': null,
      Tags: '[{"id":18,"tagName":"Churn","type":"automatic"}]',
    })
    assert.equal(overlay.ownership.salesOwner.email, 'lokesh.joshi@xindus.net')
    assert.equal(overlay.ownership.accountOwner, null)
    assert.equal(overlay.revenue.tags[0].tagName, 'Churn')
  })

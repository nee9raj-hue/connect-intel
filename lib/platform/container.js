...
import { createLeadTagRepository } from './repositories/leadTag.js'
...
export function createPlatform(overrides = {}) {
  ...
  return {
    ...
    repositories: {
      ...
      leadTags: createLeadTagRepository(),
    },
    async health() {
      const [dbOk, searchOk, storageOk] = await Promise.all([
        database.ping().catch(() => false),
        search.ping().catch(() => ({ ok: false })),
        typeof storage.ping === 'function'
          ? storage.ping().catch((error) => ({ ok: false, error: error?.message || String(error) }))
          : Promise.resolve({ ok: true, provider: storage.provider }),
      ])
      const { resolveStoreBackend } = await import('../server/storeBackend.js')
      return {
        ok: Boolean(dbOk),
        ...
        storage: { ...storageOk, provider: storage.provider },

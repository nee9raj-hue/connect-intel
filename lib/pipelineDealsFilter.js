...
/** Exclusive-end windows for Year → Month → Week Number. Multiple weeks stay disjoint. */
export function dealPeriodWindows({ year, month, weeks } = {}, timeZone = DEFAULT_TIME_ZONE) {
  const tz = resolveTimeZone({}, timeZone)
  const y = Number(year)
  if (!y) return []
...
export function formatDealPeriodLabel({ year, month, weeks } = {}) {
  const y = String(year || '').trim()
  if (!y) return ''
  const monthOpt = DEAL_MONTH_OPTIONS.find((opt) => opt.value === String(month || ''))
  const weekIds = (weeks || []).map(String).filter(Boolean)
  const weekLabel = weekIds.map((w) => `week${w}`).join(', ')
  if (!monthOpt) return y
  if (!weekLabel) return `${monthOpt.label} ${y}`
  return `${monthOpt.label} ${y} · ${weekLabel}`
}
...
export function filterPipelineDealRows(
  ...
  if (customerType) {
    out = out.filter((row) => dealMatchesCustomerType(row.deal?.freight, customerType))
  }
  return out
}

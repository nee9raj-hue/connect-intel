...
/** Estimated revenue = freight rate (per kg / CBM) × chargeable measure (higher of gross vs volumetric). */
...
export function sumEstimatedFreightRevenue(dealRows = []) {
  return (dealRows || []).reduce((sum, row) => {
    const n = estimatedFreightRevenue(row?.deal || row)
    return sum + (n || 0)
  }, 0)
}

...
const CRM_STAGE_FILTER_IDS = new Set(FREIGHT_CRM_PIPELINE_STAGE_DEFS.map((s) => s.id))

export function freightBoardColumnIds({ pipelineTrack, status } = {}) {
  const track = normalizePipelineTrack(pipelineTrack)
  const statusId = String(status || '')
    .trim()
    .toLowerCase()
  if (track === 'erp' || statusId.startsWith('erp_')) {
    return FREIGHT_ERP_PIPELINE_STAGE_DEFS.map((s) => s.id)
  }
  if (track === 'crm' || CRM_STAGE_FILTER_IDS.has(statusId)) {
    return FREIGHT_CRM_PIPELINE_STAGE_DEFS.map((s) => s.id)
  }
  return [
    ...FREIGHT_CRM_PIPELINE_STAGE_DEFS.map((s) => s.id),
    ...FREIGHT_ERP_PIPELINE_STAGE_DEFS.map((s) => s.id),
  ]
}

export function boardBucketForLeadStatus(status, columnIds) {
  const ids = Array.isArray(columnIds) && columnIds.length ? columnIds : freightBoardColumnIds()
  const raw = String(status || '')
    .trim()
    .toLowerCase()
  if (ids.includes(raw)) return raw
  for (const [id, aliases] of Object.entries(CRM_PIPELINE_FILTER_ALIASES)) {
    if (ids.includes(id) && (id === raw || aliases.includes(raw))) return id
  }
  if (raw === 'new' && ids.includes('fresh')) return 'fresh'
  const erp = mapStoredStatusToErpFilter(raw)
  if (erp && ids.includes(erp)) return erp
  const normalized = normalizeCrmLeadStatus(raw)
  if (ids.includes(normalized)) return normalized
  const erpFromNorm = mapStoredStatusToErpFilter(normalized)
  if (erpFromNorm && ids.includes(erpFromNorm)) return erpFromNorm
  return ids[0]
}

export function collectCrmStageFilterIds(filters = {}) {
  const values = []
  if (Array.isArray(filters.crmStageIds)) values.push(...filters.crmStageIds)
  if (Array.isArray(filters.crmStages)) values.push(...filters.crmStages)
  for (const extra of [filters.crmStage, ...(filters.crmStageId ? [filters.crmStageId] : [])]) {
    const csv = String(extra || '').trim()
  ...
  return [
    ...new Set(
      values
        ...
        .filter((id) => CRM_STAGE_FILTER_IDS.has(id))
    ),
  ]
}

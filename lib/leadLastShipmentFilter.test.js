...
test('last shipment accepts multiple months without filling the gap', () => {
  const filters = { lastShipmentYear: '2026', lastShipmentMonths: [1, 9] }
  assert.equal(leadMatchesLastShipmentPeriod(septLead, filters), true)
  assert.equal(leadMatchesLastShipmentPeriod(janLead, filters), true)
  const febLead = { crm: { lastOrderCreatedAt: '2026-02-10T06:30:00.000Z' } }
  assert.equal(leadMatchesLastShipmentPeriod(febLead, filters), false)
  assert.equal(lastShipmentPeriodLabel(filters), 'Last shipment: January, September 2026')
  const parts = appendLastShipmentSqlParts([], filters)
  assert.equal(parts.length, 1)
  assert.ok(parts[0].startsWith('or=('))
  assert.ok(parts[0].includes('and('))
})

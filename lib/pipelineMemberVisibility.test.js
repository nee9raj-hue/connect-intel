...
describe('pipelineMemberVisibility', () => {
  ...
  it('turns a team tag in More filters into that team book for Dakash and Vivek', () => {
    ...
    assert.deepEqual(consumeTeamScopedLeadTags(['tag-b2b'], [ocean, adminTag, b2b], []).teamIds, ['team-b2b'])
    assert.deepEqual(
      consumeTeamScopedLeadTags(
        ['tag-b2b'],
        [{ id: 'tag-b2b', name: 'Non Large B2B', teamId: 'stale-id', source: 'org_team' }],
        ['team-b2b'],
        { teams: [{ id: 'team-b2b', name: 'Non Large B2B' }] }
      ).teamIds,
      ['team-b2b']
    )
    assert.deepEqual(
      displayTagIdsForFilters({ tagIds: [], teamIds: ['team-b2b'] }, [ocean, adminTag, b2b]),
      ['tag-b2b']
    )
  })
  ...
  it('lifts assigned-only scope when a team or tag filter is applied', () => {
    ...
    assert.equal(repFilterLiftsOwnerScope({ tagIds: ['tag-ocean'] }, [ocean]), true)
  })

  it('keeps personal tags private to the owner and does not lift team book', () => {
    const mine = { id: 'tag-k', name: 'Follow up', source: 'personal', createdByUserId: 'kuldeep' }
    const other = { id: 'tag-v', name: 'Follow up', source: 'personal', createdByUserId: 'vivek' }
    assert.equal(isOrgWideLeadTag(mine), false)
    assert.equal(memberCanUseLeadTag(mine, [], { actorIds: ['kuldeep'] }), true)
    assert.equal(memberCanUseLeadTag(other, [], { actorIds: ['kuldeep'] }), false)
    assert.equal(memberCanUseLeadTag(other, [], { isOrgAdmin: true, actorIds: ['admin'] }), true)
    assert.deepEqual(
      allowedTagIdsForMember(['tag-k', 'tag-v', 'tag-vip'], [mine, other, adminTag], [], {
        actorIds: ['kuldeep'],
      }),
      ['tag-k', 'tag-vip']
    )
    assert.equal(repFilterLiftsOwnerScope({ tagIds: ['tag-k'] }, [mine]), false)
    assert.deepEqual(consumeTeamScopedLeadTags(['tag-k'], [mine], ['team-b2b']).tagIds, ['tag-k'])
  })

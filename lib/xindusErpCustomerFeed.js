...
function compactOverlay(overlay) {
  ...
  return {
    ...
    ...(Array.isArray(overlay?.erpTags) ? { erpTags: overlay.erpTags } : {}),
  }
}
...
export function getXindusErpFeedConfig() {
  ...
  const path = String(process.env.XINDUS_ERP_CUSTOMERS_PATH || '/api/customer/all/list').trim() || '/api/customer/all/list'
...
function kamFromRow(row = {}) {
  return row.kam_info || row.kamInfo || row.kam || null
}
...
export function flattenXindusApiCustomer(row = {}) {
  ...
  return {
    ...
    'Last transacted date':
      row.last_transacted_date || row.lastTransactedDate || row.last_shipment_at || row.lastShipmentAt,
    'Created On': row.customer_created_at || row.customerCreatedAt || row.created_on || row.createdOn,
    'Sales Owner': owner,
    'Kam Info': kam,
    'Service Node': row.service_node || row.serviceNode,
    Status: row.status,
    'Customer Type': row.customer_type || row.customerType,
    ...
    erp_tags: row.erp_tags || row.erpTags,
  }
}
...
export function overlayFromXindusApiCustomer(row = {}) {
  ...
  const erpTagRaw = row.erp_tags ?? row.erpTags ?? row.erp?.erpTags ?? flat.erp_tags
  if (erpTagRaw !== undefined) erp.erpTags = normalizeErpTags(erpTagRaw)
...
function extractCustomerRows(payload) {
  ...
  const rows = Array.isArray(nested)
    ? nested
    : nested && typeof nested === 'object' && Array.isArray(nested.data)
      ? nested.data
      : payload.customers || payload.results || payload.items || payload.records || []
  const nextCursor = payload.next_cursor || payload.nextCursor || payload.cursor || payload.next || null
  const recordsTotal = Number(payload.recordsTotal ?? payload.recordsFiltered ?? payload.total)
  const nextPage =
...
export async function fetchXindusErpCustomers({
  ...
  for (let i = 0; i < maxPages; i += 1) {
    ...
    target.searchParams.set('s', String(pageSize))
    target.searchParams.set('p', String(page - 1))

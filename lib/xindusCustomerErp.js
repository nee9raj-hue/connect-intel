...
function foldedRow(row = {}) {
  ...
  for (const [key, value] of Object.entries(row || {})) {
    ...
    if (compact && compact !== folded && (out[compact] == null || out[compact] === '')) {
      out[compact] = value
    }
  }
  return out
...
function pick(row, aliases) {
  for (const alias of aliases) {
    ...
    const value = row[folded] ?? row[folded.replace(/\s+/g, '')]
...
function isoDate(value) {
  ...
  if (!raw || /^(null|n\/a|na|none|-+)$/i.test(raw)) return null
  ...
  if (Number.isFinite(serial) && serial > 20000 && serial < 80000 && !raw.includes('-') && !raw.includes('/')) {
    const excelEpoch = Date.UTC(1899, 11, 30)
    const d = new Date(excelEpoch + Math.round(serial) * 86400000)
    return Number.isNaN(d.getTime()) ? null : d.toISOString().slice(0, 10)
  }
  if (/^\d{10,13}$/.test(raw)) return isoDate(Number(raw))
  ...
  if (monthName) {
    ...
    if (month && day >= 1 && day <= 31) {
      const iso = `${year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}`
      const check = new Date(`${iso}T12:00:00.000Z`)
      if (!Number.isNaN(check.getTime()) && check.getUTCDate() === day) return iso
    }
  }
  const d = new Date(raw)
  if (Number.isNaN(d.getTime())) return null
  // Date-only strings are parsed as UTC midnight; keep the calendar day.
  if (!/T\d|\d:\d/.test(raw)) return d.toISOString().slice(0, 10)
...
export function erpFromXindusCustomerRow(row = {}) {
  ...
  const lastInvoice = isoDate(
    pick(r, ['last invoice date', 'last_invoice_date', 'last invoice at', 'last_invoice_at'])
  )
  ...
  const revenue = {
    ...
    shipmentCount: num(
      pick(r, [
        'shipment count',
        'shipment_count',
        'shipment count lifetime',
        'shipment_count_lifetime',
        'shipments',
        'erp_shipment_count',
      ])
    ),
    ...
    pan: text(pick(r, ['pan'])),
    xindusId: text(pick(r, ['id', 'xindus id', 'xindus_id', 'xindus customer id', 'xindus_customer_id'])),
    crmId: text(pick(r, ['crm id', 'crmid'])),
    crn: text(pick(r, ['crn number', 'crn'])),
    channel: text(pick(r, ['channel', 'sales channel'])),
    businessType: text(pick(r, ['business type', 'business_type'])),
    tags: parseTags(pick(r, ['tags'])),
  ...
  const ownership = {
    salesOwner: parseErpPerson(
      ...
      parseErpPerson(
        pick(r, ['sales_owner_email'])
          ? {
              name: pick(r, ['sales_owner_name']),
              email: pick(r, ['sales_owner_email']),
              phone: pick(r, ['sales_owner_phone']),
              erp_staff_id: pick(r, ['sales_owner_user_id', 'sales_owner_id']),
            }
          : null
      ),
    accountOwner: parseErpPerson(
      pick(r, ['kam info', 'kam_info', 'kam', 'account owner', 'account_owner', 'account manager'])
    ),
  ...
  const finance = {
    ...
    shouldBlock,
    creditLimit: num(
      pick(r, ['pending payment limit', 'credit limit', 'pending_payment_limit', 'credit_limit'])
    ),
    creditPeriod: text(pick(r, ['credit period', 'credit period days', 'credit_period_days'])),
    ...
    bankAccountMasked:
      text(pick(r, ['bank account masked', 'bank_account_masked'])) ||
      maskAccount(pick(r, ['bank account number'])),
...
export function matchKeysFromXindusRow(row = {}) {
  ...
  const email = String(pick(r, ['email']) || '')
    .trim()
    .toLowerCase()
  const xindusId = String(pick(r, ['id', 'xindus id', 'xindus customer id', 'xindus_customer_id']) || '').replace(
    /\.0$/,
    ''
  )

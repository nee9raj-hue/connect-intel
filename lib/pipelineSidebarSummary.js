import { foldCrmStatusCounts, normalizeCrmLeadStatus } from './crmLeadStatuses.js'
...
export function normalizePipelineSummary(summary = {}) {
  const byStatus = foldCrmStatusCounts(summary.byStatus || [])
  return {
    total: summary.total ?? byStatus.reduce((sum, row) => sum + row.count, 0),
    byStatus,
...
export function bumpPipelineSummaryStatus(summary, fromStatus, toStatus) {
  const from = normalizeCrmLeadStatus(fromStatus)
  const to = normalizeCrmLeadStatus(toStatus)

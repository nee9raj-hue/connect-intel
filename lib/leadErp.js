...
export function normalizeErpRevenue(raw) {
  ...
  return {
    ...
    channel: textOrNull(src.channel),
    businessType: textOrNull(src.businessType ?? src.business_type),
  }
}
...
export function normalizeLeadErp(raw) {
  ...
  if ('erpTags' in src || 'erp_tags' in src) {
    out.erpTags = normalizeErpTags(src.erpTags ?? src.erp_tags)
  }
  return out
}
...
export function getLeadErp(leadOrEntry) {
  ...
  const built = buildErpFromFacts({
    stored: entry.erp || nested?.erp || null,
    trading: resolveLeadTradingProfile(entry),
    deals,
  })
  const erpTags = readErpTagsFromLead(entry)
  if (!erpTags.length) return built
  if (Array.isArray(built.erpTags) && built.erpTags.length) return built
  return { ...built, erpTags }
}
...
export function mergeLeadErp(existing, incoming) {
  ...
  if (Array.isArray(next.erpTags)) merged.erpTags = next.erpTags
  else if (Array.isArray(prev.erpTags)) merged.erpTags = prev.erpTags
  return merged
}
...
export function normalizeCompanyMatchKey(name) {
  return String(name || '')
    ...
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(
      /\b(private|limited|pvt|ltd|llp|inc|llc|co|company|opc|india|international)\b/g,
      ' '
    )
    .replace(/\s+/g, ' ')
    .trim()
...
/** True when two normalized company keys are the same account (prefix / contained name). */
export function companyKeysLooselyMatch(left, right) {
  ...
  // Single tokens like "express" / "galaxy" must not attach to every "* Express" company.
  if (short.split(/\s+/).filter(Boolean).length < 2) return false

...
describe('deal calendar dates', () => {
  ...
  it('treats booked and lost as exclusive outcomes', () => {
    const booked = applyDealOutcome({ wonOn: '2026-01-10', lostOn: '2026-01-11' }, 'booked', '2026-01-20')
    assert.equal(booked.bookedOn, '2026-01-20')
    assert.equal(booked.lostOn, null)
    assert.equal(booked.wonOn, '2026-01-10')
    assert.equal(resolveDealOutcome(booked), 'booked')

    const lost = applyDealOutcome(booked, 'lost', '2026-01-22')
    assert.equal(lost.lostOn, '2026-01-22')
    assert.equal(lost.bookedOn, null)
    assert.equal(lost.wonOn, null)
  })

  it('uses won date for booked filters until bookedOn is set', () => {
    const deal = { stage: 'won', wonOn: '2026-03-15', createdAt: '2026-09-01T00:00:00.000Z' }
    assert.deepEqual(dealDatesForRangeFilter(deal, ['booked']), ['2026-03-15'])
    assert.deepEqual(dealDatesForRangeFilter({ ...deal, bookedOn: '2026-03-20' }, ['booked']), ['2026-03-20'])
  })
})

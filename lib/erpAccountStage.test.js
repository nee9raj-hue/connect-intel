...
describe('erpAccountStage', () => {
  it('marks ERP accounts with no last shipment as new_account', () => {
    const entry = {
  ...
  it('marks a shipment in the last 60 days as active_trading', () => {
    const entry = {
      crm: { status: 'unqualified' },
      erp: { revenue: { lastShipmentDate: '2026-08-20', shipmentCount: 1 } },
    }
  ...
  it('marks last shipment older than 60 days as churned', () => {
    const entry = {
      crm: { status: 'active_trading' },
      erp: { revenue: { lastShipmentDate: '2026-06-01', shipmentCount: 1 } },
    }
  ...
  it('marks a negative balance older than one year as lost', () => {
    const entry = {
      crm: { status: 'active_trading' },
      erp: {
        revenue: { lastShipmentDate: '2026-09-01', shipmentCount: 4 },
  ...
  it('classifies pulled ERP rows that only have lastOrderCreatedAt', () => {
    const entry = {
      crm: { status: 'new_account', lastOrderCreatedAt: '2026-08-20T00:00:00.000Z' },
      lead: { company: 'SHAURYA HEALTHCARE' },
    }
    assert.equal(isErpImportedLead(entry), true)
    assert.equal(suggestCrmStatusFromErp(entry, NOW), 'active_trading')
    applyErpAccountStage(entry, NOW)

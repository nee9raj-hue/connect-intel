...
export async function expandManagerRosterIds(user, orgId, visibleOwnerIds, members = null) {
  const roster = members || (orgId ? await loadOrgTeamMembers(orgId) : [])
...
export async function loadViewerRoster(user, metaStore, { visibleOwnerIds = null } = {}) {
  ...
  const allMembers = orgId ? await loadOrgTeamMembers(orgId) : []

  if (!orgId) {
    const self = { userId: user.id, name: user.name || user.email || 'You', status: 'active' }
    return { role, isAdmin, isManager, rosterMembers: [self], memberOptions: memberOptionsFromTeam([self]) }
  }

  if (isAdmin) {
    const roster = allMembers.filter((m) => m.role !== 'org_admin' || m.pipelineRole === 'manager')
    return { role, isAdmin, isManager, rosterMembers: roster, memberOptions: memberOptionsFromTeam(roster) }
  }
  ...
  if (isManager) {
    ...
    const expanded = await expandManagerRosterIds(user, orgId, ownerIds, allMembers)
    const allowed = new Set(expanded.map(String))
    const roster = allMembers.filter((m) => allowed.has(String(m.userId)))
    return { role, isAdmin, isManager, rosterMembers: roster, memberOptions: memberOptionsFromTeam(roster) }
  }

  const self =
    allMembers.find((m) => String(m.userId) === String(user.id)) || {
      userId: user.id,

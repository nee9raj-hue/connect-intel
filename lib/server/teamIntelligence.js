...
import { buildActivityRollup, lastCrmActivityAtForUser } from './crmTouchpoints.js'
...
export function buildTeamIntelligence(store, user, {
  ...
  let members = rosterMembers?.length ? rosterMembers : orgId ? listTeamMembers(store, orgId) : []
  ...
  function profileForMember(m) {
    ...
    return {
      ...
      lastActiveAt: lastCrmActivityAtForUser(entries, m.userId),
      lastInAppAt: usage.lastInAppAt,
  ...
  return {
    ...
    prevMemberProfiles: members.map((m) => ({
      userId: m.userId,
      ...prevProfileForMember(m),
    })),
    comparison,

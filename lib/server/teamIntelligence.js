...
export function buildTeamIntelligence(store, user, {
  ...
  const rollup = memberUserId
    ...
    : {
        ...orgRollup,
        contactsOpened: Math.max(
          orgRollup.contactsOpened || 0,
          members.reduce(
            (sum, m) =>
              sum +
              Math.max(
                activityByUser.get(String(m.userId))?.leadsTouched || 0,
                aggregateWorkspaceUsage(usersById.get(String(m.userId)) || {}, since).leadsOpened
              ),
            0
          )
        ),

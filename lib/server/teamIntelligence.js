...
export function buildTeamIntelligence(store, user, {
  ...
  function profileForMember(m) {
    ...
    const contactsOpened = Math.max(usage.leadsOpened, crm.leadsTouched || crm.contactsOpened || 0)
    return {
      userId: m.userId,
      name: m.name,
      email: m.email,
  ...
  const memberProfiles = targetMembers.map((m) => {
    const full = allMemberProfiles.find((p) => String(p.userId) === String(m.userId))
    return full || profileForMember(m)
  })

  const rollup = memberUserId
    ? memberProfiles[0] || profileForMember(targetMembers[0])
  ...
  return {
    ...
    members: (memberUserId ? memberProfiles : allMemberProfiles).sort(

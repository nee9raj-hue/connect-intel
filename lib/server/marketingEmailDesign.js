  ...
  switch (block.type) {
    ...
    case 'form': {
      ...
      if (isGoogle) {
        ...
        return `<tr><td style="padding:16px 32px;background:${bg};text-align:${align};">
        <a href="${escapeHtml(url)}" style="display:inline-block;padding:12px 24px;background:${bgColor};text-decoration:none;border-radius:8px;${btnStyle}">${escapeHtml(label)}</a>
      </td></tr>`
      }

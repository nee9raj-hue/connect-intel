...
function renderBlockHtml(block, theme) {
  ...
  switch (block.type) {
    ...
    case 'form': {
      const url = resolveFormBlockUrl(block, { appBase: process.env.APP_URL })
      ...
      const badge = isGoogle ? 'Google Form' : 'Connect Intel form'
      return `<tr><td style="padding:16px 32px;background:${bg};text-align:${align};">
        ...
        <span style="display:inline-block;font-size:11px;font-weight:600;color:#6b7280;margin-bottom:8px;">${escapeHtml(badge)}</span>
    ...
    .filter((block) => {
      ...
      if (block.type === 'form') {
        if (isGoogleFormBlock(block)) {
          return Boolean(resolveGoogleFormUrl(block.googleUrl || block.url))
        }
        return Boolean(
          (block.formId || block.formSlug || block.url) &&
            String(block.title || 'Share your feedback').trim()
        )
      }
      return Boolean(
        block.text ||
...
export function compileTemplateContent({ blocks, design, body, previewText }, options = {}) {
  let rawBlocks = blocks
  if (rawBlocks?.length && options.marketingForms?.length) {
    rawBlocks = enrichMarketingFormBlocks(rawBlocks, options.marketingForms)
  }
...
export function resolveMessageContent(
  ...
  let sourceBlocks = blocks?.length ? blocks : template?.blocks
  if (sourceBlocks?.length && options.marketingForms?.length) {
    sourceBlocks = enrichMarketingFormBlocks(sourceBlocks, options.marketingForms)
  }
  const useBlocks = normalizeBlocks(sourceBlocks)

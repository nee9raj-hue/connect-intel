  ...
  if (user?.organizationId && user?.accountType === 'company') {
    try {
      ...
      if (assignee && assignee !== '__unassigned__' && assignee !== String(user.id)) {
        const { claimErpLeadsForFilteredMember } = await import('./erpOwnerLeadClaim.js')
        await claimErpLeadsForFilteredMember(user, assignee)
      }
    } catch (err) {
      console.warn('erp owner claim on pipeline bootstrap:', err?.message || err)
    }
    try {
      ...
      console.warn('erp stage classify on pipeline bootstrap:', err?.message || err)
    }
  }

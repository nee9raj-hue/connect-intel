...
export function collectTouchpoints(entry, since, until = Infinity) {
  ...
  for (const em of crm.emails || []) {
    ...
    points.push({
      at: em.sentAt,
      type: 'email',
      actorUserId: String(em.sentByUserId || em.createdByUserId || assignee || ''),
    })
  }
...
export function buildActivityRollupsForPeriods(entries, currentSince, prevSince, prevUntil) {
  const all = entries || []
  return {
    current: rollupPipelineActivity(all, currentSince),
    previous: rollupPipelineActivity(all, prevSince, prevUntil),
  }
}

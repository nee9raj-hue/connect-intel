...
/** Roll up KPIs from crm.activities — mirrors Activity log (reliable for emails/calls). */
export function rollupFromActivityLog(entries, since, until = Infinity) {
  const org = emptyActivityRollup()
  const byUser = new Map()

  const touch = (uid, entryId, type) => {
  ...
  for (const entry of entries || []) {
    ...
    const { activities, tasks } = rawCrmLists(entry)

    for (const act of activities) {
      if (!inActivityPeriod(act.createdAt, since, until)) continue
      touch(activityActorId(act), entryId, act.type)
    }

    for (const task of tasks) {
      if (task.createdAt && inActivityPeriod(task.createdAt, since, until)) {
        touch(activityActorId(task) || String(task.createdByUserId || ''), entryId, 'task')
      }
      if (task.completedAt && inActivityPeriod(task.completedAt, since, until)) {
        const uid = activityActorId(task) || String(task.assignedToUserId || '')
...
export function buildActivityRollupsForPeriods(entries, currentSince, prevSince, prevUntil) {
  const all = entries || []
  return {
    current: rollupFromActivityLog(all, currentSince),
    previous: rollupFromActivityLog(all, prevSince, prevUntil),
  }
}
...
export function buildActivityRollup(entries, since, until = Infinity) {
  return rollupFromActivityLog(entries || [], since, until)
}

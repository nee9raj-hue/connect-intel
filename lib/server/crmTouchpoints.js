...
function entryHasActivityInWindows(raw, windows) {
  ...
  for (const meeting of raw.meetings || []) {
    const t = new Date(meeting.createdAt).getTime()
    if (Number.isNaN(t)) continue
    for (const { since, until } of windows) {
      if (t >= since && t < until) return true
    }
...
export function buildActivityRollupsForPeriods(entries, currentSince, prevSince, prevUntil, recentLimit = 800) {
  const scanSet = entriesForActivityScanWindows(
    entries,
    [
  ...
  return {
    current: rollupRawActivities(scanSet, currentSince),
    previous: rollupRawActivities(scanSet, prevSince, prevUntil),
  }
}
...
export function buildActivityRollup(entries, since, until = Infinity) {
  ...
  return rollupRawActivities(scanSet, since, until)
}

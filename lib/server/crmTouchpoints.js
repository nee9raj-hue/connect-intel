import { normalizeExtendedCrm, resolveEntryCrm } from './crmWorkflow.js'
import { pipelineEntryFreshness } from './pipelineShard.js'
import { countCrmActivities, ACTIVITY_FEED_LIMIT, inActivityPeriod } from './crmActivityCounts.js'

export { inActivityPeriod } from './crmActivityCounts.js'
export { resolveEntryCrm } from './crmWorkflow.js'
...
/** Latest CRM activity timestamp performed by this user (strict actor — not lead assignee). */
export function lastCrmActivityAtForUser(entries, userId) {
  const uid = String(userId || '')
  if (!uid) return null
  let max = 0

  for (const entry of entries || []) {
    const { activities, emails, tasks, meetings } = rawCrmLists(entry)
    const points = [
      ...activities.map((act) => ({
        at: act.createdAt,
        actor: resolveTouchpointActor(act, entry, { strict: true }),
      })),
      ...emails.map((em) => ({
        at: em.sentAt,
        actor: resolveTouchpointActor(em, entry, { strict: true }),
      })),
      ...tasks
        .filter((task) => task.completedAt)
        .map((task) => ({
          at: task.completedAt,
          actor: resolveTouchpointActor(task, entry, { strict: true }),
        })),
      ...meetings.map((meeting) => ({
        at: meeting.startAt || meeting.createdAt,
        actor: resolveTouchpointActor(meeting, entry, { strict: true }),
      })),
    ]

    for (const point of points) {
      if (point.actor !== uid || !point.at) continue
...
/** Batch: latest strict-actor CRM touch per user (same source as activity log). */
export function lastCrmActivityAtByActorsFromEntries(entries, actorIds) {
  const wanted = new Set((actorIds || []).map(String).filter(Boolean))
  const maxByActor = new Map()
  if (!wanted.size) return maxByActor

  for (const entry of entries || []) {
    for (const point of collectTouchpoints(entry, 0, Infinity, { strictActor: true })) {
      const uid = point.actorUserId
      if (!uid || !wanted.has(uid) || !point.at) continue
      const t = new Date(point.at).getTime()
      if (Number.isNaN(t)) continue
      const prev = maxByActor.get(uid)
      if (!prev || t > new Date(prev).getTime()) {
        maxByActor.set(uid, new Date(t).toISOString())
      }
    }
  }
...
/** Roll up KPIs using the exact same feed as GET /api/crm/activity-log. */
export function rollupFromSanitizedActivityFeed(store, user, entries, since, until = Infinity) {
  const { org, perUser } = countCrmActivities(store, user, entries, {
    since,
    until,
    feedLimit: 0,
  })
...
export function entryHasActivityInWindows(raw, windows) {
...
export function entriesForActivityScanWindows(allEntries, windows, recentLimit = 800) {
  const byId = new Map()
  const add = (entry) => {
...
export function buildActivityRollupsForPeriods(store, user, entries, currentSince, prevSince, prevUntil) {
  const scanned = entriesForActivityScanWindows(
    entries || [],
    [
      { since: prevSince, until: prevUntil },
      { since: currentSince, until: Infinity },
    ],
    1500
  )
  return {
    current: rollupFromSanitizedActivityFeed(store, user, scanned, currentSince),
    previous: rollupFromSanitizedActivityFeed(store, user, scanned, prevSince, prevUntil),
  }
}

/** Roll up CRM touchpoints for org + per-user stats (used by team dashboard KPIs). */
export function buildActivityRollup(store, user, entries, since, until = Infinity) {
  return rollupFromSanitizedActivityFeed(store, user, entries || [], since, until)
}

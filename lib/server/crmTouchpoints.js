...
/** Count one CRM activity row — same source as the Activity log panel. */
...
export function rollupFromSanitizedActivityFeed(store, user, entries, since, until = Infinity) {
  const rows = (entries || []).map((entry) => ({
    ...entry,
    crm: sanitizeCrmForTenant(store, user, entry.crm, entry),
  }))
  ...
  for (const act of buildActivityFeed(rows, { limit: 0 })) {
    if (!inActivityPeriod(act.createdAt, since, until)) continue
    touch(activityActorId(act), act.leadId, act.type)
  }

  org.leadsTouched = new Set(
    [...byUser.values()].flatMap((row) => [...(row.touched || [])])
  ).size

...
/** Best-effort rollup: recent leads (activity log parity) + full touchpoint scan. */
export function buildActivityRollup(entries, since, until = Infinity) {
  ...
  const fromRecent = rollupPipelineActivity(recent, since, until)
  const fromFeed = rollupFromActivityFeed(recent, since, until)
  const fromAll = rollupPipelineActivity(entries, since, until)
  return mergeActivityRollups(mergeActivityRollups(fromRecent, fromFeed), fromAll)
}

...
export async function attachBulkRecipients(user, sendId, { source, csvText, emailsText, listId, segmentId }) {
  const store = await readStore({ only: ['marketingBulkSends', 'marketingBulkRecipients', 'marketingLists', 'marketingSegments', 'savedLeads', 'users', 'organizations'] })
  ...
  if (source === 'csv') {
    ...
    for (const leadId of ids) {
      const entry = (store.savedLeads || []).find((e) => (e.lead?.id || e.id) === leadId)

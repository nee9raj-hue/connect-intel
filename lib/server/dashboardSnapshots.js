...
export function pipelineSnapshotCollection(orgId) {
  return `pipeline_snapshot_${orgId}`
}
...
/** Read materialized pipeline snapshot (alias of pipeline_index for home/my-day). */
export async function readPipelineSnapshot(organizationId) {
  if (!organizationId) return null
  return readSnapshotPayload(pipelineSnapshotCollection(organizationId))
}

/** Write pipeline_snapshot_{org} whenever pipeline_index_* updates. */
export async function writePipelineSnapshotAlias(organizationId, indexDoc) {
  if (!organizationId || !indexDoc) return null
  const doc = {
    version: 1,
    organizationId,
    total: indexDoc.total,
    byStatus: indexDoc.byStatus,
    byAssignee: indexDoc.byAssignee,
    openDealCounts: indexDoc.openDealCounts,
    dealCounts: indexDoc.dealCounts,
    pipelineSnapshot: pipelineSnapshotFromIndex(null, indexDoc),
    pipelineHealth: pipelineHealthFromIndex(null, indexDoc),
    indexUpdatedAt: indexDoc.updatedAt,
    updatedAt: new Date().toISOString(),
  }
  await writeSnapshotPayload(pipelineSnapshotCollection(organizationId), doc)
  return doc
}
...
export async function readMyDaySnapshot(user) {
  ...
  const shardName = pipelineShardNameForUser(user)
  const orgId = user.organizationId
  let indexDoc = orgId ? await readPipelineSnapshot(orgId) : null
  if (!indexDoc && shardName) {
    indexDoc = await readPipelineIndexDoc(shardName)
  }
  ...
  return {
    myDay: {
      ...
      commandBar: [],
      priorities: [],
      pipelineSnapshot: indexDoc?.pipelineSnapshot || pipelineSnapshotFromIndex(bucket, indexDoc),
      todayTimeline: [],
      recentActivity: [],
      leadFocus: [],
      quickActions: [],

...
export async function recordSuppression({ organizationId, createdByUserId, email, reason = 'unsubscribe' }) {
  const normalized = String(email || '').trim().toLowerCase()
  if (!normalized.includes('@')) return { ok: false, error: 'Invalid email' }
  const allowed = new Set(['unsubscribe', 'bounce', 'complaint', 'manual', 'blocked'])
  const safeReason = allowed.has(reason) ? reason : 'manual'
...
export async function recordUnsubscribe({ organizationId, createdByUserId, email }) {
  ...
  return recordSuppression({ organizationId, createdByUserId, email, reason: 'unsubscribe' })
}

...
export async function writeCampaignStatsShard(campaignId, patch) {
  ...
  const { syncCampaignStatsToSql } = await import('./campaignsV3Sync.js')
  syncCampaignStatsToSql(campaignId, patch)
}
...
export async function mergeCampaignStatsShards(campaigns, { maxCampaigns = 0 } = {}) {
  if (!campaigns?.length) return campaigns || []
  const cap = Math.max(0, Number(maxCampaigns) || 0)
  const shardTargets =
    cap > 0 && campaigns.length > cap
      ? [...campaigns]
          .sort((a, b) => new Date(b.updatedAt || 0) - new Date(a.updatedAt || 0))
          .slice(0, cap)
      : campaigns
  const names = shardTargets.map((c) => campaignStatsShardName(c.id))
  ...
  return campaigns.map((c) => {
    const shard = shardById.get(c.id)
    if (!shard) {

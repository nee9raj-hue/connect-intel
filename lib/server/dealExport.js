...
const COLUMN_HEADERS = {
  ...
  queryReceivedOn: 'Query received',
  ratesQuotedOn: 'Rates quoted',
  bookedOn: 'Booked',
  wonOn: 'Won on',
  lostOn: 'Lost on',
  updatedAt: 'Updated',
...
export function dealExportCell(row, column) {
  ...
  switch (column) {
    ...
    case 'amount':
      return deal.amount ?? ''
    case 'currency':
      return resolveFreightDealCurrency(deal)
    case 'customerType':
    ...
    case 'queryReceivedOn':
      return formatIsoDate(deal.queryReceivedOn)
    case 'ratesQuotedOn':
      return formatIsoDate(deal.ratesQuotedOn)
    case 'bookedOn':
      return formatIsoDate(deal.bookedOn)
    case 'wonOn':
      return formatIsoDate(deal.wonOn)
    case 'lostOn':
      return formatIsoDate(deal.lostOn)
    case 'updatedAt':
...
export async function loadAllDealsForExport(
  ...
  if (hasClientFilters) {
    deals = filterPipelineDealRows(deals, {
      ...
      dateStages:
        filters.dealStage && filters.dealStage !== 'all' ? [filters.dealStage] : filters.stages || [],
    })
  }

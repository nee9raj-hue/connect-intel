...
export async function loadPipelineStoreContext(user) {
  ...
  const [shardRaw, monolithLeads] = await Promise.all([
    readPipelineShardEntries(shardName, { bypassCache: true }),
    readStore({ only: ['savedLeads'] }).then((s) => s.savedLeads || []),
  ])
  const fromShard = Array.isArray(shardRaw) ? shardRaw : []
  ...
  let entries = fromShard.length ? mergePipelineEntries(fromMonolith, fromShard) : fromMonolith
  if (!fromShard.length && entries.length) {
    await writePipelineShardEntries(shardName, entries)
  }

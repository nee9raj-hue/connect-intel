...
function mergeCrmRecords(primaryCrm, secondaryCrm) {
  ...
  const mergeById = (left, right, max, sortKey = 'createdAt') =>
    [...byId(left).values(), ...byId(right).values()]
  ...
  const meetings = [...byId(a.meetings).values(), ...byId(b.meetings).values()]
    ...
    .slice(0, 200)
  const deals = mergeById(a.deals, b.deals, 50)
...
export function overlayMonolithCrmForRead(shardEntries, monolithEntries) {
  ...
  for (const shard of shardEntries || []) {
    ...
    merged.push(mono ? mergePipelineEntry(shard, mono) : shard)
  }
...
export async function loadPipelineStoreContext(user, { dashboard = false, mergeMonolithCrm = false, shardOnly = false } = {}) {
  const shardName = pipelineShardNameForUser(user)

...
export async function loadPipelineStoreForLeadIds(user, leadIds) {
  ...
  if (pipelineLeadsTableActive() && ids.length) {
    ...
    if (!visible.length) {
      const { pipelineStore, visible: shardVisible } = await loadPipelineStoreContext(user)
      const idSet = new Set(ids.map(String))
      const fromShard = shardVisible.filter((entry) => {
    ...
    return {
      ...
      visible,
      shardName,
      pipelineSource: visible.length && !entries.length ? 'shard_fallback_by_id' : 'pipeline_leads_table',

...
/** Fast read merge: pick the CRM copy with more activity instead of deep mergeCrmRecords. */
export function overlayMonolithCrmForRead(shardEntries, monolithEntries) {
  ...
  for (const shard of shardEntries || []) {
    ...
    const shardActs = shard.crm?.activities?.length || 0
    const monoActs = mono.crm?.activities?.length || 0
    const shardFresh = pipelineEntryFreshness(shard)
    const monoFresh = pipelineEntryFreshness(mono)
    if (monoActs > shardActs || monoFresh > shardFresh) {
      merged.push({
        ...shard,
        ...mono,
        crm: mono.crm || shard.crm,
      })
    } else {
      merged.push(shard)
    }
...
export async function loadPipelineStoreContext(user, { dashboard = false } = {}) {
  const shardName = pipelineShardNameForUser(user)
  ...
  let entries
  if (fromShard.length && dashboard) {
    // Dashboard reads shard only — writes land on shard; avoids loading 6k+ monolith rows.
    entries = fromShard
    ...
    entries = overlayMonolithCrmForRead(fromShard, fromMonolith)
  } else {
    const monolithLeads = (await readStore({ only: ['savedLeads'] })).savedLeads || []

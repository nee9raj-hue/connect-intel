 ...
 */
export async function updatePipelineLeadViaTable(user, leadId, mutator) {
  ...
  const shardName = pipelineShardNameForUser(user)
  const previousEntry = await readPipelineLeadById(shardName, leadId)
  ...
  await persistPipelineEntryUpdates(shardName, [entry], { refreshIndex: true })

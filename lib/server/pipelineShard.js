...
import { filterPipelineEntriesVisibleAsync } from './pipelineVisibility.js'
import { PIPELINE_STORE_COLLECTIONS } from './pipelineStore.js'
import { normalizeDealsList } from './crmWorkflow.js'
import { assertPipelineStoreTenant } from './tenantWriteGuard.js'
import { recordPipelineRead } from './infra/apiPipelineContext.js'

export const META_STORE_COLLECTIONS = ['users', 'organizations', 'organizationMemberships']

/** Session-only org meta so company Pipeline never downloads the users JSON blob. */
export function pipelineMetaStoreFromSessionUser(user) {
  if (!user?.id) {
    return { users: [], organizations: [], organizationMemberships: [], savedLeads: [] }
  }
  const organizationId = user.organizationId || null
  const isAdmin = Boolean(user.isOrgAdmin || user.orgRole === 'org_admin')
  const organization = organizationId
    ? {
        id: organizationId,
        name: user.organizationName || user.company || 'Company',
        accountType: user.accountType || 'company',
        sqlOrganizationId: user.organizationUuid || null,
        workspaceFeatures: user.workspaceFeatures || {},
        logoUrl: user.organizationLogoUrl || null,
      }
    : null
  const membership = organizationId
    ? {
        userId: user.id,
        organizationId,
        role: isAdmin ? 'org_admin' : user.orgRole || 'member',
        pipelineRole: user.pipelineRole || (isAdmin ? 'org_admin' : 'member'),
        status: 'active',
  ...
  return {
    ...
    organizationMemberships: membership ? [membership] : [],
    savedLeads: [],
  }
}
...
/** Skip sync monolith mirror + full index rebuild — defer via queue/background job. */
export const FAST_PIPELINE_WRITE = { mirrorToSavedLeads: false, refreshIndex: false }
...
export function mergePipelineEntry(prev, incoming) {
  ...
  return {
    ...secondary,
    ...primary,
    ...pickNewerAssignment(prev, incoming),
...
/** Keep the most recent lead assignment when merging table + monolith rows. */
export function pickNewerAssignment(prev, incoming) {
  const a = prev || {}
  const b = incoming || {}
  ...
  if (bAt > aAt) {
    return {
      ...
      assignedByUserId: b.assignedByUserId ?? null,
    }
  }
  if (aAt > bAt) {
    return {
  ...
  const aPipe = Date.parse(a.pipelineUpdatedAt || '') || 0
  const bPipe = Date.parse(b.pipelineUpdatedAt || '') || 0
  const pick = bPipe >= aPipe ? b : a
  return {
    assignedToUserId: pick.assignedToUserId ?? null,
    assignedAt: pick.assignedAt ?? null,
    assignedByUserId: pick.assignedByUserId ?? null,
  }
}

async function mergeEntriesIntoPipelineShard(shardName, entries, { refreshIndex = true } = {}) {
  const updatesByKey = new Map()
  for (const entry of entries || []) {
    const key = pipelineEntryKey(entry)
    if (key) updatesByKey.set(key, entry)
  }
  if (!updatesByKey.size) return

  const existing = (await readPipelineShardEntries(shardName, { bypassCache: true })) || []
  const merged = existing.map((entry) => {
    const key = pipelineEntryKey(entry)
    return key && updatesByKey.has(key) ? updatesByKey.get(key) : entry
  })
  const seen = new Set(merged.map((entry) => pipelineEntryKey(entry)).filter(Boolean))
  for (const [key, entry] of updatesByKey) {
    if (!seen.has(key)) merged.push(entry)
  }
  await writePipelineShardEntries(shardName, merged, {
    mirrorToSavedLeads: false,
    refreshIndex,
  })
}

async function deferredMirrorPipelineEntries(shardName, list) {
  try {
    await mergeEntriesIntoPipelineShard(shardName, list, { refreshIndex: false })
    await mirrorShardEntriesToSavedLeads(list)
  } catch (err) {
    console.warn('deferred pipeline mirror:', err?.message || err)
  }
}

async function deferredMeilisearchSync(entries) {
  const list = (entries || []).filter((entry) => entry?.lead?.id && entry.organizationId)
  if (!list.length) return
  const { syncMeilisearchAfterSave } = await import('./meilisearchSync.js')
  const byOrg = new Map()
  for (const entry of list) {
    const oid = entry.organizationId
    if (!byOrg.has(oid)) byOrg.set(oid, [])
    byOrg.get(oid).push(entry)
  }
  for (const [organizationId, batch] of byOrg) {
    syncMeilisearchAfterSave({ organizationId, entries: batch })
  }
}

/**
 * Authoritative write for assignment/CRM patches: pipeline_leads + JSON shard + savedLeads mirror.
 * When pipeline_leads table is active, only the row upsert blocks the response; shard/index mirror is deferred.
 */
export async function persistPipelineEntryUpdates(
  shardName,
  entries,
  { refreshIndex = false, deferHeavyMirror = true } = {}
) {
  const list = (entries || []).filter((entry) => entry?.lead?.id)
  if (!list.length) return

  const { upsertPipelineLeadRows, pipelineLeadsTableActive } = await import('./pipelineLeadsTable.js')
  if (pipelineLeadsTableActive()) {
    const result = await upsertPipelineLeadRows(shardName, list)
    if (!result.upserted) {
      throw new Error('Could not save pipeline changes. Please try again.')
    }
    invalidatePipelineShard(shardName)
    void deferredMeilisearchSync(list)
    if (deferHeavyMirror) {
      void deferredMirrorPipelineEntries(shardName, list)
    } else {
      await mergeEntriesIntoPipelineShard(shardName, list, { refreshIndex })
      await mirrorShardEntriesToSavedLeads(list)
    }
    return
  }
...
/** Read specific leads from a JSON shard without loading meta or running visibility merge. */
export function filterPipelineEntriesByLeadIds(entries, leadIds) {
  const ids = [...new Set((leadIds || []).map(String).filter(Boolean))]
  if (!ids.length) return []
  const idSet = new Set(ids)
  return (entries || []).filter((entry) => {
    const lid = entry?.lead?.id || entry?.leadId || entry?.id
    return lid && idSet.has(String(lid))
  })
}

export async function readPipelineShardEntriesByIds(shardName, leadIds) {
  if (!shardName) return []
  const entries = (await readPipelineShardEntries(shardName, { bypassCache: true })) || []
  return filterPipelineEntriesByLeadIds(entries, leadIds)
}

/**
 * Table-first load for single-lead mutations: read pipeline_leads row, heal from shard if missing.
 */
export async function loadPipelineLeadForMutation(user, leadId) {
  ...
  let entry = await readPipelineLeadById(shardName, leadId)
  let source = 'pipeline_leads_table'

  if (!entry) {
    const fromShard = await readPipelineShardEntriesByIds(shardName, [leadId])
    entry = fromShard[0] || null
    if (entry) {
      await upsertPipelineLeadRow(shardName, entry)
      source = 'shard_heal_to_table'
    }
  }
  ...
  return {
    entry,
    pipelineStore: attachPipelineEntriesToStore(metaStore, [entry]),
    metaStore,
    shardName,
    source,
  }
...
export async function patchPipelineEntriesCrmBatch(
  ...
  return withStoreLock(async () => {
    ...
    const { findPipelineEntryWithScope, resolvePipelineVisibleOwnerIds } = await import(
      './pipelineVisibility.js'
    )
    const visibleOwnerIds = await resolvePipelineVisibleOwnerIds(user, metaStore)
    const updated = []
    for (const { leadId, updateCrm } of list) {
      const entry = findPipelineEntryWithScope(draft, user, leadId, { visibleOwnerIds })
      if (!entry) continue
...
async function upsertPipelineLeadsTableRows(shardName, entries) {
  const list = (entries || []).filter((entry) => entry?.lead?.id)
  if (!list.length) return
  try {
    const { pipelineLeadsTableActive, upsertPipelineLeadRows } = await import('./pipelineLeadsTable.js')
    if (pipelineLeadsTableActive()) {
      await upsertPipelineLeadRows(shardName, list)
    }
  } catch (err) {
    console.warn('pipeline_leads table sync:', err?.message || err)
  }
}
...
export async function syncPipelineShardsFromSavedLeads(savedLeads) {
  ...
  await Promise.all(
    [...byShard.entries()].map(async ([name, savedEntries]) => {
      ...
      await upsertPipelineLeadsTableRows(name, savedEntries)
    })
  )
}
...
export async function loadPipelineStoreForLeadIds(user, leadIds) {
  ...
  if (pipelineLeadsTableActive() && ids.length) {
    ...
    let visible = await filterPipelineEntriesVisibleAsync(user, entries, metaStore)

    if (!visible.length) {
      const fromShard = await readPipelineShardEntriesByIds(shardName, ids)
      if (fromShard.length) {
        visible = await filterPipelineEntriesVisibleAsync(user, fromShard, metaStore)
      }
    }
    ...
    recordPipelineRead({
      rows: visible.length,
      source: visible.length && !entries.length ? 'shard_by_id_fallback' : 'pipeline_leads_table',
    })
    return {
      pipelineStore,
      visible,
      shardName,
      pipelineSource:
        visible.length && !entries.length ? 'shard_by_id_fallback' : 'pipeline_leads_table',
      pipelineRowsRead: entries.length || visible.length,
    }
  }

  const { pipelineStore, visible } = await loadPipelineStoreContext(user)
  if (!ids.length) {
    recordPipelineRead({ rows: visible.length, source: 'shard_full' })
    return {
  ...
  recordPipelineRead({ rows: visible.length, source: 'shard_full_filtered' })
  return {
...
export async function patchPipelineEntryCrm(user, leadId, updateCrm, options = {}) {
  const { pipelineLeadsTableActive } = await import('./pipelineLeadsTable.js')
  if (pipelineLeadsTableActive()) {
    const result = await updatePipelineLeadViaTable(
      user,
      leadId,
      async (draft) => {
        const { findPipelineEntryAsync } = await import('./pipelineVisibility.js')
        const metaStore =
          options.metaStore ||
          (await readStore({ only: META_STORE_COLLECTIONS }))
        const entry = await findPipelineEntryAsync(draft, user, leadId, metaStore)
      ...
      {
        preloadedEntry: options.preloadedEntry,
...
/**
 * Table-first single-lead update: read one row, mutate, persist to table + shard + savedLeads mirror.
 * Returns null when the table is off or the lead row is missing (caller should fall back to shard).
 */
export async function updatePipelineLeadViaTable(
  user,
  leadId,
  mutator,
  { preloadedEntry = null, metaStore: preloadedMetaStore = null } = {}
) {
  const { pipelineLeadsTableActive, readPipelineLeadById } = await import('./pipelineLeadsTable.js')
  if (!pipelineLeadsTableActive() || !leadId) return null

  const shardName = pipelineShardNameForUser(user)
  const previousEntry = preloadedEntry || (await readPipelineLeadById(shardName, leadId))
  if (!previousEntry) return null

  const metaStore = preloadedMetaStore || (await readStore({ only: META_STORE_COLLECTIONS }))
  const draft = attachPipelineEntriesToStore(metaStore, [previousEntry])
  const { findPipelineEntryAsync } = await import('./pipelineVisibility.js')
  const next = (await mutator(draft)) || draft
  const entry = await findPipelineEntryAsync(next, user, leadId, metaStore)
  if (!entry) return { error: 'not_found' }

  await persistPipelineEntryUpdates(shardName, [entry], {
    refreshIndex: false,
    deferHeavyMirror: true,
  })

  return { store: next, entry, previousEntry, mode: 'table' }
}
...
export async function updatePipelineStore(user, mutator, { writeOptions = {} } = {}) {
  return withStoreLock(async () => {
    ...
    if (entries) {
      const draft = attachPipelineEntriesToStore(metaStore, entries)
      const next = (await mutator(draft)) || draft
      assertPipelineStoreTenant(user, next)
...
export async function loadPipelineStoreContext(
  ...
  if (dashboard && !mergeMonolithCrm) {
    ...
    if (cached && Date.now() - cached.at < DASHBOARD_CACHE_TTL_MS) {
      const metaStore = await readStore({ only: META_STORE_COLLECTIONS })
      const ownerIds = await resolveManagerVisibleOwnerIds(user, metaStore)
      const pipelineStore = attachPipelineEntriesToStore(metaStore, cached.entries)
      const visible = listPipelineSavedEntries(pipelineStore, user, { visibleOwnerIds: ownerIds })
      const scopedStore = attachPipelineEntriesToStore(metaStore, visible)
      recordPipelineRead({ rows: visible.length, source: 'shard_dashboard_cache' })
      return { pipelineStore: scopedStore, visible, shardName, pipelineSource: 'shard_dashboard_cache' }
    }
  }
  ...
  const { readPipelineLeadsForUser, pipelineLeadsTableActive } = await import('./pipelineLeadsTable.js')
  let rawEntries = null
  let pipelineSource = 'shard'

  if (pipelineLeadsTableActive()) {
    rawEntries = await readPipelineLeadsForUser(user, metaStore, shardName)
    if (rawEntries) pipelineSource = 'pipeline_leads_table'
  }

  if (!rawEntries?.length) {
    const shardRaw = await readPipelineShardEntries(shardName)
    rawEntries = Array.isArray(shardRaw) ? shardRaw : []
    pipelineSource = 'shard'
  }

  let entries
  if (rawEntries.length && !useMonolithMerge) {
    entries = rawEntries
    ...
    entries = overlayMonolithCrmForRead(rawEntries, fromMonolith, { activitySinceMs })
  } else {
    const monolithLeads = (await readStore({ only: ['savedLeads'] })).savedLeads || []
  ...
  const scopedStore = attachPipelineEntriesToStore(metaStore, visible)
  recordPipelineRead({ rows: visible.length, source: pipelineSource })
  return { pipelineStore: scopedStore, visible, shardName, pipelineSource, pipelineRowsRead: visible.length }
}

...
async function sendViaResend({ fromMeta, content, inviterEmail, skipDeliveryStatusCheck = false }) {
...
export async function sendOrgNotificationEmail({
  ...
  if (provider === 'gmail') {
    ...
    return { ...result, from: fromMeta.from, to: normalizedTo }
  }

  if (provider === 'resend') {
    try {
      return await sendViaResend({ fromMeta, content, inviterEmail: replyTo })
    } catch (error) {

...
import {
  normalizeOrgNotificationPlainText,
  sanitizeEmailSubject,
  wrapOrgNotificationHtml,
} from './orgNotificationEmailLayout.js'
...
async function sendViaResend({ fromMeta, content, inviterEmail, skipDeliveryStatusCheck = false, attachments }) {
    ...
    payload.attachments = attachments.map((file) => ({
      ...
      content:
        typeof file.content === 'string'
          ? Buffer.from(file.content, file.encoding || 'utf8').toString('base64')
  ...
  if (!skipDeliveryStatusCheck && data.id) {
...
/** Transactional email to a teammate (assignment alerts, etc.) — same delivery as team invites. */
export async function sendOrgNotificationEmail({
  ...
  const fromMeta = buildInviteFromHeader({
    inviterName: organizationName ? `${organizationName} (Connect Intel)` : 'Connect Intel',
    inviterEmail: replyTo,
    organizationName,
  })
  ...
  const safeHtml = wrapOrgNotificationHtml(html)
  const safeText = normalizeOrgNotificationPlainText(text)

  let orgOAuth = null
  if (organizationId) {
    const { readStore, INVITE_STORE_COLLECTIONS } = await import('./store.js')
    const store = await readStore({ only: INVITE_STORE_COLLECTIONS })
    orgOAuth = resolveInviteGmailOAuthForOrg(store, organizationId)
  }

  const content = { normalizedTo, subject: safeSubject, html: safeHtml, text: safeText }
  ...
  if (provider === 'gmail_oauth') {
    ...
    return { ...result, from: fromMeta.from, to: normalizedTo, mailbox: orgOAuth.email }
  }
  ...
  if (provider === 'gmail') {
    ...
    const attachmentNote = attachments?.length
      ? ' Attachment omitted — reconnect Gmail OAuth or Resend for CSV delivery.'
      : ''
    return {
      ...result,
      from: fromMeta.from,
      to: normalizedTo,
      text: safeText + attachmentNote,
    }
  }

  if (provider === 'resend') {
    try {
      return await sendViaResend({ fromMeta, content, inviterEmail: replyTo, attachments })
    } catch (error) {
      return { sent: false, error: error.message, provider: 'resend', to: normalizedTo }
    }
  }

  return {
    sent: false,
    error: 'Email is not configured on the server — assignment saved but notification was not sent.',
    provider: 'none',
    to: normalizedTo,
  }
}
...
export async function sendTeamInviteEmail({
  ...
  store: storeHint = null,
  skipDeliveryStatusCheck = false,
}) {
  ...
  if (organizationId) {
    ...
    if (!store) {
      const { readStore, INVITE_STORE_COLLECTIONS } = await import('./store.js')
      store = await readStore({ only: INVITE_STORE_COLLECTIONS })
    }
    orgOAuth = resolveInviteGmailOAuthForOrg(store, organizationId)
  }
  ...
  if (provider === 'resend') {
    try {
      return await sendViaResend({ fromMeta, content, inviterEmail, skipDeliveryStatusCheck })
    } catch (error) {
      return {
        sent: false,
        error: error.message,
  ...
  const resendDomain = { status: 'not_configured', hint: null }
...
/** Persist invite first, then deliver email without blocking the HTTP response when possible. */
export async function deliverTeamInviteEmail(emailParams) {
  try {
    const { waitUntil } = await import('@vercel/functions')
    waitUntil(
      sendTeamInviteEmail(emailParams).catch((err) => {
        console.warn('team invite email failed:', err?.message || err)
      })
    )
    return { deferred: true }
  } catch {
    return sendTeamInviteEmail(emailParams)
  }
}

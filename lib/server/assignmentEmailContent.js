...
const BULK_ASSIGNMENT_LIST_MAX = 50

export function buildBulkAssignmentEmailContent({
  to,
  assigneeName,
  actorName,
  actorEmail,
  ...
  const orgLabel = organizationName || 'Connect Intel'

  const rows = entries.slice(0, BULK_ASSIGNMENT_LIST_MAX)
  ...
  const tableRowsHtml = rows
    .map((entry) => {
      ...
      const status = STATUS_LABELS[crm.status || 'new'] || crm.status || 'New'
      return `<tr>
        <td style="padding:8px 10px;border-bottom:1px solid #eee;color:#111;font-weight:600">${escapeHtml(name)}</td>
        <td style="padding:8px 10px;border-bottom:1px solid #eee;color:#444">${escapeHtml(company)}</td>
        <td style="padding:8px 10px;border-bottom:1px solid #eee;color:#444">${escapeHtml(status)}</td>
      </tr>`
    })
    .join('')

  const tableRowsText = rows
    .map((entry) => {
      const lead = entry.lead || {}
      const crm = entry.crm || {}
      const name = leadDisplayName(lead)
  ...
  const subject =
    count === 1
      ? `${actorLabel} assigned you a lead: ${leadDisplayName(rows[0]?.lead)}`
      : `${actorLabel} assigned you ${count} leads`

  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;padding:24px">
      <h2 style="color:#242424;margin:0 0 8px">${count === 1 ? 'New lead assigned to you' : `${count} leads assigned to you`}</h2>
      <p style="color:#444;line-height:1.5;margin:0 0 16px">
        Hi ${escapeHtml(assigneeName || 'there')}, <strong>${escapeHtml(actorLabel)}</strong>
        assigned ${count === 1 ? 'a pipeline lead' : `<strong>${count} pipeline leads</strong>`} to you on
        <strong>${escapeHtml(orgLabel)}</strong>.
      </p>
      <table style="width:100%;border-collapse:collapse;margin:0 0 16px;font-size:14px">
        <thead>
          <tr>
            <th style="text-align:left;padding:8px 10px;border-bottom:2px solid #e8eaed;color:#666;font-size:12px">Lead</th>
            <th style="text-align:left;padding:8px 10px;border-bottom:2px solid #e8eaed;color:#666;font-size:12px">Company</th>
            <th style="text-align:left;padding:8px 10px;border-bottom:2px solid #e8eaed;color:#666;font-size:12px">Status</th>
          </tr>
        </thead>
        <tbody>${tableRowsHtml}</tbody>
      </table>
      ${overflow > 0 ? `<p style="color:#666;font-size:13px;margin:0 0 16px">And ${overflow} more in your Pipeline.</p>` : ''}
      <p style="color:#444;line-height:1.5;margin:0 0 20px;white-space:pre-wrap;background:#fffbeb;border:1px solid #fde68a;padding:12px;border-radius:8px">Open Pipeline to review each lead, send intro emails, and log your first touch.</p>
      <p style="margin:24px 0">
        <a href="${appUrl}" style="background:#ffcb2b;color:#242424;padding:12px 20px;border-radius:8px;font-weight:600;text-decoration:none;display:inline-block">
          Open Connect Intel -> Pipeline
        </a>
      </p>
      <p style="color:#888;font-size:12px">Reply to reach ${escapeHtml(actorLabel)}${actorEmail ? ` (${escapeHtml(actorEmail)})` : ''}.</p>
    </div>
  `

  const text = `${count === 1 ? 'New lead assigned to you' : `${count} leads assigned to you`}

Hi ${assigneeName || 'there'},

${actorLabel} assigned ${count === 1 ? 'a lead' : `${count} leads`} to you on ${orgLabel}.

${tableRowsText}
${overflow > 0 ? `\nAnd ${overflow} more in your Pipeline.\n` : ''}
Open Pipeline to review each lead and log your first touch.

Open pipeline: ${appUrl}

Reply to ${actorEmail || actorLabel}.`

...
/**
 * After enrollments exist: queue+worker when Redis is on (all batch sizes);
 * inline only as degraded fallback when background email is disabled.
 */
export async function executeCampaignSendByMode(user, campaignId, recipientCount, options = {}) {
  ...
  if (mode === EMAIL_SEND_MODE.INLINE) {
    ...
    const burst = await processCampaignSendBurst(user, campaignId, {
      limit: Math.min(count, INLINE_EMAIL_MAX_RECIPIENTS),

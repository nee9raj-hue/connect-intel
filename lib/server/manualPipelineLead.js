...
import { defaultCrm, normalizeCrmLeadStatus } from './crm.js'
...
function pipelineStatusFromInput(raw, { freightOrg = false } = {}) {
  const status = String(raw || '')
    .trim()
    .toLowerCase()
    .replace(/\s+/g, '_')
  if (freightOrg && (!status || status === 'new' || status === 'fresh')) return 'fresh'
  return raw ? normalizeCrmLeadStatus(status) : 'unqualified'
}
...
export function addManualPipelineLead(store, { user, organizationId, fields }) {
  ...
  if (organizationId) {
    ...
    if (org) assertWithinPlanLimits(store, org, { extraLeads: 1 })
  }
  ...
  crm.status = pipelineStatusFromInput(fields.status, { freightOrg })

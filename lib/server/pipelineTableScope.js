...
export async function resolvePipelineTableScopeAsync(user, metaStore, filters = {}) {
  ...
  return {
    ...
    repOwnAndUnassigned: scoped.scope.repOwnAndUnassigned,
    managerWithUnassigned: scoped.scope.managerWithUnassigned,
    viewerUserId: user.id,
    includeUnassigned: Boolean(
      scoped.scope.repOwnAndUnassigned || scoped.scope.managerWithUnassigned
    ),
    role: scoped.role,
    scoped,
    repSharedMode: scoped.scope.repSharedMode,
    listFilters: scoped.scope.listFilters,
  }
}

/** Visible rows after loading a raw entry batch from the table. */
export function visiblePipelineFromEntries(metaStore, user, entries, options = {}) {

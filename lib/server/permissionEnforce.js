...
const PERM_META_COLLECTIONS = ['users', 'organizations', 'organizationMemberships']
...
export async function userHasOrgPermission(user, action, store) {
  ...
  if (action === 'delete_leads' && role !== 'admin') return false
...
/** Session user + org matrix slice; asserts edit_leads for company CRM mutations. */
export async function loadMetaUserAndAssertEditLeads(sessionUser, preloadedStore) {
  const metaStore =
    preloadedStore || (await readStore({ only: PERM_META_COLLECTIONS }))
  const user = metaStore.users?.find((u) => u.id === sessionUser.id) || sessionUser
  await assertOrgPermission(user, 'edit_leads', metaStore)
  return { user, metaStore }
}
...
/** Delete leads/deals: Org Admin only (never manager or rep, including via edit_leads). */
export async function assertPipelineDeleteAccess(user, store) {
  await assertOrgPermission(user, 'delete_leads', store)
}

...
function findCompany(store, row, organizationId = null) {
  const pool =
    organizationId != null
  ...
  const companyId = pool.find((entry) => companyKey(entry) === key)?.id
  if (companyId) return pool.find((entry) => entry.id === companyId) || null

  const target = String(row.company || '').trim().toLowerCase()
  if (!target) return null
  return pool.find((entry) => String(entry.name || '').trim().toLowerCase() === target) || null
}

function findContact(store, companyId, row, organizationId = null) {
  const pool =
    organizationId != null
...
export function upsertMasterRecordFromLeadFields(store, fields, actor = null) {
  ...
  let company = findCompany(store, row, organizationId)
  let contact = company ? findContact(store, company.id, row, organizationId) : null
  ...
  if (!contact) {
    ...
    company = findCompany(store, row, organizationId)
    contact =
      (company && findContact(store, company.id, row, organizationId)) ||

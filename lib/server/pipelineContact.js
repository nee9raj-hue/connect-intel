...
function pipelineEntriesForUser(store, user) {
  const orgId = user.organizationId
  return store.savedLeads.filter((entry) =>
    orgId ? entry.organizationId === orgId : entry.userId === user.id && !entry.organizationId
  )
}

function allowedImportJobIds(store, user) {
  const orgId = user.organizationId
  return new Set(
    store.importJobs
      .filter((job) => {
        if (orgId) return job.organizationId === orgId || job.sourceLabel === 'org-pipeline'
        return job.datasetType === 'pipeline-manual' || job.sourceLabel === 'org-pipeline'
      })
      .map((job) => job.id)
  )
}
...
export function userCanAccessContact(store, user, contactId) {
  ...
  for (const entry of pipelineEntriesForUser(store, user)) {
    if (entry.contactId === contactId || entry.lead?.id === contactId) return true
  }

  if (contact.importJobId && allowedImportJobIds(store, user).has(contact.importJobId)) {
    return true
  }
...
export function listContactsForUser(store, user, { search = '', limit = 100, offset = 0 } = {}) {
  const allowedJobs = allowedImportJobIds(store, user)
  const pipelineIds = new Set()

  for (const entry of pipelineEntriesForUser(store, user)) {
    if (entry.contactId) pipelineIds.add(entry.contactId)
    if (entry.lead?.id) pipelineIds.add(entry.lead.id)
  }

  const companyById = new Map(store.companies.map((company) => [company.id, company]))
  let rows = store.contacts.filter((contact) => {
    if (pipelineIds.has(contact.id)) return true
    return contact.importJobId && allowedJobs.has(contact.importJobId)
  })
...
function syncPipelineSnapshotsForContact(store, contact, company) {
...
export function updateMasterContactById(store, user, contactId, patch = {}) {
  ...
  syncPipelineSnapshotsForContact(store, contact, company)

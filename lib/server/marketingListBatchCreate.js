...
import { crmLeadStatusLabel, isCrmLeadStatusFilter, normalizeCrmLeadStatus } from '../crmLeadStatuses.js'
...
import { getMembership, getPipelineLeadIds, listPipelineSavedEntries, resolveOrgRole } from './organizations.js'
import {
  chunkArray,
  formatBatchListName,
...
function normalizePipelineStatus(status) {
  ...
  if (!isCrmLeadStatusFilter(id)) return null
  return normalizeCrmLeadStatus(id)
}

function entryPipelineStatus(entry) {
  return normalizeCrmLeadStatus(entry?.crm?.status)
}

export function pipelineStatusLabel(statusId) {
  const id = normalizePipelineStatus(statusId)
  if (!id) return null
  return crmLeadStatusLabel(id)
}
...
/** Static lists from explicit pipeline selection — no assignee pool, snapshot frozen for campaigns. */
export function createSelectionListBatches(store, user, options = {}) {
  const {
    namePrefix,
    leadIds: requestedIds,
    batchSize = MARKETING_SEND_BATCH_SIZE,
  ...
  let leadIds = [...new Set((requestedIds || []).filter((id) => visible.has(id)))]
  const listChannel = normalizeMarketingChannel(channel)
  leadIds = filterLeadIdsForMarketingChannel(store, user, leadIds, listChannel)
  const { allowed, blocked } = partitionLeadsBySuppression(store, user, leadIds)
  leadIds = allowed

  if (!leadIds.length) {
    throw new Error(
      listChannel === 'whatsapp'
        ? 'No selected contacts have a valid mobile number'
        : 'No selected contacts have a sendable email address'
    )
  }

  const size = Math.min(Math.max(Number(batchSize) || MARKETING_SEND_BATCH_SIZE, 1), 200)
  const chunks = chunkArray(leadIds, size)
  if (chunks.length > MAX_BATCH_LISTS_PER_REQUEST) {
    throw new Error(
      `Too many batches (${chunks.length}). Max ${MAX_BATCH_LISTS_PER_REQUEST} lists (${MAX_BATCH_LISTS_PER_REQUEST * size} contacts) per action.`
    )
  }
  ...
  const lists = chunks.map((ids, index) => {
    const list = {
      ...
      description: [
        ...
        `${ids.length} contacts`,
        'Static — pipeline selection',
      ].join(' · '),
      leadIds: ids,
      channel: listChannel,
      audienceType: 'static',
      source: 'pipeline_selection_batches',
      createdByUserId: user.id,
      createdAt: now,
      updatedAt: now,
    }

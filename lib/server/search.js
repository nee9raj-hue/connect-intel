...
import { hasCompleteContact, hasReachableContact } from './leadQuality.js'
...
function applyLeadAccess(lead, store, viewer, rank) {
  ...
  const fullContactVisible = hasReachableContact(lead)
  const isUnlocked = unlockable
    ? previouslyUnlocked || previewUnlocked || fullContactVisible
    : true
...
export function searchStoredLeads(store, filters, count, viewer = null, excludeIds = null) {
  ...
  const contactLeads = excludePipelineIds(
    store.contacts
      ...
      .filter((record) => hasReachableContact(record) && recordMatches(record, filters)),
    excludeIds
  )

  const leads = [...contactLeads]
  ...
  return {
    ...
    netNew: Math.max(leads.length, Math.floor(contactLeads.length * 0.82)),
    provider: 'database',
  }
}

export function getMockLeadsForViewer(store, viewer, filters, count, excludeIds = null) {
  const filtered = excludePipelineIds(
    MOCK_LEADS.filter((lead) => hasReachableContact(lead) && recordMatches(lead, filters)),
    excludeIds
  )

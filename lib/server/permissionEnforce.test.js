...
describe('assertPipelineDeleteAccess', () => {
  it('blocks a rep even when they can edit leads', async () => {
    const user = { id: 'u1', organizationId: 'org1', accountType: 'company', pipelineRole: 'member' }
    const store = {
      users: [user],
      organizations: [{ id: 'org1' }],
    ...
    assert.equal(await userHasOrgPermission(user, 'delete_leads', store), false)
    assert.equal(await userHasOrgPermission(user, 'edit_leads', store), true)
    await assert.rejects(() => assertPipelineDeleteAccess(user, store), /permission/i)
  })

  it('blocks a manager even when they can edit leads', async () => {
    const user = { id: 'u1', organizationId: 'org1', accountType: 'company', pipelineRole: 'manager' }
    const store = {
      users: [user],
      organizations: [{ id: 'org1' }],
  ...
  it('allows org admin to delete', async () => {
    const user = {
      id: 'u1',
      organizationId: 'org1',
      accountType: 'company',
    ...
    const store = {
      users: [user],
      organizations: [{ id: 'org1', ownerUserId: 'u1' }],
      organizationMemberships: [{ userId: 'u1', organizationId: 'org1', role: 'org_admin' }],
    }
    await assertPipelineDeleteAccess(user, store)
  })

  it('blocks marketing executive without edit or delete', async () => {
    const user = {
      id: 'u1',
      organizationId: 'org1',
      accountType: 'company',
      marketingRole: 'marketing_executive',
    }
    const store = {
      users: [user],
      organizations: [{ id: 'org1' }],
      organizationMemberships: [
        { userId: 'u1', organizationId: 'org1', marketingRole: 'marketing_executive' },
      ],
    }
    await assert.rejects(() => assertPipelineDeleteAccess(user, store), /permission/i)
  })

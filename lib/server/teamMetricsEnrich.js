...
/** When filtering team metrics to one rep, align member row with activity-log rollup (same period). */
export function overlayScopedMemberIntelligence(payload, memberUserId, activityResult = {}, usage = {}) {
  if (!payload || !memberUserId || !activityResult?.rollup) return payload
  const mid = String(memberUserId)
  const rollup = activityResult.rollup || emptyRollup()
  const prevRollup = activityResult.prevRollup || emptyRollup()
  const intel = payload.teamIntelligence || {}
  const existing = new Map((intel.members || []).map((m) => [String(m.userId), m]))
  const base = existing.get(mid) || { userId: mid, name: activityResult.memberName || 'Member' }

  const memberRow = {
    ...base,
    emails: rollup.emails ?? 0,
    calls: rollup.calls ?? 0,
    meetings: rollup.meetings ?? base.meetings ?? 0,
    tasksCreated: rollup.tasksCreated ?? 0,
    notes: rollup.notes ?? base.notes ?? 0,
    whatsapp: rollup.whatsapp ?? base.whatsapp ?? 0,
    activitiesTotal: rollup.activitiesTotal ?? 0,
    leadsTouched: rollup.leadsTouched ?? rollup.contactsOpened ?? 0,
    contactsOpened: rollup.contactsOpened ?? rollup.leadsTouched ?? 0,
    hoursInApp: usage.hours ?? base.hoursInApp ?? 0,
  }

  const members = [...(intel.members || []).filter((m) => String(m.userId) !== mid), memberRow]
  ...
  return {
    ...
    teamIntelligence: {
      ...
      members,
      rollup: memberRow,
      comparison: payload.teamIntelligence?.comparison || null,
    },
    summary: {
      ...(payload.summary || {}),
      emailsSent: memberRow.emails,
      callsLogged: memberRow.calls,
      meetingsSet: memberRow.meetings,
      tasksCreated: memberRow.tasksCreated,
      contactsWorked: memberRow.contactsOpened,
    },
  }
}

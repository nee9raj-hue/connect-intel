...
const GATE_STORE_COLLECTIONS = ['users', 'organizations', 'organizationMemberships']

export async function loadMarketingGateContext(sessionUser) {
  const store = await readStore({ only: GATE_STORE_COLLECTIONS })
...
export async function requireMarketingHubAccess(user, store) {
  const check = requireMarketingUser(user)
  if (!check.ok) return { ...check, status: 401 }
  ...
  if (CRM_SOLO_FREE_TIER && (await userHasOrgPermission(user, 'edit_leads', store))) {
    return { ok: true, user }
  }

  try {
    await assertOrgPermission(user, 'access_marketing', store)
    return { ok: true, user }
  } catch (error) {
    const denied = permissionDeniedResponse(error)
    return {
      ok: false,
      status: denied.status,
      error: denied.body.error,
      code: denied.body.code,
      action: denied.body.action,
    }
  }
}

export async function requireMarketingSendAccess(user, store) {
  const hub = await requireMarketingHubAccess(user, store)
  ...
  try {
    await assertOrgPermission(user, 'send_campaigns', store)
    return { ok: true, user }
  } catch (error) {
    const denied = permissionDeniedResponse(error)
    return {
...
export const MARKETING_SEND_ACTIONS = new Set([
  'start',
  'test_send',
  'process_sends',
  'log_whatsapp_sent',
...
/** Archive, pause, or permanently delete a campaign. */
export function canManageMarketingCampaign(row, user) {
  if (!canViewMarketingCampaign(row, user)) return false
  if (user.organizationId && isOrgMarketingAdmin(user)) return true
  return canAccessMarketingAsset(row, user)
}
...
export function filterMarketingRows(rows, user) {
  return asRowArray(rows).filter((row) => canAccessMarketingAsset(row, user))
}
...
export function filterMarketingCampaignsVisible(rows, user) {
  return asRowArray(rows).filter((row) => canViewMarketingCampaign(row, user))
}
...
export function enrichMarketingRows(store, viewer, rows) {
  return asRowArray(rows).map((row) => ({
    ...row,

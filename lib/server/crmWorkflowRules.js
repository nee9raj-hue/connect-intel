...
export function applyWorkflowRules(store, entry, { trigger, previousStatus, newStatus, actor, organizationId, meta = {}, firedRules = null } = {}) {
  ...
  for (const rule of rules) {
    ...
    } else if (rule.trigger === 'lead_created') {
      if (trigger !== 'lead_created') continue
    } else if (rule.trigger === 'no_activity_days') {
      if (trigger !== 'no_activity_days') continue
...
export function seedDefaultWorkflowRules(organizationId) {
  return [
    {
      id: createId('wfr'),
      name: 'Sales opportunity → send proposal task',
      enabled: true,
    ...
    {
      id: createId('wfr'),
      name: 'Onboarding → paperwork note',
      enabled: true,
      trigger: 'status_enter',
      status: 'onboarding',
      actions: [{ type: 'add_note', summary: 'Contract signed — complete credit checks, KYC, and POA paperwork.' }],
    },
  ]
}

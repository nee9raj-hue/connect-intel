...
/** Latest CRM `occurred_at` per actor — one indexed query per rep (never a shared global limit). */
  ...
  await Promise.all(
    ids.map(async (actorId) => {
      try {
        ...
        if (row?.occurred_at) map.set(String(actorId), row.occurred_at)
      } catch {
        // try CRM entry fallback for this actor
      }
    })
  )

...
/** Latest CRM `occurred_at` per actor — strict actor_id only (all orgs, no period cap). */
export async function fetchLastCrmActivityAtByActors(orgId, actorIds = []) {
  const map = new Map()
  const ids = [...new Set((actorIds || []).map(String).filter(Boolean))]
  if (!orgId || !ids.length || !isPipelineActivitiesTableEnabled()) return map

  try {
    const parts = [
      `organization_id=eq.${encodeURIComponent(orgId)}`,
      `actor_id=in.(${ids.map(encodeURIComponent).join(',')})`,
      'order=occurred_at.desc',
      `limit=${Math.min(Math.max(ids.length * 4, ids.length), 2000)}`,
    ...
    for (const row of rows) {
      ...
      if (!aid || map.has(aid) || !row.occurred_at) continue
      map.set(aid, row.occurred_at)
    }
  } catch {
    return map
  }

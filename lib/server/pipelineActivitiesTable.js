...
function buildFilters(orgId, { since, until = Infinity, actorId = null, type = null, types = null } = {}) {
...
export async function countActivityRollup(orgId, { since, until = Infinity, actorId = null } = {}) {
  if (!orgId) return { org: emptyActivityRollup(), perUser: new Map(), total: 0 }

  const base = buildFilters(orgId, { since, until, actorId })
  const [
    total,
    calls,
    emails,
    ...
    supabaseCount(`${TABLE}?${buildFilters(orgId, { since, until, actorId, types: EMAIL_TYPES }).join('&')}`),
    supabaseCount(`${TABLE}?${buildFilters(orgId, { since, until, actorId, types: MEETING_TYPES }).join('&')}`),
    ...
    supabaseCount(
      `${TABLE}?${buildFilters(orgId, { since, until, actorId, types: ['status', 'assignment', 'transfer'] }).join('&')}`
    ),
...
export async function listPipelineActivities(
  orgId,
  { since, until = Infinity, actorId = null, type = null, limit = 50, offset = 0, usersById = new Map() } = {}
) {
  ...
  const parts = [
    ...buildFilters(orgId, { since, until, actorId, type }),
  ...
  const [total, response] = await Promise.all([
    supabaseCount(`${TABLE}?${buildFilters(orgId, { since, until, actorId, type }).join('&')}`),
    fetch(url, {
      headers: { apikey: key, Authorization: `Bearer ${key}` },

...
function isRetryableSupabaseStatus(status) {
  return [408, 425, 500, 502, 503, 504, 520, 521, 522, 524].includes(status)
}
...
function formatSupabaseError(response, data) {
  ...
  if (response.status === 521 || response.status === 522) {
    return (
      `Supabase connection failed (${response.status}). The database may be waking from pause or under heavy load — wait 1–2 minutes and refresh. ` +
      'If this persists, open your Supabase project dashboard → confirm the project is Active (Resume if paused), then try Settings → Infrastructure → Restart project.'
    )
  }
...
export async function supabaseRest(path, options = {}, { attempts = 3, timeoutMs = 20_000, bypassCircuit = false } = {}) {
  if (!isSupabaseEnabled()) {
    throw new Error('Supabase is not configured')
  }
  if (!bypassCircuit) assertCircuitClosed()
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    if (!response.ok) {
      ...
      recordCircuitFailure(lastError)
      throw lastError
    }
...
/** Cheap PostgREST count (no row body). Returns null on timeout/error. */
export async function supabaseRestCount(path, { timeoutMs = 8_000 } = {}) {
...
export async function testSupabaseConnection() {
  ...
  try {
    await supabaseRest('store_collections?select=collection&limit=1', {}, { attempts: 3, timeoutMs: 35_000 })
...
/** Load one collection row (smaller payload than multi-collection fetch). */
export async function fetchStoreCollectionJson(collection) {
  const name = String(collection || '').trim()
  if (!name) return []
  const rows = await supabaseRest(
    `store_collections?select=json&collection=eq.${encodeURIComponent(name)}`,
    {},
    { attempts: 3, timeoutMs: 90_000 }
  )
...
export async function upsertCollection(collection, json) {
  const size = Array.isArray(json) ? json.length : 0
  const timeoutMs = size > 8000 ? 120_000 : size > 2000 ? 60_000 : 20_000
  return supabaseRest(
    'store_collections',
    {
      method: 'POST',
...
export async function fetchAllCollections(path = 'store_collections?select=collection,json', options = {}) {

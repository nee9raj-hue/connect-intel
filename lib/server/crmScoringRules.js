...
function marketingEventsForLead(marketingEvents, entry) {
  const leadId = entry?.lead?.id
  const visitorId = entry?.crm?.visitorId
  if (!leadId && !visitorId) return []
  return (marketingEvents || []).filter((event) => {
    if (!event) return false
    if (leadId && event.leadId === leadId) return true
    if (visitorId && event.visitorId === visitorId) return true
    return false
  })
}
...
function ruleMatches(event, entry, mktEvents) {
  ...
  switch (event) {
    ...
    case 'pricing_page_visit':
      return mktEvents.some((e) => {
        if (e.type !== 'site_pageview') return false
        const url = String(e.url || '').toLowerCase()
        return /\/(pricing|plans|price)(\/|$|\?|#)/.test(url) || /\bpricing\b/.test(url)
      })
    case 'form_submit':
...
export function computeCrmLeadScore(entry, context = {}) {
  ...
  const leadId = entry.lead?.id
  const mkt = leadId || entry.crm?.visitorId ? marketingEventsForLead(marketingEvents, entry) : []

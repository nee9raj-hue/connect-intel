...
import { attachPipelineEntriesToStore, loadPipelineLeadForMutation } from './pipelineShard.js'
...
async function findExistingCaptureMatch(user, fields) {
  ...
  if (linkedinKey) {
    const byLinkedin = result.matches.find((m) => {
      const stored = normalizeLinkedinKey(m.linkedin)
      return stored && (stored.includes(linkedinKey) || linkedinKey.includes(stored))
    })
  ...
  if (first && last) {
    const byName = result.matches.find((m) => {

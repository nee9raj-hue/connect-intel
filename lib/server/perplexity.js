          ...
          messages: [
            {
              role: 'system',
              content: strict
                ? 'You are a B2B data assistant for India. Output ONLY a raw JSON array (no markdown). Each object must include real business email and Indian phone (+91).'
                : 'Output ONLY a raw JSON array (no markdown). Include business email and Indian phone for every contact.',
            },
...
Use publicly listed export/sales contacts. Both email and phone required on every row.`
  }
...
- Skip rows if you cannot provide both email and phone.`
}
...
export async function discoverLeadsWithPerplexity(filters, count = 5) {
    ...
    try {
      ...
      if (leads.length) {
        return {
          ...
          notice:
            'AI discovery found contacts with email and phone. Verify before outreach — saved to your master database when search runs.',
        }
      }

      errors.push(
        focused
          ? 'Retry returned no contacts with both email and phone'
          : `Parsed ${parsed.length} row(s) but none passed email+phone validation`
      )
    } catch (error) {
  ...
  return {
    leads: [],
    error:
      'No contacts with both email and phone were returned. Upload your pipeline CSV in Team → Import, or try one state + keyword "exporter".',
    notice: errors.join(' · '),
  }
}

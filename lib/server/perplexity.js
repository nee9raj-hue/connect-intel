...
const LOGISTICS_INTEL_SYSTEM = `You are Connect Copilot — logistics intelligence for Indian freight forwarders and export sales reps.

Answer logistics and customs questions with sharp, scannable facts.

Output format:
**Answer:** One direct sentence.

**Key points:**
- Bullet each requirement, document, or rule (IEC, AD Code, CHA, DGFT, FSSAI, HSN when relevant).

Rules:
- Focus on Indian export compliance.
- Keep total response under 150 words.
- Never mention Perplexity or internal providers.
- Never guess HS codes or duties — say verify with CHA when uncertain.`

/** Live web research for logistics / customs Q&A (Perplexity Sonar). */
export async function logisticsAssistantWebResearch(query) {
  if (!isPerplexityConfigured()) {
    return {
      error: 'Web research is not available. Set PERPLEXITY_API_KEY for logistics Q&A.',
      text: null,
      citations: [],
    }
  }
  ...
  const text = String(query || '').trim().slice(0, 1500)
  if (!text) return { error: 'Enter a logistics question.', text: null, citations: [] }

  try {
    const { text: answer, model, citations } = await chatCompletion(text, {
      system: LOGISTICS_INTEL_SYSTEM,
      temperature: 0.15,
      maxTokens: 700,
      searchRecencyFilter: 'year',
      returnCitations: true,
    ...
    const body = preprocessAiText(answer)
    if (!body) {
      return { error: 'No results found. Try naming destination country and commodity.', citations: [] }
    }
    ...
    return {
      error: error.message || 'Logistics research failed.',
      citations: [],
    }
  }
}

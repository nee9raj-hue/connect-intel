...
export async function fetchKnowledgeEntities(semanticQuery, { count = 15 } = {}) {
  ...
  const prompt = `Knowledge research: ${query}

Return ONLY a JSON array of up to ${limit} real entries.
Each object when known: company, firstName, lastName, title, product, industry, website, linkedin, exportMarkets, city, season.
Only include real companies/people from credible public sources. Never invent data.`

  try {
    const { text, model } = await chatCompletion(prompt, {
      system:
        'You extract structured business facts from public web sources. Output only a valid JSON array. No markdown, no commentary.',
      temperature: 0.05,
      maxTokens: 14000,
      searchRecencyFilter: 'year',
    })
    ...
    return { leads, model, method: 'knowledge-sonar' }
  } catch (error) {
    return { leads: [], error: error.message || 'Knowledge search failed' }
  }
}

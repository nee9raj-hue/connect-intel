...
const DISCOVERY_SYSTEM = `You are a B2B prospecting researcher for India. Your job is to return REAL companies and contacts from live web search.
...
- Only include email/phone if found in sources — never invent addresses.
- Prefer SMEs: exporters, manufacturers, wholesalers, distributors matching the user's product and location.

Example output shape:
[{"company":"ABC Textiles Pvt Ltd","firstName":"Rajesh","lastName":"Kumar","title":"Export Manager","email":"","phone":"+91-98765-43210","city":"Ludhiana","state":"Punjab","industry":"Textiles","website":"abctextiles.in"}]`
...
function buildWebSearchQuery(naturalQuery, filters = {}) {
...
export async function discoverLeadsWithPerplexity(filters, count = 5, options = {}) {
  ...
  const requestCount = Math.min(limit + 15, 55)
  const errors = []
  const promptOptions = {
    naturalQuery,
  ...
  const webQuery = buildWebSearchQuery(naturalQuery, filters)
  try {
    const webResults = await perplexityWebSearch(webQuery, { maxResults: 18 })
...
const CRM_RESEARCH_SYSTEM = `You are Connect Intel CRM AI — web research for B2B sales teams.

Rules:
- Answer using current web sources. Cite facts; say when uncertain.
- Help with: company research, LinkedIn/profile discovery hints, Amazon/product/market context, industry news, competitor checks.
- B2B and professional use only. No personal harassment, credential theft, or bypassing site terms.
- Never mention Perplexity, Apollo, or internal providers.
- Concise answer: short paragraphs or bullets. Include source URLs inline when available.`

function inferResearchFocus(query) {
  const q = String(query || '').toLowerCase()
  if (/linkedin|linked in/i.test(q)) return 'linkedin'
  if (/amazon\.| on amazon|amazon seller|amazon product/i.test(q)) return 'amazon'
  return 'general'
}
...
export async function crmAssistantWebResearch(query, options = {}) {
  ...
  if (focus === 'linkedin') searchDomainFilter = ['linkedin.com']
  if (focus === 'amazon') searchDomainFilter = ['amazon.in', 'amazon.com', 'amazon.co.uk']

  try {
    const { text: answer, model, citations } = await chatCompletion(text, {

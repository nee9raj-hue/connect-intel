...
async function chatCompletion(prompt, { system, temperature = 0.15, maxTokens = 12000 } = {}) {
...
function parseLinkedinMatchesFromText(text, max = 5) {
  ...
  const urlMatches = String(text || '').match(/https?:\/\/(?:www\.)?linkedin\.com\/in\/[a-zA-Z0-9_-]+/gi)
...
/** Find LinkedIn profile candidates for an existing contact using Perplexity Sonar. */
export async function discoverLinkedinForContact(contact = {}) {
  ...
  const prompt = `Find LinkedIn profile URL(s) for this B2B contact in India. Use live web search.

Contact we have:
- Name: ${name || 'unknown'}
- Job title: ${title || 'unknown'}
- Company: ${company || 'unknown'}
- Work email: ${email || 'unknown'}
- Phone: ${contact.phone || 'unknown'}
- Location: ${location || 'unknown'}
- Industry: ${contact.industry || 'unknown'}
- Company website: ${contact.website || 'unknown'}

Return ONLY a JSON array with up to 5 objects, best match first:
{
  ...
  "reason": "one short sentence why this profile matches"
}

Rules:
- linkedin must be a real linkedin.com/in/ profile URL from search results.
- Do not invent URLs. Skip uncertain profiles.
- Prefer profiles that match company and title when known.
- Rank the most likely person first.`

  try {
    const { text, model } = await chatCompletion(prompt, {
      system:
        'You find LinkedIn profiles for business contacts. Respond with ONLY a valid JSON array. No markdown or commentary.',
      temperature: 0.1,
      maxTokens: 2500,
    })

    const matches = parseLinkedinMatchesFromText(text, 5)
    ...
    if (!matches.length) {
      return {
        ...
        provider: 'perplexity',
        model,
        error: 'No LinkedIn profiles found. Add company and job title, then try again.',
        notice: 'Try spelling the company name as on their website.',
      }
    }

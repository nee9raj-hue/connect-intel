...
- Rank the most likely person first.`
  ...
  try {
    const { text, model, citations = [] } = await chatCompletion(prompt, {
      system:
        'You find LinkedIn profiles for business contacts. Respond with ONLY a valid JSON array. No markdown or commentary. Prefer linkedin.com/in/ URLs from search citations.',
      temperature: 0.1,
      maxTokens: 2500,

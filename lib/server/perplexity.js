...
export async function generatePerplexityEmail(lead, options = {}) {
  ...
  const prompt = `Write a short B2B outreach email for an Indian export/B2B audience.

Tone: ${tone}
Purpose: ${purpose}
Recipient: ${name}, ${lead.title || 'Business contact'} at ${lead.company || 'their company'}
Location: ${location}
Industry: ${lead.industry || 'B2B'}
${lead.email ? `Email on file: ${lead.email}` : ''}
${lead.phone ? `Phone on file: ${lead.phone}` : ''}

Return ONLY valid JSON with keys "subject" and "body".
Body under 180 words, plain text (no markdown). Sign off as "[Your name]" from Connect Intel.`

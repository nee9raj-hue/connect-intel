import crypto from 'node:crypto'
import { SESSION_TTL_MS } from './config.js'

function getSecret() {
  const secret =
    process.env.SESSION_SECRET ||
    process.env.APOLLO_API_KEY ||
    process.env.ANTHROPIC_API_KEY ||
    'connect-intel-dev-only-secret'

  if (
    (process.env.VERCEL || process.env.NODE_ENV === 'production') &&
    !process.env.SESSION_SECRET
  ) {
    throw new Error('SESSION_SECRET is required in production')
  }

  return String(secret)
}

function signBody(body) {
  return crypto.createHmac('sha256', getSecret()).update(body).digest('base64url')
}

export function signSessionToken(user) {
  const exp = Date.now() + SESSION_TTL_MS
  const payload = {
    userId: user.id,
    email: user.email,
    name: user.name,
    company: user.company,
    picture: user.picture || null,
    plan: user.plan || 'free',
    role: user.role || 'member',
    searchesLeft: user.searchesLeft ?? 25,
    creditsPaise: user.creditsPaise ?? 5000,
    authProvider: user.authProvider || 'google',
    organizationId: user.organizationId || null,
    organizationUuid: user.organizationUuid || user.sqlOrganizationId || null,
    accountType: user.accountType || 'individual',
    orgRole: user.orgRole || 'individual',
    isPlatformAdmin: Boolean(user.isPlatformAdmin),
    isOrgAdmin: Boolean(user.isOrgAdmin),
    canSearch: user.canSearch !== false,
    organizationName: user.organizationName || user.company,
    organizationLogoUrl: user.organizationLogoUrl || null,
    onboardingComplete: Boolean(user.onboardingComplete),
    pipelineRole: user.pipelineRole || 'member',
    mobile: user.mobile || null,
    mobileE164: user.mobileE164 || null,
    orgOutboundEmailReady: Boolean(user.orgOutboundEmailReady),
    workspaceFeatures: user.workspaceFeatures && typeof user.workspaceFeatures === 'object' ? user.workspaceFeatures : {},
    exp,
  }

  const body = Buffer.from(JSON.stringify(payload)).toString('base64url')
  return `${body}.${signBody(body)}`
}

export function verifySessionToken(token) {
  if (!token || typeof token !== 'string') return null

  const dot = token.indexOf('.')
  if (dot <= 0) return null

  const body = token.slice(0, dot)
  const signature = token.slice(dot + 1)
  if (signBody(body) !== signature) return null

  try {
    const payload = JSON.parse(Buffer.from(body, 'base64url').toString('utf8'))
    if (!payload?.userId || !payload?.email || !payload?.exp) return null
    if (payload.exp < Date.now()) return null
    return payload
  } catch {
    return null
  }
}

export function payloadToUserRecord(payload) {
  const email = String(payload.email || '').toLowerCase()
  return {
    id: payload.userId,
    email: payload.email,
    name: payload.name,
    company: payload.company,
    picture: payload.picture,
    plan: payload.plan,
    role: payload.role,
    searchesLeft: payload.searchesLeft,
    creditsPaise: payload.creditsPaise,
    authProvider: payload.authProvider,
    organizationId: payload.organizationId,
    organizationUuid: payload.organizationUuid || null,
    accountType: payload.accountType,
    orgRole: payload.orgRole,
    isPlatformAdmin: payload.isPlatformAdmin,
    isOrgAdmin: payload.isOrgAdmin,
    canSearch: payload.canSearch,
    organizationName: payload.organizationName,
    organizationLogoUrl: payload.organizationLogoUrl,
    onboardingComplete: payload.onboardingComplete,
    pipelineRole: payload.pipelineRole,
    mobile: payload.mobile || null,
    mobileE164: payload.mobileE164 || null,
    orgOutboundEmailReady: Boolean(payload.orgOutboundEmailReady),
    workspaceFeatures: (() => {
      const fromToken =
        payload.workspaceFeatures && typeof payload.workspaceFeatures === 'object'
          ? payload.workspaceFeatures
          : {}
      if (
        email.endsWith('@xindus.net') ||
        String(payload.organizationName || '')
          .toLowerCase()
          .includes('xindus')
      ) {
        return { freightDealRfq: true, ...fromToken }
      }
      return fromToken
    })(),
  }
}

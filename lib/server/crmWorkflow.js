...
import { DEAL_STAGES, defaultCrm, isClosedDealStage } from './crm.js'
import { normalizeCrmLeadStatus, leadStatusToStandardDealStage } from '../crmLeadStatuses.js'
...
import { resolveFreightDealCurrency } from '../freightDeal.js'
import { applyDealMilestonePatch, pickDealMilestones } from '../dealMilestones.js'
import { buildAutoDealName, cloneDealPayloadForDuplicate } from '../dealNaming.js'
...
export function normalizeDealRow(raw = {}, now = new Date().toISOString()) {
  ...
  const freight = raw.freight ? normalizeFreightRfq(raw.freight) : null
  const milestones = pickDealMilestones(raw)
  return {
    id: raw.id || createId('deal'),
    name: String(raw.name || 'Deal').slice(0, 200),
    stage,
    amount: normalizeDealAmount(raw.amount),
    currency: resolveFreightDealCurrency({ ...raw, freight }),
    expectedCloseDate: raw.expectedCloseDate || null,
    queryReceivedOn: milestones.queryReceivedOn,
    ratesQuotedOn: milestones.ratesQuotedOn,
    bookedOn: milestones.bookedOn,
    wonOn: milestones.wonOn,
    lostOn: milestones.lostOn,
    wonAt: raw.wonAt || (stage === 'won' ? raw.updatedAt || raw.createdAt || null : null),
    lostAt: raw.lostAt || (stage === 'lost' ? raw.updatedAt || raw.createdAt || null : null),
...
function migrateLegacyDealFields(crm) {
  ...
  let stage = leadStatusToStandardDealStage(crm?.status)
...
export function normalizeExtendedCrm(crm) {
  ...
  const normalized = {
    ...
    ...extra,
    status: extra.status ? normalizeCrmLeadStatus(extra.status) : base.status,
...
export function duplicateDeal(crm, dealId, { company, stage } = {}, creator) {
  const normalized = normalizeExtendedCrm(crm)
  const source = normalized.deals.find((d) => d.id === dealId)
  if (!source) return { crm: normalized, deal: null }
  const copy = cloneDealPayloadForDuplicate(source)
  const name = buildAutoDealName({ company, existingDeals: normalized.deals })
  const notePrefix = `Duplicated from ${source.name}`
  const notes = copy.notes ? `${notePrefix}\n${copy.notes}` : notePrefix
  const nextStage =
    stage ||
    (source.stage === 'won' || source.stage === 'lost' ? 'rfq' : source.stage) ||
    'new'
  return addDeal(
    normalized,
    {
      ...
      expectedCloseDate: copy.expectedCloseDate,
      notes,
      freight: copy.freight,
    },
    creator
  )
}
...
export function updateDeal(crm, dealId, patch, actor) {
  ...
  applyDealMilestonePatch(deal, patch)
  deal.currency = resolveFreightDealCurrency(deal)
...
export function deleteDeal(crm, dealId, actor) {
  const normalized = normalizeExtendedCrm(crm)
  const deal = normalized.deals.find((d) => d.id === dealId)
  if (!deal) return normalized

  normalized.deals = normalized.deals.filter((d) => d.id !== dealId)

  if (normalized.primaryDealId === dealId) {
    const nextPrimary =
      normalized.deals.find((d) => !isClosedDealStage(d.stage)) || normalized.deals[0] || null
    normalized.primaryDealId = nextPrimary?.id || null
  }

  if (!normalized.deals.length) {
    normalized.primaryDealId = null
    normalized.dealValue = null
    normalized.expectedCloseDate = null
  }

  return finishDealMutation(normalized, {
    type: 'status',
    summary: `Deal deleted: ${deal.name}`,
    userId: actor.userId,
    userName: actor.name,
    meta: { dealId: deal.id, dealStage: deal.stage, deleted: true },
  })
}
...
export function addMeeting(crm, payload, creator) {
  ...
  const note = String(meeting.notes || '').trim()
  const summary = [
    `${typeLabel} scheduled: ${meeting.title}`,
  ...
  return {
    crm: appendActivity(normalized, {
      ...
      userId: creator.userId,
      userName: creator.name,
      meta: { meetingId: meeting.id, notes: note || null },
    }),
    meeting,
  }
}
...
/** Read tasks/meetings without full CRM normalize (hot path for large pipelines). */
function readCalendarCrmFields(crm) {
  if (!crm || typeof crm !== 'object') {
    return { tasks: [], meetings: [], nextFollowUpAt: null, lastCommunicationSummary: '' }
  }
  return {
    tasks: Array.isArray(crm.tasks) ? crm.tasks : [],
    meetings: Array.isArray(crm.meetings) ? crm.meetings : [],
    nextFollowUpAt: crm.nextFollowUpAt || null,
    lastCommunicationSummary: crm.lastCommunicationSummary || '',
  }
}

function entryMayHaveCalendarInRange(entry, rangeStart, rangeEnd) {
  const crm = entry?.crm
  ...
  if (Array.isArray(tasks)) {
    for (const task of tasks) {
      ...
      if (!Number.isNaN(at) && at >= rangeStart && at <= rangeEnd) return true
    }
  }
...
export function collectCalendarEvents(entries, user, { fromMs, toMs } = {}) {
  ...
  const events = []
  const canSeeAll = Boolean(user?.isOrgAdmin || user?.orgRole === 'org_admin')

  for (const entry of entries) {
    if (!entryMayHaveCalendarInRange(entry, rangeStart, rangeEnd)) continue

    const crm = readCalendarCrmFields(entry.crm)
    ...
    for (const task of crm.tasks) {
      ...
      if (!canSeeAll && !userCanSeeCalendarEvent(user, meta, entry)) continue
    ...
    if (crm.nextFollowUpAt) {
      ...
      if (!Number.isNaN(at) && at >= rangeStart && at <= rangeEnd) {
        if (canSeeAll || userCanSeeCalendarEvent(user, { assignedToUserId: entry.assignedToUserId }, entry)) {
          events.push({

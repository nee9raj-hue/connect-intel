...
function faqFallbackReply(message, ctx) {
  ...
  if (/credit|balance|rupee/i.test(message)) {
    return {
      reply:
        'AI prospect credits and lead search are a separate Connect Intel product — not part of the core CRM workspace. For CRM billing and seats, open Team → Billing.',
      actions: [{ type: 'navigate', panel: 'team', teamTab: 'billing', label: 'Open billing' }],

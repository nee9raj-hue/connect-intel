...
function truncateThreadForEdit(thread, editFromMessageId) {
  if (!editFromMessageId || !thread?.messages?.length) return false
  const idx = thread.messages.findIndex((m) => m.id === editFromMessageId)
  if (idx < 0) return false

  thread.messages = thread.messages.slice(0, idx)
  thread.lastDiscovery = null

  if (thread.copilotState) {
    thread.copilotState.knowledgeContext = null
    thread.copilotState.researchSession = null
    thread.copilotState.focus = {
      company: null,
      person: null,
      lead: thread.copilotState.focus?.lead || null,
      selectedResult: null,
    }
  }
...
export async function processAssistantMessage(store, user, message, uiContext = {}) {
  ...
  const editFromMessageId = uiContext.editFromMessageId || null
  if (editFromMessageId) {
    truncateThreadForEdit(thread, editFromMessageId)
  }
  ...
  return {
    ...
    people: assistantMsg.people,
    understanding: assistantMsg.understanding,
    nbsa: assistantMsg.nbsa,
    planSteps: assistantMsg.planSteps,
    discoveryMeta: assistantMsg.discoveryMeta,
    emailDraft: assistantMsg.emailDraft,
    rfqPrefill: assistantMsg.rfqPrefill,
...
export function getAssistantHistory(store, userId) {
  ...
  return {
    ...
    messages: (thread.messages || []).map((m) => ({
      ...
      understanding: m.understanding || null,
      nbsa: m.nbsa || null,
      planSteps: m.planSteps || null,
      discoveryMeta: m.discoveryMeta || null,
      rfqPrefill: m.rfqPrefill || null,

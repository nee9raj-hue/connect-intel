...
/** Parse pasted RFQ / WhatsApp shipment text into LogiCopilot JSON schema. */
export async function parseRfqPasteWithGemini(rawMessage) {
  if (!isGeminiConfigured()) return null

  const text = String(rawMessage || '').trim().slice(0, 4000)
  if (!text) return null

  const prompt = `Extract shipping RFQ parameters from this message. Never guess — use null or empty string when not stated.
...
{
  "is_rfq": true,
  "commodity": "type of goods or empty string",
  "hsn_code": "4-8 digit HSN if mentioned or empty string",
  "transport_mode": "Air" | "Ocean" | "Air + Ocean" | null,
  "incoterm": "THREE_LETTER_CODE or null",
  "gross_weight_kg": number or null,
  "box_count": number or null,
  "dimensions": { "length": number or null, "width": number or null, "height": number or null },
  "origin_city": "city or empty string",
  "origin_pincode": "pin/zip or empty string",
  "destination_country": "country or empty string",
  "destination_city": "city or empty string"
}

Rules:
- Convert dimensions to cm when units are inches (×2.54).
- Normalize incoterms to EXW, FOB, CIF, DDP, etc.
- gross_weight_kg is total shipment weight in kg when stated.`

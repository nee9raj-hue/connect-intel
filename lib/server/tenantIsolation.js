...
import { populateLeadErp } from './leadErpPopulate.js'
import { lastOrderCreatedAtFromErp, laterIsoTimestamp } from '../leadLastOrder.js'
...
export function mergeLeadForTenant(store, user, entry) {
  ...
  const erp = populateLeadErp(store, user, entry)
  const crm = sanitizeCrmForTenant(store, user, merged.crm, entry)
  ...
  return {
    ...merged,
    erp,
    crm: lastOrderCreatedAt ? { ...crm, lastOrderCreatedAt } : crm,
  }
}

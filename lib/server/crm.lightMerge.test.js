...
test('mergeLeadForClientLight keeps ERP tags off CRM tagIds', async () => {
  const lead = mergeLeadForClientLight({
    savedAt: '2026-01-01T00:00:00.000Z',
    lead: { id: 'l3', company: 'Acme' },
    crm: { status: 'new', tagIds: ['keep'] },
    erp: { erpTags: [{ name: 'Dummy', color: '#B4876E', type: 'manual' }] },
  })
  assert.deepEqual(lead.crm.tagIds, ['keep'])
  assert.deepEqual(lead.erpTags, [{ name: 'Dummy', color: '#b4876e', type: 'manual' }])
})

...
export function pipelineCrmStatusPostgrestFilter(status) {
  ...
  if (!st || st === 'all' || !CRM_STATUSES.includes(st)) return null
  return `entry->crm->>status=eq.${encodeURIComponent(st)}`
}
...
/** No assignee on pipeline row (claim pool — matches UI assignedToUserId). */
export function pipelineUnassignedAssigneePostgrestFilter() {
  return 'or=(entry->assignedToUserId.is.null,entry->>assignedToUserId.eq.)'
}

/** Assigned leads only — excludes claim pool from default pipeline/search. */
export function pipelineExcludeUnassignedAssigneePostgrestFilter() {
  return 'not.or=(entry->assignedToUserId.is.null,entry->>assignedToUserId.eq.)'
}

/** Rep inbox: leads explicitly assigned to this user. */
export function pipelineRepMyAssignedPostgrestFilter(userId) {
  const enc = encodeURIComponent(String(userId || '').trim())
  if (!enc) return pipelineExcludeUnassignedAssigneePostgrestFilter()
  return `entry->>assignedToUserId.eq.${enc}`
}
...
export function filterPipelineEntries(
  ...
  if (!assigneeUserId) {
    list = list.filter((e) => !isPipelineLeadUnassigned(e))

...
/** PostgREST filter for pipeline Owner column (assigned rep only). */
...
/** Match pipeline entry to Owner filter — same field as the pipeline table Owner column. */
export function pipelineEntryMatchesAssignee(entry, assigneeUserId) {
  ...
  if (id === '__unassigned__') return isPipelineLeadUnassigned(entry)
  return String(entry?.assignedToUserId || '') === id
}

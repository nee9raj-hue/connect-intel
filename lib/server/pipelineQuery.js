...
export function pipelineCrmStatusPostgrestFilter(status, { pipelineTrack, crmStageIds, crmStages } = {}) {
...
export function pipelineLeadStatusColumnFilter(status, { pipelineTrack, crmStageIds, crmStages } = {}) {
  if (collectCrmStageFilterIds({ crmStageIds, crmStages }).length) return null
  const st = String(status || 'all').trim()
  if (st && st !== 'all' && isCrmLeadStatusFilter(st)) {
    const inn = crmLeadStatusPostgrestIn(st)

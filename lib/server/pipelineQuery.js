...
import { collectCrmStageFilterIds, pipelineTrackSqlAliases, boardBucketForLeadStatus, freightBoardColumnIds } from '../crmPipelineFlow.js'
...
export function boardPipelineSlice(entries, defaultPerColumn = 50, columnLimits = {}, columnIds = CRM_STATUSES) {
  const ids = Array.isArray(columnIds) && columnIds.length ? columnIds : CRM_STATUSES
  const columns = Object.fromEntries(ids.map((s) => [s, []]))
  const totals = Object.fromEntries(ids.map((s) => [s, 0]))
  const sorted = entries
    .slice()
    .sort((a, b) => new Date(b.savedAt).getTime() - new Date(a.savedAt).getTime())

  for (const entry of sorted) {
    const bucketKey = boardBucketForLeadStatus(entry.crm?.status, ids)
    ...
    if (bucket && bucket.length < max) bucket.push(entry)
  }

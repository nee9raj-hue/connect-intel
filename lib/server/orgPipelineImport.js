...
export function importOrgPipeline(
  store,
  { rows, datasetType = 'general', actor, organizationId, addToPipeline = true, assignToActor = false }
) {
  ...
  if (addToPipeline && organizationId) {
    ...
    for (const contact of newContacts) {
      ...
      nextStore.savedLeads.push({
        ...
        assignedToUserId: assignToActor
          ? actor.id
          : row?.assigned_to || row?.assignedTo
            ? String(row.assigned_to || row.assignedTo)
            : actor.id,

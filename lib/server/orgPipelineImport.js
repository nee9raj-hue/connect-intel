...
import { resolveAssigneeUserIdForOrg, importRowHasAssigneeHint } from './importAssignee.js'
import {
  normalizeLeadTagIds,
  resolveOrgLeadTagIdsFromNames,
} from './orgLeadTags.js'
import { parseEmailConsentValue, applyCommercialEmailConsent } from '../emailConsent.js'
...
import { erpFromImportRow, mergeLeadErp } from '../leadErp.js'
...
function pipelineStatusFromRow(row) {
  return explicitImportLeadStatus(row) || 'unqualified'
}

function commercialEmailConsentFromRow(row) {
  ...
  return parseEmailConsentValue(raw)
}

function mergeImportEmailConsent(lead, row, { isUpdate = false } = {}) {
  const parsed = commercialEmailConsentFromRow(row)
  if (parsed === true) {
    return applyCommercialEmailConsent(lead, { granted: true, source: 'import' })
  }
  if (parsed === false) {
    return applyCommercialEmailConsent(lead, { granted: false, source: 'import' })
  }
  if (isUpdate) return lead
  return applyCommercialEmailConsent(lead, { granted: false, source: 'import' })
}

function tagIdsFromNamedColumns(row, store, organizationId, { actorUserId = null, createMissing = false } = {}) {
  const raw = getValue(row, ['lead_tags', 'tags', 'tag_names', 'tag', 'lead_tag'])
  if (!raw) return []
  return resolveOrgLeadTagIdsFromNames(store, organizationId, raw.replace(/\|/g, ','), {
    createMissing,
    actorUserId,
  })
}
...
export function importOrgPipeline(
  ...
  const org =
    organizationId && addToPipeline
      ? (nextStore.organizations || []).find((o) => o.id === organizationId) || null
      : null
  ...
  if (addToPipeline && organizationId) {
    for (const rawRow of rows) {
      ...
      if (existing) {
        ...
        const nextCrm = mergeImportedCrmOntoExisting(prevCrm, {
          status: explicitImportLeadStatus(row),
          notes:
            row?.notes !== undefined && String(row.notes).trim() ? String(row.notes).trim() : null,
        ...
        existing.lead = mergeImportEmailConsent(
          mergeImportedLeadOntoExisting(existing.lead, {
            ...lead,
            savedAt: now,
            inPipeline: true,
          }),
        ...
        const erpOverlay = erpFromImportRow(row)
        if (erpOverlay) {
          existing.lead.erp = mergeLeadErp(existing.lead.erp, erpOverlay)
          existing.erp = mergeLeadErp(existing.erp, erpOverlay)
        }
        existing.crm = nextCrm
        stampLastOrderCreatedAt(existing, lastOrderCreatedAtFromErp(existing.erp))
        if (assignToActor) {
          existing.assignedToUserId = actor.id
        } else if (importRowHasAssigneeHint(row)) {
        ...
        if (org) {
          assertWithinPlanLimits(nextStore, org, { extraLeads: 1 })
        }
        ...
        const newLead = mergeImportEmailConsent(
          {
            ...lead,
            savedAt: now,
            inPipeline: true,
        ...
        const erpOverlay = erpFromImportRow(row)
        if (erpOverlay) newLead.erp = mergeLeadErp(newLead.erp, erpOverlay)
        const created = {
          id: createId('saved'),
          userId: actor.id,
          organizationId,
        ...
        stampLastOrderCreatedAt(created, lastOrderCreatedAtFromErp(created.erp))
        nextStore.savedLeads.push(created)
        pipelineAdded += 1
      }
    }
  }

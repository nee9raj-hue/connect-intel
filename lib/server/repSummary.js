import { normalizeCrmLeadStatus, isClosedCrmLeadStatus } from '../crmLeadStatuses.js'
...
export function computeRepLeadStats(entries, userId) {
  ...
  for (const entry of mine) {
    ...
    if (normalizeCrmLeadStatus(crm.status) === 'opportunity' || crm.nextFollowUpAt) followups += 1
...
function countsFromSql(byStatus = []) {
  ...
  for (const row of byStatus) {
    const id = normalizeCrmLeadStatus(st)
    const n = Number(row.count) || 0
    if (id === 'opportunity') followups += n
    if (id === 'onboarding') won += n
    if (!isClosedCrmLeadStatus(id)) open += n
  }

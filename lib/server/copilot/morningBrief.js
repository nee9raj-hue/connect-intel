...
export async function buildMorningBrief(store, user) {
  ...
  for (const entry of entries) {
    ...
    if (status === 'at_risk' || status === 'churned') {
      atRisk.push({
        name,
        company,
        leadId,
        reason: status === 'churned' ? 'Churned' : 'At risk',
      })
    } else {
      const dueRaw = crm.nextFollowUpAt || crm.followUpAt
      if (dueRaw) {
  ...
  const reply = buildStructuredReply({
    ...
    nextStep:
      ctx.overdueFollowUps > 0
        ...
        : 'Open your first due lead and log the call or email.',
  })

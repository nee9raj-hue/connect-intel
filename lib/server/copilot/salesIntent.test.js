import { describe, it } from 'node:test'
import assert from 'node:assert/strict'
import { inferSalesIntent, isDiscoveryRefinement, isFreshDiscoveryQuery, discoveryCountForDepth } from './salesIntent.js'
import { planCopilotTurn } from './planner.js'

describe('inferSalesIntent', () => {
  it('detects vague lead gen: toy exporters Delhi NCR', () => {
    const intent = inferSalesIntent('I need toys exporter from Delhi NCR')
    assert.equal(intent.isLeadGeneration, true)
    assert.equal(intent.category, 'lead_generation')
    assert.ok(intent.filters.keywords || intent.naturalQuery)
  })

  it('detects CRM follow-up intent', () => {
    const intent = inferSalesIntent('Who needs follow-up today?')
    assert.equal(intent.isCrmFollowUp, true)
    assert.equal(intent.category, 'crm_follow_up')
  })

  it('asks industry clarification for vague location + export query', () => {
    const intent = inferSalesIntent('I need exporters from Delhi exporting to USA')
    assert.equal(intent.isLeadGeneration, true)
    assert.equal(intent.needsIndustryClarification, true)
    assert.equal(intent.needsExportClarification, false)
  })

  it('does not ask industry clarification when product is specific', () => {
    const intent = inferSalesIntent('I need toys exporter from Delhi NCR exporting to USA')
    assert.equal(intent.needsIndustryClarification, false)
  })

  it('sets research depth', () => {
    assert.equal(discoveryCountForDepth('quick'), 6)
    assert.equal(discoveryCountForDepth('deep'), 15)
    assert.equal(discoveryCountForDepth('standard'), 10)
  })
})

describe('planCopilotTurn lead discovery', () => {
  it('does not route toy exporters to lead discovery', () => {
    const plan = planCopilotTurn('I need toys exporter from Delhi NCR', { copilotTab: 'copilot' })
    assert.equal(plan.runLeadDiscovery, false)
    assert.equal(plan.runWeb, false)
  })

  it('routes who needs follow-up to CRM pipeline query', () => {
    const plan = planCopilotTurn('Who needs follow-up today?', { copilotTab: 'crm' })
    assert.equal(plan.intents.crmFollowUp, true)
    assert.equal(plan.runWeb, false)
    assert.equal(plan.runLeadDiscovery, false)
  })

  it('blocks lead discovery when industry clarification needed', () => {
    const plan = planCopilotTurn('exporters from Delhi exporting to USA', { copilotTab: 'copilot' })
    assert.equal(plan.salesIntent.needsIndustryClarification, true)
    assert.equal(plan.runLeadDiscovery, false)
  })
})

describe('isFreshDiscoveryQuery', () => {
  it('detects FBA exporter list with city and amazon.com', () => {
    const msg =
      'I need FBA exporter list from Jaipur, who are selling on amazon.com with contact information or linkedin id'
    assert.equal(isFreshDiscoveryQuery(msg), true)
  })

  it('does not treat contextual contact follow-up as fresh discovery', () => {
    assert.equal(isFreshDiscoveryQuery('I need his linkedin profile'), false)
    assert.equal(isFreshDiscoveryQuery('get email for the second company'), false)
  })
})

describe('isDiscoveryRefinement', () => {
  it('detects filter-only follow-up', () => {
    const thread = {
      lastDiscovery: {
        companies: [{ company: 'ABC Toys', exportMarkets: 'USA UK' }],
      },
    }
    assert.equal(isDiscoveryRefinement('only exporting to USA', thread), true)
  })
})

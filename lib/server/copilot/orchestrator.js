...
import { processLeadGenerationReply } from './leadGeneration.js'
...
export async function processCopilotTurn(store, user, message, uiContext = {}, { history = [], thread = null } = {}) {
  ...
  if (!result && plan.runWeb && (plan.runCrmCrossRef || plan.intents?.leadGeneration)) {
    result = processLeadGenerationReply({ message: text, webRaw, crmSearch, plan })
  }
  ...
  if (!result) {
    result = {
      reply:
        '**Answer:** I can search your CRM, research companies on the web, and guide you through Connect Intel.\n\n**Next step:** Ask something specific — e.g. lead counts, a company name, or how Pipeline bulk email works.',
      source: 'default',

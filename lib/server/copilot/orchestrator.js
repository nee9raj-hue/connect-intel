...
import { isDiscoveryRefinement, isFreshDiscoveryQuery } from './salesIntent.js'
...
import { understandCopilotQuery, applyQueryUnderstanding } from './queryUnderstanding.js'
import { runEntityResearchAgent } from './entityResearchAgent.js'
import { runKnowledgeAgent } from './knowledgeAgent.js'
import { validateCopilotResponse, filterValidatedCompanies } from './responseValidator.js'
import { runRfqParseAgent } from './rfqParseAgent.js'
import { runLogisticsIntelAgent } from './logisticsIntelAgent.js'
import { runMessagingCopilotAgent } from './messagingCopilotAgent.js'
import { attachCopilotPipelineStore, copilotTurnFailureReply } from './pipelineStore.js'

const META_COLLECTIONS = ['users', 'organizations', 'organizationMemberships']

const PRODUCTIVE_FALLBACK = {
  reply:
    '**Answer:** Ask about your book — follow-ups, stage counts, at-risk accounts, or a draft on the open lead.\n\n**Next step:** Try **“who needs follow-up today?”** or **“brief me”**.',
  source: 'copilot',
  sources: [{ type: 'copilot', label: 'Connect Copilot' }],
  confidence: 'medium',
  suggestions: ['Who needs follow-up today?', 'Brief me', 'How many leads by stage?'],
  actions: [{ type: 'navigate', panel: 'pipeline', label: 'Open Pipeline' }],
}

export async function processCopilotTurn(store, user, message, uiContext = {}, extras = {}) {
  try {
    const pipelineStore = await attachCopilotPipelineStore(store, user)
    return await runCopilotTurn(pipelineStore, user, message, uiContext, extras)
  } catch (err) {
    console.error('copilot turn failed:', err)
    return copilotTurnFailureReply(err)
  }
}

async function runCopilotTurn(store, user, message, uiContext = {}, { history = [], thread = null } = {}) {
  ...
  const queryUnderstanding = understandCopilotQuery(text, state)
  salesIntent = applyQueryUnderstanding(salesIntent, queryUnderstanding)
  salesIntent = enrichSalesIntentFromState(salesIntent, state, resolved, entities)
  const plan = planCopilotTurn(text, uiContext, { salesIntent, resolved, understanding: queryUnderstanding })
  ...
  if (
    ...
    resolved.contactFromResearch &&
    !isFreshDiscoveryQuery(text)
  ) {
  ...
  if (plan.intents?.crmAtRisk) {
    const atRisk = retrieveAtRiskAccounts(store, user)
    return finalizeCopilotResult({
      result: formatAtRiskReply(atRisk),
      plan,
      text,
      uiContext,
      thread,
  ...
  if (plan.runRfqParse) {
    const rfqResult = await runRfqParseAgent({ message: text, user, store, uiContext })
    if (rfqResult) {
      return finalizeCopilotResult({
        result: rfqResult,
        plan,
  ...
  if (plan.runLogisticsIntel) {
    const logisticsResult = await runLogisticsIntelAgent({ message: text })
    if (logisticsResult) {
      return finalizeCopilotResult({
  ...
  if (plan.runMessagingCopilot) {
    const messagingResult = await runMessagingCopilotAgent({ store, user, message: text, plan })
    if (messagingResult) {
      return finalizeCopilotResult({
  ...
  if (plan.salesIntent?.needsIndustryClarification && !plan.runKnowledgeLookup) {
  ...
  if (plan.salesIntent?.needsExportClarification && !plan.runKnowledgeLookup) {
  ...
  if (plan.runEntityResearch) {
    const entityResult = await runEntityResearchAgent({
      store,
      user,
      message: text,
      understanding: queryUnderstanding,
...
function finalizeCopilotResult({
  ...
  const understanding =
    queryUnderstanding || plan?.salesIntent?.queryUnderstanding || turnCtx?.state?.knowledgeContext || null

  if (result?.companies?.length && understanding) {
    result = {
      ...result,
      companies: filterValidatedCompanies(result.companies, understanding),
    }
  }

  const validation = validateCopilotResponse(result, plan, understanding, turnCtx?.state)
  if (!validation.valid && result?.companies?.length && understanding) {
    result = {
      ...result,
      companies: filterValidatedCompanies(result.companies, understanding),
    }
  ...
  const nbsa =
    result.source === 'knowledge_lookup'
      ? null
      : result.nbsa ||
        buildNBSA({
          plan,
  ...
  if (thread) {
    appendCopilotLog(thread, {
      ...
      plan: {
        ...
        runKnowledgeLookup: plan.runKnowledgeLookup,
  ...
  return {
    ...
    rfqPrefill: result.rfqPrefill || null,

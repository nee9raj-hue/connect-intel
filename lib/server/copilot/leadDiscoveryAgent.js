...
export async function runLeadDiscoveryAgent({ store, user, message, salesIntent, thread }) {
  ...
  const rawLeads = (discovery.leads || []).filter(isRealCompany)
  ...
  const nbsa = buildNBSA({ plan: { intentCategory: 'lead_generation' }, companies, discoveryMeta: { total: companies.length, inCrm, newLeads: companies.length - inCrm } })
  const reply = buildV3Reply({
    understanding: `I understand you're looking for ${understanding}.`,
    answer: buildDiscoveryAnswer({ companies, product, location, inCrm, intent }),
    crmFindings: [
      inCrm > 0

...
/** New list/search with geography or marketplace — not a follow-up on prior research. */
export function isFreshDiscoveryQuery(text) {
  const lower = String(text || '').trim().toLowerCase()
  if (!lower) return false

  const isContextualFollowUp =
    /\b(his|her|their|that company|this company|the company|that founder|the founder|that exporter|this exporter)\b/i.test(
      lower
    ) ||
    /\b(?:the\s+)?(first|second|third|fourth|fifth|1st|2nd|3rd|4th|5th)\s*(?:one|company|exporter|lead|prospect)?\b/i.test(
      lower
    )

  if (isContextualFollowUp) return false

  const hasListIntent =
    /\b(?:i need|need|looking for|want|find me|get me|show me|list|find|search|discover)\b/i.test(lower) &&
    /\b(?:exporter|exporters|manufacturer|manufacturers|supplier|suppliers|seller|sellers|companies|leads?)\b/i.test(
      lower
    )

  const hasGeo =
    CITY_RE.test(lower) || /\b(?:from|in)\s+[a-z][a-z\s]{2,28}\b/i.test(lower)

  const hasMarketplace =
    /\bamazon(?:\s+fba|\.com)?\b/i.test(lower) ||
    (/\bfba\b/i.test(lower) &&
      /\b(?:seller|selling|exporter|export|amazon)\b/i.test(lower))

  if (hasMarketplace && /\b(?:list|exporter|exporters|seller|sellers|companies|leads?)\b/i.test(lower)) {
    return true
  }

  if (hasListIntent && (hasGeo || hasMarketplace)) return true

  return /\bfba\b/i.test(lower) && hasGeo
}
...
export function inferSalesIntent(message, uiContext = {}) {
  ...
  const isStalled = /\b(stalled deals?|stale leads?|no activity)\b/i.test(lower)
  const isAtRisk = /\b(at.?risk|churned|declining accounts?)\b/i.test(lower)

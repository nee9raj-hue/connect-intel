import { tryGroundedAssistantReply, tryHighConfidenceFaq } from '../assistantCrmFacts.js'
...
export async function retrieveCrmFacts(store, user, message, uiContext) {
  return tryGroundedAssistantReply(message, store, user, uiContext)
}
...
export async function retrieveCrmSearch(store, user, query, { limit = 6 } = {}) {
  ...
  try {
    ...
    console.warn('copilot CRM search failed:', err?.message || err)
    return { results: [], query: q, error: String(err?.message || err) }
  }
}
...
export async function retrieveLeadContext(store, user, leadId, metaStore) {
  ...
  return {
    ...
    overdueFollowUp: Boolean(
      (crm.nextFollowUpAt || crm.followUpAt) &&
        new Date(crm.nextFollowUpAt || crm.followUpAt) < new Date()
    ),

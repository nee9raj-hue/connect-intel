...
import {
  ...
  saveResearchSession,
  clearStaleResearchForFreshQuery,
...
describe('conversationState', () => {
  ...
  it('does not contact-lookup when user asks for a fresh FBA exporter list', () => {
    const msg =
    ...
    const state = createEmptyConversationState('t5')
    state.knowledgeContext = { entity: 'Shark Tank India', entityType: 'TV_SHOW', season: 'all' }
    state.researchSession = {
      originalCompanies: [
        {
          company: 'BluePine Industries',
          linkedinUrl: 'linkedin.com/in/aditimadan',
          contactName: 'Aditi Madan',
        },
      ],
    }
    state.focus.company = { name: 'BluePine Industries' }

    clearStaleResearchForFreshQuery(state, msg)
    const resolved = resolveReferences(msg, state)

    assert.notEqual(resolved.inferredIntent, 'contact_lookup')
    assert.equal(resolved.contactFromResearch, null)
  })
})

...
import { isFreshDiscoveryQuery } from './salesIntent.js'
...
function understandFromMessage(text) {
  ...
  const city = extractCityScope(text)
  ...
  if (marketplace) {
    return buildMarketplaceLeadUnderstanding(marketplace.name, text, {
      industry,
      exporterFilter,
      city,
      wantsLinkedIn,
...
function detectMarketplace(text) {
  ...
  if (/\bamazon\.com\b/i.test(text)) {
    return { name: 'Amazon.com' }
  }
  if (/\bamazon\s+fba\b/i.test(text) || (/\bfba\b/i.test(lower) && /\bamazon\b/i.test(lower))) {
    return { name: 'Amazon FBA' }
  }
  if (/\bfba\b/i.test(lower) && /\b(?:seller|selling|exporter|export)\b/i.test(lower)) {
    return { name: 'Amazon FBA' }
  }
  return matchFirst(text, MARKETPLACE_ENTITIES)
}

function extractCityScope(text) {
  const fromMatch = String(text || '').match(
    /\b(?:from|in)\s+([A-Za-z][A-Za-z\s]{2,24}?)(?:\s*,|\s+who|\s+with|\s+selling|\s+that|\s*$)/i
  )
  if (fromMatch?.[1]) return fromMatch[1].trim()
  const cityMatch = String(text || '').match(
    /\b(jaipur|mumbai|delhi|bangalore|bengaluru|chennai|ahmedabad|punjab|surat|kolkata|noida|gurugram|gurgaon|pune|ludhiana|hyderabad|indore|kochi)\b/i
  )
  return cityMatch?.[1] || ''
}
...
function mergeStateIntoUnderstanding(text, state, partial) {
  ...
  if (isFreshDiscoveryQuery(text)) return partial
  ...
  const isFollowUp =
    /\b(contestants?|information|info|founders?|names?|seasons?|them|those|these|continuing|same (?:list|show))\b/i.test(
      lower
    ) ||
    (/\blinkedin\b/i.test(lower) &&
      /\b(contestants?|founders?|their|them|those|these|shark)\b/i.test(lower))
...
function buildMarketplaceLeadUnderstanding(marketplace, text, { industry, exporterFilter, city, wantsLinkedIn }) {
  const cityPhrase = city ? ` in ${city}` : ''
  const contactPhrase = wantsLinkedIn ? ' with public contact or LinkedIn' : ''
  return {
    mode: 'entity_research',
    intent: 'lead_generation',
    domain: 'marketplace',
    ...
    target: 'Sellers',
    season: null,
    filters: { exporter: Boolean(exporterFilter), industry: industry || null, city: city || null },
    requiredOutput: wantsLinkedIn ? [...LEAD_OUTPUT, 'Email'] : LEAD_OUTPUT,
    semanticQuery: buildMarketplaceSemanticQuery(marketplace, { industry, exporterOnly: exporterFilter, city }),
    blockedTokens: [],
    explanation: `**${industry || 'exporters'}** on **${marketplace}**${cityPhrase}${contactPhrase}`,
    rawMessage: text,
  }
}
...
function buildMarketplaceSemanticQuery(marketplace, { industry, exporterOnly, city }) {
  const parts = ['India', city, industry || 'exporters', 'selling on', marketplace]
  if (exporterOnly) parts.push('exporters', 'exporting')
  parts.push('contact', 'LinkedIn')
...
export function buildExecutionPlan(understanding) {
  ...
  if (understanding.mode === 'knowledge_lookup') {
    return [
      {
        id: 'entity',
        label: `${understanding.entity}${understanding.season === 'all' ? ' · all seasons' : ''}`,
        status: 'done',
      },

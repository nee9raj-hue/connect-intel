...
function understandFromMessage(text) {
  ...
  if (tvShow && (wantsContestants || season || exporterFilter)) {
...
function buildTvShowUnderstanding(show, text, opts = {}) {
  const { exporterFilter, industry, season = 'all', wantsLinkedIn, fromState } = opts
  const seasonPhrase =
  ...
  if (wantsLinkedIn) semanticParts.push('LinkedIn profiles')
  if (industry) semanticParts.push(industry)
  if (exporterFilter) semanticParts.push('that export internationally', 'export markets')
  ...
  return {
    ...
    target: 'Contestants',
    season,
    filters: { exporter: Boolean(exporterFilter), industry: industry || null, linkedin: wantsLinkedIn },
    ...
    explanation: fromState
      ...
      : exporterFilter
        ? `contestants from **${show}**${seasonPhrase} who **export**`

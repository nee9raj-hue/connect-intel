...
import { pickBestLinkedinMatch } from '../linkedinProfileMatch.js'
...
const LINKEDIN_LOOKUP_LIMIT = 20
const LINKEDIN_LOOKUP_CONCURRENCY = 4
...
export function buildCompactKnowledgeReply(u, companies, { linkedinCount = 0, exportCount = 0 } = {}) {
  ...
  const bits = []
  if (u.filters?.linkedin) bits.push(`**${linkedinCount}** verified LinkedIn`)
...
async function enrichKnowledgeLinkedin(companies, { maxLookups = LINKEDIN_LOOKUP_LIMIT, context = '' } = {}) {
  const stripped = companies.map((c) => ({ ...c, linkedinUrl: '' }))
  const targets = stripped
    .map((c, index) => ({ c, index }))
    .filter(({ c }) => parseContactName(c.contactName))
    .slice(0, maxLookups)

  if (!targets.length) return stripped

  await mapWithConcurrency(targets, LINKEDIN_LOOKUP_CONCURRENCY, async ({ c, index }) => {
    ...
    const { matches } = await discoverLinkedinForContact(contact, { context }).catch(() => ({ matches: [] }))
    const best = pickBestLinkedinMatch(matches, contact)
    if (best?.linkedin) {
      stripped[index] = { ...stripped[index], linkedinUrl: best.linkedin }
    }
  })
...
export async function runKnowledgeAgent({ store, user, understanding, salesIntent, thread }) {
  ...
  if (u.filters?.linkedin) {
    const linkedinContext = [u.entity, u.season === 'all' ? 'all seasons' : u.season ? `season ${u.season}` : '']
      .filter(Boolean)
      .join(' ')
    companies = await enrichKnowledgeLinkedin(companies, {
      maxLookups: Math.min(companies.length, LINKEDIN_LOOKUP_LIMIT),

...
const LEAD_GEN_RE =
  /\b(find|search|list|discover)\b.*\b(exporter|manufacturer|supplier|buyer|companies?|leads?)\b|\bexporters?\s+(?:in|from)\b|\bexporting to\b/i
...
export function planCopilotTurn(message, uiContext = {}) {
  ...
  const intents = {
    morningBrief: MORNING_BRIEF_RE.test(lower),
    leadGeneration: LEAD_GEN_RE.test(lower),
  ...
  return {
    text,
    intents,
    intentCategory: classifyIntentCategory(intents, copilotTab),
    copilotTab,
    runCrmFacts: true,
    runKnowledge: true,
    runCrmSearch,
...
function classifyIntentCategory(intents, copilotTab) {

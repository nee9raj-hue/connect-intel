...
import {
  looksLikeRfqPaste,
  LOGISTICS_INTEL_RE,
  MESSAGING_COPILOT_RE,
} from './rfqSignals.js'

const CRM_COUNT_RE =
  /how many|count|leads in (my )?pipeline|leads do i have|overdue follow|pipeline total|leads by stage/i
const CRM_HOWTO_RE =
  /how (do|to)|what is|what's|difference| vs |versus|setup|connect|bulk email|marketing campaign|consent|form/i
const CRM_SEARCH_RE =
  /\b(find|search|show me|list|lookup|look up)\b.*\b(lead|contact|company|deal|pipeline)\b|\bwho needs\b|\bstalled\b|\bfollow.?up\b|\boverdue\b|\bat.?risk\b|\bchurned\b|\bsimilar compan/i
const WEB_SIGNAL_RE =
...
export function planCopilotTurn(message, uiContext = {}, options = {}) {
  ...
  const copilotTab = uiContext.copilotTab === 'market' ? 'crm' : uiContext.copilotTab || 'crm'
  ...
  const runKnowledgeLookup =
    understanding?.mode === 'knowledge_lookup' || understanding?.intent === 'knowledge_lookup'
  const runEntityResearch =
    understanding?.mode === 'entity_research' && !runKnowledgeLookup
  ...
  const intents = {
    ...
    crmAtRisk: salesIntent.isAtRisk,
    ...
    rfqParse: looksLikeRfqPaste(text),
    logisticsIntel:
      LOGISTICS_INTEL_RE.test(lower) &&
      !looksLikeRfqPaste(text) &&
      !/\b(find|discover|exporter|manufacturer)\b/i.test(lower),
    messagingSummary: MESSAGING_COPILOT_RE.test(lower),
    messagingRetry: /\bretry failed\b/i.test(lower),
  }
  ...
  if (intents.morningBrief || intents.crmFollowUp || intents.crmStalled || intents.crmAtRisk) {
    runWeb = false
  }
  ...
  const runLogisticsIntel =
    intents.logisticsIntel &&
    !runRfqParse &&
    !intents.leadGeneration &&
    !intents.morningBrief &&
    !intents.crmFollowUp &&
  ...
  const runMessagingCopilot =
    (intents.messagingSummary || intents.messagingRetry) &&
    !runRfqParse &&
    !intents.morningBrief &&
    !intents.crmFollowUp

  if (runRfqParse || runLogisticsIntel || runMessagingCopilot) {
    runWeb = false
  }

  const runPeopleDiscovery = false
  const runLeadDiscovery = false

  if (runKnowledgeLookup) {
    runWeb = false
  }
  ...
  let runCrmSearch =
    (intents.crmSearch && !intents.crmFollowUp && !intents.crmStalled && !intents.crmAtRisk) ||
    intents.forecast ||
    isExplicitCrmSearch
  ...
  // Workspace copilot: CRM records only.
  runWeb = false
  ...
  return {
    ...
    runCrmSearch: runCrmSearch && !intents.crmFollowUp && !intents.crmStalled && !intents.crmAtRisk,
    runCrmPipelineQuery: intents.crmFollowUp || intents.crmStalled || intents.crmAtRisk,
    runCrmCrossRef: runCrmCrossRef && !runLeadDiscovery && !runPeopleDiscovery,
    runLeadDiscovery,
    runPeopleDiscovery,
    runEntityResearch: false,
    runKnowledgeLookup,
    runRfqParse,
    runLogisticsIntel,
    runMessagingCopilot,
...
function classifyIntentCategory(intents, copilotTab, salesIntent) {
  if (intents.rfqParse) return 'rfq_parse'
  if (intents.logisticsIntel) return 'logistics_intel'
  if (intents.messagingSummary || intents.messagingRetry) return 'messaging_copilot'
  if (salesIntent?.isKnowledgeLookup || salesIntent?.category === 'knowledge_lookup') {
  ...
  if (intents.crmAtRisk) return 'crm_at_risk'
  if (intents.draftEmail || intents.scheduleMeeting || intents.createTask) return 'actions'
  if (copilotTab === 'lead') return 'crm_lead'
  if (copilotTab === 'crm' || intents.crmSearch || intents.crmCounts || intents.leadGeneration) return 'crm'
  return 'general'
}

...
export function planCopilotTurn(message, uiContext = {}) {
  ...
  const salesIntent = inferSalesIntent(text, uiContext)
  ...
  const intents = {
    ...
    leadGeneration:
      !isExplicitCrmSearch && (LEAD_GEN_RE.test(lower) || salesIntent.isLeadGeneration),
  ...
  return {
    ...
    runCrmCrossRef: runCrmCrossRef && !runLeadDiscovery,
    runLeadDiscovery,

...
import { isFreshDiscoveryQuery, parseDiscoveryRefinement } from './salesIntent.js'
...
export function clearStaleResearchForFreshQuery(state, text, thread = null) {
  if (!state || !isFreshDiscoveryQuery(text)) return false
  ...
  state.focus = {
    company: null,
    person: null,
    lead: state.focus?.lead || null,
    selectedResult: null,
  }
  state.recommendations = state.recommendations || { lastNbsa: '', suggestedActions: [] }
...
export function prepareTurnContext(thread, message, uiContext = {}) {
  ...
  clearStaleResearchForFreshQuery(state, text, thread)
...
export function resolveReferences(message, state, uiContext = {}) {
  ...
  const wantsContact =
    ...
    (/\b(contact details?|his email|her email|their email|his phone|her phone|linkedin profile|email address|phone number)\b/i.test(
      ...
      (/\b(email|phone|mobile|linkedin)\b/i.test(lower) &&
        /\b(his|her|their|that company|this company|the company|for (?:the|that|this))\b/i.test(lower)))
...
export function saveResearchSession(thread, session) {
  ...
  if (session.intent === 'knowledge_lookup' || session.entityType === 'TV_SHOW') {
    state.knowledgeContext = {
      entity: session.entity || '',
      entityType: session.entityType || 'TV_SHOW',
      target: 'Contestants',
      season: session.season || 'all',

...
describe('planCopilotTurn', () => {
  ...
  it('does not route company research to web', () => {
  ...
  it('does not run lead generation or web from market-style queries', () => {
  ...
  it('routes at-risk accounts to CRM pipeline query', () => {
    const plan = planCopilotTurn('Show at-risk and churned accounts', { copilotTab: 'crm' })
    assert.equal(plan.intents.crmAtRisk, true)
    assert.equal(plan.runWeb, false)
    assert.equal(plan.runCrmPipelineQuery, true)
  })
})

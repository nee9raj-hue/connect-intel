...
export async function previewSegmentCount(user, segmentOrFilters) {
  ...
  const segment =
    typeof segmentOrFilters === 'object' && segmentOrFilters.filterJson !== undefined
      ? segmentOrFilters
      : { type: 'dynamic', filterJson: segmentOrFilters }
...
export async function updateMarketingSegment(user, segmentId, patch) {
  ...
  return { ...updated, memberCount: leadIds.length, leadIds: updated.type === 'static' ? leadIds : updated.leadIds }
}

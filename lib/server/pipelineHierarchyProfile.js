...
import { pickCanonicalAuthProfile, siblingProfileActorIds } from './storeUserLookup.js'
...
function mapProfileRow(row) {
  ...
  return {
    ...
    pipelineRole: String(row.pipeline_role || '').trim() || null,
    aliasLegacyUserIds: [],
  }
}
...
export async function loadHierarchyProfile(user, metaStore = null) {
  ...
  try {
    ...
    let matches = Array.isArray(rows) && rows[0] ? [rows[0]] : []
    if (user.email) {
      const email = String(user.email).trim().toLowerCase()
      const byEmail = await supabaseRest(
        `profiles?email=eq.${encodeURIComponent(email)}&select=id,legacy_user_id,organization_id,email,full_name,role,team_id,department_id,pipeline_role&limit=20`,
        {},
        { timeoutMs: 8_000, attempts: 2, bypassCircuit: true }
      )
      const extra = Array.isArray(byEmail) ? byEmail : []
      const seen = new Set(matches.map((p) => String(p.legacy_user_id || '')))
      for (const p of extra) {
        const id = String(p?.legacy_user_id || '')
        if (!id || seen.has(id)) continue
        seen.add(id)
        matches.push(p)
      }
    }
    const row = pickCanonicalAuthProfile(matches) || matches[0] || null
    const profile = mapProfileRow(row)
    if (profile) {
      profile.aliasLegacyUserIds = siblingProfileActorIds(matches)

  ...
  try {
    ...
    if (!row && user.email) {
      ...
      const byEmail = await supabaseRest(
        `profiles?email=eq.${encodeURIComponent(email)}&select=id,legacy_user_id,organization_id,email,full_name,role,team_id,department_id,pipeline_role&limit=5`,
        {},
        { timeoutMs: 8_000, attempts: 1, bypassCircuit: true }
      )
      const matches = Array.isArray(byEmail) ? byEmail : []
      row =
        matches.find((p) => String(p.organization_id) === String(user.organizationId)) ||
        matches[0] ||
        null
    }

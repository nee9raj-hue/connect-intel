...
export function localizeSampleDraftForLead({ subject, body }, sampleLead, targetLead) {
  ...
  const pairs = [
    ...
    [String(sampleLead.city || '').trim(), String(targetLead.city || '').trim()],
    [String(sampleLead.state || '').trim(), String(targetLead.state || '').trim()],
    [String(sampleLead.location || '').trim(), String(targetLead.location || '').trim()],

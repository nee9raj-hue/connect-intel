...
/** Async SQL run log + marketing graph fan-out (after sync rule application). */
export async function postCrmWorkflowDispatch({
  firedRules = [],
  trigger,
  leadId,
  organizationId,
  ...
  try {
    if (firedRules?.length && leadId && organizationId) {
      await recordFiredCrmWorkflowRuns({
        firedRules,
        trigger,
        leadId,
        organizationId,
    ...
    return await fireWorkflowAutomationsAndAudit({
      trigger,
      leadId,
      organizationId,
      actor,
...
export async function dispatchCrmWorkflowEvent(store, entry, ctx) {
  ...
  return postCrmWorkflowDispatch({
    firedRules,
    trigger: ctx.trigger,
    leadId,

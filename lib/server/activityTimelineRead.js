...
export async function readActivityTimeline(
  ...
  if (timelineStale || singleRepDominated || !activityTimeline.length) {
    try {
      const log = await readActivityLogCached(user, {
        ...
        limit: TIMELINE_LIMIT,
        offset: 0,
        preferCrm: Boolean(preferCrm || fresh),
        fresh,
      })
      ...
      if (
        crmTimeline.length &&
        (timelineStale || crmNewer || singleRepDominated || !activityTimeline.length)
      ) {
...
export async function readActivityTimelineCached(user, options = {}) {
  ...
  if (!fresh) {
    ...
    if (cached.value) {
      ...
      if (!isTimelineStale(cachedTimeline, since) || cached.stale) {
        return {
          ...cached.value,

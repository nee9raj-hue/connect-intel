...
export async function sendMarketingMessage({
  ...
  if (!gmail && !orgCanSend) {
    ...
    let error =
      'Connect your work Gmail under Team → CRM email (or Integrations) before sending marketing email.'
    if (org?.emailDomain?.name && orgCheck?.reason === 'email_domain_mismatch') {
      ...
      error = 'Your company email domain is not verified yet — ask your admin to finish DNS under Team.'
    }
    return {
      sent: false,
      error,

...
test('rep cannot see cross-org row even with null assignee', () => {
  const store = storeWith([
    entry({ organizationId: 'org2', assignedToUserId: null, savedByUserId: 'other', lead: { id: 'x1' } }),
  ])
  const rep = { id: 'rep1', organizationId: 'org1', accountType: 'company', orgRole: 'member' }
  assert.equal(findPipelineEntryWithScope(store, rep, 'x1'), null)
})

test('canUserDeletePipelineEntry — only org admin, not rep or manager', () => {
  const rep = { id: 'rep1', organizationId: 'org1', accountType: 'company', orgRole: 'member' }
  assert.equal(canUserDeletePipelineEntry(rep, entry({ assignedToUserId: 'rep1' }), metaStore), false)
  const manager = {
    id: 'mgr1',
    organizationId: 'org1',
    accountType: 'company',
  ...
  assert.equal(canUserDeletePipelineEntry(manager, entry({ assignedToUserId: 'rep1' }), metaStore), false)
})

import { normalizeExtendedCrm } from './crmWorkflow.js'
import { computeCrmLeadScore } from './crmLeadScore.js'
import { FREIGHT_DEAL_STAGE_IDS } from '../freightDeal.js'
import { getLeadErp } from '../leadErp.js'
import { getLeadTradeProfile } from '../leadTradeProfile.js'
import { resolveLeadLastOrderCreatedAt } from '../leadLastOrder.js'
import { slimPipelineDealsForList } from '../pipelineDealClips.js'
import { readDisplayErpTagsFromLead } from '../erpTags.js'
import {
  CRM_STATUS_IDS,
  DEFAULT_CRM_LEAD_STATUS,
  crmLeadStatusLabel,
  foldCrmStatusCounts,
  isCrmLeadStatusFilter,
  normalizeCrmLeadStatus,
} from '../crmLeadStatuses.js'

export const CRM_STATUSES = CRM_STATUS_IDS
export {
  DEFAULT_CRM_LEAD_STATUS,
  crmLeadStatusLabel,
  foldCrmStatusCounts,
  isCrmLeadStatusFilter,
  normalizeCrmLeadStatus,
}

/** Standard (non-freight) deal stages — independent of account pipeline. */
const STANDARD_DEAL_STAGES = ['new', 'contacted', 'follow_up', 'replied', 'won', 'lost']

/** All valid deal.stage values (standard CRM + freight shipment pipeline). */
export const DEAL_STAGES = [...new Set([...FREIGHT_DEAL_STAGE_IDS, ...STANDARD_DEAL_STAGES])]

export function isClosedDealStage(stage) {
  return stage === 'won' || stage === 'lost'
}

export function defaultCrm() {
  return {
    status: DEFAULT_CRM_LEAD_STATUS,
    notes: '',
    lastEmailSentAt: null,
    lastResponseAt: null,
    responseReceived: false,
    emails: [],
  }
}

export function normalizeCrm(crm) {
  return normalizeExtendedCrm(crm)
}

export function mergeLeadForClient(entry) {
  const crm = normalizeCrm(entry.crm)
  const lead = entry.lead || {}
  return {
    ...lead,
    emailBouncedAt: lead.emailBouncedAt || null,
    emailBounceReason: lead.emailBounceReason || null,
    commercialEmailOptIn: Boolean(lead.commercialEmailOptIn || lead.commercialEmailConsentAt),
    commercialEmailConsentAt: lead.commercialEmailConsentAt || null,
    commercialEmailConsentSource: lead.commercialEmailConsentSource || null,
    contactId: entry.contactId || lead.contactId || null,
    companyId: entry.companyId || entry.lead?.companyId || null,
    savedAt: entry.savedAt,
    assignedToUserId: entry.assignedToUserId || null,
    teamId: entry.teamId || null,
    savedByUserId: entry.savedByUserId || entry.userId || null,
    crm,
    tradingProfile: entry.tradingProfile || lead.tradingProfile || null,
    tradeProfile: getLeadTradeProfile(entry),
    erp: getLeadErp(entry),
    erpTags: readDisplayErpTagsFromLead(entry),
  }
}

/** Slim pipeline row for list/kanban — omits email bodies and full activity history. */
export function mergeLeadForClientLight(entry) {
  const crm = normalizeCrm(entry.crm)
  const lead = entry.lead || entry
  const tasks = crm.tasks || []
  const meetings = crm.meetings || []
  const activities = crm.activities || []
  const emails = crm.emails || []

  return {
    ...entry.lead,
    emailBouncedAt: lead.emailBouncedAt || null,
    emailBounceReason: lead.emailBounceReason || null,
    commercialEmailOptIn: Boolean(lead.commercialEmailOptIn || lead.commercialEmailConsentAt),
    commercialEmailConsentAt: lead.commercialEmailConsentAt || null,
    commercialEmailConsentSource: lead.commercialEmailConsentSource || null,
    contactId: entry.contactId || entry.lead?.contactId || null,
    companyId: entry.companyId || entry.lead?.companyId || null,
    savedAt: entry.savedAt,
    assignedToUserId: entry.assignedToUserId || null,
    teamId: entry.teamId || null,
    savedByUserId: entry.savedByUserId || entry.userId || null,
    updatedAt:
      crm.lastCommunicationAt ||
      crm.lastEmailSentAt ||
      entry.savedAt,
    listLight: true,
    tradingProfile: entry.tradingProfile || null,
    tradeProfile: getLeadTradeProfile(entry),
    crm: {
      status: crm.status,
      notes: crm.notes,
      leadScore: crm.leadScore ?? computeCrmLeadScore(entry),
      dealValue: crm.dealValue,
      expectedCloseDate: crm.expectedCloseDate,
      lastEmailSentAt: crm.lastEmailSentAt,
      lastResponseAt: crm.lastResponseAt,
      lastCommunicationAt: crm.lastCommunicationAt,
      lastCommunicationType: crm.lastCommunicationType || null,
      lastCommunicationSummary: crm.lastCommunicationSummary || '',
      lastOrderCreatedAt: resolveLeadLastOrderCreatedAt(entry) || null,
      responseReceived: crm.responseReceived,
      nextFollowUpAt: crm.nextFollowUpAt,
      tagIds: crm.tagIds || [],
      emailCount: emails.length,
      activityCount: activities.length,
      tasks: tasks.map((t) => ({
        id: t.id,
        title: t.title,
        dueAt: t.dueAt,
        completed: t.completed,
        assignedToUserId: t.assignedToUserId,
      })),
      meetings: meetings.map((m) => ({
        id: m.id,
        title: m.title,
        scheduledAt: m.scheduledAt,
        type: m.type,
        location: m.location || '',
        notes: m.notes || '',
        visitRecordedAt: m.visitRecordedAt,
      })),
      deals: crm.deals || [],
      emails: [],
      activities: activities.slice(0, 60).map((a) => ({
        id: a.id,
        type: a.type,
        summary: a.summary,
        createdAt: a.createdAt,
        userId: a.userId,
        userName: a.userName,
        meta: a.meta || null,
      })),
    },
    erpTags: readDisplayErpTagsFromLead(entry),
  }
}

/** Fast pipeline list row — no CRM normalize/score (full lead loads on open). */
export function mergeLeadForClientListMinimal(entry) {
  const raw = entry.crm && typeof entry.crm === 'object' ? entry.crm : {}
  const lead = entry.lead || {}
  const status = normalizeCrmLeadStatus(raw.status)

  const emailRows = Array.isArray(raw.emails) ? raw.emails : []
  const lastEmailRow = emailRows.length ? emailRows[emailRows.length - 1] : null
  const lastEmailSubject = lastEmailRow
    ? String(lastEmailRow.subject || lastEmailRow.subjectLine || '').trim().slice(0, 240)
    : ''

  return {
    ...lead,
    emailBouncedAt: lead.emailBouncedAt || null,
    emailBounceReason: lead.emailBounceReason || null,
    commercialEmailOptIn: Boolean(lead.commercialEmailOptIn || lead.commercialEmailConsentAt),
    commercialEmailConsentAt: lead.commercialEmailConsentAt || null,
    commercialEmailConsentSource: lead.commercialEmailConsentSource || null,
    contactId: entry.contactId || lead.contactId || null,
    companyId: entry.companyId || lead.companyId || null,
    savedAt: entry.savedAt,
    assignedToUserId: entry.assignedToUserId || null,
    teamId: entry.teamId || null,
    savedByUserId: entry.savedByUserId || entry.userId || null,
    updatedAt: raw.lastCommunicationAt || raw.lastEmailSentAt || entry.savedAt,
    listLight: true,
    tradingProfile: entry.tradingProfile || null,
    tradeProfile: getLeadTradeProfile(entry),
    crm: {
      status,
      notes: typeof raw.notes === 'string' ? raw.notes.slice(0, 500) : '',
      leadScore: typeof raw.leadScore === 'number' ? raw.leadScore : null,
      dealValue: raw.dealValue ?? null,
      expectedCloseDate: raw.expectedCloseDate || null,
      lastEmailSentAt: raw.lastEmailSentAt || null,
      lastEmailSubject,
      lastCommunicationSummary:
        typeof raw.lastCommunicationSummary === 'string'
          ? raw.lastCommunicationSummary.slice(0, 240)
          : '',
      lastResponseAt: raw.lastResponseAt || null,
      lastCommunicationAt: raw.lastCommunicationAt || null,
      lastOrderCreatedAt: resolveLeadLastOrderCreatedAt(entry) || null,
      responseReceived: Boolean(raw.responseReceived),
      nextFollowUpAt: raw.nextFollowUpAt || null,
      tagIds: Array.isArray(raw.tagIds) ? raw.tagIds : [],
      emailCount: emailRows.length,
      activityCount: Array.isArray(raw.activities) ? raw.activities.length : 0,
      tasks: [],
      meetings: [],
      deals: slimPipelineDealsForList(raw.deals),
      emails: [],
      activities: [],
    },
    erpTags: readDisplayErpTagsFromLead(entry),
  }
}

export function buildTemplateEmail(lead, options = {}) {
  const purpose = options.purpose || 'introduction'
  const tone = options.tone || 'professional'
  const senderName = options.senderName || 'Your name'
  const senderCompany = options.senderCompany || 'Your company'
  const agenda =
    options.agenda ||
    'Introduce our company and explore a potential B2B partnership with the recipient.'
  const name = [lead.firstName, lead.lastName].filter(Boolean).join(' ') || 'there'
  const company = lead.company || 'your company'
  const title = lead.title || 'your role'

  const subject =
    purpose === 'follow_up'
      ? `Following up — ${company}`
      : purpose === 'meeting'
        ? `Meeting request — ${company}`
        : `Introduction — ${senderCompany} & ${company}`

  const body = `Hi ${name},

${agenda}

Given your role as ${title} at ${company}, I wanted to reach out from ${senderCompany}.

${options.keyPoints ? `${options.keyPoints}\n\n` : ''}Would you be open to a brief call to discuss next steps?

Best regards,
${senderName}
${senderCompany}`

  return { subject, body, aiGenerated: false, tone, purpose }
}

export async function generateAiEmail(lead, options = {}) {
  const template = buildTemplateEmail(lead, options)

  try {
    const { isPerplexityConfigured, generatePerplexityEmail } = await import('./perplexity.js')
    if (isPerplexityConfigured()) {
      const draft = await generatePerplexityEmail(lead, options)
      if (draft?.subject && draft?.body) return draft
    }
  } catch {
    // fall through
  }

  try {
    const { isGeminiConfigured, generateGeminiEmail } = await import('./gemini.js')
    if (isGeminiConfigured()) {
      return await generateGeminiEmail(lead, options)
    }
  } catch {
    // fall through
  }

  const apiKey = process.env.ANTHROPIC_API_KEY
  if (!apiKey) {
    return {
      ...template,
      aiGenerated: false,
      notice: 'AI draft uses templates until PERPLEXITY_API_KEY, GEMINI_API_KEY, or ANTHROPIC_API_KEY is configured.',
    }
  }

  const { emailPromptBlock } = await import('./crmEmailPrompt.js')
  const prompt = emailPromptBlock(lead, options)

  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': apiKey,
        'anthropic-version': '2023-06-01',
      },
      body: JSON.stringify({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 1024,
        messages: [{ role: 'user', content: prompt }],
      }),
    })

    const data = await response.json()
    if (!response.ok) {
      return { ...template, aiGenerated: false, notice: data.error?.message || 'AI unavailable' }
    }

    const text = (data.content || [])
      .filter((b) => b.type === 'text')
      .map((b) => b.text)
      .join('\n')

    const match = text.match(/\{[\s\S]*\}/)
    if (match) {
      const parsed = JSON.parse(match[0])
      if (parsed.subject && parsed.body) {
        return {
          subject: String(parsed.subject),
          body: String(parsed.body),
          aiGenerated: true,
        }
      }
    }
  } catch {
    // fall through to template
  }

  return { ...template, aiGenerated: false, notice: 'Using template draft.' }
}

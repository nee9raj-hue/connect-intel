...
import { readErpTagsFromLead } from '../erpTags.js'
import {
  CRM_STATUS_IDS,
...
export function mergeLeadForClientListMinimal(entry) {
  ...
  return {
    ...
    crm_payload: {
      lastOrderCreatedAt: resolveLeadLastOrderCreatedAt(entry) || null,
      erp_tags: erpTags,
      erpDuplicatePending: Boolean(raw.erpDuplicatePending),
    },
    erp: erpTags.length ? { erpTags } : undefined,
    crm: {
      status,
      ...
      erpDuplicatePending: Boolean(raw.erpDuplicatePending),
      erpPipelineOwned: Boolean(raw.erpPipelineOwned),

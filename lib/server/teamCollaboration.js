...
export function parseLeadMentions(body) {
  ...
  for (const match of text.matchAll(LEAD_MENTION_RE)) {
...
export function searchMentionLeads(store, user, query = '', limit = 15) {
  ...
  const scored = leads
    .map((lead) => {
      const name = [lead.firstName, lead.lastName].filter(Boolean).join(' ').trim()
      const hay = [name, lead.company, lead.email, lead.title].filter(Boolean).join(' ').toLowerCase()

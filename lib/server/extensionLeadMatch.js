...
import { normalizeLinkedinKey } from './pipelineLeadDedup.js'
...
async function findLeadIdsByLinkedInShard(user, metaStore, linkedinUrl) {
  ...
  const matches = []
  for (const entry of pipelineStore.savedLeads || []) {
    const stored = normalizeLinkedinKey(entry.lead?.linkedin)
    if (!stored) continue
    if (!(stored === key || stored.includes(key) || key.includes(stored))) continue
    const visible = await isPipelineEntryVisibleAsync(user, entry, metaStore)
    if (!visible) continue
    const leadId = entry.lead?.id || entry.id
    if (leadId) matches.push(String(leadId))
...
async function findLeadIdsByLinkedInSql(user, metaStore, linkedinUrl) {
  const key = normalizeLinkedinKey(linkedinUrl)
  if (!key || !pipelineLeadsTableActive() || !user?.organizationId) return []
  ...
  const slug = key.replace(/^linkedin\.com\/in\//, '')
  parts.push(`entry->lead->>linkedin=ilike.${encodeURIComponent(`%${slug}%`)}`)

  const rows = await supabaseRest(
    `${TABLE}?${parts.join('&')}&select=lead_id&limit=10`,
    {},
    { timeoutMs: 15_000, attempts: 1 }
  )
  if (!Array.isArray(rows)) return []
...
export async function matchPipelineLeadsForExtension(
  ...
  if (linkedin) {
    try {
      leadIds = await findLeadIdsByLinkedInSql(scopedUser, metaStore, linkedin)
    } catch {
      leadIds = []
    }
    if (!leadIds.length) {
      leadIds = await findLeadIdsByLinkedInShard(scopedUser, metaStore, linkedin)
    }
    if (leadIds.length) matchedBy = 'linkedin'
  }

  if (!leadIds.length && normalized.length) {
    try {

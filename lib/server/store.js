...
import { isPostgresStoreBackend, isSupabaseRestStoreBackend, resolveStoreBackend } from './storeBackend.js'
...
/** Session GET / JWT refresh — skip sessions + creditLedger JSON blobs. */
export const SESSION_STORE_COLLECTIONS = [
  'users',
  'organizations',
  'organizationMemberships',
]

/** Login / upsert — invites for pending org join; never pull sessions ledger. */
export const LOGIN_STORE_COLLECTIONS = [
  'users',
  'organizations',
  'organizationMemberships',
  'organizationInvites',
...
/** Team invite mutation + invite email OAuth lookup — never load pipeline shards. */
export const INVITE_STORE_COLLECTIONS = [
  'users',
  'organizations',
  'organizationMemberships',
  'organizationInvites',
...
/** Onboarding — user + org + membership only (no full CRM store read/write). */
export const ONBOARDING_STORE_COLLECTIONS = [
  'users',
  'organizations',
  'organizationMemberships',
]
...
async function readPostgresStoreWithRetry(attempts = 4) {
  const { readPostgresStorePartial } = await import('./storePg.js')
  let lastError = null
  for (let i = 0; i < attempts; i += 1) {
    try {
      const partial = await readPostgresStorePartial(COLLECTIONS)
      return mergeStore({ ...cloneDefaultStore(), ...partial })
    } catch (error) {
      lastError = error
      if (i < attempts - 1) {
  ...
  throw lastError || new Error('Postgres readStore failed')
}
...
async function readSupabaseStorePartial(collections, extra = {}) {
  ...
  for (let i = 0; i < names.length; i += SUPABASE_READ_BATCH_SIZE) {
    ...
    const timeoutMs =
      extra.timeoutMs || (needsLongRead ? 90_000 : 20_000)
    const attempts = extra.attempts || (timeoutMs <= 8_000 ? 2 : 3)
...
/** One-time copy: local/SQLite data → cloud store when cloud is empty. */
export async function migrateSqliteToSupabaseIfNeeded() {
  const backend = resolveStoreBackend()
  if (backend === 'sqlite') return { migrated: false }
  if (backend === 'postgres') {
    if (!isPostgresStoreBackend()) return { migrated: false }
    if (migrationChecked) return { migrated: false }
    migrationChecked = true

    const { readPostgresStorePartial } = await import('./storePg.js')
    const partial = await readPostgresStorePartial(['users', 'contacts', 'companies'])
    const cloud = mergeStore({ ...cloneDefaultStore(), ...partial })
    const hasCloudData =
      (cloud.contacts?.length || 0) > 0 ||
      (cloud.companies?.length || 0) > 0 ||
      (cloud.users?.length || 0) > 0
    if (hasCloudData) return { migrated: false }
    if (isServerlessCloud()) return { migrated: false }
    ...
    const { writePostgresCollections } = await import('./storePg.js')
    await writePostgresCollections(local, COLLECTIONS)
    return { migrated: true, contacts: local.contacts?.length || 0, engine: 'postgres' }
  }
  ...
  if (backend === 'postgres') {
    if (!only?.length) {
      try {
        ...
        return await readPostgresStoreWithRetry()
      } catch (error) {
        console.error('Postgres readStore failed:', error.message)
        if (isServerlessCloud()) {
          throw new Error('Could not load your workspace. Wait a moment and try again.')
        }
      }
    } else {
      const { readPostgresStorePartial } = await import('./storePg.js')
      const partial = await readPostgresStorePartial(only)
      return mergeStore({ ...cloneDefaultStore(), ...partial })
    }
  }

  if (isSupabaseRestStoreBackend() && isSupabaseEnabled()) {
...
export async function writeStoreCollections(store, collections) {
  ...
  if (isPostgresStoreBackend()) {
    ...
    const { writePostgresCollections } = await import('./storePg.js')
    await writePostgresCollections(snapshot, [...names, ...shardNames])
...
export async function writeSavedLeadsCollection(savedLeads) {
  ...
  if (isPostgresStoreBackend()) {
    const { upsertPostgresCollection } = await import('./storePg.js')
    await upsertPostgresCollection('savedLeads', list)
    return
  }
...
export async function writeStore(store) {
  ...
  if (isPostgresStoreBackend()) {
    try {
      const before = await readPostgresStoreWithRetry()
      assertSafeWrite(before, snapshot)
      const { writePostgresCollections } = await import('./storePg.js')
      await writePostgresCollections(snapshot, COLLECTIONS)
      const { syncPipelineShardsFromSavedLeads } = await import('./pipelineShard.js')
      await syncPipelineShardsFromSavedLeads(snapshot.savedLeads || [])
      return
    } catch (error) {
      console.error('Postgres writeStore failed:', error.message)
...
export async function getStoreMetadata() {
  ...
  if (backend === 'postgres') {
    try {
      const { pingPostgresStore } = await import('./storePg.js')
      await pingPostgresStore()
      return { engine: 'postgres', backend }
    } catch (error) {
      return { engine: 'postgres', backend, supabaseFallbackReason: error.message }
    }
  }

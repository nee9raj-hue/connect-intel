...
import { enrichCampaignEngagementFromEnrollments } from './marketingCampaigns.js'
...
export async function buildMarketingDashboard(store, user, { period = '30d', statsOnly = false } = {}) {
  ...
  for (const c of campaigns) {
    ...
    const stats = c.stats || {}
    const cSent = stats.recipientsSent || stats.sent || analytics.sent || 0
    const uniqueOpens = stats.uniqueOpens ?? analytics.uniqueOpens ?? 0
    const uniqueClicks = stats.uniqueClicks ?? analytics.uniqueClicks ?? 0
    sent += cSent
    delivered += Math.max(0, cSent - (stats.bounced || analytics.bounced || 0) - (stats.failed || analytics.failed || 0))
    opens += uniqueOpens || analytics.opens || 0
    clicks += uniqueClicks || analytics.clicks || 0
    bounced += stats.bounced || analytics.bounced || 0
    unsubscribed += stats.unsubscribed || analytics.unsubscribed || 0
    ...
    campaignRows.push({
      ...
      openRate: stats.openRate ?? analytics.openRate ?? 0,
      clickRate: stats.clickRate ?? analytics.clickRate ?? 0,

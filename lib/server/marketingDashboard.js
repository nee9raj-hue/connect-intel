...
export async function buildMarketingDashboard(store, user, { period = '30d' } = {}) {
  ...
  for (const c of campaigns) {
    ...
    const cSent = analytics.sent || 0
    sent += cSent
    delivered += Math.max(0, cSent - (analytics.bounced || 0) - (analytics.failed || 0))
    opens += analytics.uniqueOpens || analytics.opens || 0
    clicks += analytics.uniqueClicks || analytics.clicks || 0

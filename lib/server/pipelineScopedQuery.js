...
export async function getScopedLeadsQuery(currentUser, rawFilters = {}, metaStore = null) {
  ...
  if (role === 'manager') {
    ...
    // Owner picker is one person's book. Do this before the team page, or the
    // filter only keeps whoever happened to be on the first team page.
    if (assignee && allowedOwners.has(assignee)) {
      return applyOwnerBookFilter(base, assignee, metaStore, organizationId, filters)
    }
...
function applyRepPipelineScope(base, currentUser, filters, actorIds, shared = {}) {
  ...
  if (mode === 'team') {
    if (assignee && assignee !== '__unassigned__') {
      const self = new Set([String(currentUser.id), ...(actorIds || [])].map(String))
      const allowed =
        base.role === 'manager'
          ? new Set([...(shared.ownerIds || []), ...self].map(String))
          : self
      if (allowed.has(String(assignee))) {
        const extra = self.has(String(assignee)) ? [...self] : []
        return applyOwnerBookFilter(
          base,
          assignee,
          null,
          currentUser.organizationId,
          filters,
          extra.filter((id) => id !== String(assignee))
        )
      }
    }
...
/** One owner's assigned leads (owner_id or assignee), not the team page. */
function applyOwnerBookFilter(base, assignee, metaStore, organizationId, filters, fallbackIds = []) {
  const actorIds = metaStore
    ? listPipelineActorIdsForMember(metaStore, organizationId, assignee)
    : []
  const ids = [...new Set([assignee, ...actorIds, ...fallbackIds].map(String).filter(Boolean))]
  const ownerFilter = pipelineAssigneePostgrestFilter(ids.length ? ids : [assignee])
  if (ownerFilter) base.postgrestParts.push(ownerFilter)
  base.scope.ownerId = assignee
  base.rpc.p_owner_id = assignee
  base.scope.repSharedMode = 'own'
  return finalizeStatusFilter(base, {
    ...filters,
    teamIds: [],
    teamId: null,

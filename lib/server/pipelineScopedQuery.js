  ...
  if (mode === 'team') {
    ...
    if (teamIds.length > 1) {
      base.postgrestParts.push(
        `team_id.in.(${teamIds.map((id) => encodeURIComponent(id)).join(',')})`
      )
    }
    base.postgrestParts.push(managerFilter)
    ...
    return finalizeStatusFilter(base, {
      ...filters,
      teamIds: teamIds.length === 1 ? [] : filters.teamIds,
    })
  }
...
async function resolveRepSharedPipelineScope({
  ...
  try {
    ...
    for (const team of teams) {
      if ((team.members || []).some((m) => String(m.userId) === String(currentUser.id))) {
        memberTeamIds.push(String(team.id))
      }
    }
  } catch {
    /* profile.teamId is enough */
  }
  memberTeamIds = [...new Set(memberTeamIds)]

  const allowedTeams = allowedTeamIdsForMember(collectPipelineTeamIds(filters), memberTeamIds, {
    isOrgAdmin,
  })
  const tags = listOrgLeadTagDefinitions(metaStore || {}, organizationId)
  const allowedTags = allowedTagIdsForMember(filters.tagIds, tags, memberTeamIds, { isOrgAdmin })
  const nextFilters = { ...filters, teamIds: allowedTeams, tagIds: allowedTags }
  ...
  if (allowedTeams.length) {
    let memberIds = (filters.teamMemberUserIds || []).map(String).filter(Boolean)
    if (!memberIds.length) {
      try {
        const teams = await loadHierarchyTeams(organizationId)
        memberIds = memberUserIdsForTeams(teams, allowedTeams)
      } catch {
        memberIds = []
      }
    }
...
function finalizeStatusFilter(base, filters) {
  ...
  const statusFilter = pipelineCrmStatusPostgrestFilter(status, { pipelineTrack: filters.pipelineTrack })
  if (statusFilter) {

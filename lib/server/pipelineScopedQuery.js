...
async function resolveRepSharedPipelineScope({
  ...
  try {
    ...
    const selfIds = new Set(
      [currentUser?.id, profile?.legacyUserId, currentUser?.legacyUserId].map(String).filter((id) => id && id !== 'undefined')
    )
    for (const team of teams) {
      if (
        (team.members || []).some(
          (m) => selfIds.has(String(m.userId || '')) || selfIds.has(String(m.legacyUserId || ''))
        )
      ) {
  ...
  const merged = mergeTeamScopedTagFilters(filters, tags, memberTeamIds, { isOrgAdmin })

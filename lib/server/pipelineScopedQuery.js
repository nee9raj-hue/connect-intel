...
export async function getScopedLeadsQuery(currentUser, rawFilters = {}, metaStore = null) {
  ...
  if (!isPipelineHierarchyRbacEnabled()) {
    ...
    return applyRepPipelineScope(base, currentUser, shared.filters, actorIds, shared, metaStore)
  }
  ...
  if (profile?.profileId) actorIds.push(String(profile.profileId))
  ...
  if (role === 'manager') {
    ...
    if (shared.mode === 'team' || shared.mode === 'tags') {
      return applyRepPipelineScope(base, currentUser, shared.filters, uniqueActorIds, shared, metaStore)
    }
...
function applyRepPipelineScope(base, currentUser, filters, actorIds, shared = {}, metaStore = null) {
  const assignee = String(filters.assigneeUserId || '').trim()
  if (assignee === '__unassigned__') {
    base.postgrestParts.push(pipelineUnassignedPostgrestFilter())
    base.scope.unassigned = true
  ...
  if (mode === 'team') {
    ...
    base.postgrestParts.push(pipelineRepVisibilityPostgrestFilter(ownerIds, { includeCollaborators: true }))
...
async function resolveRepSharedPipelineScope({
  ...
  const hasSharedFilter = Boolean(
    collectPipelineTeamIds(filters).length || (filters.tagIds || []).length
  )
  if (!hasSharedFilter) {
    return { mode: 'own', filters, teamIds: [], ownerIds: actorIds }
  }
  ...
  const selfEmail = String(currentUser?.email || profile?.email || '')
    .trim()
    .toLowerCase()
  const selfIds = new Set(
    [currentUser?.id, profile?.legacyUserId, profile?.profileId, currentUser?.legacyUserId]
      .map(String)
    ...
    if (
      (team.members || []).some(
        (m) =>
          ...
          (selfEmail && String(m.email || '').trim().toLowerCase() === selfEmail)
      )
    ) {
  ...
  const allowedTags = allowedTagIdsForMember(merged.tagIds, tags, memberTeamIds, { isOrgAdmin })

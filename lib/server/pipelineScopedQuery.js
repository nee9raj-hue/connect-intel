...
function applyRepPipelineScope(base, currentUser, filters) {
  ...
  const ownerId = assignee && assignee !== '__unassigned__' ? assignee : repId
  if (ownerId !== repId) {
    base.scope.ownerId = repId
    base.rpc.p_owner_id = repId
    const ownerFilter = pipelineAssigneePostgrestFilter(repId)
    if (ownerFilter) base.postgrestParts.push(ownerFilter)
    return finalizeStatusFilter(base, filters)
  }
  base.scope.ownerId = repId

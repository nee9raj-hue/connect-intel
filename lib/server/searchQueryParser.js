...
export function parseSearchQueryFallback(rawQuery, existingFilters = {}) {
  ...
  keywords = keywords
    .replace(
      /\b(in|at|from|the|and|for|of|a|an|to|need|i want|looking for|find|email|id|ceo|phone|contact|india|indian|bharat)\b/gi,
      ' '
    )
    .replace(/\s+/g, ' ')
    .trim()

...
describe('xindusErpOverlayApply owner reclaim', () => {
  ...
  it('keeps CRM-created assignee and does not reclaim unmatched CRM leads', () => {
    const store = {
      users: [
        { id: 'u-vivek', email: 'vivek.kumar@xindus.net', name: 'Vivek Kumar Singh' },
        { id: 'u-navya', email: 'navya@xindus.net', name: 'Navya Sagar' },
    ...
    const overlays = [
      {
        ...
        ownerAuthoritative: true,
        erp: {
          revenue: { lastShipmentDate: '2026-07-25', shipmentCount: 2, tags: [{ tagName: 'Active' }] },
          ownership: { salesOwner: { name: 'Vivek Kumar Singh', email: 'vivek.kumar@xindus.net' } },
        },
      },
    ]
    const overlayIndex = indexOverlays(overlays)
    ...
    const crmLead = {
      assignedToUserId: 'u-navya',
      savedByUserId: 'u-navya',
      crm: { status: 'fresh' },
      source: 'manual',
      lead: { company: 'Acme Traders', phone: '9999999999' },
    }
    const hit = applyErpOverlayToLeadEntry(crmLead, {
      overlayIndex,
      ownerIndex,
      overlayOwnerIds,
      reclaimUnmatchedOwners: true,
    ...
    assert.equal(hit.matched, true)
    assert.equal(crmLead.assignedToUserId, 'u-navya')
    assert.equal(crmLead.crm.status, 'fresh')
    assert.equal(crmLead.crm.erpDuplicatePending, true)
    assert.equal(crmLead.erp.ownership.salesOwner.email, 'vivek.kumar@xindus.net')
    ...
    const unmatchedCrm = {
      ...
      source: 'manual',
      lead: { company: 'Handmade CRM Lead' },
    }
    const reclaim = applyErpOverlayToLeadEntry(unmatchedCrm, {
      overlayIndex,
      ownerIndex,
      overlayOwnerIds,
      reclaimUnmatchedOwners: true,
    ...
    assert.equal(unmatchedCrm.assignedToUserId, 'u-vivek')
  })
})

...
import { indexOverlays, matchLeadToOverlay, applyErpOverlayToLeadEntry, bestShipmentOverlay, bestSalesOwnerOverlay, stampErpShipmentFacts, stampErpSalesOwner, overlayOwnerUserIds, resetErpOwnerMappingOnEntry } from './xindusErpOverlayApply.js'
...
describe('xindusErpOverlayApply shipment facts', () => {
  it('keeps the same-name customer that actually shipped when an empty duplicate is indexed first', () => {
    const index = indexOverlays([
      {
        company: 'LAVINAS',
        xindusId: '1226',
        ownerAuthoritative: true,
        erp: { revenue: { xindusId: '1226', shipmentCount: 0 } },
      },
      {
        company: 'LAVINAS',
        xindusId: '717',
        erp: { revenue: { lastShipmentDate: '2025-08-25', shipmentCount: 6 } },
      },
    ])
    const entry = {
      assignedToUserId: 'u-keep',
      lead: { company: 'LAVINAS' },
      erp: { revenue: { xindusId: '1226' } },
      crm: { status: 'churned' },
    }
    const identity = matchLeadToOverlay(entry, index)
    assert.equal(identity?.xindusId, '1226')
    const shipment = bestShipmentOverlay(index, entry, identity)
    assert.equal(shipment?.xindusId, '717')
    assert.equal(stampErpShipmentFacts(entry, shipment), true)
    assert.equal(entry.erp.revenue.lastShipmentDate, '2025-08-25')
    assert.equal(entry.erp.revenue.xindusId, '1226')
    assert.equal(entry.assignedToUserId, 'u-keep')
    assert.equal(entry.crm.status, 'churned')
    assert.ok(String(entry.crm.lastOrderCreatedAt).startsWith('2025-08-25'))
  })
})

describe('xindusErpOverlayApply sales owner sync', () => {
  it('moves the lead to the ERP sales owner when that person changes', () => {
    const ownerIndex = buildOwnerMemberIndex(
      {
        users: [
          { id: 'u-dakash', email: 'dakash.rantiya@xindus.net', name: 'Dakash Rantiya' },
          { id: 'u-vivek', email: 'vivek.singh@xindus.net', name: 'Vivek Kumar Singh' },
        ],
        organizationMemberships: [
          { userId: 'u-dakash', organizationId: 'org1', status: 'active' },
    ...
    const index = indexOverlays([
      {
        company: 'DEE DEE EXPORTS',
        xindusId: '1756',
        erp: {
          revenue: { xindusId: '1756', lastShipmentDate: '2026-09-01' },
          ownership: { salesOwner: { name: 'Vivek Kumar Singh', email: 'vivek.singh@xindus.net' } },
        },
      },
    ])
    const entry = {
      assignedToUserId: 'u-dakash',
      savedByUserId: 'u-dakash',
      crm: { status: 'active_trading' },
      lead: { company: 'DEE DEE EXPORTS' },
      erp: { revenue: { xindusId: '1756', lastShipmentDate: '2026-09-01' } },
    }
    const packed = bestSalesOwnerOverlay(index, entry, null)
    assert.equal(stampErpSalesOwner(entry, packed, ownerIndex), true)
    assert.equal(entry.assignedToUserId, 'u-vivek')
    assert.equal(entry.savedByUserId, 'u-dakash')
    assert.equal(entry.crm.status, 'active_trading')
    assert.equal(stampErpSalesOwner(entry, packed, ownerIndex), false)
  })
})
...
describe('xindusErpOverlayApply owner reclaim', () => {
  ...
  it('follows the ERP sales owner, and a later ops-list owner replaces it', () => {
    const store = {
      users: [
        { id: 'u-vivek', email: 'vivek.singh@xindus.net', name: 'Vivek Kumar Singh' },
    ...
    assert.equal(chuninda.assignedToUserId, 'u-vivek')
    assert.equal(chuninda.erp.ownership.salesOwner.email, 'vivek.singh@xindus.net')

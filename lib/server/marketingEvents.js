...
export async function recordMarketingEvent({
  ...
  if (
    organizationId &&
    leadId &&
    (type === 'open' || type === 'click' || type === 'unsubscribe')
  ) {
    try {
      const { refreshLeadScoreAfterMarketingEvent } = await import('./leadScoreRecompute.js')
      await refreshLeadScoreAfterMarketingEvent({
        organizationId,
        leadId,
        actorUserId: createdByUserId,
      })
    } catch (err) {
      console.error('lead score refresh failed:', err?.message || err)
    }
  }
}

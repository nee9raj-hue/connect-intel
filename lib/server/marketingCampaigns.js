...
/** Backfill list metrics from events + enrollment shards (works in light overview). */
export async function enrichCampaignsListMetrics(campaigns, events = []) {
  if (!campaigns?.length) return campaigns || []
  ...
  const needsEnrollment = campaigns
    .filter((c) => {
      ...
      return sent === 0 && enrolled === 0
    })
    .slice(0, 48)

  const enrollmentStats = new Map()
  await Promise.all(
    needsEnrollment.map(async (c) => {
      try {
        const rows = await readCampaignEnrollments(c.id)
...
export async function marketingOverview(store, user, options = {}) {
  ...
  campaigns = await enrichCampaignsListMetrics(campaigns, events)
  if (!light) {

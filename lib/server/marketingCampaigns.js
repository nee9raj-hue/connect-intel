...
/** Gmail sends are slow — keep each serverless request to ~2–3 messages (10s Hobby limit). */
export const MARKETING_SEND_CHUNK = 2
...
export async function processDueEnrollments({
  ...
  const store = await readStore({ only: MARKETING_STORE_SLICES })
  const now = new Date().toISOString()
  ...
  if (pendingWrites.length) {
    await updateStorePartial(['marketingCampaigns', 'marketingEnrollments', 'savedLeads'], (draft) => {
      for (const write of pendingWrites) {
        ...
        if (write.kind === 'failed') {
          ...
          const entry = findPipelineEntry(draft, write.user, write.result.leadId)
          if (entry?.crm && write.result.logPayload) {
            entry.crm = recordOutboundEmail(entry.crm, write.result.logPayload, {

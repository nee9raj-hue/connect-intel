...
export async function marketingOverview(store, user, options = {}) {
  ...
  const withEngagement = Boolean(options.withEngagement)
  ...
  let campaigns = enrichMarketingRows(
    ...
    filterMarketingCampaignsVisible(store.marketingCampaigns, user).map((c) => {
      ...
      return {
        ...c,
        analytics,
        stats: { ...analytics, ...(c.stats || {}) },
      }
    })
  )
  campaigns = await mergeCampaignStatsShards(campaigns)
  campaigns = await enrichCampaignsListMetrics(campaigns, events, { light: light && !withEngagement })
  if (withEngagement || !light) {
    ...
    if (needsEngagement.length) {
      const enriched = await enrichCampaignEngagementFromEnrollments(needsEngagement, store, user, {
        maxCampaigns: withEngagement ? Math.min(needsEngagement.length, 100) : 16,
        skipWhenStatsComplete: !withEngagement,
      })

...
import { filterMarketingRows, marketingScopeKey, rowInMarketingScope } from './marketingAccess.js'
...
export function getMarketingList(store, user, listId) {
  ...
  if (!row || !rowInMarketingScope(row, user)) return null
  return row
}
...
export function marketingOverview(store, user) {
  ...
  const campaigns = filterMarketingRows(store.marketingCampaigns, user).map((c) => {
    const enrollments = (store.marketingEnrollments || []).filter(
      (e) => e.campaignId === c.id && rowInMarketingScope(e, user)
    )
  ...
  return {
    lists: filterMarketingRows(store.marketingLists, user),
    templates: filterMarketingRows(store.marketingTemplates, user),
    campaigns,
    forms: filterMarketingRows(store.marketingForms, user),

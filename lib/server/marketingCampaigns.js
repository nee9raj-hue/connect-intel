...
/** Gmail sends are slow — batch within Vercel maxDuration (120s). */
export const MARKETING_SEND_CHUNK = 5
...
export function countPendingCampaignSends(store, campaignId) {
  const now = new Date().toISOString()
  return (store.marketingEnrollments || []).filter(
    (e) =>
...
export async function enrollCampaign(store, user, campaign, list) {
  ...
  await updateStorePartial(['marketingCampaigns', 'marketingEnrollments'], (draft) => {
...
async function persistCampaignSendCrm(user, leadId, logPayload) {
  if (!user || !leadId || !logPayload) return
...
async function hydrateStorePipeline(store, actorUsers) {
  ...
  if (!senders.length) {
    return readStore({ only: MARKETING_STORE_SLICES })
  }
...
export async function processDueEnrollments({
  ...
  if (actorUser) {
    ...
    const senders = due
      .map((e) => {
        const campaign = (store.marketingCampaigns || []).find((c) => c.id === e.campaignId)
        return campaign ? resolveCampaignSender(store, campaign) : null
      })
      .filter(Boolean)
    store = await hydrateStorePipeline(store, senders)
  }
  ...
  if (pendingWrites.length) {
    await updateStorePartial(['marketingCampaigns', 'marketingEnrollments'], (draft) => {
      for (const write of pendingWrites) {
        const e = draft.marketingEnrollments.find((x) => x.id === write.enrollmentId)
        if (!e) continue
    ...
    for (const write of pendingWrites) {
      if (write.kind === 'sent' && write.result?.logPayload) {
        await persistCampaignSendCrm(write.user, write.result.leadId, write.result.logPayload)
      }
    }
  }

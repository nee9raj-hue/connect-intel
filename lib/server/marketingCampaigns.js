...
export function getMarketingCampaign(store, user, campaignId, { manage = false } = {}) {
  ...
  if (!row) return null
  if (manage) {
    if (!canAccessMarketingAsset(row, user)) return null
...
async function enrichCampaignEngagementFromEnrollments(campaigns) {
    ...
    await Promise.all(
      batch.map(async (c) => {
        try {
          ...
          const engagement = summarizeEnrollmentEngagement(rows, c.stats?.sent || 0)
...
export async function marketingOverview(store, user, options = {}) {
  ...
  campaigns = await enrichCampaignEngagementFromEnrollments(campaigns)

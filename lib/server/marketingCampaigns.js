...
/** Minimal store read for sending — never load monolithic marketingEnrollments. */
...
async function syncCampaignRowAfterSends(campaignId, enrollments, pendingWrites) {
  const pending = enrollments.some((e) => e.status === 'active' && e.nextSendAt)
  const needsCampaignRow =
    !pending ||
    pendingWrites.some((w) => w.kind === 'failed' || w.kind === 'unsubscribed' || w.kind === 'sent')
  if (!needsCampaignRow) return
  ...
  await updateStorePartial(['marketingCampaigns'], (draft) => {
    ...
    applyCampaignCompletionForCampaign(c, enrollments)
...
export async function processDueEnrollments({
  ...
  let store = await readStore({
    only: campaignId ? MARKETING_SEND_READ_SLICES : MARKETING_META_SLICES,
  })
  const now = new Date().toISOString()
  const campaignEnrollments = campaignId ? await readCampaignEnrollments(campaignId) : []
  ...
  if (pendingWrites.length) {
    ...
    try {
      ...
      const stillPending = updatedEnrollments.some((e) => e.status === 'active' && e.nextSendAt)
      const needsCampaignSync =
        !stillPending ||
        pendingWrites.some((w) => w.kind === 'failed' || w.kind === 'unsubscribed')
      if (needsCampaignSync) {
        await syncCampaignRowAfterSends(campaignId, updatedEnrollments, pendingWrites)
      }
    } catch (err) {

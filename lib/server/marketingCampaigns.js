...
export async function marketingOverview(store, user, options = {}) {
  ...
  let campaigns = enrichMarketingRows(
    ...
    filterMarketingCampaignsVisible(store.marketingCampaigns, user).map((c) => {
      const rollup = eventRollups.get(c.id)
      const analytics = buildCampaignAnalyticsFromStats(c, events)
      const merged = rollup
        ? {
            ...analytics,
            opens: Math.max(analytics.opens || 0, rollup.opens),
            clicks: Math.max(analytics.clicks || 0, rollup.clicks),
            uniqueOpens: Math.max(analytics.uniqueOpens || 0, rollup.uniqueOpens),
            uniqueClicks: Math.max(analytics.uniqueClicks || 0, rollup.uniqueClicks),
          }
        : analytics
      return {
        ...c,
        analytics: merged,
        stats: { ...merged, ...(c.stats || {}) },
      }
    })
  )

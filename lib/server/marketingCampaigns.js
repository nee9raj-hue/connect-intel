...
import {
  applyPipelineBulkCrmPatches,
  attachPipelineLeadsForBulkSend,
  resolvePipelineBulkStepContent,
} from './pipelineBulkSendWorker.js'
...
/** Resolve list or segment into an audience object with leadIds. */
export async function resolveCampaignAudience(store, user, campaign) {
  if (campaign.segmentId) {
    ...
    const leadIds = await resolveSegmentLeadIds(store, user, segment, {
      channel: campaign.channel || segment.channel || 'email',
    })
    return {
      id: segment.id,
      name: segment.name,
      channel: segment.channel || campaign.channel || 'email',
      leadIds,
      sourceType: 'segment',
    }
  }
...
export async function processDueEnrollments({
  ...
  if (campaignForBulk?.source === 'pipeline_bulk' && due.length) {
    ...
    if (bulkSender) {
      store = await attachPipelineLeadsForBulkSend(store, bulkSender, campaignForBulk, due)
    }
  }
...
export async function processCampaignSendBurst(user, campaignId, options = {}) {
  ...
  const metaStore = await readStore({ only: [...MARKETING_SEND_META_SLICES, 'marketingCampaigns'] })
  const sendCampaign = await readCampaignSendShard(campaignId)
  const campaign =
    sendCampaign ||
    getMarketingCampaign(metaStore, user, campaignId, { manage: true })
  if (!campaign) throw new Error('Campaign not found')
  if (campaign.status !== 'active') {
    return {
      ...agg,
      pendingSends: await countPendingCampaignEnrollments(campaignId),
      queuedSends: await countQueuedCampaignEnrollments(campaignId),
      campaign,
      skipped: true,
    }
  }
  ...
  while (
    ...
    const pendingBefore = await countPendingCampaignEnrollments(campaignId)
    ...
    const pendingAfter = await countPendingCampaignEnrollments(campaignId)
  ...
  const refreshed = await readStore({ only: ['marketingCampaigns'] })
  const updated =
    getMarketingCampaign(refreshed, user, campaignId) ||
    ({ ...campaign, status: stats.status || campaign.status, stats })
  ...
  return {
    ...
    campaign: {
      ...updated,
      status: stats.status || updated.status,

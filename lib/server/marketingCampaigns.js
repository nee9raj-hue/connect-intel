...
function applyCampaignCompletionInDraft(draft, onlyCampaignId = null) {
  const campaigns = onlyCampaignId
    ? (draft.marketingCampaigns || []).filter((c) => c.id === onlyCampaignId)
...
export async function processDueEnrollments({
  ...
  if (pendingWrites.length) {
    await updateStorePartial(['marketingCampaigns', 'marketingEnrollments'], (draft) => {
      ...
      applyCampaignCompletionInDraft(draft, campaignId)
      return draft
    })

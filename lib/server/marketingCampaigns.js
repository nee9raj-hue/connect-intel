...
export async function marketingOverview(store, user, options = {}) {
  ...
  if (light) {
    campaigns = await mergeCampaignStatsShards(campaigns, { maxCampaigns: 24 })
  } else {
    campaigns = await mergeCampaignStatsShards(campaigns)
  }
  campaigns = await enrichCampaignsListMetrics(campaigns, events, { light })
  // Full (non-light) overview only: capped enrollment reads. Light overview uses events rollups only.
  if (!light) {
  ...
  const listRows = light
    ? filterMarketingRows(store.marketingLists, user).map((row) => ({
        ...
        memberCount:
          row.memberCount ??
          (Array.isArray(row.leadIds) ? row.leadIds.length : 0),
      }))

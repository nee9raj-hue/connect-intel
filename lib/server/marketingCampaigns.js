...
export const MARKETING_SEND_CHUNK = 12
...
export async function syncMarketingCampaignStatus(user, campaignId, patch) {
...
export async function pauseMarketingCampaign(user, campaignId) {
  ...
  return syncMarketingCampaignStatus(user, campaignId, { status: 'paused' })
}
...
export async function resumeMarketingCampaign(user, campaignId) {
  ...
  return syncMarketingCampaignStatus(user, campaignId, { status: 'active' })
}
...
export async function stopMarketingCampaign(user, campaignId) {
  ...
  await writeCampaignEnrollments(campaignId, next)
  await syncMarketingCampaignStatus(user, campaignId, {
    status: 'stopped',
    completedAt: now,
  })
...
/** Consent-aware audience breakdown before campaign launch (mirrors listEligibleLeads). */
export function summarizeAudienceEligibility(store, user, leadIds) {
  const unique = [...new Set(leadIds || [])]
  const pipelineByLeadId = new Map(
    listPipelineSavedEntries(store, user)
      .filter((e) => e.lead?.id)
  ...
  const scope = user.organizationId
    ? { organizationId: user.organizationId, createdByUserId: null }
    : { organizationId: null, createdByUserId: user.id }
  const skipped = { not_in_pipeline: 0, no_email: 0, no_consent: 0, suppressed: 0 }
  let eligible = 0
  for (const leadId of unique) {
    const entry = pipelineByLeadId.get(leadId)
    if (!entry) {
      skipped.not_in_pipeline += 1
      continue
    }
    const lead = entry.lead || entry
    const email = getLeadEmail(lead)
    if (!email.includes('@') || !leadHasSendableEmail(lead)) {
      skipped.no_email += 1
      continue
    }
  ...
  return { total: unique.length, eligible, skipped }
}
...
export async function enrollCampaign(store, user, campaign, list) {
  ...
  syncRecipientsAfterEnroll({ campaign, enrollments, store, user })
...
export function applyEnrollmentWrites(enrollments, pendingWrites) {
  for (const write of pendingWrites) {
    const e = enrollments.find((x) => x.id === write.enrollmentId)
    if (!e) continue
    if (write.kind === 'retry') {
      e.status = 'active'
      e.nextSendAt = write.nextSendAt || nextRetryAtIso(write.attempts || 0)
      e.lastError = String(write.error || 'Retry scheduled').slice(0, 240)
      e.retryAttempts = (write.attempts || 0) + 1
...
export async function processDueEnrollments({
  ...
  const now = new Date().toISOString()
  let useSqlRecipients = false
  if (campaignId) {
    const { campaignsV3TableActive, orgHasCampaignRecipients, readDueCampaignRecipients } =
      await import('./campaignsV3Table.js')
    if (campaignsV3TableActive()) {
      useSqlRecipients = await orgHasCampaignRecipients(campaignId)
    }
  }
  ...
  const due = campaignId
    ? sendSession
      ...
      : useSqlRecipients
        ? await readDueCampaignRecipients(campaignId, limit)
        : await readDueCampaignEnrollments(campaignId, limit)
    : []

  const campaignForBulk = campaignId ? (store.marketingCampaigns || [])[0] : null
  if (due.length && sendSession?.leadById?.size) {
    store = attachLeadsFromSession(store, sendSession, due)
  } else if (useSqlRecipients && due.length) {
    const { leadFromRecipientEnrollment } = await import('./campaignsV3Table.js')
    store = {
      ...store,
      savedLeads: due.map((e) => ({
        id: e.leadId,
        lead: leadFromRecipientEnrollment(e),
      })),
    }
  } else if (campaignForBulk?.source === 'pipeline_bulk' && due.length) {
  ...
  } else if (due.length && campaignForBulk?.channel === 'email') {
    const emailSender = actorUser || resolveCampaignSender(store, campaignForBulk)
    if (emailSender) {
      const leadIds = due.map((e) => e.leadId).filter(Boolean)
      const { pipelineStore } = await loadPipelineStoreForLeadIds(emailSender, leadIds)
      store = { ...store, savedLeads: pipelineStore.savedLeads || [] }
    }
  }
  ...
  for (const enrollment of due) {
    ...
    if (!result.sent) {
      ...
      const attempts = enrollment.retryAttempts || 0
      if (shouldRetrySendError(err, attempts, { code: result.code })) {
        pendingWrites.push({
          kind: 'retry',
          enrollmentId: enrollment.id,
          chunkIndex: enrollment.chunkIndex,
          ...
          error: err,
          attempts,
          nextSendAt: nextRetryAtIso(attempts),
        })
        continue
      }
      errors.push(err)
  ...
  if (pendingWrites.length && campaignId) {
    try {
      if (sendSession) {
        ...
        const { syncRecipientWritesToSql } = await import('./campaignsV3Sync.js')
        syncRecipientWritesToSql(campaignId, pendingWrites, due)
...
function campaignHasEngagementStats(c) {
  const s = c?.stats || c?.analytics || {}
  if ((s.uniqueOpens || 0) > 0 || (s.uniqueClicks || 0) > 0) return true
  if ((s.openRate || 0) > 0 && (s.sent || s.recipientsSent || 0) > 0) return true
  if ((s.clickRate || 0) > 0 && (s.sent || s.recipientsSent || 0) > 0) return true
  return false
}

export async function enrichCampaignEngagementFromEnrollments(
  campaigns,
  store,
  user,
  { maxCampaigns = 12, skipWhenStatsComplete = true } = {}
) {
  if (!campaigns?.length) return campaigns || []

  let needsEngagement = campaigns.filter(
    (c) =>
      (c.stats?.sent || c.stats?.recipientsSent || 0) > 0 ||
      ['active', 'completed', 'paused', 'stopped', 'archived'].includes(c.status)
  )
  if (skipWhenStatsComplete) {
    needsEngagement = needsEngagement.filter((c) => !campaignHasEngagementStats(c))
  }
  needsEngagement = needsEngagement
    .sort((a, b) => new Date(b.startedAt || b.updatedAt || 0) - new Date(a.startedAt || 0))
...
/** Backfill list metrics from events (+ optional enrollment shards for full overview). */
export async function enrichCampaignsListMetrics(campaigns, events = [], options = {}) {
  const { light = false } = options
  if (!campaigns?.length) return campaigns || []

  const testByCampaign = new Map()
  const lastTestAtByCampaign = new Map()
  for (const ev of events || []) {
    if (ev.type !== 'test_send' || !ev.campaignId) continue
    testByCampaign.set(ev.campaignId, (testByCampaign.get(ev.campaignId) || 0) + 1)
    const prev = lastTestAtByCampaign.get(ev.campaignId)
    if (!prev || String(ev.createdAt) > String(prev)) {
      lastTestAtByCampaign.set(ev.campaignId, ev.createdAt)
    }
  }

  const enrollmentStats = new Map()
  if (!light) {
    const needsEnrollment = campaigns
      .filter((c) => {
        const sent = c.stats?.sent || c.stats?.recipientsSent || 0
        const enrolled = c.stats?.enrolled || 0
        if (sent > 0 || enrolled > 0) return false
        const status = String(c.status || 'draft').toLowerCase()
        return status === 'active' || status === 'completed' || c.startedAt
      })
      .slice(0, 8)

    for (const c of needsEnrollment) {
      try {
        const rows = await readCampaignEnrollments(c.id)
        ...
        let failed = 0
        for (const e of rows) {
          const count = e.sentCount || (e.status === 'completed' ? 1 : 0)
          sent += count
          if (e.status === 'failed') failed += 1
        }
        enrollmentStats.set(c.id, { enrolled: rows.length, sent, failed })
      } catch {
        /* skip shard read errors */
      }
    }
  }
  ...
  return campaigns.map((c) => {
    const stats = { ...(c.stats || {}) }
    const testFromEvents = testByCampaign.get(c.id) || 0
    if (testFromEvents > 0) {
      stats.testSent = Math.max(stats.testSent || 0, testFromEvents)
    }
    const enroll = enrollmentStats.get(c.id)
    if (enroll) {
      stats.enrolled = Math.max(stats.enrolled || 0, enroll.enrolled)
      stats.sent = Math.max(stats.sent || 0, enroll.sent)
      stats.failed = Math.max(stats.failed || 0, enroll.failed)
    }

    const lastTest =
      lastTestAtByCampaign.get(c.id) || c.lastTestSentAt || stats.lastTestSentAt || null
    if (lastTest) stats.lastTestSentAt = lastTest

    const merged = {
      ...c,
      stats,
      lastTestSentAt: lastTest,
      testSendCount: Math.max(c.testSendCount || 0, stats.testSent || 0),
    }
    const status = resolveMergedCampaignStatus(merged, stats)
    return { ...merged, status }
  })
}

/** Fast campaigns-tab payload: stats shards only — no marketingEvents scan. */
export async function marketingOverviewLight(store, user) {
  let campaigns = enrichMarketingRows(
    store,
    user,
    filterMarketingCampaignsVisible(store.marketingCampaigns, user)
  )
  campaigns = await mergeCampaignStatsShards(campaigns, { maxCampaigns: 32 })
  campaigns = campaigns.map((c) => {
    const stats = finalizeCampaignEngagementStats({ ...(c.stats || {}) })
    const status = resolveMergedCampaignStatus(c, stats)
    return { ...c, stats, analytics: stats, status }
  })

  const listRows = filterMarketingRows(store.marketingLists, user).map((row) => ({
    ...row,
    leadIds: Array.isArray(row.leadIds) ? row.leadIds : [],
    memberCount:
      row.memberCount ?? (Array.isArray(row.leadIds) ? row.leadIds.length : 0),
  }))
  ...
  return {
    ...
    summary: marketingSummary(campaigns, []),
...
export async function marketingOverview(store, user, options = {}) {
  ...
  if (light) return marketingOverviewLight(store, user)
  ...
  campaigns = await enrichCampaignsListMetrics(campaigns, events, { light: false })
  // Full overview: capped enrollment reads for send progress backfill.
  {
    const needsEngagement = campaigns.filter(
      (c) =>
        (c.stats?.sent || c.stats?.recipientsSent || c.stats?.enrolled || 0) > 0 ||
        ['active', 'completed', 'paused', 'stopped', 'archived'].includes(c.status)
    )
    if (needsEngagement.length) {
      const enriched = await enrichCampaignEngagementFromEnrollments(needsEngagement, store, user, {
        maxCampaigns: 8,
        skipWhenStatsComplete: true,
      })
      const byId = new Map(enriched.map((c) => [c.id, c]))
      campaigns = campaigns.map((c) => byId.get(c.id) || c)
    }
  }

  campaigns = campaigns.map((c) => {
    const stats = finalizeCampaignEngagementStats({ ...(c.stats || {}), ...(c.analytics || {}) })
    return { ...c, stats, analytics: { ...(c.analytics || {}), ...stats } }
  })

  const listRows = filterMarketingAssets(store, user, store.marketingLists, {
    filterLeadIds: true,
...
export async function sendCampaignTestEmails(store, user, campaign, { emails = [] } = {}) {
  ...
  for (const email of targets.slice(0, 5)) {
    ...
    const ok = Boolean(result.sent)
    results.push({ email, sent: ok, error: result.error || null })
    if (ok) {
      await recordMarketingEvent({
        organizationId: campaign.organizationId || user.organizationId || null,
  ...
  if (sent > 0) {
    ...
    await updateStorePartial(['marketingCampaigns'], (draft) => {
      ...
      if (row) {
        ...
        row.testSendCount = (row.testSendCount || 0) + sent
        row.stats = row.stats || {}
        row.stats.testSent = (row.stats.testSent || 0) + sent
        row.stats.lastTestSentAt = now
        row.updatedAt = now
      }
      return draft
    })
    await bumpCampaignStatsShard(campaign.id, { testSent: sent, lastTestSentAt: now })
  }

  return { results, sent, campaignId: campaign.id, lastTestSentAt: sent > 0 ? new Date().toISOString() : null }
}

/** After SQL queue sends, align enrollments + campaign status with stats. */
export async function reconcileCampaignAfterQueueSends(campaignId) {
  ...
  if (enrolled > 0 && sent + failed >= enrolled) {
    const { setCampaignSendStatus } = await import('./email/campaignLifecycle.js')
    await setCampaignSendStatus(campaignId, 'completed')
    return
  }

  if (sent > 0) {
    await bumpCampaignStatsShard(campaignId, { status: 'active' })
    await updateStorePartial(['marketingCampaigns'], (draft) => {
      const row = (draft.marketingCampaigns || []).find((c) => c.id === campaignId)
...
export async function recordCampaignQueueSend(campaignId, enrollmentId, sendResult) {
  if (!campaignId) return

  if (enrollmentId && sendResult?.sent) {
    await patchCampaignEnrollments(
      campaignId,
      [
        {
          ...
          result: {
            ...
            id: sendResult.logPayload?.gmailMessageId || sendResult.logPayload?.resendId,
            logPayload: sendResult.logPayload,
          },
          nextStep: 0,
          delayDays: 0,
        },
  ...
  await reconcileCampaignAfterQueueSends(campaignId)
}
...
export async function processScheduledCampaigns({ limit = 5 } = {}) {
  ...
  for (const campaign of due) {
    try {
      ...
      await syncMarketingCampaignStatus(user, campaign.id, {
        status: 'active',
        startedAt: now,
      })

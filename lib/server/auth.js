...
async function readAuthStoreForLogin() {
  return readStore({ only: LOGIN_STORE_COLLECTIONS, ...SESSION_READ_OPTIONS })
}
...
async function userFromSessionToken(token) {
  ...
  if (payload) {
    ...
    try {
      ...
      console.warn('session user lookup:', error?.message || error)
...
/**
 * Page-load session: cache first, DB refresh at most every 5 minutes.
 * If the database times out, keep the existing JWT/cache so Pipeline still opens.
 */
export async function loadSessionForRequest(req, res) {
  ...
  if (cached && !shouldRefreshSessionFromDatabase(payload.userId)) {
  ...
  try {
    ...
    console.warn('session db refresh:', error?.message || error)
    if (cached) return { user: cached, token, stale: true }
    const jwtView = payloadToUserRecord(payload)
    if (jwtView?.id) {
      setCachedSessionUser(jwtView.id, jwtView)
      return { user: jwtView, token, stale: true }
    }
    throw new Error(customerSafeErrorMessage(error))
  }

  if (cached) return { user: cached, token }
  return { user: payloadToUserRecord(payload), token }
}

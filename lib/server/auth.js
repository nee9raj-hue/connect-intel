import {
  AUTH_STORE_COLLECTIONS,
  LOGIN_STORE_COLLECTIONS,
  SESSION_STORE_COLLECTIONS,
  createId,
  readStore,
  updateStore,
  updateStorePartial,
  writeStoreCollections,
} from './store.js'
import { parseCookies, setSessionCookie, clearSessionCookie } from './cookies.js'
import {
  DEFAULT_TRIAL_CREDITS_PAISE,
  FREE_FULL_LEAD_PREVIEW_COUNT,
  LEAD_UNLOCK_PRICE_PAISE,
  SESSION_COOKIE,
  SESSION_TTL_MS,
  getAdminEmails,
} from './config.js'
import { CRM_SOLO_FREE_TIER } from './crmProductFlags.js'
import {
  payloadToUserRecord,
  signSessionToken,
  verifySessionToken,
} from './sessionJwt.js'
import {
  applyPendingInvites,
  buildOrgUserResponse,
  consumeOrgSearchQuota,
  isPlatformOperatorUser,
} from './organizations.js'
import { COMPANY_MAILBOX } from './gmailOAuth.js'
import {
  getCachedSessionUser,
  getCachedSessionUserDistributed,
  invalidateSessionUserCache,
  markSessionDatabaseRefreshed,
  setCachedSessionUser,
} from './authSessionCache.js'
import { repairOrgSqlSyncIfNeeded, deferMemberSqlSync } from './orgSqlSync.js'
import {
  customerSafeErrorMessage,
  customerSignInBusyMessage,
} from './customerError.js'
import { findAuthUserByEmail, persistLastLoginFast } from './storeUserLookup.js'

function getEmailDomain(email) {
  return String(email || '').split('@')[1]?.toLowerCase() || 'personal'
}

function resolvePlatformRole(email) {
  const normalized = String(email || '').toLowerCase()
  if (normalized === COMPANY_MAILBOX) return 'admin'
  const admins = getAdminEmails()
  if (admins.includes(normalized)) return 'admin'
  return 'member'
}

function markPlatformOperatorReady(user) {
  if (!isPlatformOperatorUser(user)) return
  user.onboardingComplete = true
  user.accountType = user.accountType || 'individual'
}

const SESSION_READ_OPTIONS = { timeoutMs: 8_000, attempts: 2 }

function viewFromLookup(userSnapshot, looked, email) {
  const store = {
    users: [userSnapshot],
    organizations: looked.organization ? [looked.organization] : [],
    organizationMemberships: looked.memberships || [],
  }
  userSnapshot.role = resolvePlatformRole(email)
  markPlatformOperatorReady(userSnapshot)
  return buildOrgUserResponse(userSnapshot, store)
}

async function readAuthStoreForLogin() {
  return readStore({ only: LOGIN_STORE_COLLECTIONS, timeoutMs: 45_000, attempts: 1 })
}

export async function upsertUser(profile) {
  const now = new Date().toISOString()
  const email = String(profile.email || '').toLowerCase()

  const looked = await findAuthUserByEmail(email)
  if (looked?.user?.id) {
    const userSnapshot = {
      ...looked.user,
      name: profile.name || looked.user.name,
      company: profile.company || looked.user.company,
      picture: profile.picture || looked.user.picture || null,
      lastLoginAt: now,
      authProvider: profile.authProvider || looked.user.authProvider || 'google',
    }
    if (profile.passwordHash) userSnapshot.passwordHash = profile.passwordHash
    const view = viewFromLookup(userSnapshot, looked, email)
    void persistLastLoginFast(email, now, userSnapshot.authProvider)
    setCachedSessionUser(view.id, view)
    return view
  }

  if (email.endsWith('@xindus.net')) {
    throw new Error(customerSignInBusyMessage())
  }

  let store
  try {
    store = await readAuthStoreForLogin()
  } catch (error) {
    throw new Error(customerSafeErrorMessage(error, customerSignInBusyMessage()))
  }
  const existing = store.users.find((entry) => String(entry.email || '').toLowerCase() === email)
  const dirty = new Set(['users'])
  let userSnapshot = null

  const isNewUser = !existing
  if (existing) {
    existing.name = profile.name || existing.name
    existing.company = profile.company || existing.company
    existing.picture = profile.picture || existing.picture || null
    existing.lastLoginAt = now
    existing.authProvider = profile.authProvider || existing.authProvider || 'google'
    if (profile.passwordHash) existing.passwordHash = profile.passwordHash
    existing.role = resolvePlatformRole(email)
    const beforeOrg = existing.organizationId
    applyPendingInvites(store, existing)
    if (existing.organizationId !== beforeOrg) {
      dirty.add('organizationMemberships')
      dirty.add('organizations')
    }
    markPlatformOperatorReady(existing)
    userSnapshot = { ...existing }
  } else {
    const created = {
      id: createId('user'),
      email,
      name: profile.name || email.split('@')[0] || 'User',
      company: profile.company || getEmailDomain(email),
      picture: profile.picture || null,
      plan: profile.plan || 'free',
      role: resolvePlatformRole(email),
      searchesLeft: CRM_SOLO_FREE_TIER ? 0 : (profile.searchesLeft ?? 25),
      authProvider: profile.authProvider || 'google',
      passwordHash: profile.passwordHash || null,
      organizationId: null,
      creditsPaise: CRM_SOLO_FREE_TIER ? 0 : DEFAULT_TRIAL_CREDITS_PAISE,
      onboardingComplete: false,
      accountType: null,
      createdAt: now,
      lastLoginAt: now,
    }
    store.users.push(created)
    applyPendingInvites(store, created)
    markPlatformOperatorReady(created)
    if (created.organizationId) {
      dirty.add('organizationMemberships')
      dirty.add('organizations')
    }
    userSnapshot = created
  }

  const persist = writeStoreCollections(store, [...dirty]).catch((err) => {
    console.warn('sign-in persist:', err?.message || err)
  })
  if (isNewUser) {
    await persist
  } else {
    void import('@vercel/functions')
      .then(({ waitUntil }) => waitUntil(persist))
      .catch(() => {
        void persist
      })
  }
  const view = buildOrgUserResponse(userSnapshot, store)
  if (view.organizationId && view.accountType === 'company') {
    deferMemberSqlSync(view.organizationId, view.id, store)
  }
  invalidateSessionUserCache(view.id)
  if (isNewUser && !CRM_SOLO_FREE_TIER) {
    void updateStorePartial(['creditLedger'], (draft) => {
      draft.creditLedger.push({
        id: createId('credit'),
        userId: view.id,
        kind: 'grant',
        amountPaise: DEFAULT_TRIAL_CREDITS_PAISE,
        description: 'Welcome trial credits',
        createdAt: now,
      })
    }).catch((err) => console.warn('welcome credits:', err?.message || err))
  }
  return view
}

export async function createSession(res, user, req) {
  const token = signSessionToken(user)
  setSessionCookie(res, token, req)
  return { token }
}

async function getLegacySessionUser(sessionId) {
  const now = Date.now()
  const store = await readStore({ only: AUTH_STORE_COLLECTIONS })
  const session = store.sessions.find((entry) => entry.id === sessionId)

  if (!session) return null
  if (new Date(session.expiresAt).getTime() <= now) {
    await updateStore((fresh) => {
      fresh.sessions = fresh.sessions.filter((entry) => entry.id !== sessionId)
      return fresh
    })
    return null
  }

  const user = store.users.find((entry) => entry.id === session.userId)
  return user ? buildOrgUserResponse(user, store) : null
}

function readBearerToken(req) {
  const header = String(req.headers?.authorization || '')
  if (header.startsWith('Bearer ')) {
    return header.slice(7).trim()
  }
  return null
}

export function findUserInStore(store, payload) {
  if (!payload) return null
  const byId = store.users.find((u) => u.id === payload.userId)
  if (byId) return byId
  const email = String(payload.email || '').toLowerCase()
  if (!email) return null
  return store.users.find((u) => String(u.email || '').toLowerCase() === email) || null
}

async function userFromSessionToken(token) {
  if (!token) return null

  const payload = verifySessionToken(token)
  if (payload) {
    const cached =
      (await getCachedSessionUserDistributed(payload.userId)) ||
      getCachedSessionUser(payload.userId)
    if (cached) return cached

    const jwtView = payloadToUserRecord(payload)
    if (jwtView?.id) {
      setCachedSessionUser(jwtView.id, jwtView)
      return jwtView
    }
    return null
  }

  try {
    return await getLegacySessionUser(token)
  } catch (error) {
    console.warn('session legacy lookup:', error?.message || error)
    return null
  }
}

export async function getSessionUser(req) {
  const cookies = parseCookies(req)
  const cookieValue = cookies[SESSION_COOKIE]
  const fromCookie = await userFromSessionToken(cookieValue)
  if (fromCookie) return fromCookie

  const bearer = readBearerToken(req)
  if (bearer) return userFromSessionToken(bearer)

  return null
}

/** Reload user from database and refresh session cookie (keeps Gmail OAuth + org data in sync). */
export async function refreshSessionFromDatabase(req, res) {
  const cookies = parseCookies(req)
  const token = cookies[SESSION_COOKIE] || readBearerToken(req)
  const payload = verifySessionToken(token)
  if (!payload) return null

  const store = await readStore({ only: SESSION_STORE_COLLECTIONS, ...SESSION_READ_OPTIONS })
  const user = findUserInStore(store, payload)
  if (!user) return null

  const email = String(user.email || '').toLowerCase()
  const nextRole = resolvePlatformRole(email)
  if (user.role !== nextRole) {
    user.role = nextRole
    await updateStorePartial(['users'], (draft) => {
      const row = findUserInStore(draft, payload)
      if (row) row.role = nextRole
    })
    invalidateSessionUserCache(user.id)
  }

  repairOrgSqlSyncIfNeeded(user, store)

  const view = buildOrgUserResponse(user, store)
  let sessionToken = token
  if (res) {
    sessionToken = await refreshSessionCookie(res, view, req)
  }
  markSessionDatabaseRefreshed(user.id, view)
  return { user: view, token: sessionToken }
}

function readSessionTokenFromRequest(req) {
  const cookies = parseCookies(req)
  return cookies[SESSION_COOKIE] || readBearerToken(req) || null
}

/**
 * Page-load session from the JWT/cookie only. Never wait on store_collections.
 */
export async function loadSessionForRequest(req, res) {
  const token = readSessionTokenFromRequest(req)
  if (!token) return { user: null, token: null }

  const payload = verifySessionToken(token)
  if (!payload) {
    try {
      const user = await getSessionUser(req)
      return { user: user || null, token }
    } catch (error) {
      console.warn('session legacy lookup:', error?.message || error)
      return { user: null, token, error: customerSafeErrorMessage(error) }
    }
  }

  const cached =
    (await getCachedSessionUserDistributed(payload.userId)) || getCachedSessionUser(payload.userId)
  const user = cached || payloadToUserRecord(payload)
  if (user?.id) setCachedSessionUser(user.id, user)

  if (res && user) {
    try {
      setSessionCookie(res, token, req)
    } catch {
      /* cookie refresh is best-effort */
    }
  }

  return { user, token }
}

export async function destroySession(req, res) {
  clearSessionCookie(res, req)
}

export async function requireUser(req, res) {
  const user = await getSessionUser(req)
  if (!user) {
    res.status(401).json({ error: 'Authentication required. Please sign out and sign in again.' })
    return null
  }
  return user
}

export async function requireAdmin(req, res) {
  const user = await requireUser(req, res)
  if (!user) return null
  if (!user.isPlatformAdmin) {
    res.status(403).json({ error: 'Platform admin access required' })
    return null
  }
  return user
}

/** Company org_admin or platform admin — for team pipeline upload. */
export async function requireOrgAdmin(req, res) {
  const user = await requireUser(req, res)
  if (!user) return null
  if (user.isPlatformAdmin) return user
  if (user.isOrgAdmin && user.accountType === 'company' && user.organizationId) return user
  res.status(403).json({ error: 'Company admin access required' })
  return null
}

export async function consumeSearchQuota(req, res) {
  const sessionUser = await getSessionUser(req)
  if (!sessionUser) {
    throw new Error('Authentication required')
  }

  const QUOTA_COLLECTIONS = ['users', 'organizations', 'organizationMemberships']
  let refreshed = null

  await updateStorePartial(QUOTA_COLLECTIONS, (draft) => {
    let user = findUserInStore(draft, {
      userId: sessionUser.id,
      email: sessionUser.email,
    })
    if (!user) {
      throw new Error('Session out of date — please sign out and sign in again.')
    }

    const orgView = buildOrgUserResponse(user, draft)
    if (!orgView.canSearch) {
      throw new Error('Lead search is not enabled for your account. Ask your company admin.')
    }

    if (orgView.accountType === 'individual' || !orgView.organizationId) {
      const left = user.searchesLeft ?? 25
      if (left <= 0) throw new Error('No searches remaining on your plan.')
      user.searchesLeft = left - 1
    } else {
      const result = consumeOrgSearchQuota(draft, orgView)
      user = result.user
    }

    refreshed = buildOrgUserResponse(user, draft)
    return draft
  })

  setSessionCookie(res, signSessionToken(refreshed), req)
  return refreshed
}

export async function refreshSessionCookie(res, user, req) {
  const token = signSessionToken(user)
  setSessionCookie(res, token, req)
  return token
}

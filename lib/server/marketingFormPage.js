...
function fieldHtml(field, prefill = {}) {
  ...
  if (field.type === 'consent') {
    return `<div class="consent-field">${help}<label class="choice consent"><input type="checkbox" id="${escapeHtml(field.id)}" name="${escapeHtml(field.id)}" value="on"${req} /> ${escapeHtml(field.label)}${field.required ? ' *' : ''}</label></div>`
  }
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .consent-field { margin-top:20px; padding:14px; background:#f9fafb; border-radius:10px; border:1px solid #e5e7eb; }
    .consent-field .choice { margin:0; font-size:0.85rem; line-height:1.45; }

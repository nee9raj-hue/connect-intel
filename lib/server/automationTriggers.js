...
/** Legacy marketing UI trigger ids → catalog types. */
const MARKETING_TRIGGER_ALIASES = {
  lead_stage_changed: 'status_enter',
  deal_created: 'deal_won',
}

function automationTriggerType(automation) {
  return (
    automation.trigger?.type ||
    automation.graph?.nodes?.find((n) => n.type === 'trigger')?.config?.type ||
    ''
  )
}

function triggerMatches(automationType, firedType) {
  if (!automationType || !firedType) return false
  if (automationType === firedType) return true
  return MARKETING_TRIGGER_ALIASES[automationType] === firedType
}
...
export async function fireAutomationTrigger({
  ...
  const automations = (store.marketingAutomations || []).filter((a) => {
    ...
    const triggerType = automationTriggerType(a)
    return triggerMatches(triggerType, type)
  })
  ...
  for (const automation of automations) {
    ...
    if (type === 'sequence_completed' && config.sequenceId && meta.sequenceId !== config.sequenceId) {
      continue
    }

...
export async function cacheSet(key, value, { ttlSeconds = 60, staleSeconds } = {}) {
  const fetchedAt = Date.now()
  const retainSeconds = Math.max(
    ttlSeconds * 2,
    Number(staleSeconds) > 0 ? Number(staleSeconds) : ttlSeconds + 30
  )
  localCache.set(key, { value, fetchedAt, expiresAt: fetchedAt + retainSeconds * 1000 })
  try {
    await redisSet(
      key,
      JSON.stringify({ value, fetchedAt }),
      { exSeconds: retainSeconds }
    )
  } catch (err) {
...
export function pipelineBootstrapCacheKey(user, query = {}, generation = 0) {
  ...
  return `pipe:bootstrap:v2:${org}:${uid}:${parts.join(':')}`
}

...
export function isTracedApiRoute(route = '') {
  const key = String(route || '')
  return key.startsWith('crm/') || key.startsWith('marketing')
}

/** Sentry performance span for CRM / marketing API routes. */
export async function withRequestSpan({ name, op = 'http.server' }, fn) {
  const mod = await getSentry()
  if (!mod?.startSpan) {
    return fn()
  }
  return mod.startSpan({ name, op, attributes: { 'http.route': name } }, fn)
}

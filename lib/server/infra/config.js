...
import { isEmailSendsEnabled } from '../emailSends.js'
import { isWorkflowRunsEnabled } from '../workflowRuns.js'
...
/** Campaign Engine V3 — campaigns_v3 + campaign_recipients SQL mirror. */
export function isCampaignsV3TableEnabled() {
  if (flag('CAMPAIGNS_V3_TABLE_OFF') || flag('DISABLE_CAMPAIGNS_V3_TABLE')) return false
  if (flag('USE_CAMPAIGNS_V3_TABLE') || flag('CAMPAIGNS_V3_TABLE')) return true
  return isSupabaseConfigured() && isPipelineLeadsTableEnabled()
}

/** Prefer SQL campaign_recipients as enrollment source of truth when mirrored. */
export function isMessagingSqlRecipientsPrimary() {
  if (flag('MESSAGING_SQL_RECIPIENTS_OFF')) return false
  if (flag('MESSAGING_SQL_RECIPIENTS') || flag('USE_SQL_RECIPIENTS')) return true
  return isCampaignsV3TableEnabled()
}
...
export function getInfraStatus() {
  return {
    ...
    pipelineTasksTable: isPipelineLeadsTableEnabled() && isSupabaseConfigured(),
    pipelineMeetingsTable: isPipelineLeadsTableEnabled() && isSupabaseConfigured(),
    pipelineCompaniesTable: isPipelineLeadsTableEnabled() && isSupabaseConfigured(),
    campaignsV3Table: isCampaignsV3TableEnabled(),
    ...
    auditEvents: isAuditEventsEnabled(),
    emailSends: isEmailSendsEnabled(),
    workflowRuns: isWorkflowRunsEnabled(),
  }
}

...
function entryMine(entry, userId) {
  ...
  return String(entry.assignedToUserId || entry.savedByUserId || entry.userId || '') === uid
}
...
function buildCommandBar(counts) {
  const items = [
    {
      id: 'tasks',
      label: "Today's tasks",
      ...
      action: { panel: 'pipeline', hint: 'View tasks' },
    },
    {
      id: 'followups',
    ...
    },
    {
      ...
      trend: null,
      action: { panel: 'pipeline', view: 'deals' },
    },
    {
      id: 'meetings',
...
function buildLeadFocus(scanned, userId) {
  ...
  for (const entry of scanned) {
    ...
    if (crm.status === 'follow_up') followUpDue += 1
...
function buildSmartInsights(counts, pipeline, leadFocus, role) {
  const insights = []
...
function buildGoals(rollup, comparison) {
  const weeklyTarget = 25
  const achievement = rollup?.activitiesTotal || 0
  const prev = comparison?.activitiesTotal?.previous || 0
...
function scanCounts(scanned, userId, tz) {
  ...
  for (const entry of scanned) {
    ...
    if (crm.status === 'replied' || crm.status === 'follow_up') {
      counts.dealsClosing += 1
    }

    for (const deal of crm.deals || []) {
      if (!deal.wonAt && !deal.lostAt) {
        const close = deal.expectedCloseAt || deal.updatedAt
        if (close && new Date(close).getTime() <= weekEnd) counts.dealsClosing += 1
      }
    }
...
export function buildMyDayDashboard(store, user, entries, { timeZone = null, since = 0 } = {}) {
  ...
  const insights = buildSmartInsights(counts, pipeline, leadFocus, role)

  const rollup = { activitiesTotal: priorities.length }
  const comparison = { activitiesTotal: { previous: Math.max(0, priorities.length - 2) } }

  return {
    role,
    greeting: 'What should you do next?',
    commandBar: buildCommandBar(counts),
    priorities,
    ...
    goals: buildGoals(rollup, comparison),
    teamIntelLink: role === 'manager' || role === 'admin',
  }
}

...
import { CRM_STATUS_IDS, isClosedCrmLeadStatus, normalizeCrmLeadStatus } from '../crmLeadStatuses.js'
...
function buildCommandBarDetails(mine, userId, tz) {
  ...
  for (const entry of mine) {
    ...
    const followDue = crm.nextFollowUpAt && new Date(crm.nextFollowUpAt).getTime() <= endToday
    if (followDue) {
    ...
    if (normalizeCrmLeadStatus(crm.status) === 'unqualified' && entry.savedAt && now - new Date(entry.savedAt).getTime() < 7 * MS_DAY) {
...
function buildCommandBar(counts, details, { freightOrg = false } = {}) {
  ...
  const items = [
    ...
    },
    {
      ...
      action: { panel: 'pipeline', followUpDue: true },
...
function buildPriorities(scanned, userId, tz, { useSqlTasksMeetings = false, sqlTasks = [], sqlMeetings = [] } = {}) {
  const now = Date.now()
  const endToday = endOfLocalDay(tz)
  const priorities = []
  ...
  for (const entry of scanned) {
    ...
    if (normalizeCrmLeadStatus(crm.status) === 'opportunity') {
  ...
  if (useSqlTasksMeetings) {
    for (const row of sqlTasks || []) {
      const task = row.payload?.task || {}
      const leadId = row.lead_id
      const name = row.payload?.leadName || 'Lead'
      const company = row.payload?.company || ''
      const due = row.due_at ? new Date(row.due_at).getTime() : null
      const taskId = row.task_id || task.id
      if (!taskId) continue
      if (due && due < now) {
        priorities.push({
          ...
          score: 100,
          kind: 'task',
          title: row.title || task.title || `Task · ${name}`,
          subtitle: company || 'Overdue',
          leadId,
          dueAt: row.due_at,
          action: { panel: 'pipeline', leadId },
        ...
        priorities.push({
          ...
          score: 85,
          kind: 'task',
          title: row.title || task.title || `Task today · ${name}`,
          subtitle: company,
          leadId,
          dueAt: row.due_at,
          action: { panel: 'pipeline', leadId },
    ...
    for (const row of sqlMeetings || []) {
      const meeting = row.payload?.meeting || {}
      const leadId = row.lead_id
      const name = row.payload?.leadName || 'Lead'
      const company = row.payload?.company || ''
      const at = row.starts_at ? new Date(row.starts_at).getTime() : null
      const meetingId = row.meeting_id || meeting.id
      if (!meetingId || !at || at < now - 3600000) continue
      if (at <= endToday) {
        const mins = Math.round((at - now) / 60000)
        priorities.push({
          id: `meet-${meetingId}`,
...
function buildPipelineSnapshot(scanned, userId) {
  const stages = Object.fromEntries(CRM_STATUS_IDS.map((id) => [id, 0]))
  let dealValue = 0
  ...
  for (const entry of scanned) {
    ...
    if (last && now - new Date(last).getTime() > MS_14D && !isClosedCrmLeadStatus(st)) {
      stuckDeals += 1
    }
  }
...
function buildLeadFocus(scanned, userId) {
  ...
  for (const entry of scanned) {
    ...
    if (crm.nextFollowUpAt && new Date(crm.nextFollowUpAt).getTime() <= now + MS_DAY) {
      followUpDue += 1
    }
...
function buildRevenueProgress(scanned, userId, tz) {
  ...
  for (const entry of scanned) {
    ...
    if (!isClosedCrmLeadStatus(crm.status)) {
      openPipeline += val
      if (normalizeCrmLeadStatus(crm.status) === 'opportunity') forecast += val * 0.35
    }
  }
...
function scanCounts(scanned, userId, tz, { useSqlTasksMeetings = false, sqlTasks = [], sqlMeetings = [] } = {}) {
  const now = Date.now()
  const endToday = endOfLocalDay(tz)
  const weekEnd = endOfWeek(tz)
  const counts = {
 ...
 */
export function buildMyDayDashboard(
  store,
  user,
  entries,
  { timeZone = null, since = 0, sqlEntities = null } = {}
) {
  ...
  const sqlOpts = sqlEntities?.useSql
    ? {
        useSqlTasksMeetings: true,
        sqlTasks: sqlEntities.tasks || [],
        sqlMeetings: sqlEntities.meetings || [],
      }
    : {}
  const counts = scanCounts(mine, userId, tz, sqlOpts)
  const barDetails = buildCommandBarDetails(mine, userId, tz)

  const priorities = buildPriorities(mine, userId, tz, sqlOpts)

...
function buildCommandBar(counts, { freightOrg = false } = {}) {
...
function scanCounts(scanned, userId, tz) {
  ...
  for (const entry of scanned) {
    ...
    for (const deal of crm.deals || []) {
      ...
      if (t >= startOfLocalDay(tz) && t <= weekEnd) closingThisWeek = true
    }
    if (!closingThisWeek && crm.expectedCloseAt) {
      const t = new Date(crm.expectedCloseAt).getTime()
      if (t >= startOfLocalDay(tz) && t <= weekEnd) closingThisWeek = true
    }
    if (closingThisWeek) counts.dealsClosing += 1
...
export function buildMyDayDashboard(store, user, entries, { timeZone = null, since = 0 } = {}) {
  ...
  return {
    ...
    commandBar: buildCommandBar(counts, { freightOrg }),
    priorities,

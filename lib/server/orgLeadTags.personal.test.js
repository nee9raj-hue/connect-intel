...
describe('personal lead tags', () => {
  it('creates a personal tag without loading the CRM store blob', () => {
    const tag = createLeadTagRecord([], { name: 'Hot Follow Up', personal: true }, 'user_neeraj')
    assert.equal(tag.source, 'personal')
    assert.equal(tag.name, 'Hot Follow Up')
    assert.equal(tag.createdByUserId, 'user_neeraj')
  })

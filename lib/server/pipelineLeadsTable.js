...
export function buildPipelineLeadsScopedQuery(
  ...
  if (scope.assigneeUserId) {
    parts.push(`entry->>assignedToUserId=eq.${encodeURIComponent(scope.assigneeUserId)}`)
  }
  if (scope.unassigned) {
    parts.push('entry->>assignedToUserId=is.null')
  }
...
export async function readPipelineLeadsScopedPage(shardName, scope, options = {}) {
  if (!pipelineLeadsTableActive()) return null
  const rows = await supabaseRest(
    `${TABLE}?${buildPipelineLeadsScopedQuery(shardName, scope, options)}`,
    {},
    { timeoutMs: 30_000 }
  )

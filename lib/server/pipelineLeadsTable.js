import { CRM_STATUSES } from './crm.js'
import { getLeadCityFromFields, getLeadStateFromFields } from '../pipelineLeadLocation.js'
import { collectPipelineCollaboratorUserIds } from '../pipelineCollaborators.js'
import {
  pipelineAssigneeOrUnassignedPostgrestFilter,
  pipelineAssigneePostgrestFilter,
  pipelineCrmStatusPostgrestFilter,
  pipelineRepVisibilityPostgrestFilter,
  pipelineScopeOrUnassignedPostgrestFilter,
  pipelineUnassignedPostgrestFilter,
} from './pipelineQuery.js'
import { isPipelineLeadsTableEnabled } from './infra/config.js'
import { isSupabaseEnabled, supabaseRest } from './supabaseClient.js'
import { pipelineShardNameForUser } from './pipelineShard.js'
import { resolvePipelineTableScope, resolvePipelineTableScopeAsync } from './pipelineTableScope.js'
import { isPipelineHierarchyRbacEnabled } from './infra/config.js'
import { appendPipelineFilterSqlParts } from './pipelineFilterSql.js'
import { readStore } from './store.js'
import {
  isPipelineEntryInUserOrg,
  isPipelineEntryVisibleAsync,
} from './pipelineVisibility.js'

const META_STORE_COLLECTIONS = ['users', 'organizations', 'organizationMemberships']
const TABLE_READ_PAGE = 200

const TABLE = 'pipeline_leads'

/** Columns present on every deployed pipeline_leads table (base migration). */
const PIPELINE_LEADS_CORE_KEYS = [
  'lead_id',
  'shard_name',
  'organization_id',
  'user_id',
  'entry',
  'updated_at',
]

function isMissingPipelineColumnError(err) {
  const msg = String(err?.message || '')
  return /could not find the .* column/i.test(msg) && /schema cache/i.test(msg)
}

/** Runtime-safe row: JSON entry + core columns only (DB trigger fills denormalized cols when present). */
export function buildPipelineLeadRowCore(shardName, entry) {
  if (!entry || typeof entry !== 'object') return null
  const lead = entry.lead || entry
  const leadId = lead?.id || entry.leadId || entry.id
  if (!leadId) return null

  const updatedAt = entry.updatedAt || entry.savedAt || new Date().toISOString()

  return {
    lead_id: String(leadId),
    shard_name: shardName,
    organization_id: entry.organizationId || null,
    user_id: entry.userId || entry.savedByUserId || null,
    entry,
    updated_at: updatedAt,
  }
}

/** Map a pipeline shard entry to a `pipeline_leads` table row. */
export function buildPipelineLeadRow(shardName, entry, { denormalized = true } = {}) {
  const core = buildPipelineLeadRowCore(shardName, entry)
  if (!core || !denormalized) return core

  const lead = entry.lead || entry
  const ownerId = entry.assignedToUserId ? String(entry.assignedToUserId) : null
  const leadStatus = entry.crm?.status || 'new'

  return {
    ...core,
    owner_id: ownerId ? String(ownerId) : null,
    team_id: entry.teamId ? String(entry.teamId) : null,
    department_id: entry.departmentId ? String(entry.departmentId) : null,
    lead_status: leadStatus,
    email: lead.email || null,
    phone: lead.phone || null,
    city: getLeadCityFromFields(lead) || null,
    state: getLeadStateFromFields(lead) || null,
    lead_score:
      entry.crm?.leadScore != null && entry.crm?.leadScore !== ''
        ? Math.floor(Number(entry.crm.leadScore)) || null
        : null,
    deal_count: Array.isArray(entry.crm?.deals) ? entry.crm.deals.length : 0,
    collaborator_ids: collectPipelineCollaboratorUserIds(entry),
  }
}

function pickPipelineLeadUpsertRow(shardName, entry, { denormalized }) {
  return denormalized
    ? buildPipelineLeadRow(shardName, entry, { denormalized: true })
    : buildPipelineLeadRowCore(shardName, entry)
}

function toCorePipelineLeadRows(rows) {
  return rows.map((row) => {
    const out = {}
    for (const key of PIPELINE_LEADS_CORE_KEYS) {
      if (row[key] !== undefined) out[key] = row[key]
    }
    return out
  })
}

async function postPipelineLeadRows(chunk, { denormalized }) {
  try {
    await supabaseRest(
      `${TABLE}?on_conflict=shard_name,lead_id`,
      {
        method: 'POST',
        headers: { Prefer: 'resolution=merge-duplicates,return=minimal' },
        body: JSON.stringify(chunk),
      },
      { timeoutMs: 60_000 }
    )
  } catch (err) {
    if (!denormalized || !isMissingPipelineColumnError(err)) throw err
    await supabaseRest(
      `${TABLE}?on_conflict=shard_name,lead_id`,
      {
        method: 'POST',
        headers: { Prefer: 'resolution=merge-duplicates,return=minimal' },
        body: JSON.stringify(toCorePipelineLeadRows(chunk)),
      },
      { timeoutMs: 60_000 }
    )
  }
}

export function pipelineLeadsTableActive() {
  return isPipelineLeadsTableEnabled() && isSupabaseEnabled()
}

/** Batch-patch CRM fields without reading the full org shard (when table is enabled). */
export async function patchPipelineLeadsTable(user, patches, options = {}) {
  if (!pipelineLeadsTableActive()) return { patched: 0, mode: 'disabled' }
  const { trustOrgScope = false, metaStore: metaStoreHint = null } = options
  const shardName = pipelineShardNameForUser(user)
  const list = Array.isArray(patches) ? patches.filter((p) => p?.leadId) : []
  if (!list.length) return { patched: 0 }

  const metaStore =
    metaStoreHint || (await readStore({ only: META_STORE_COLLECTIONS }))

  let patched = 0
  let skipped = 0
  for (const { leadId, updateCrm } of list) {
    const rows = await supabaseRest(
      `${TABLE}?shard_name=eq.${encodeURIComponent(shardName)}&lead_id=eq.${encodeURIComponent(leadId)}&select=entry`,
      {},
      { timeoutMs: 15_000 }
    )
    const existing = Array.isArray(rows) && rows[0]?.entry ? rows[0].entry : null
    if (!existing) {
      skipped += 1
      continue
    }

    const allowed = trustOrgScope
      ? isPipelineEntryInUserOrg(user, existing)
      : await isPipelineEntryVisibleAsync(user, existing, metaStore)
    if (!allowed) {
      skipped += 1
      continue
    }

    const nextEntry = {
      ...existing,
      crm: updateCrm(existing.crm || {}),
      updatedAt: new Date().toISOString(),
    }
    const patchBody = {
      entry: nextEntry,
      updated_at: new Date().toISOString(),
    }
    const denorm = buildPipelineLeadRow(shardName, nextEntry, { denormalized: true })
    if (denorm) {
      for (const key of [
        'owner_id',
        'team_id',
        'department_id',
        'lead_status',
        'email',
        'phone',
        'city',
        'state',
        'lead_score',
        'deal_count',
      ]) {
        if (denorm[key] !== undefined) patchBody[key] = denorm[key]
      }
    }
    await supabaseRest(
      `${TABLE}?shard_name=eq.${encodeURIComponent(shardName)}&lead_id=eq.${encodeURIComponent(leadId)}`,
      {
        method: 'PATCH',
        headers: { Prefer: 'return=minimal' },
        body: JSON.stringify(patchBody),
      },
      { timeoutMs: 15_000 }
    )
    patched += 1
  }
  return { patched, skipped, mode: 'table' }
}

/** Remove one row from pipeline_leads (after shard/mirror delete). */
export async function deletePipelineLeadRow(shardName, leadId) {
  if (!pipelineLeadsTableActive() || !leadId) return { deleted: false, mode: 'disabled' }
  await supabaseRest(
    `${TABLE}?shard_name=eq.${encodeURIComponent(shardName)}&lead_id=eq.${encodeURIComponent(leadId)}`,
    { method: 'DELETE', headers: { Prefer: 'return=minimal' } },
    { timeoutMs: 15_000 }
  )
  return { deleted: true, mode: 'table' }
}

/** Remove many rows from pipeline_leads in one request. */
export async function deletePipelineLeadRows(shardName, leadIds = []) {
  const ids = [...new Set((leadIds || []).map(String).filter(Boolean))]
  if (!pipelineLeadsTableActive() || !ids.length) return { deleted: 0, mode: 'disabled' }
  const inList = ids.map((id) => encodeURIComponent(id)).join(',')
  await supabaseRest(
    `${TABLE}?shard_name=eq.${encodeURIComponent(shardName)}&lead_id=in.(${inList})`,
    { method: 'DELETE', headers: { Prefer: 'return=minimal' } },
    { timeoutMs: 30_000 }
  )
  return { deleted: ids.length, mode: 'table' }
}

/** Upsert rows into pipeline_leads (backfill or runtime). Skips feature flag when force=true. */
export async function upsertPipelineLeadRows(
  shardName,
  entries,
  { batchSize = 50, force = false, denormalized = true, skipMembershipGuard = false, metaStore = null, skipEnterpriseSync = false } = {}
) {
  if (!force && !pipelineLeadsTableActive()) return { upserted: 0, skipped: 0, mode: 'disabled' }
  if (!isSupabaseEnabled()) throw new Error('Supabase is not configured')

  let entriesToWrite = entries
  const orgId = organizationIdFromShardName(shardName)
  if (orgId && !skipMembershipGuard) {
    const store =
      metaStore || (await readStore({ only: META_STORE_COLLECTIONS }))
    const { enforceOrgMembershipOnPipelineEntries } = await import('./tenantPipelineCleanup.js')
    entriesToWrite = enforceOrgMembershipOnPipelineEntries(store, entries)
  }

  const rowsToUpsert = (entriesToWrite || [])
    .map((entry) => pickPipelineLeadUpsertRow(shardName, entry, { denormalized }))
    .filter(Boolean)

  if (!rowsToUpsert.length) return { upserted: 0, skipped: entries?.length || 0, mode: force ? 'backfill' : 'table' }

  let upserted = 0
  for (let i = 0; i < rowsToUpsert.length; i += batchSize) {
    const chunk = rowsToUpsert.slice(i, i + batchSize)
    await postPipelineLeadRows(chunk, { denormalized })
    upserted += chunk.length
  }

  if (upserted > 0 && entries?.length && !skipEnterpriseSync) {
    void import('./enterpriseLeadsTable.js')
      .then(({ syncEnterpriseLeadsFromEntries }) =>
        syncEnterpriseLeadsFromEntries(entries, { batchSize })
      )
      .catch((err) => {
        console.warn('enterprise leads sync:', err?.message || err)
      })
  }

  return {
    upserted,
    skipped: (entries?.length || 0) - upserted,
    mode: force ? 'backfill' : 'table',
  }
}

export async function upsertPipelineLeadRow(shardName, entry, options = {}) {
  if (!entry) return null
  const result = await upsertPipelineLeadRows(shardName, [entry], {
    batchSize: 1,
    force: options.force,
    skipMembershipGuard: options.skipMembershipGuard,
  })
  if (!result.upserted) return null
  return buildPipelineLeadRow(shardName, entry)
}

export function buildPipelineLeadsScopedQuery(
  shardName,
  scope = {},
  options = {}
) {
  const { offset = 0, limit = 100, status = 'all' } = options
  const parts = [
    `shard_name=eq.${encodeURIComponent(shardName)}`,
    'select=entry',
    'order=updated_at.desc',
  ]
  if (scope.organizationId) {
    parts.push(`organization_id=eq.${encodeURIComponent(scope.organizationId)}`)
  }
  if (scope.userId) {
    parts.push(`user_id=eq.${encodeURIComponent(scope.userId)}`)
  }
  if (scope.unassigned) {
    parts.push(pipelineUnassignedPostgrestFilter())
  } else if (scope.repOwnAndUnassigned && scope.viewerUserId) {
    parts.push(pipelineRepVisibilityPostgrestFilter(scope.viewerUserId))
  } else if (scope.managerWithUnassigned && scope.teamId) {
    parts.push(
      pipelineScopeOrUnassignedPostgrestFilter(`team_id.eq.${encodeURIComponent(scope.teamId)}`)
    )
  } else if (scope.managerWithUnassigned && scope.departmentId) {
    parts.push(
      pipelineScopeOrUnassignedPostgrestFilter(
        `department_id.eq.${encodeURIComponent(scope.departmentId)}`
      )
    )
  } else if (scope.managerWithUnassigned && scope.ownerId) {
    parts.push(pipelineAssigneeOrUnassignedPostgrestFilter(scope.ownerId))
  } else if (scope.includeUnassigned && scope.ownerId) {
    parts.push(pipelineRepVisibilityPostgrestFilter(scope.ownerId))
  } else if (scope.ownerId || scope.assigneeUserId) {
    const ownerFilter = pipelineAssigneePostgrestFilter(scope.ownerId || scope.assigneeUserId)
    if (ownerFilter) parts.push(ownerFilter)
  }
  const statusFilter = pipelineCrmStatusPostgrestFilter(status, {
    pipelineTrack: options.pipelineTrack,
    crmStageIds: options.crmStageIds,
    crmStages: options.crmStages,
  })
  if (statusFilter) parts.push(statusFilter)
  if (isPipelineHierarchyRbacEnabled()) {
    if (scope.teamId) parts.push(`team_id=eq.${encodeURIComponent(scope.teamId)}`)
    if (scope.departmentId) parts.push(`department_id=eq.${encodeURIComponent(scope.departmentId)}`)
  }
  appendPipelineFilterSqlParts(parts, options)
  parts.push(`offset=${Math.max(0, Math.floor(Number(offset) || 0))}`)
  parts.push(`limit=${Math.max(1, Math.floor(Number(limit) || 100))}`)
  return parts.join('&')
}

export async function readPipelineLeadsScopedPage(shardName, scope, options = {}, user = null, metaStore = null) {
  if (!pipelineLeadsTableActive()) return null

  const { offset = 0, limit = 100 } = options

  try {
    if (user && metaStore && isPipelineHierarchyRbacEnabled()) {
      const { getScopedLeadsQuery, scopedLeadsListUrl } = await import('./pipelineScopedQuery.js')
      const scoped = await getScopedLeadsQuery(
        user,
        {
          ...options,
          assigneeUserId: options.assigneeUserId || scope.assigneeUserId,
          scope: options.scope || options.hierarchyScope,
          offset,
          limit,
        },
        metaStore
      )
      const url = scopedLeadsListUrl(scoped, { select: 'entry' })
      const rows = await supabaseRest(url, {}, { timeoutMs: 8_000, attempts: 2, bypassCircuit: true })
      if (!Array.isArray(rows)) return []
      return rows.map((r) => r.entry).filter(Boolean)
    }

    let effectiveScope = scope
    if (user && metaStore) {
      effectiveScope = resolvePipelineTableScope(user, metaStore, {
        ...options,
        assigneeUserId: options.assigneeUserId || scope.assigneeUserId,
        scope: options.scope || options.hierarchyScope,
      })
    }

    const rows = await supabaseRest(
      `${TABLE}?${buildPipelineLeadsScopedQuery(shardName, effectiveScope, { ...options, offset, limit })}`,
      {},
      { timeoutMs: 8_000, attempts: 2, bypassCircuit: true }
    )
    if (!Array.isArray(rows)) return []
    return rows.map((r) => r.entry).filter(Boolean)
  } catch (error) {
    console.warn('pipeline_leads page:', error?.message || error)
    return null
  }
}

/** Unscoped page (all rows in shard) — prefer readPipelineLeadsScopedPage for list/bootstrap. */
export async function readPipelineLeadsPage(shardName, { offset = 0, limit = 100 } = {}) {
  return readPipelineLeadsScopedPage(shardName, {}, { offset, limit })
}

/** Walk scoped table pages until exhausted (replaces single blob shard download). */
export async function readAllPipelineLeadsScoped(
  shardName,
  scope = {},
  { pageSize = TABLE_READ_PAGE, user = null, metaStore = null, filters = {} } = {}
) {
  if (!pipelineLeadsTableActive()) return null
  const all = []
  let offset = 0
  const size = Math.max(50, Math.floor(Number(pageSize) || TABLE_READ_PAGE))

  while (true) {
    const page = await readPipelineLeadsScopedPage(
      shardName,
      scope,
      { ...filters, offset, limit: size },
      user,
      metaStore
    )
    if (!page?.length) break
    all.push(...page)
    if (page.length < size) break
    offset += page.length
  }

  return all
}

/** All pipeline rows visible to this user from pipeline_leads; null when table off or empty. */
export async function readPipelineLeadsForUser(user, metaStore, shardName, filters = {}) {
  if (!pipelineLeadsTableActive()) return null
  const scope = await resolvePipelineTableScopeAsync(user, metaStore, filters)
  const entries = await readAllPipelineLeadsScoped(shardName, scope, { user, metaStore, filters })
  return entries?.length ? entries : null
}

const LEAD_IDS_QUERY_BATCH = 40

function organizationIdFromShardName(shardName) {
  const match = String(shardName || '').match(/^pipeline_org_(.+)$/)
  return match ? match[1] : null
}

/** Load only specific pipeline rows by lead_id (no full shard download). */
export async function readPipelineLeadsByIds(shardName, leadIds) {
  if (!pipelineLeadsTableActive()) return null
  const ids = [...new Set((leadIds || []).filter(Boolean))]
  if (!ids.length) return []

  const orgId = organizationIdFromShardName(shardName)
  const entries = []
  for (let i = 0; i < ids.length; i += LEAD_IDS_QUERY_BATCH) {
    const chunk = ids.slice(i, i + LEAD_IDS_QUERY_BATCH)
    const inList = chunk.map((id) => encodeURIComponent(id)).join(',')
    const orgPart = orgId ? `&organization_id=eq.${encodeURIComponent(orgId)}` : ''
    const rows = await supabaseRest(
      `${TABLE}?shard_name=eq.${encodeURIComponent(shardName)}${orgPart}&lead_id=in.(${inList})&select=entry`,
      {},
      { timeoutMs: 25_000 }
    )
    if (!Array.isArray(rows)) continue
    entries.push(...rows.map((r) => r.entry).filter(Boolean))
  }

  // Imported rows may lack organization_id while still belonging to this shard — retry misses.
  if (orgId && entries.length < ids.length) {
    const found = new Set(entries.map((e) => String(e.lead?.id || e.id || '')))
    const missing = ids.filter((id) => !found.has(String(id)))
    for (let i = 0; i < missing.length; i += LEAD_IDS_QUERY_BATCH) {
      const chunk = missing.slice(i, i + LEAD_IDS_QUERY_BATCH)
      const inList = chunk.map((id) => encodeURIComponent(id)).join(',')
      const rows = await supabaseRest(
        `${TABLE}?shard_name=eq.${encodeURIComponent(shardName)}&lead_id=in.(${inList})&select=entry`,
        {},
        { timeoutMs: 25_000 }
      )
      if (!Array.isArray(rows)) continue
      entries.push(...rows.map((r) => r.entry).filter(Boolean))
    }
  }

  return entries
}

export async function readPipelineLeadById(shardName, leadId) {
  if (!leadId) return null
  const rows = await readPipelineLeadsByIds(shardName, [leadId])
  return rows[0] || null
}

/** Distinct lead owners in SQL — catches assignees missing from membership/index sync. */
export async function listDistinctOwnerIdsForOrg(organizationId, { limit = 8000 } = {}) {
  if (!organizationId || !isSupabaseEnabled() || !isPipelineLeadsTableEnabled()) return []
  try {
    const rows = await supabaseRest(
      `${TABLE}?organization_id=eq.${encodeURIComponent(organizationId)}&owner_id=not.is.null&select=owner_id&limit=${Math.max(1, Math.min(10000, Number(limit) || 8000))}`,
      {},
      { timeoutMs: 18_000, attempts: 1 }
    )
    if (!Array.isArray(rows)) return []
    return [...new Set(rows.map((r) => r.owner_id).filter(Boolean))]
  } catch {
    return []
  }
}

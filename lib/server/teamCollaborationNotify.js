...
function bodyForEmail(body, appUrl) {
  const mentionRe = /@\[([^\]]+)\]\(lead:([^)]+)\)/g
  const html = escapeHtml(String(body || ''))
    .replace(mentionRe, (_, label, leadId) => {
    ...
    .replace(/\n/g, '<br>')
  const text = String(body || '').replace(mentionRe, (_, label) => label)
  return { html, text }
}
...
export async function notifyTeamNoteRecipient({ store, note, actor }) {
  ...
  return sendCollaborationEmail({
    ...
    toUser: recipient,
    actor,
    subject: `Team note from ${actor?.name || 'teammate'}`,
    intro: `${actor?.name || 'A teammate'} shared a note with you:`,
    body: note.body,
    ctaLabel: 'Open Notes',
    ctaPath: '/?panel=team-notes',
  })
}

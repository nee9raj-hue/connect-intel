...
export async function buildDashboardBootstrap(sessionUser, { assigneeUserId = null } = {}) {
  ...
  const { entries, visibleOwnerIds, sqlSummary, assigneeUserId: scopedAssignee } = await resolveScopedEntries(
    user,
    store,
    { assigneeUserId }
  )
  ...
  if (role === 'manager') {
    ...
    payload.repPerformance = buildRepPerformance(entries, perfMembers, perfOwners)

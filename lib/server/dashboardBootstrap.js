...
import { getOrganization, listTeamMembers, resolveOrgRole } from './organizations.js'
...
function buildStatStrip(role, counts) {
  const mk = (id, label, count, linkLabel, action) => ({
    id,
    label,
    count,
    linkLabel,
  ...
  return [
    mk(
      ...
      'Open tasks',
      role === 'rep'
        ? { panel: 'pipeline', view: 'tasks', due: 'today', scopeOwner: 'me', returnTo: 'overview' }
        : role === 'manager'
          ? { panel: 'pipeline', view: 'tasks', due: 'today', hierarchyTeam: 'mine', returnTo: 'overview' }
          : { panel: 'pipeline', view: 'tasks', due: 'today', scope: 'all', returnTo: 'overview' }
    ),
    mk(
      'followups_due',
      'Follow-ups due',
    ...
    mk(
      ...
      'Activity',
      pipelineAction(role, { view: 'activity', activityFilter: 'unread' })
    ),
    mk(
      'new_assignments',
      'New assignments',
...
function buildActivityFeed(store, user, entries, { limit = 15, since = 0 } = {}) {
...
function buildTeamLeaderboard(entries, hierarchy) {
  ...
  return teams.map((team) => {
    ...
    return {
      ...
      action: { panel: 'pipeline', hierarchyTeam: team.id, returnTo: 'overview' },
    }
  }).sort((a, b) => b.activities7d - a.activities7d)
}
...
async function resolveScopedEntries(user, store) {
  const metaStore = {
    users: store.users,
  ...
  return { role, entries, visibleOwnerIds, sqlSummary }
}

export async function buildDashboardBootstrap(sessionUser) {
  ...
  if (role === 'marketing_manager') {
    const overview = await marketingOverview(store, user, { light: true })
  ...
  const { entries, visibleOwnerIds, sqlSummary } = await resolveScopedEntries(user, store)
  ...
  const thisWeek = buildThisWeek(store, user, entries, user.id)
  const activity = buildActivityFeed(store, user, entries, { limit: 15, since })
  const members = user.organizationId ? listTeamMembers(store, user.organizationId) : []
  ...
  if (role === 'manager') {
    const ownerIds = visibleOwnerIds || [user.id]
    ...
    payload.repPerformance = buildRepPerformance(entries, members, ownerIds)
  ...
  if (role === 'org_admin') {
    ...
    payload.repPerformance = buildRepPerformance(
      entries,

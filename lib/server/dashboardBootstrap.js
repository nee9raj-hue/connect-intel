...
import { isClosedCrmLeadStatus } from '../crmLeadStatuses.js'
...
function teamReportQuickAction(label) {
  if (!isTeamIntelligenceHubEnabled()) return null
  return { id: 'report', label, action: { panel: 'crm-dashboard', returnTo: 'overview' } }
}
...
function scanStatCounts(entries, tz) {
  ...
  for (const entry of entries) {
    ...
    if (crm.nextFollowUpAt && new Date(crm.nextFollowUpAt).getTime() <= endToday) {
    ...
    if (normalizeCrmLeadStatus(crm.status) === 'unqualified' && entry.savedAt && new Date(entry.savedAt).getTime() >= yesterday) {
...
function buildStatStrip(role, counts) {
  ...
  return [
    ...
    mk(
      ...
      counts.followUpsDue,
      'View leads',
      pipelineAction(role, { followUpDue: true })
    ),
    mk(
      'deals_closing',
      'Deals closing',
...
function buildPipelineSummary(entries, { stuckDays = 7 } = {}) {
  ...
  for (const entry of entries) {
    ...
    if (!isClosedCrmLeadStatus(st)) {
      dealValue += val
      const weight =
        st === 'active_trading' ? 1 : st === 'onboarding' ? 0.8 : st === 'opportunity' ? 0.5 : 0.2
      expectedRevenue += val * weight
    }
    const last = crm.lastCommunicationAt || crm.updatedAt || entry.savedAt
    if (last && now - new Date(last).getTime() > stuckMs && !isClosedCrmLeadStatus(st)) {
      stuck += 1
    }
  }

  const maxCount = Math.max(1, ...Object.values(stages))
  const stageRows = CRM_STATUSES.filter((id) => stages[id] > 0 || id === 'opportunity' || id === 'unqualified').map(
    (id) => ({
      id,
      count: stages[id] || 0,
...
function pipelineSummaryFromSqlAndSnapshot(sqlSummary, pipeSnap) {
  ...
  for (const row of sqlSummary?.byStatus || []) {
    const st = normalizeCrmLeadStatus(row.status)
...
function buildPriorities(entries, tz, limit = 8) {
  ...
  for (const entry of entries) {
    ...
    if (
      normalizeCrmLeadStatus(crm.status) === 'opportunity' &&
      ...
      rows.push({
        ...
        dueToday: true,
        action: { panel: 'pipeline', leadId, status: 'opportunity', returnTo: 'overview' },
      })
    }
  }
...
function buildLeadFocus(entries, tz) {
  ...
  for (const entry of entries) {
    ...
    if (normalizeCrmLeadStatus(crm.status) === 'unqualified') {
      newLeads += 1
...
function buildRepPerformance(entries, members, ownerIds) {
  ...
  return reps.map((m) => {
    ...
    for (const entry of mine) {
      const crm = normalizeExtendedCrm(entry.crm)
      if (!isClosedCrmLeadStatus(crm.status)) open += 1
      if (normalizeCrmLeadStatus(crm.status) === 'opportunity') followups += 1
      if (normalizeCrmLeadStatus(crm.status) === 'onboarding') {
...
function buildTeamLeaderboard(entries, hierarchy) {
    ...
    return [
      {
        ...
        openLeads: entries.filter((e) => {
          ...
          return !isClosedCrmLeadStatus(st)
        }).length,
        followups: entries.filter((e) => normalizeCrmLeadStatus(e.crm?.status) === 'opportunity').length,
        activities7d: 0,
        wonMonth: entries.filter((e) => normalizeCrmLeadStatus(e.crm?.status) === 'onboarding').length,
        value: 0,
  ...
  return teams.map((team) => {
    ...
    for (const entry of teamEntries) {
      const crm = normalizeExtendedCrm(entry.crm)
      if (!isClosedCrmLeadStatus(crm.status)) openLeads += 1
      if (normalizeCrmLeadStatus(crm.status) === 'opportunity') followups += 1
      if (normalizeCrmLeadStatus(crm.status) === 'onboarding') {
        const wonAt = crm.updatedAt || crm.lastCommunicationAt
...
function buildInsights(role, counts, pipeline, leadFocus, topRep) {
  ...
  if (counts.followUpsDue > 0) {
    insights.push({
      ...
      action: pipelineAction(role, { status: 'opportunity', followUpDue: true }),
    })
  }
  if (topRep) {
    insights.push({
...
async function resolveScopedEntries(user, store, { assigneeUserId = null } = {}) {
  ...
  // Snapshot-first for org dashboards without assignee filter — avoids full pipeline shard reads.
  if (!assignee && isPipelineHierarchyRbacEnabled() && user.organizationId) {
    return {
      role,
      entries: [],
      visibleOwnerIds,
      sqlSummary: sqlSummary || { ready: false, byStatus: [], total: 0 },
      assigneeUserId: null,
      snapshotFirst: true,
    }
  }
...
async function buildDashboardBootstrapSnapshotFirst(
  ...
  if (sqlSummary?.byStatus) {
    const fu = sqlSummary.byStatus.find((r) => r.status === 'opportunity')
  ...
  if (role === 'rep') {
    ...
    payload.leadFocusActions = {
      ...
      followUp: pipelineAction('rep', { status: 'opportunity', followUpDue: true }),
    }
  }
  ...
  if (role === 'org_admin') {
    ...
    payload.quickActions = [
      teamReportQuickAction('Org report'),
...
export async function buildDashboardBootstrap(sessionUser, { assigneeUserId = null } = {}) {
  ...
  if (role === 'manager') {
    ...
    payload.quickActions = [
      ...
      teamReportQuickAction('Team report'),
  ...
  if (role === 'org_admin') {
    ...
    payload.revenue = {
      monthlyTarget: 1_000_000,
      achieved: entries
        .filter((e) => normalizeCrmLeadStatus(e.crm?.status) === 'onboarding')

...
export async function fetchResendReceivingEmail(emailId) {
  if (!emailId) return { ok: false, error: 'No email id' }
  const result = await fetchResendApi(`/emails/receiving/${encodeURIComponent(emailId)}`)
  if (!result.ok) return result
  return { ok: true, email: result.data }
}

export async function sendResendEmail({ from, to, subject, html, text, replyTo, cc, attachments, headers }) {
  const payload = {
    from,

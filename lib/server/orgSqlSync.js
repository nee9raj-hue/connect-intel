...
function membershipForSqlSync(store, org, userId) {
  const membership = findMembership(store, userId, org?.id)
...
export function buildProfileSqlPayload(user, orgUuid, membership) {
  const isOwner =
    ...
    String(membership?.pipelineRole || '').toLowerCase() === 'org_admin'
  return {
    ...
    can_search: Boolean(membership?.canSearch ?? isOwner),
    metadata: {
...
async function persistSqlProfileId(userId, sqlProfileId) {
  if (!userId || !sqlProfileId) return
  await updateStorePartial(['users'], (draft) => {
    const user = draft.users?.find((u) => u.id === userId)
    if (user && user.sqlProfileId !== sqlProfileId) {
      user.sqlProfileId = sqlProfileId
    }
    return draft
  })
}
...
export async function syncOrgMembershipToSql({
  ...
  const membership = membershipForSqlSync(store, org, userId)
  let profileRow = null
  if (user) {
    profileRow = await upsertProfileRow(user, orgUuid, membership)
    if (profileRow?.id) {
      await persistSqlProfileId(userId, profileRow.id)
    }
  }
...
export async function syncOrganizationMembersToSql(organizationId, { store: preloadedStore } = {}) {
  ...
  if (org.ownerUserId) memberIds.add(org.ownerUserId)
  ...
  for (const userId of [...memberIds]) {
    ...
    if (row?.id) {
      profiles += 1
      await persistSqlProfileId(userId, row.id)
    }
  }

  return { organizationUuid: orgUuid, profiles, members: memberIds.size }
}

/** Sync profiles for active company members (Phase 4 — org UUID may already exist). */
export async function syncAllMemberProfilesToSql({ store: preloadedStore, orgId = null } = {}) {
  if (!isOrgSqlSyncEnabled()) {
    throw new Error('Organization SQL sync is not enabled')
  }
  ...
  const orgIds = (store.organizations || [])
    .filter((org) => org?.id && org.accountType === 'company')
    .filter((org) => !orgId || org.id === orgId)
    .map((org) => org.id)

  const summary = { orgs: 0, profiles: 0, errors: [] }

  for (const organizationId of orgIds) {
    try {
      const result = await syncOrganizationMembersToSql(organizationId, { store })
      summary.orgs += 1
      summary.profiles += result.profiles
    } catch (error) {
      summary.errors.push({ orgId: organizationId, error: error?.message || String(error) })
    }
  }
...
/** Backfill all company orgs missing sqlOrganizationId (Phase 3+ legacy repair). */
export async function syncAllOrganizationsToSql({ store: preloadedStore, orgId = null } = {}) {
  if (!isOrgSqlSyncEnabled()) {
    throw new Error('Organization SQL sync is not enabled')
  }
  ...
  const targets = (store.organizations || []).filter((org) => {
    if (org.accountType !== 'company' || !org.id) return false
    if (orgId && org.id !== orgId) return false
    return true
  })
  ...
  for (const org of targets) {
    try {
      const result = await syncOrganizationMembersToSql(org.id, { store })
      summary.orgs += 1
      summary.profiles += result.profiles
    } catch (error) {
      summary.errors.push({ orgId: org.id, error: error?.message || String(error) })
    }
  }
...
/** Fire background sync when org or profile SQL UUID is missing (Phase 4 lazy repair). */
export function repairOrgSqlSyncIfNeeded(user, store) {
  if (!user?.id || !user?.organizationId || !isOrgSqlSyncEnabled()) return
  const org = findOrg(store, user.organizationId)
  if (!org || org.accountType !== 'company') return

  if (!org.sqlOrganizationId) {
    void syncOrganizationMembersToSql(user.organizationId, { store }).catch((error) => {
      console.warn('legacy org SQL repair:', error?.message || error)
    })
    return
  }

  const dbUser = store.users?.find((u) => u.id === user.id)
  if (dbUser && !dbUser.sqlProfileId) {
    void syncOrgMembershipToSql({
      organizationId: user.organizationId,
      userId: user.id,
      store,
    }).catch((error) => {
      console.warn('legacy profile SQL repair:', error?.message || error)
    })
  }
}

/** Defer profile sync after membership mutations (invite, permissions, hierarchy). */
export function deferMemberSqlSync(organizationId, userId, store) {
  if (!organizationId || !userId || !isOrgSqlSyncEnabled()) return
  const work = syncOrgMembershipToSql({ organizationId, userId, store }).catch((error) => {
    console.warn('member SQL sync:', error?.message || error)
  })

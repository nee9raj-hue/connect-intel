...
import { processNoActivityWorkflows } from '../workflowInactivityCron.js'
...
export default async function handler(req, res) {
  ...
  const queueOnly = req.query?.queue === '1' || body.queueOnly === true
  if (queueOnly) {
    const marketingSqlQueue = await processMarketingEmailQueue({
      limit: Math.min(200, Math.max(1, Number(body.limit || req.query?.limit) || 100)),
      maxMs: 240_000,
    })
    return sendJson(res, 200, { ok: true, queueOnly: true, marketingSqlQueue })
  }
  ...
  let marketingSqlQueue = { ok: true, mode: 'skipped' }
  try {
    marketingSqlQueue = await processMarketingEmailQueue({ limit: 200, maxMs: 240_000 })
  } catch (err) {
    marketingSqlQueue = { ok: false, error: err?.message || 'marketing sql queue failed' }
  }

  let audienceMaintenance = { orgs: 0, segmentsRefreshed: 0, recommendationCaches: 0, errors: 0 }
  let workflowInactivity = { orgsScanned: 0, leadsScanned: 0, fired: 0, skippedDuplicate: 0 }
  try {
    workflowInactivity = await processNoActivityWorkflows()
  } catch (err) {
    console.error('workflow inactivity cron failed:', err?.message || err)
    workflowInactivity = { ok: false, error: err?.message || 'workflow inactivity failed' }
  }

...
export default async function handler(req, res) {
  ...
  if (cached.value && !cached.stale && !filters.status && !filters.tagId && !params.get('from')) {
    const hit = { ...cached.value, _cache: { hit: true, source: cached.source } }
    if (!hit.memberOptions?.length && user.organizationId) {
      const meta = await readStore({ only: ['users', 'organizations', 'organizationMemberships'] })
      hit.memberOptions = listTeamMembers(meta, user.organizationId)
  ...
  const meta = await readStore({ only: ['users', 'organizations', 'organizationMemberships'] })
  const members = user.organizationId ? listTeamMembers(meta, user.organizationId) : []
  payload.memberOptions = members
    .filter((m) => (m.status || 'active') === 'active')
    .map((m) => ({ userId: m.userId, name: m.name }))

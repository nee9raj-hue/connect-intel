  ...
  if (!sendResult.sent) {
    ...
    return sendJson(res, 400, {
      error:
        sendResult.error ||
        (orgDomain
          ? `Outbound email is not ready. Company admin: Team → Outbound email → add DNS for ${orgDomain}.`
          : 'Outbound email is not set up. Company admin: open Team → Outbound email (automatic setup).'),

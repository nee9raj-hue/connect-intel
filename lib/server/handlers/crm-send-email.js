...
import { loadPipelineStoreForLeadIds, patchPipelineEntryCrm } from '../pipelineShard.js'
...
export default async function handler(req, res) {
  ...
  const { pipelineStore } = await loadPipelineStoreForLeadIds(user, [leadId])
  ...
  const updatedEntry = await patchPipelineEntryCrm(user, leadId, (crm) =>
    recordOutboundEmail(
      crm,
      {
        subject: subject.trim(),
  ...
  const savedLeads = pipelineStore.savedLeads.map((row) =>
    row?.lead?.id === leadId ? updatedEntry : row
  )

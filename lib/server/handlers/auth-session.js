...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const { token } = await createSession(res, view, req)
      return sendJson(res, 200, { user: view, token })
    } catch (error) {
      const message = error.message || 'Sign-in failed'
      const isDb =
        /supabase|database|521|522|503|504|timed out|unavailable|redploy|circuit open|workspace is taking longer|Could not load your workspace/i.test(
          message
        )

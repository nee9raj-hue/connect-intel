...
import {
  ...
  createSelectionListBatches,
  resolveSmartListLeadIds,
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'create_batches_from_selection') {
      try {
        const leadIds = [...new Set(Array.isArray(body.leadIds) ? body.leadIds : [])]
        if (!leadIds.length) {
          return sendJson(res, 400, { error: 'Select at least one contact' })
        }
        let result = null
        await updateStore((draft) => {
          const ctx = { ...draft, savedLeads: store.savedLeads }
          result = createSelectionListBatches(ctx, user, {
            namePrefix: body.namePrefix,
            leadIds,
            batchSize: body.batchSize,
            channel: body.channel,
          })
          draft.marketingLists = ctx.marketingLists
          return draft
        })
    ...
    if (body.action === 'create_batches' || body.action === 'create_batches_from_filters') {
      try {
        assertCanCreateListBatches(store, user, body.assigneeUserId)
        let leadIds = [...new Set(Array.isArray(body.leadIds) ? body.leadIds : [])]
        if (body.action === 'create_batches_from_filters') {
          leadIds = await resolveSmartListLeadIds(store, user, {
  ...
  if (req.method === 'PATCH') {
    ...
    if (action === 'add_leads' || action === 'remove_leads') {
      ...
      await updateStore((draft) => {
        ...
        row.leadIds = merged
        applySnapshotToAudience(row, merged)
        row.updatedAt = new Date().toISOString()
        return draft
      })
      const updated = (await readStore()).marketingLists.find((l) => l.id === id)
    ...
    if (patchLeadIds !== undefined) {
      ...
      await updateStore((draft) => {
        ...
        applySnapshotToAudience(row, row.leadIds)
        row.updatedAt = now
        return draft
      })
    } else {

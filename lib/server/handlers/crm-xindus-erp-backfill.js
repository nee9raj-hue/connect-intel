...
export default async function handler(req, res) {
  ...
  const resetOwners = Boolean(body.resetOwners)
  if (!overlays.length && !classifyOnly && !fetchFromErp && !resetOwners) {
  ...
  try {
    ...
    if (resetOwners && !overlays.length) {
      ...
      return sendJson(res, 200, { ok: true, organizationId, dryRun, resetOwners: true, ...result })
    }
    ...
    const result = await applyOverlaysToLeadPage({
      ...
      reclaimUnmatchedOwners:
        body.reclaimUnmatchedOwners === true ||
        body.reclaimUnmatchedOwners === false
          ? Boolean(body.reclaimUnmatchedOwners)
          : overlays.length >= 500,
    })

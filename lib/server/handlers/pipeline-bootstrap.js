...
async function refreshPipelineBootstrapCache(user, query, cacheKey) {
  try {
    const payload = await buildPipelineBootstrap(user, query)
    const response = {
      ...payload,
      scopeRole: payload.summary?.scopeRole || null,
    }
    const hasRows = (response.leads || []).length > 0 || Number(response.total) > 0
    if (hasRows) await cacheSet(cacheKey, response, { ttlSeconds: TTL, staleSeconds: STALE })
  } catch (error) {
    console.warn('pipeline/bootstrap refresh:', error?.message || error)
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const cached = fresh
      ...
      : await cacheGet(cacheKey, { ttlSeconds: TTL, staleSeconds: STALE })
    if (cached.value) {
      if (cached.stale) {
        try {
          const { waitUntil } = await import('@vercel/functions')
          waitUntil(refreshPipelineBootstrapCache(user, query, cacheKey))
        } catch {
          void refreshPipelineBootstrapCache(user, query, cacheKey)
        }
      }
      return sendJson(res, 200, {
        ...cached.value,
        scopeRole: cached.value.summary?.scopeRole || null,
        _cache: { hit: true, stale: Boolean(cached.stale) },
      })
    }
    ...
    const hasRows = (response.leads || []).length > 0 || Number(response.total) > 0
    if (hasRows) void cacheSet(cacheKey, response, { ttlSeconds: TTL, staleSeconds: STALE })
    return sendJson(res, 200, { ...response, _cache: { hit: false, stale: false } })
  } catch (error) {

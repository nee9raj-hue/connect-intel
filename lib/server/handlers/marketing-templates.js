...
import { readStore, updateStorePartial, createId } from '../store.js'
...
const TEMPLATE_READ_COLLECTIONS = ['marketingTemplates', 'marketingForms', 'users']

function buildTemplatePayload(body, user, store) {
  ...
  const compiled = compileTemplateContent(
    ...
    { marketingForms: filterMarketingRows(store?.marketingForms || [], user) }
  )
...
export default async function handler(req, res) {
  ...
  const store = await readStore({ only: TEMPLATE_READ_COLLECTIONS })
  ...
  if (req.method === 'POST') {
    const payload = buildTemplatePayload(getBody(req), user, store)
    if (payload.error) return sendJson(res, 400, { error: payload.error })
    await updateStorePartial(['marketingTemplates'], (draft) => {
  ...
  if (req.method === 'PATCH') {
    ...
    const updated = (await readStore({ only: ['marketingTemplates'] })).marketingTemplates.find(
      (t) => t.id === id
    )

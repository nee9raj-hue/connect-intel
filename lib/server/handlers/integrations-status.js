...
import { canOfferCustomerGmailConnect, getGoogleOAuthVerificationEnv, paidApisEnabled } from '../config.js'
...
export default async function handler(req, res) {
  ...
  const googleOAuth = getGoogleOAuthVerificationEnv()
  const whatsappPlatform = getPlatformWhatsAppCloudStatus(store)
  const whatsappEnv = Boolean(
  ...
  return sendJson(res, 200, {
    ...
    providers: {
      ...
      crmGmailConnectOffered: canOfferCustomerGmailConnect(),
      googleOAuthVerified: googleOAuth.verified,
      googleOAuthPhase: googleOAuth.phase,
      googleOAuthAllowConnectEnv: googleOAuth.allowConnectEnv,
      googleOAuthVerifiedEnv: googleOAuth.verifiedEnv,
      whatsappAutoSendReady: whatsappPlatform.configured || whatsappEnv,
      whatsappPlatformConfigured: whatsappPlatform.configured,
      whatsappEnvConfigured: whatsappEnv,
    },
  })
}

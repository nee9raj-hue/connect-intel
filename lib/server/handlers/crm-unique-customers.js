...
async function loadFactRows({ organizationId, ownerIds, sinceIso, untilIso }) {
  ...
  for (;;) {
    ...
    if (sinceIso) {
      parts.push(`entry->crm->>lastOrderCreatedAt=gte.${encodeURIComponent(sinceIso)}`)
    }
    if (untilIso) {
      parts.push(`entry->crm->>lastOrderCreatedAt=lt.${encodeURIComponent(untilIso)}`)
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const filtered = raw.filter((row) =>
      matchUniqueCustomerFilters(row.entry, row, { tagIds, statuses, ownerIds: ownerFilter })
    )
    const report = buildUniqueCustomersReport(filtered, {
      ...
      weeks,
      timeZone,
      ownerNames: ownerNameMap(metaStore, user.organizationId),
    })

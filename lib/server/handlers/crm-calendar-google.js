...
export default async function handler(req, res) {
  ...
  if (action === 'push_meeting') {
    const result = await pushSavedCrmMeetingToGoogle(user, {
      leadId: body.leadId,
      meetingId: body.meetingId,
    })
    if (!result?.pushed && result?.needsCalendarConsent) {
      return sendJson(res, 403, result)
    }
    if (!result?.pushed && result?.error && !result?.skipped) {
      return sendJson(res, 400, result)
    }
    return sendJson(res, 200, result || { pushed: false })
  }
  ...
  return sendJson(res, 400, { error: 'Unknown action. Use sync, connect, enable, disable, or push_meeting.' })
}

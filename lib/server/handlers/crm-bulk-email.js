...
export default async function handler(req, res) {
  ...
  if (!orgCanSend && !gmail) {
    return sendJson(res, 400, {
      error: 'Outbound email not ready. Company admin: Team → Outbound email (DNS).',
  ...
  for (const leadId of leadIds) {
    ...
    if (orgCanSend) {
      sendResult = await sendCrmEmailViaOrgResend({ user, lead, subject: subj, body: bodyText, org })
    }
    if (!sendResult.sent && gmail) {

...
import {
  checkEmailWorkerReady,
  getEmailWorkerReadiness,
  isWorkerOnlyEmailRequired,
} from '../infra/emailWorkerPolicy.js'
...
export default async function handler(req, res) {
  ...
  if (body.action === 'queue' || body.action === 'drain') {
    ...
    if (body.action === 'queue' && isWorkerOnlyEmailRequired()) {
      const { redis, backgroundEmail } = getEmailWorkerReadiness()
      if (!redis || !backgroundEmail) {
    ...
    if (body.action === 'queue') {
      ...
      try {
        ...
        if (isWorkerOnlyEmailRequired() && result.queued && !result.background) {
          return sendJson(res, 503, {
            error: 'Campaign enrolled but background worker did not start. Check REDIS_URL and Railway worker.',
        ...
        if (result.queued) {
          ...
          triggerQueueDrainNow({ afterMs: 10_000 })
        }
        return sendJson(res, 200, {
          ...result,
          sendStatus: result.sendStatus || (result.background ? 'queued' : 'queued'),
        })
      } catch (err) {

...
import { createPipelineBulkMessagingJob } from '../messaging/engine.js'
import { EMAIL_SEND_MODE } from '../email/sendMode.js'
import { isWorkerOnlyEmailRequired } from '../infra/emailWorkerPolicy.js'
import { pipelineEmailGuard } from '../resourceProtectionEnforce.js'
...
async function resolveBulkRecipients(res, user, leadIds) {
  ...
  for (const leadId of leadIds) {
    ...
    leads.push(shaped)
    if (leadCanReceiveCommercialEmail(shaped)) {
      sendableIds.push(leadId)
    } else if (!leadHasSendableEmail(shaped)) {
...
export default async function handler(req, res) {
  ...
  try {
    await loadMetaUserAndAssertEditLeads(sessionUser)
  } catch (permError) {
  ...
  if (body.action === 'resolve') {
    ...
    const metaForGuard = await readStore({ only: ['users', 'organizations', 'organizationMemberships'] })
    const guardUser = metaForGuard.users.find((u) => u.id === sessionUser.id) || sessionUser
    const emailGuard = pipelineEmailGuard(leadIds.length, guardUser, metaForGuard)
    if (emailGuard) return sendJson(res, emailGuard.status, emailGuard.body)
  ...
  if (body.action === 'queue' || body.action === 'drain') {
    ...
    if (body.action === 'queue') {
      ...
      const emailGuard = pipelineEmailGuard(leadIds.length, user, metaStore)
      if (emailGuard) return sendJson(res, emailGuard.status, emailGuard.body)
      if (leadIds.length > MAX_BULK) {
        return sendJson(res, 400, {
          code: 'LARGE_AUDIENCE_MARKETING',
      ...
      try {
        const result = await createPipelineBulkMessagingJob(user, metaStore, body)
    ...
    const campaign = await findPipelineBulkCampaignByShard(user, campaignId)
    if (!campaign) {
  ...
  for (const leadId of leadIds) {
    ...
    if (!leadCanReceiveCommercialEmail(lead)) {
      skippedCount += 1
      results.push({ leadId, ok: false, error: 'No commercial email consent on lead' })
      continue
    }
  ...
  if (validRows.length) {
    if (existingCampaignId) {
      campaign = await findPipelineBulkCampaignByShard(user, existingCampaignId)
      if (!campaign) {
        return sendJson(res, 400, { error: 'Campaign not found for this bulk send' })
      }
    } else {
      campaign = await createPipelineBulkCampaign(user, {

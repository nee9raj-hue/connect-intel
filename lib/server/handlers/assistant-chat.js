...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const store = await readStore({
      only: ['assistantThreads', 'assistantSupportTickets'],
    })
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'escalate') {
      let escalation
      await updateStorePartial(['assistantThreads', 'assistantSupportTickets'], (draft) => {
      ...
      return sendJson(res, 200, {
        ...
        myTickets: getAssistantHistory(
          await readStore({ only: ['assistantSupportTickets'] }),
          user.id
        ).myTickets,
      })
    }
    ...
    const uiContext = {
      ...
      copilotTab: body.copilotTab || body.uiContext?.copilotTab || 'crm',
      mode: body.mode || body.uiContext?.mode || null,
      editFromMessageId: body.editFromMessageId || body.uiContext?.editFromMessageId || null,
    }
    let out
    await updateStorePartial(['assistantThreads', 'assistantSupportTickets'], async (draft) => {

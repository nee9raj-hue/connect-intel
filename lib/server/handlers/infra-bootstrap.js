...
import { inviteTeamMember } from '../organizations.js'
...
import { applyCampaignsV3Bootstrap, applyPipelineCollaboratorIdsBootstrap, applyPipelineTasksMeetingsBootstrap } from '../supabaseSqlApply.js'
import {
  backfillAllCampaignsV3,
  backfillCampaignsV3ForOrg,
  verifyCampaignsV3Backfill,
} from '../campaignsV3Backfill.js'
...
export default async function handler(req, res) {
  ...
  if (action === 'pipeline-collaborators-migrate') {
    const migration = await applyPipelineCollaboratorIdsBootstrap()
    let columnProbe = { ok: false }
    try {
      await supabaseRest('pipeline_leads?select=collaborator_ids&limit=1', {}, { timeoutMs: 15_000, attempts: 1 })
      columnProbe = { ok: true }
    } catch (error) {
      columnProbe = { ok: false, error: error?.message || String(error) }
    }
    return sendJson(res, migration.applied || columnProbe.ok ? 200 : 503, {
      ok: migration.applied || columnProbe.ok,
      migration,
      columnProbe,
    })
  }
  ...
  if (action === 'campaigns-v3-migrate') {
    const migration = await applyCampaignsV3Bootstrap()
    let tableProbe = { ok: false }
    try {
      await supabaseRest('campaigns_v3?select=id&limit=1', {}, { timeoutMs: 15_000, attempts: 1 })
      tableProbe = { ok: true }
    } catch (error) {
      tableProbe = { ok: false, error: error?.message || String(error) }
    }
  ...
  if (action === 'campaigns-v3-sync') {
    const orgId = body.orgId || req.query?.orgId || null
    const dryRun = body.dryRun === true || req.query?.dryRun === '1'
    const verifyBefore = await verifyCampaignsV3Backfill({ orgId })
    let backfill = null
    if (!verifyBefore.ok && !dryRun) {
      let migration = null
      try {
        ...
        migration = await applyCampaignsV3Bootstrap()
        if (!migration.applied) {
          return sendJson(res, 503, {
            error: 'campaigns_v3 table missing',
            hint: 'Run supabase/migrations/20260702140000_campaigns_v3_bootstrap.sql in Supabase SQL editor, or set SUPABASE_DB_PASSWORD / SUPABASE_ACCESS_TOKEN on Vercel',
            verifyBefore,
            migration,
            detail: error?.message || String(error),
          })
      ...
      backfill = orgId
        ? [await backfillCampaignsV3ForOrg(orgId, { dryRun: false })]
        : await backfillAllCampaignsV3({ dryRun: false, orgId })
    }
    const verifyAfter = backfill ? await verifyCampaignsV3Backfill({ orgId }) : verifyBefore
    return sendJson(res, 200, {
      ok: verifyAfter.ok,
      orgId: orgId || null,
      dryRun,
  ...
  if (action === 'step10-probe-user') {
    const email = String(body.email || req.query?.email || '').trim().toLowerCase()
    const store = await readStore({ only: ['users'] })
    const users = store.users || []
    const findByEmail = (target) =>
      users.find((u) => String(u.email || '').toLowerCase() === String(target || '').toLowerCase())
    ...
    if (!user) {
      user =
        users.find((u) => u.orgRole === 'org_admin' || u.isOrgAdmin) ||
        users.find((u) => u.organizationId) ||
        users[0] ||
        null
    }
    if (!user?.id) return sendJson(res, 404, { error: 'user_not_found', email: email || null })
    return sendJson(res, 200, {
      ok: true,
      userId: user.id,
      email: user.email,
  ...
  if (action === 'org-transfer-admin') {
    ...
    const fromPipelineRole = body.fromPipelineRole || req.query?.fromPipelineRole || 'member'
  ...
  if (action === 'org-invite-member') {
    const dryRun = body.dryRun !== false && req.query?.dryRun !== '0'
    const orgId = body.orgId || req.query?.orgId || null
    const nameQuery = body.nameQuery || req.query?.nameQuery || null
    const email = String(body.email || req.query?.email || '')
      .trim()
      .toLowerCase()
    if (!email) return sendJson(res, 400, { error: 'email is required' })

    const roster = await getOrganizationRoster({ orgId, nameQuery })
    const inviterUserId = roster.organization.ownerUserId
    if (!inviterUserId) {
      return sendJson(res, 400, { error: 'Organization has no owner/admin to invite from' })
    }
    ...
    const outcome = await inviteTeamMember(roster.organization.id, inviterUserId, {
      email,
      skipEmail: body.skipEmail !== false,
    })
    const after = await getOrganizationRoster({ orgId: roster.organization.id })
    return sendJson(res, 200, {
      ok: true,
      dryRun: false,
      organizationId: roster.organization.id,
      email,
      inviteUrl: outcome.inviteUrl || null,

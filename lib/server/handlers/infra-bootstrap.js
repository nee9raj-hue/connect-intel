...
export default async function handler(req, res) {
  ...
  if (action === 'activity-log-setup') {
    ...
    let indexes = { ok: false, skipped: true }
    try {
      await supabaseRest(
      ...
      indexes = {
        ...
        hint: 'Run supabase/migrations/20260617120100_activity_log_indexes_rpc.sql in Supabase SQL editor once',
      }
    }
    steps.push({ step: 'indexes', ...indexes })

    const backfill = orgId
      ? [await backfillPipelineActivitiesForOrg(orgId, { dryRun })]
      : await backfillAllPipelineActivities({ dryRun, orgId })

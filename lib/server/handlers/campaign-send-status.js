...
import { getMessagingJobAnalytics } from '../messaging/analytics.js'
...
async function maybeSelfHealPipelineBulk(user, campaignId, progress) {
  ...
  try {
    // When background email is on, the worker drains queued jobs — skip self-heal if online.
...
export default async function handler(req, res) {
  ...
  const analytics = await getMessagingJobAnalytics(campaignId, storeUser, meta)

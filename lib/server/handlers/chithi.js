...
const CHITHI_V2_EXTRA = ['savedLeads']

async function readChithiStore(includePipeline = false) {
  const only = includePipeline ? [...CHITHI_READ_COLLECTIONS, ...CHITHI_V2_EXTRA] : CHITHI_READ_COLLECTIONS
  return readStore({ only })
}
...
export default async function handler(req, res) {
  ...
  if (resource === 'workspace') {
    ...
    const store = await readChithiStore(true)
    const user = store.users.find((u) => u.id === sessionUser.id) || sessionUser
    await persistChithiChannelsIfNeeded(store, user)
    const workspace = buildChithiWorkspace(store, user)
  ...
  if (resource === 'context') {
    ...
    const store = await readChithiStore(true)
    const user = store.users.find((u) => u.id === sessionUser.id) || sessionUser
    const context = buildChannelContext(store, user, channelId)
  ...
  if (resource === 'search') {
    ...
    const store = await readChithiStore(true)
    const user = store.users.find((u) => u.id === sessionUser.id) || sessionUser
    const results = searchChithiV2(store, user, q)
    return sendJson(res, 200, { results, query: q })
  }

  if (resource === 'entity-channel') {
    if (req.method !== 'POST') return methodNotAllowed(res, ['POST'])
    const { roomType, leadId, dealId, label } = getBody(req)
    try {
      let channelRow = null
      await updateStorePartial(['chithiChannels', 'savedLeads'], (draft) => {
        const result = openEntityChannel(draft, sessionUser, { roomType, leadId, dealId, label })
        channelRow = result.channel
        return draft
      })
      const store = await readChithiStore(true)
      const channel = formatChannelV2(store, sessionUser, channelRow)
      return sendJson(res, 201, { channel })
    } catch (error) {
      return sendJson(res, 400, { error: error.message || 'Could not open channel' })
    }
  }

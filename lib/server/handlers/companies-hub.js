...
import { buildCompaniesHub, getCompanyDetail, getCompanyDetailForLeadIds } from '../companiesHub.js'
import {
...
export default async function handler(req, res) {
  ...
  if (companyId) {
    if (user.organizationId && (await orgHasPipelineCompanies(user.organizationId))) {
      ...
      if (sqlCompany) {
        const company = getCompanyDetailForLeadIds(store, user, sqlCompany, sqlCompany.leadIds)
        if (company) return sendJson(res, 200, { company, fromTable: true })
      }
    }
  ...
  if (user.organizationId && (await orgHasPipelineCompanies(user.organizationId))) {
    ...
    if (tablePage) {
      return sendJson(res, 200, { ...tablePage, limit, offset, fromTable: true })
    }
  }

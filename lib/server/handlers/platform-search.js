...
export default async function handler(req, res) {
  ...
  let payload = await timeAsync('connectintel_platform_search', { provider: meiliActive ? 'meili' : 'json' }, () =>
    searchPlatformFast(store, user, { q, limit })
  )

  if (meiliActive && payload.provider === 'postgres-json' && !store.savedLeads?.length) {
    const { pipelineStore } = await loadPipelineStoreContext(sessionUser)
    store.savedLeads = pipelineStore.savedLeads
    payload = await timeAsync('connectintel_platform_search', { provider: 'json_fallback' }, () =>
      searchPlatformFast({ ...store, ...pipelineStore, savedLeads: pipelineStore.savedLeads }, user, { q, limit })
    )
  }

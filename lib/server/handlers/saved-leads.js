...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (!leadId && !leadIdsOnly && user.organizationId && user.accountType === 'company') {
      const { scheduleErpPipelineMaintenance } = await import('../erpPipelineMaintenance.js')
      scheduleErpPipelineMaintenance(user, filtersEarly)
    }
  ...
  if (crmPatch?.status && organizationId && !crmPatch?.moveToErpPipeline) {
    ...
    const applyPatch = async (draft) => {
      ...
      if (crmPatch) {
        ...
        if (crmPatch.status !== undefined && !crmPatch.moveToErpPipeline) {
          crm = markCrmStatusManual(crm, { userId: user.id })
        }
        if (crmPatch.moveToErpPipeline) {
          crm.erpPipelineOwned = true
          crm.erpDuplicatePending = false
          delete crm.pipelineStatusManual
          delete crm.pipelineStatusManualAt
          delete crm.pipelineStatusManualBy
        }
      ...
      if (activity && (String(activity.summary || '').trim() || activity.type === 'call')) {
        ...
        if (activitySummary) {
          ...
          if (activity.type === 'call' && (crm.status === 'unqualified' || crm.status === 'new' || crm.status === 'fresh')) {
      ...
      entry.crm = crm
      if (crmPatch?.moveToErpPipeline) {
        const { applyErpAccountStage, suggestCrmStatusFromErp } = await import('../../erpAccountStage.js')
        applyErpAccountStage(entry)
        const stillCrm = ['fresh', 'qualified', 'unqualified', 'account_created_erp', 'new', 'contacted'].includes(
          String(entry.crm?.status || '').toLowerCase()
        )
        if (stillCrm) {
          entry.crm = {
            ...(entry.crm || {}),
            status: suggestCrmStatusFromErp(entry) || 'new_account',
            erpPipelineOwned: true,
            erpDuplicatePending: false,
          }
        }
        crm = entry.crm
      }

...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (pipelineLeadsTableActive()) {
      tableEntryForPatch = await readPipelineLeadById(shardName, leadId)
      if (tableEntryForPatch) {
        metaStoreForPatch = await readStore({ only: META_STORE_COLLECTIONS })
        pipelineStore = attachPipelineEntriesToStore(metaStoreForPatch, [tableEntryForPatch])
      }
    }
    if (!pipelineStore) {
    ...
    if (organizationId && merged) {
      ...
      if (dealAction?.action) {
        ...
        if (dealAction.action === 'won') {
          const { fireCrmWorkflowAutomations } = await import('../crmAutomationBridge.js')
          void fireCrmWorkflowAutomations({
            trigger: 'deal_won',
            leadId,
            organizationId,
            actor: user,

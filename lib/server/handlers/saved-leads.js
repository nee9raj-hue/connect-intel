...
function parsePipelineQueryParams(url) {
  ...
  return { status, q, assigneeUserId, tagIds }
}
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (isSummary) {
      const summary = summarizePipelineEntries(filtered.length ? filtered : visible)
      return sendJson(res, 200, { ...summary, ready: true })
    }

    if (isBoard) {
      const columns = boardPipelineSlice(filtered, 50)
      const board = {}

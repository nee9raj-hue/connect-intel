...
import { attachOrgWorkspaceImports } from '../leadErpPopulate.js'
import {
  addMeeting,
  addTask,
  addDeal,
...
import { dealMilestonePatchFrom, pickDealMilestones } from '../../dealMilestones.js'
import { pipelineOwnerUserId } from '../../pipelineOwner.js'
import { refreshPipelineCollaboratorUserIds } from '../../pipelineCollaborators.js'
import { findPipelineEntryAsync } from '../pipelineVisibility.js'
import { markCrmStatusManual } from '../../erpAccountStage.js'
...
import { relinkOrphanDealsToEntry } from '../relinkOrphanDeals.js'
...
import { assertWithinPlanLimits } from '../crmPlanLimits.js'

/** When a manager schedules work for another rep, transfer pipeline ownership so they see the lead. */
function maybeManagerShareLeadWithAssignee(entry, user, taskAssigneeId) {
  const assignee = String(taskAssigneeId || '').trim()
  if (!assignee || assignee === String(user.id)) return null
  if (!isCompanyPipelineManager(user)) return null
  const owner = pipelineOwnerUserId(entry)
  if (!owner || owner !== String(user.id)) return null
  const prev = entry.assignedToUserId || null
  if (prev === assignee) return null
  entry.assignedToUserId = assignee
  entry.assignedAt = new Date().toISOString()
  entry.assignedByUserId = user.id
  return { assignToUserId: assignee, previousAssignee: prev, via: 'task_or_meeting' }
}
...
import { normalizeTradeProfile } from '../../leadTradeProfile.js'
import { applyCrmWorkflowRules, postCrmWorkflowDispatch } from '../workflowEngine.js'
...
import { normalizeLeadTagIds, mergeVisibleLeadTagIds, visibleLeadTagIdsForUser } from '../orgLeadTags.js'
import { applyAssigneeTeamOnPipelineEntry, attachTeamMemberUserIdsToFilters, loadHierarchyTeams } from '../orgTeamLeadTags.js'
import {
  DEFAULT_PIPELINE_PAGE_SIZE,
  MAX_PIPELINE_PAGE_SIZE,
} from '../pipelineStore.js'
import { assertOrgPermission, assertEditLeadsForPipelinePatch, assertPipelineDeleteAccess, permissionDeniedResponse } from '../permissionEnforce.js'
...
import { syncPipelineTasksAfterSave } from '../pipelineTasksSync.js'
import { syncPipelineMeetingsAfterSave } from '../pipelineMeetingsSync.js'
import { syncPipelineCompaniesAfterSave } from '../pipelineCompaniesSync.js'
import { syncMeilisearchAfterSave } from '../meilisearchSync.js'
import { enqueuePipelineIndexRefresh, enqueueDashboardSnapshotRefresh } from '../queue/producer.js'
import { bumpPipelineBootstrapGeneration } from '../infra/cache.js'
...
import { countDealsByStage, flattenDealsFromEntries, dealStageIncludesClosed } from '../../dealPipeline.js'
import { buildAutoDealName } from '../../dealNaming.js'
import { isFreightDealOrg } from '../../freightDeal.js'
import { freightBoardColumnIds } from '../../crmPipelineFlow.js'
...
function isEmailConsentOnlyPipelinePatch(body) {
  const keys = Object.keys(body || {}).filter((k) => k !== 'leadId')
  return keys.length === 1 && keys[0] === 'emailConsent'
}
...
function isTagsOnlyPipelinePatch(body) {
  const keys = Object.keys(body || {}).filter((k) => k !== 'leadId')
  if (keys.length !== 1 || keys[0] !== 'crm') return false
  const crm = body.crm
  if (!crm || typeof crm !== 'object') return false
  const crmKeys = Object.keys(crm)
  return crmKeys.length === 1 && crmKeys[0] === 'tagIds'
}

function isTradeProfileOnlyPipelinePatch(body) {
  const keys = patchMetaKeys(body)
  return keys.length === 1 && keys[0] === 'tradeProfile'
}
...
async function deferPipelineMaterialization(user, entry, previousEntry = null) {
  ...
  void bumpPipelineBootstrapGeneration(user).catch(() => {})
...
async function withRelinkedDeals(user, entry) {
  if (!entry || !user?.organizationId) return entry
  try {
    const { entry: next } = await relinkOrphanDealsToEntry(user, entry)
    return next || entry
  } catch (err) {
    console.warn('deal relink:', err?.message || err)
    return entry
  }
}
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
    const pipeline = getPlatform().repositories.pipeline
    const url = new URL(req.url || '', 'http://local')
    const light = url.searchParams.get('light') !== '0'
    const leadId = String(url.searchParams.get('leadId') || '').trim()
    const leadIdsCsv = String(url.searchParams.get('leadIds') || '').trim()
    const leadIdsOnly = url.searchParams.get('idsOnly') === '1'
    const leadIdsFromQuery = [
      ...url.searchParams.getAll('leadId').map((id) => String(id).trim()).filter(Boolean),
      ...leadIdsCsv.split(',').map((id) => String(id).trim()).filter(Boolean),
    ]
    const uniqueLeadIds = [...new Set(leadIdsFromQuery)]
    ...
    const filtersEarly = await attachTeamMemberUserIdsToFilters(
      user.organizationId,
      parsePipelineQueryParams(url)
    )

    if (isSummary && !leadId && !isBoard && !isDealsView) {
      try {
        const indexed = await pipeline.loadSummaryOnly(user)
        if (indexed?.ready) {
    ...
    if (leadIdsOnly && uniqueLeadIds.length > 0) {
      const byIds = await pipeline.loadLeadsByIds(user, uniqueLeadIds, { light })
      return sendPipelineJson(
        ...
        user,
        {
          leads: byIds.leads,
          total: byIds.total,
          filtered_total: byIds.filtered_total,
          limit: byIds.limit,
          offset: byIds.offset,
          hasMore: false,
          pipelineTotal: byIds.pipelineTotal,
          pipelineSource: 'pipeline_leads_by_id',
        },
        filtersEarly
      )
    }
    ...
    if (leadId) {
      ...
      if (pipelineLeadsTableActive()) {
        ...
        if (tableEntry) {
          ...
          await attachOrgWorkspaceImports(metaStore)
          ...
          const linked = await withRelinkedDeals(user, entry)
          return sendPipelineJson(
            res,
            200,
            user,
            { lead: mergeLeadForTenant(storeForMerge, user, linked) },
            filtersEarly
          )
        }
    ...
    if (light && !leadId && !isBoard && !isDealsView && !isSummary) {
      ...
      const pageCursor = filtersEarly.cursor || null
      const pageOffset = pageCursor
        ? 0
        : Math.max(0, Math.floor(Number(url.searchParams.get('offset') || 0)) || 0)
      const list = await pipeline.loadListPage(user, {
        offset: pageOffset,
        limit: pageLimit,
      ...
      const pipelineSource = pipeline.resolveListSource(list)
      return sendPipelineJson(
        res,
        200,
        user,
    ...
    if (isDealsView) {
      ...
      const dealsPage = await pipeline.loadDealsPage(user, {
        filters: filtersEarly,
        dealStage,
        offset: dealOffset,
        limit: dealLimit,
    ...
    if (isBoard) {
      ...
      const boardView = await pipeline.loadBoardView(user, {
        filters: filtersEarly,
        columnLimits,
        defaultPerColumn: 50,
      })
    ...
    if (leadId) {
      ...
      const linked = await withRelinkedDeals(user, entry)
      await attachOrgWorkspaceImports(storeForMerge)
      return sendPipelineJson(
        res,
        200,
        user,
    ...
    if (isDealsView) {
      ...
      const rows = flattenDealsFromEntries(filtered, {
        ...
        includeClosed: dealStageIncludesClosed(dealStage),
        freightOrg,
      })
    ...
    if (isSummary) {
      const indexedSummary = await pipeline.loadSummaryWithDeals(user, { freightOrg })
      if (indexedSummary) {
    ...
    if (isBoard) {
      const columnLimits = parseBoardColumnLimits(url)
      const { columns, totals } = boardPipelineSlice(
        filtered,
        50,
        columnLimits,
        freightOrg
  ...
  if (req.method === 'POST') {
      ...
      try {
        ...
        if (entry?.lead?.id) {
          recordPipelineAudit({
            ...
            resourceId: entry.lead.id,
            metadata: { manual: true },
          })
        }
        await attachOrgWorkspaceImports(updated)
      ...
      const updated = await updateStore((draft) => {
        ...
        if (!exists) {
          if (organizationId) {
            ...
            if (org) assertWithinPlanLimits(draft, org, { extraLeads: 1 })
          }
          let contactId = null
          let companyId = null
    ...
    if (entry) {
      ...
      recordPipelineAudit({
        ...
        actorUserId: user.id,
        action: 'pipeline.lead_created',
        resourceId: entry.lead?.id || lead.id,
    ...
    return sendPipelineJson(res, 200, user, {
      ...
      return sendJson(res, 400, { error: error.message || 'Could not save lead' })
    }
  }
  ...
  if (req.method === 'PATCH') {
    ...
    const tradeProfilePatch = body.tradeProfile
    ...
    const emailConsentOnlyPatch = isEmailConsentOnlyPipelinePatch(body)
    const tagsOnlyPatch = isTagsOnlyPipelinePatch(body)
    const tradeProfileOnlyPatch = isTradeProfileOnlyPipelinePatch(body)
    const fastActionPatch =
      ...
      dealOnlyPatch ||
      notesOnlyPatch ||
      emailConsentOnlyPatch
    const lightCrmPatch = assignOnlyPatch || statusOnlyCrmPatch || fastActionPatch || tagsOnlyPatch

    let pipelineStore = null
    let tableEntryForPatch = null
    let metaStoreForPatch = null
    const { pipelineLeadsTableActive } = await import('../pipelineLeadsTable.js')
    const tablePatchPath = pipelineLeadsTableActive()
    ...
    if (tablePatchPath) {
      ...
      tableEntryForPatch = tableLoad.entry
      metaStoreForPatch = tableLoad.metaStore
      pipelineStore = tableLoad.pipelineStore
    } else {
    ...
    try {
      await assertEditLeadsForPipelinePatch(user, metaStoreForPatch || pipelineStore, entryBefore, {
        assignOnly: assignOnlyPatch,
        emailConsentOnly: emailConsentOnlyPatch,
        activityOnly: activityOnlyPatch,
        fieldVisitOnly: fieldVisitOnlyPatch,
        notesOnly: notesOnlyPatch,
        activityFollowUp: activityFollowUpPatch,
        scheduleOnly: scheduleOnlyPatch,
        tagsOnly: tagsOnlyPatch,
        tradeProfileOnly: tradeProfileOnlyPatch,
        dealOnly: dealOnlyPatch && dealAction?.action !== 'delete',
      })
    } catch (permError) {
    ...
    if (dealAction?.action === 'delete') {
      try {
        await assertPipelineDeleteAccess(user, metaStoreForPatch || pipelineStore)
      } catch (permError) {
        const denied = permissionDeniedResponse(permError)
        return sendJson(res, denied.status, denied.body)
      }
    ...
    if (assignToUserId !== undefined) {
      if (!canAssignLead(user, entryBefore, assignToUserId)) {
        return sendJson(res, 403, {
          error:
            assignToUserId === null
              ? 'You cannot unassign this lead'
              : entryBefore.assignedToUserId
    ...
    const assigneeHierarchyTeams =
      assignToUserId !== undefined && organizationId
        ? await loadHierarchyTeams(organizationId)
        : []
    ...
    let workflowStatusChange = null
    let workflowFiredRules = []
    let updated
    const patchMeta = { managerShare: null }
    const applyPatch = async (draft) => {
      ...
      if (assignToUserId !== undefined) {
        ...
        if (organizationId) {
          crm = applyAssigneeTeamOnPipelineEntry(
            draft,
            organizationId,
            entry,
            crm,
            assignToUserId || null,
            assigneeHierarchyTeams
          )
        }
      }
      ...
      if (emailConsent !== undefined) {
        entry.lead = applyCommercialEmailConsent(entry.lead || entry, {
          granted: Boolean(emailConsent),
          source: 'manual',
        })
      }

      if (tradeProfilePatch !== undefined) {
        entry.tradeProfile = normalizeTradeProfile(tradeProfilePatch, {
          actor: { userId: user.id, name: user.name || user.email },
        })
      }
      ...
      if (crmPatch) {
        ...
        if (patch.tagIds !== undefined && organizationId) {
          const requested = normalizeLeadTagIds(patch.tagIds, draft, organizationId)
          const visible = visibleLeadTagIdsForUser(draft, organizationId, user)
          patch.tagIds = mergeVisibleLeadTagIds(requested, crm.tagIds, visible)
        ...
        if (crmPatch.status !== undefined) {
          crm = markCrmStatusManual(crm, { userId: user.id })
        }
        ...
        if (crmPatch?.responseReceived === true && !crm.lastResponseAt) {
          ...
          if (['unqualified', 'qualified', 'new', 'contacted'].includes(crm.status)) {
            crm.status = 'opportunity'
          }
        }
      }
      ...
      if (activity && (String(activity.summary || '').trim() || activity.type === 'call')) {
        ...
        const activityNotes = String(activity.meta?.notes || '').trim()
        if (activityNotes && activitySummary && !activitySummary.includes(activityNotes)) {
          activitySummary = `${activitySummary} · ${activityNotes}`.slice(0, 500)
        }
        if (activitySummary) {
          crm = appendActivity(crm, {
          ...
          if (activity.type === 'call' && (crm.status === 'unqualified' || crm.status === 'new')) {
      ...
      if (dealAction?.action === 'add') {
        ...
        crm = updateDeal(
          ...
          dealAction.dealId,
          {
            ...
            notes: dealAction.notes,
            freight: dealAction.freight,
            ...dealMilestonePatchFrom(dealAction),
          },
          creator
        )
      ...
      if (crmPatch?.status && crmPatch.status !== previousStatus && organizationId) {
        // Rules must see the stage the rep just chose. Reading entry.crm first
        // drops the patch and the overview dropdown snaps back.
        entry.crm = crm
        workflowFiredRules = applyCrmWorkflowRules(draft, entry, {
        ...
        workflowStatusChange = { previousStatus, newStatus: crm.status }
      }
      ...
      const delegatedAssignee =
        taskAction?.action === 'add' && taskAction.title
          ? taskAction.assignedToUserId || user.id
          : meetingAction?.action === 'add' && meetingAction.scheduledAt
            ? meetingAction.assignedToUserId || user.id
            : null
      const managerShare = maybeManagerShareLeadWithAssignee(entry, user, delegatedAssignee)
      refreshPipelineCollaboratorUserIds(entry)
      if (managerShare) {
        patchMeta.managerShare = managerShare
        crm = appendActivity(crm, {
          type: 'assignment',
          summary: 'Lead assigned for teammate follow-up',
          userId: user.id,
          userName: user.name,
          meta: managerShare,
        })
    ...
    try {
      const tableResult = await updatePipelineLeadViaTable(user, leadId, applyPatch, {
        preloadedEntry: tableEntryForPatch,
        metaStore: metaStoreForPatch,
      })
    ...
    if (workflowStatusChange && organizationId) {
      void postCrmWorkflowDispatch({
        firedRules: workflowFiredRules,
        trigger: 'status_change',
        leadId,
        organizationId,
        actor: user,
    ...
    const calendarOwnerId = lastAddedMeeting?.assignedToUserId || user.id
    const calendarOwner = updated.users?.find((u) => u.id === calendarOwnerId) || storeUser
    const leadForGoogle = entry.lead || entry
    const calendarOauth = getUserCrmGmail(calendarOwner)
    const googleCalendar = lastAddedMeeting
      ? {
          meetingId: lastAddedMeeting.id,
          leadId,
          queued: Boolean(calendarOauth && oauthHasCalendarScope(calendarOauth)),
          needsCalendarConsent: Boolean(
            calendarOwnerId === user.id && calendarOauth && !oauthHasCalendarScope(calendarOauth)
          ),
        }
      : null
    ...
    if (
      ...
      (patchMeta.managerShare?.assignToUserId &&
        organizationId &&
        patchMeta.managerShare.assignToUserId !== previousAssignee)
    ) {
      void notifyLeadAssigned({
        store: updated,
        entry,
        assigneeUserId: assignToUserId || patchMeta.managerShare?.assignToUserId,
        actorUser: user,
        organizationId,
      }).catch((error) => {
    ...
    if (organizationId && merged?.crm) {
      syncPipelineDealsAfterSave({ organizationId, entry: merged })
      syncPipelineTasksAfterSave({ organizationId, entry: merged })
      syncPipelineMeetingsAfterSave({ organizationId, entry: merged })
      syncPipelineCompaniesAfterSave({ user, metaStore: updated })
      syncMeilisearchAfterSave({ organizationId, entry: merged })
    }
    ...
    const mergeLead = emailConsentOnlyPatch
      ? mergeLeadForTenant
      : lightCrmPatch ||
          notesOnlyPatch ||
          (crmPatch &&
    ...
    recordPipelineAudit({
      organizationId,
      actorUserId: user.id,
      action: dealAction ? 'pipeline.deal_updated' : 'pipeline.lead_updated',
      resourceId: leadId,
      resourceType: dealAction ? 'deal' : 'lead',
      metadata: {
        assign: assignToUserId !== undefined,
        crm: Boolean(crmPatch),
        tradeProfile: tradeProfilePatch !== undefined,
        deal: Boolean(dealAction),
        activity: Boolean(activity),
        task: Boolean(taskAction),
        meeting: Boolean(meetingAction),
      },
    })

    if (mergeLead === mergeLeadForTenant) {
      await attachOrgWorkspaceImports(updated)
    }
  ...
  if (req.method === 'DELETE') {
    ...
    const { visible } = await loadPipelineStoreForLeadIds(user, [leadId])
    ...
    try {
      await assertPipelineDeleteAccess(user, metaStore)
    } catch (permError) {
      const denied = permissionDeniedResponse(permError)
      return sendJson(res, denied.status, denied.body)
    }

    if (!canUserDeletePipelineEntry(user, entry, metaStore)) {
      return sendJson(res, 403, { error: 'You do not have permission to delete this lead' })
    }

    const { deletePipelineLeadRow, pipelineLeadsTableActive } = await import('../pipelineLeadsTable.js')
    if (pipelineLeadsTableActive()) {
      await deletePipelineLeadRow(shardName, leadId)
      invalidatePipelineShard(shardName)
    } else {
      await updatePipelineStore(user, async (draft) => {
        draft.savedLeads = (draft.savedLeads || []).filter((e) => e.lead?.id !== leadId)
        return draft
      })
      await deletePipelineLeadRow(shardName, leadId)
    }

...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (!explicitPaging) {
      const leads = listPipelineEntries(store, user, { light })
      const total = leads.length
      return sendJson(res, 200, { leads, total, limit: total, offset: 0, hasMore: false })
    }
    ...
    const total = countPipelineEntries(store, user)
    const leads = listPipelineEntries(store, user, { light, limit, offset })

...
import { pushCrmMeetingToGoogle } from '../googleCalendarSync.js'
...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    ...
    const canAssignLeads = isCompanyPipelineManager(user) && organizationId
    const assignOnlyPatch = isAssignOnlyPipelinePatch(body)

    if (assignToUserId !== undefined && !canAssignLeads) {
      return sendJson(res, 403, { error: 'Only company admins can assign leads to teammates' })
    }

    if (assignToUserId && canAssignLeads) {
    ...
    try {
      ...
      if (taskAction?.action === 'add' && taskAction.title) {
        ...
        if (assignee !== user.id && !isCompanyPipelineManager(user)) {
    ...
    if (lastAddedMeeting) {
      ...
      googleCalendar = await pushCrmMeetingToGoogle(storeUser, lastAddedMeeting, entry.lead || entry)
    ...
    const mergeLead =
      assignOnlyPatch || (crmPatch && !activity && !taskAction && !meetingAction && !fieldVisit && !contactPatch &&

...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (isSummary) {
      ...
      if (freightOrg) {
        payload.openDealCounts = countDealsByStage(visible, { openOnly: true })
        payload.dealCounts = countDealsByStage(visible, { openOnly: false })
      }
  ...
  if (req.method === 'PATCH') {
    ...
    const fastActionPatch = scheduleOnlyPatch || activityOnlyPatch || fieldVisitOnlyPatch || dealOnlyPatch
    const lightCrmPatch = assignOnlyPatch || statusOnlyCrmPatch || fastActionPatch

    const { pipelineStore } = await loadPipelineStoreContext(user, { shardOnly: fastActionPatch })
    ...
    try {
      ...
      if (dealAction?.action === 'add' && dealAction.name) {

...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const explicitPaging = url.searchParams.has('limit')
    ...
    if (leadId) {
      ...
      return sendJson(res, 200, { lead: mergeLeadForTenant(store, user, entry) })
    }
    ...
    if (!explicitPaging) {
      let limit = DEFAULT_PIPELINE_PAGE_SIZE
      limit = Math.min(limit, MAX_PIPELINE_PAGE_SIZE)
      const { leads, total } = listPipelinePage(store, user, { light, limit, offset: 0 })
      const hasMore = leads.length < total
      return sendJson(res, 200, { leads, total, limit, offset: 0, hasMore })
    }
    ...
    const { leads, total } = listPipelinePage(store, user, { light, limit, offset })
    const hasMore = offset + leads.length < total
    return sendJson(res, 200, { leads, total, limit, offset, hasMore })
  }

...
import { loadPipelineLeadsByIds } from '../pipelineListLoad.js'
import {
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (leadIdsOnly && uniqueLeadIds.length > 0) {
      const byIds = await loadPipelineLeadsByIds(user, uniqueLeadIds, { light })
    ...
    if (light && !leadId && !isBoard && !isDealsView && !isSummary) {
      ...
      const pipelineSource = list.fromTableSearch
        ? 'pipeline_leads_search'
        : list.fromPlatformSearch
          ? 'platform_search'
          : list.fromMeili
            ? 'meilisearch'
            : list.fromSqlScope

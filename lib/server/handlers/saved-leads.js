...
import { loadPipelineStoreContext, updatePipelineStore, touchPipelineEntry } from '../pipelineShard.js'
import {
  boardPipelineSlice,
...
import { pushCrmMeetingToGoogle, pushCrmTaskToGoogle, removeCrmItemFromGoogle } from '../googleCalendarSync.js'
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (leadId) {
      ...
      return sendJson(res, 200, { lead: mergeLeadForTenant(storeForMerge, user, entry) })
    }
  ...
  if (req.method === 'PATCH') {
    ...
    const lightCrmPatch = assignOnlyPatch || statusOnlyCrmPatch
    ...
    let updated
    try {
      updated = await updatePipelineStore(user, async (draft) => {
    ...
    if (lastAddedMeeting) {
      googleCalendar = await pushCrmMeetingToGoogle(storeUser, lastAddedMeeting, leadForGoogle)
    ...
    if (lastAddedTask) {
      ...
      if (taskGoogle?.pushed && taskGoogle.googleEventId) {
        ...
        googleCalendar = googleCalendar || taskGoogle
      } else if (taskGoogle?.error) {
        googleCalendar = googleCalendar || taskGoogle
      }
    }
    ...
    const mergeLead =
      ...
      (crmPatch && !activity && !taskAction && !meetingAction && !fieldVisit && !contactPatch &&
        Object.keys(crmPatch).every((key) => key === 'status' || key === 'responseReceived'))

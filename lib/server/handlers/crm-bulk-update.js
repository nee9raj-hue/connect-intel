...
import { isCrmLeadStatusFilter, mergeLeadForClientListMinimal, normalizeCrmLeadStatus } from '../crm.js'
...
import { notifyBulkLeadsAssigned } from '../assignmentNotify.js'
import { normalizeLeadTagIds, visibleLeadTagIdsForUser } from '../orgLeadTags.js'
import { applyAssigneeTeamOnPipelineEntry, loadHierarchyTeams } from '../orgTeamLeadTags.js'
...
export default async function handler(req, res) {
  ...
  try {
    await assertOrgPermission(user, 'edit_leads', storeBefore)
  } catch (permError) {
    const denied = permissionDeniedResponse(permError)
    return sendJson(res, denied.status, denied.body)
  }

  const visibleTagIds = organizationId
    ? new Set(visibleLeadTagIdsForUser(storeBefore, organizationId, user))
    : new Set()
  const addTagIds = organizationId
    ? normalizeLeadTagIds(Array.isArray(body.addTagIds) ? body.addTagIds : [], storeBefore, organizationId).filter(
        (id) => visibleTagIds.has(String(id))
      )
    : []
  const removeTagIds = organizationId
    ? normalizeLeadTagIds(Array.isArray(body.removeTagIds) ? body.removeTagIds : [], storeBefore, organizationId).filter(
        (id) => visibleTagIds.has(String(id))
      )
    : []
  ...
  if (status && !isCrmLeadStatusFilter(status)) {
  ...
  const assigneeHierarchyTeams =
    (assignToUserId !== undefined || clearAssignee) && organizationId
  ...
  const { store, updated, skipped, updatedEntries } = await bulkMutatePipelineEntries(
    ...
    (entry, draft) => {
      ...
      if (status) {
        ...
        crm.status = normalizeCrmLeadStatus(status)
        if (prev !== crm.status) {
          crm = markCrmStatusManual(crm, { userId: user.id })
          crm = appendActivity(crm, {
            type: 'status',
            summary: `Status changed to ${crm.status}`,
            userId: user.id,
            userName: user.name,
            meta: { from: prev, to: crm.status },
          })
        }
      }
      ...
      if (markReplied) {
        ...
        crm = appendActivity(crm, {
          type: 'note',
          summary: 'Marked reply received (bulk)',
          userId: user.id,
          userName: user.name,
        })
      }
  ...
  if (organizationId && assignmentNotifies.length) {
    ...
    for (const { leadId, assigneeUserId } of assignmentNotifies) {
      if (!byAssignee.has(assigneeUserId)) byAssignee.set(assigneeUserId, [])
      byAssignee.get(assigneeUserId).push(leadId)
    }
    void Promise.allSettled(
      [...byAssignee.entries()].map(async ([assigneeUserId, leadIds]) => {
        const entries = leadIds
          .map((leadId) => findPipelineEntry(store, user, leadId))
          .filter(Boolean)
        if (!entries.length) return null
        return notifyBulkLeadsAssigned({
          store,
          entries,
          assigneeUserId,
          actorUser: user,

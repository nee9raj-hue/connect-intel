...
async function loadFactRows({ organizationId, ownerIds, teamIds, sinceIso, untilIso }) {
  ...
  const dateOr =
    sinceIso && untilIso
      ? `or=(and(entry->erp->revenue->>onboardedAt.gte.${encodeURIComponent(sinceIso)},entry->erp->revenue->>onboardedAt.lt.${encodeURIComponent(untilIso)}),and(entry->erp->revenue->>firstShipmentAt.gte.${encodeURIComponent(sinceIso)},entry->erp->revenue->>firstShipmentAt.lt.${encodeURIComponent(untilIso)}),and(entry->erp->revenue->>customerCreatedAt.gte.${encodeURIComponent(sinceIso)},entry->erp->revenue->>customerCreatedAt.lt.${encodeURIComponent(untilIso)}))`
      : null
  for (;;) {
    const parts = [

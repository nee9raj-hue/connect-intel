...
import { syncOrgTeamLeadTags } from '../orgTeamLeadTags.js'
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const skipLeadCounts =
        url.searchParams.get('skipLeadCounts') === '1' || url.searchParams.get('summary') === '1'
      const hierarchy = await listOrgHierarchy(user.organizationId, { skipLeadCounts })
      await syncOrgTeamLeadTags(user.organizationId, user.id, hierarchy).catch(() => {})

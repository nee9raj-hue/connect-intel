...
export default async function handler(req, res) {
  ...
  if (!orgHierarchyActive()) {
    return sendJson(res, 503, { error: 'Team structure is temporarily unavailable. Refresh in a moment.' })
  }

  try {
    if (req.method === 'GET') {
      const url = new URL(req.url || '', 'http://local')
      const skipLeadCounts = url.searchParams.get('skipLeadCounts') !== '0'
      const hierarchy = await listOrgHierarchy(user.organizationId, { skipLeadCounts })
    ...
    return sendJson(res, 400, { error: customerSafeErrorMessage(error, 'Could not load teams. Refresh in a moment.') })
  }
}

...
import { stampMarketingAutomationVersion } from '../workflowPublish.js'
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      recordMarketingAudit({
        ...
        actorUserId: user.id,
        action: 'marketing.automation_created',
  ...
  if (req.method === 'PATCH') {
    ...
    try {
      let automation = await updateMarketingAutomation(user, body.id, body)
      if (automation?.status === 'active' && user.organizationId) {
        automation = await stampMarketingAutomationVersion(user.organizationId, automation, store)
        if (automation.publishedVersionId) {
          await updateStore((draft) => {
            const row = (draft.marketingAutomations || []).find((a) => a.id === body.id)
            if (row) {
              row.publishedVersionId = automation.publishedVersionId
              row.definitionHash = automation.definitionHash
              row.publishedAt = automation.publishedAt
            }
            return draft
          })
        }
      ...
      recordMarketingAudit({
        organizationId: user.organizationId,
        actorUserId: user.id,
        action: 'marketing.automation_updated',
        resourceType: 'automation',
        resourceId: body.id,
      })
  ...
  if (req.method === 'DELETE') {
    ...
    recordMarketingAudit({
      organizationId: user.organizationId,
      actorUserId: user.id,
      action: 'marketing.automation_deleted',
      resourceType: 'automation',
      resourceId: body.id,
    })

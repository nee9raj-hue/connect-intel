...
import { readMarketingDashboardFromSnapshot } from '../marketingSnapshots.js'
import { enqueueMarketingSnapshotRefresh } from '../queue/producer.js'
...
export default async function handler(req, res) {
  ...
  if (gateUser.organizationId && gateUser.accountType === 'company') {
    try {
      await assertOrgPermission(gateUser, 'view_analytics', gateStore)
    } catch (permError) {
  ...
  const cached = await readMarketingDashboardFromSnapshot(user, { period, allowStale: true })
  if (cached?.hub && cached?.dashboard) {
    if (!cached.fresh) {
      void enqueueMarketingSnapshotRefresh(user.organizationId, user.id, period)
    }
    ...
    const payload = {
      ...
      deliverability: hub.deliverability,
      snapshot: cached.dashboard,
      _snapshot: { fresh: cached.fresh, source: cached.source, updatedAt: cached.updatedAt },
    }

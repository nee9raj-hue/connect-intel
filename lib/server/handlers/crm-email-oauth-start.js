...
export default async function handler(req, res) {
  ...
  if (!verifiedForCustomers && String(process.env.GOOGLE_OAUTH_VERIFIED || '').toLowerCase() === 'false') {
    if (!user.isPlatformAdmin && user.role !== 'admin') {
      return sendJson(res, 403, {
        error: 'Work Gmail connect is disabled for your account.',
        googleVerificationPending: true,
        hint: 'Ask your admin to enable Google OAuth on the server.',
      })
    }
  }

...
export default async function handler(req, res) {
  ...
  try {
    ...
    const includeCalendarScope = params.get('calendar') === '1'
    const connected = getUserCrmGmail(freshUser)

    const scopeMode = includeCalendarScope ? 'calendar_only' : 'send_only'
    ...
    return sendJson(res, 200, {
      ...
      replySyncEnabled: inboundEnabled ? Boolean(connected) : false,

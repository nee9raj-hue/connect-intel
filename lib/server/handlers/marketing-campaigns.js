...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (campaignId && recipientFilter) {
      ...
      const result = await resolveCampaignRecipientLeadIds(
        reportStore,
        user,
        campaignId,
        recipientFilter
      )
      if (!result.campaignName && result.count === 0 && recipientFilter !== 'opened' && recipientFilter !== 'clicked') {
...
async function startCampaign(res, user, campaignId, { scheduledAt: overrideSchedule } = {}) {
  ...
  if (campaign.channel === 'email' && pendingSends > 0) {
    if (marketingSqlQueueActive()) {
      ...
      triggerMarketingQueueWorker({ limit: 50 })
      triggerMarketingQueueWorker({ afterMs: 4_000, limit: 50 })
      modeResult = {
        ...
        failed: 0,
        firstError: null,
        pendingSends: queueResult.enqueued ?? pendingSends,
        queuedSends: queueResult.enqueued ?? pendingSends,
        done: false,
        background: true,
        ...
        workerOnline: false,
        workerHint:
          'Campaign queued in the database. Emails send in the background — you can close this tab.',
      }
    } else {

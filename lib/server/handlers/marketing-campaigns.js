...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'start') {
      if (isWorkerOnlyEmailRequired()) {
        const workerCheck = await checkEmailWorkerReady()
        if (!workerCheck.ok) {
          return sendJson(res, workerCheck.status, workerCheck.body)
        }
      }
...
async function startCampaign(res, user, campaignId, { scheduledAt: overrideSchedule } = {}) {
  ...
  await writeCampaignSendShard(store, user, { ...campaign, status: 'active' })
  if (!enrolled) {
    return sendJson(res, 400, {
      error:
  ...
  let sendResult = { sent: 0, failed: 0, processed: 0, firstError: null }
  let pendingAfter = pendingSends
  let background = null

  if (campaign.channel === 'email' && pendingSends > 0) {
    sendResult = await processCampaignSendBurst(user, campaignId, {
      limit: Math.min(pendingSends, 50),
      maxMs: 240_000,
    })
    pendingAfter = sendResult.pendingSends ?? 0
    if (pendingAfter > 0 && isBackgroundEmailEnabled()) {
      background = await startBackgroundCampaignSend(campaignId, user.id, { source: 'marketing' })
    }
  }

  const sendStatus =
    pendingAfter <= 0 && sendResult.sent > 0
      ? 'completed'
      : sendResult.sent > 0
        ? 'sending'
        : background?.started
  ...
  return sendJson(res, 200, {
    ...
    queued: campaign.channel === 'email' && pendingAfter > 0,
    background: background?.started || false,
    sendStatus,

...
async function startCampaign(res, user, campaignId) {
  ...
  const chunkLimit =
    campaign.channel === 'email'
      ? MARKETING_SEND_CHUNK
      : Math.min(enrolled, MARKETING_SEND_CHUNK)
  const sendResult =
    campaign.channel === 'whatsapp' && isWhatsAppCloudConfigured(user, freshStore)
      ? await processDueEnrollments({ campaignId, limit: chunkLimit })
      : campaign.channel === 'whatsapp'
  ...
  const fresh = marketingOverview(afterStore, user)
...
async function processCampaignSends(res, user, campaignId, limitRaw) {
  ...
  const limit = Math.min(Math.max(Number(limitRaw) || MARKETING_SEND_CHUNK, 1), 15)

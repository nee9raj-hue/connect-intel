...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'start') {
      ...
      return startCampaign(res, startUser, body.id)
    }
  }
...
async function processCampaignSends(res, user, campaignId, limitRaw) {
  const limit = Math.min(Math.max(Number(limitRaw) || MARKETING_SEND_CHUNK, 1), 1)
  const [sendCampaign, metaStore, statsBefore] = await Promise.all([
    readCampaignSendShard(campaignId),
    readStore({ only: MARKETING_SEND_META_SLICES }),
  ...
  if (!campaign) {
    ...
    campaign = getMarketingCampaign(withRows, user, campaignId, { manage: true })
  }
  if (!campaign) return sendJson(res, 404, { error: 'Campaign not found' })
  const status = statsBefore?.status || campaign.status
  if (status !== 'active') {
  ...
  const stats = (await readCampaignStatsShard(campaignId)) || statsBefore
  const updated = {
    ...campaign,

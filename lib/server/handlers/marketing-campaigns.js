...
import {
  ...
  marketingScopeKey,
  MARKETING_SEND_ACTIONS,
  requireMarketingHubAccess,
  requireMarketingSendAccess,
...
import { startMessagingCampaignSend } from '../messaging/engine.js'
...
function auditCampaign(user, action, campaignId, metadata = {}) {
...
export default async function handler(req, res) {
  ...
  async function denyIfCannotSend(user = gateUser, store = gateStore) {
    const sendCheck = await requireMarketingSendAccess(user, store)
    if (!sendCheck.ok) {
  ...
  const MARKETING_WRITE_SLICES = [
    ...MARKETING_LIGHT_SLICES,
    'marketingSuppressions',
  ]
  ...
  if (req.method === 'POST') {
    ...
    if (MARKETING_SEND_ACTIONS.has(body.action)) {
      const denied = await denyIfCannotSend()
    ...
    if (body.action === 'preview_audience') {
      const { user: previewUser, store: previewSlice } = await marketingUser(MARKETING_START_SLICES)
      const { pipelineStore } = await loadPipelineStoreContext(previewUser)
      const store = { ...previewSlice, savedLeads: pipelineStore.savedLeads }
      const channel = body.channel || 'email'
      const stub = { channel }
      if (body.audienceMode === 'segment' && body.segmentId) {
        stub.segmentId = body.segmentId
      } else if (body.listId || body.audienceMode === 'list') {
        stub.listId = body.listId
      } else if (body.audienceMode === 'all') {
        const orgLists = filterMarketingRows(store.marketingLists || [], previewUser).filter(
          (l) => (l.channel || 'email') === channel
        )
        const biggest = [...orgLists].sort(
          (a, b) =>
        ...
        if (!biggest) {
          return sendJson(res, 200, {
            ...
            audienceName: 'Entire audience',
          })
        }
        stub.listId = biggest.id
      } else {
        return sendJson(res, 400, { error: 'Select a list or segment' })
      }
      const audience = await resolveCampaignAudience(store, previewUser, stub)
      if (!audience) return sendJson(res, 404, { error: 'Audience not found' })
      const summary = summarizeAudienceEligibility(store, previewUser, audience.leadIds || [])
      return sendJson(res, 200, {
        ...summary,
  ...
  const isLightOverview =
    req.method === 'GET' && req.query?.overview === '1' && req.query?.light === '1'
  const storeSlices = isLightOverview
    ? MARKETING_LIGHT_SLICES
    : req.method === 'PATCH'
      ? MARKETING_WRITE_SLICES
      : MARKETING_READ_SLICES

  const store = await readStore({ only: storeSlices })
  ...
  if (req.method === 'POST') {
    ...
    if (body.action === 'duplicate') {
      ...
      auditCampaign(user, 'marketing.campaign_duplicated', created.id, { sourceId })
    ...
    const { syncCampaignV3AfterSave } = await import('../campaignsV3Sync.js')
    syncCampaignV3AfterSave({ campaign, user })
    auditCampaign(user, 'marketing.campaign_created', campaign.id)
  ...
  if (req.method === 'PATCH') {
    ...
    if (body.action === 'pause') {
      ...
      try {
        ...
        auditCampaign(user, 'marketing.campaign_paused', campaignId)
    ...
    if (body.action === 'resume') {
      ...
      try {
        ...
        auditCampaign(user, 'marketing.campaign_resumed', campaignId)
        return sendJson(res, 200, {
    ...
    if (body.action === 'stop') {
      ...
      try {
        ...
        auditCampaign(user, 'marketing.campaign_stopped', campaignId)
        return sendJson(res, 200, { campaign })
      } catch (e) {
    ...
    if (body.action === 'archive') {
      ...
      try {
        ...
        auditCampaign(user, 'marketing.campaign_archived', campaignId)
        return sendJson(res, 200, { campaign })
      } catch (e) {
    ...
    if (updated) {
      const { syncCampaignV3AfterSave } = await import('../campaignsV3Sync.js')
      syncCampaignV3AfterSave({ campaign: updated, user })
    }
    if (updated) auditCampaign(user, 'marketing.campaign_updated', id)
  ...
  if (req.method === 'DELETE') {
    ...
    if (!permanent) {
      try {
        ...
        auditCampaign(user, 'marketing.campaign_archived', id)
    ...
    auditCampaign(user, 'marketing.campaign_deleted', id, { permanent: true })
...
const SYNC_QUEUE_DRAIN_LIMIT = 12
const SYNC_QUEUE_DRAIN_MS = 42_000

async function scheduleBackgroundMarketingQueueDrain(remaining) {
  triggerMarketingQueueWorker({ afterMs: 1_500, limit: 50 })
  try {
    const { waitUntil } = await import('@vercel/functions')
    const { processMarketingEmailQueue } = await import('../marketingEmailQueueWorker.js')
    waitUntil(
      processMarketingEmailQueue({
        limit: Math.min(Math.max(remaining, 1), 80),
        maxMs: 240_000,
      }).catch((err) => {
        console.warn('waitUntil marketing queue drain:', err?.message || err)
      })
    )
  } catch {
    /* non-Vercel runtime */
...
async function startCampaign(res, user, campaignId, { scheduledAt: overrideSchedule } = {}) {
  ...
  if (campaign.channel === 'email' && pendingSends > 0) {
    if (isBackgroundEmailEnabled()) {
      modeResult = await startMessagingCampaignSend(user, campaignId, pendingSends, {
        source: 'marketing',
      })
    } else if (marketingSqlQueueActive() && pendingSends > INLINE_EMAIL_MAX_RECIPIENTS) {
      const { readCampaignEnrollments } = await import('../marketingEnrollmentShard.js')
      ...
      if (enqueued > 0) {
        ...
        drainResult = await processMarketingEmailQueue({
          limit: Math.min(enqueued, SYNC_QUEUE_DRAIN_LIMIT),
          maxMs: SYNC_QUEUE_DRAIN_MS,
        })
        ...
        if (remaining > 0) {
          await scheduleBackgroundMarketingQueueDrain(remaining)
        }
      } else {
        modeResult = await startMessagingCampaignSend(
          user,
          campaignId,
  ...
  auditCampaign(user, 'marketing.campaign_started', campaignId, { enrolled })

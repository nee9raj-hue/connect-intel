  ...
  if (req.method === 'GET') {
    if (req.query?.overview === '1') {
      ...
      return sendJson(res, 200, {
        ...(await marketingOverview({ ...store, savedLeads: pipelineStore.savedLeads }, user)),
      })
    }
...
async function startCampaign(res, user, campaignId) {
  const { pipelineStore } = await loadPipelineStoreContext(user)
  const store = {
    ...(await readStore({ only: MARKETING_START_SLICES })),
  ...
  if (campaign.channel === 'email') {
    const sender =
      resolveCampaignSender(store, campaign) ||
      buildOrgUserResponse(store.users.find((u) => u.id === user.id) || user, store)
  ...
  const afterStore = await readStore({ only: ['marketingCampaigns'] })
...
async function processCampaignSends(res, user, campaignId, limitRaw) {
  const metaStore = await readStore({ only: MARKETING_SEND_READ_SLICES })
  const campaign = getMarketingCampaign(metaStore, user, campaignId)
  if (!campaign) return sendJson(res, 404, { error: 'Campaign not found' })
  if (campaign.status !== 'active') {
    return sendJson(res, 400, { error: 'Campaign is not active' })
  }
  const sender = resolveCampaignSender(metaStore, campaign) || user
  ...
  let updated = getMarketingCampaign(afterStore, user, campaignId)
  if (updated) {
    ;[updated] = await mergeCampaignStatsShards([updated])
  }
  return sendJson(res, 200, {
    campaign: updated,
    sendResult,

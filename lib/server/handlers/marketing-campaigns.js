...
async function startCampaign(res, user, campaignId) {
  ...
  if (campaign.channel === 'email') {
    ...
    if (!gmail && !orgCanSend) {
      ...
      let error =
        check?.hint ||
        'Email sending is not set up for your account yet. Open Work email in the sidebar for options.'
      if (!domain) {
        error =
          'Your company has not set up outbound email yet. Ask your admin to complete Outbound email under Team & email, or add you as a Google OAuth test user (see Work email in the sidebar).'
      } else if (domain && check?.reason === 'email_domain_mismatch') {
        error = check.hint
      } else if (domain && org?.emailDomain?.status !== 'verified') {
        error = `Company domain @${domain} is not verified yet. Ask your admin to finish DNS under Team → Outbound email.`
      }

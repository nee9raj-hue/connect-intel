...
export default async function handler(req, res) {
  ...
  let gmailRefresh = null
  try {
    const { refreshAllCrmGmailTokens } = await import('../crmUserGmail.js')
    gmailRefresh = await refreshAllCrmGmailTokens()
  } catch {
    gmailRefresh = null
  }
  await writeWorkerHeartbeat({
    source: 'vercel_cron_drain',
    jobs: result.processed?.length || 0,
    gmailRefresh,
  })
  return sendJson(res, 200, { ...result, gmailRefresh })
}

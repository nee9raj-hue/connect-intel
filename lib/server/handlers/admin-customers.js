  ...
  if (req.method === 'PATCH') {
    ...
    if (userId) {
      ...
      if (organizationId) {
        ...
        if (action === 'grant_org_admin' && payload.userId) {
          invalidateSessionUserCache(payload.userId)
        }
        if (
          ...
          await unassignOrgMemberPipelineLeads(organizationId, payload.userId)
        }

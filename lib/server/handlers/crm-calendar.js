...
export default async function handler(req, res) {
  ...
  let freshUser = store.users.find((u) => u.id === user.id) || user

  try {
    const calSync = await syncGoogleCalendarIfStale(freshUser)
    if (calSync?.synced) {
      const refreshed = await readStore({ only: ['users'] })
      freshUser = refreshed.users.find((u) => u.id === user.id) || freshUser
    }
  } catch (err) {
    console.warn('calendar auto-sync failed:', err?.message || err)
  }

  const crmEvents = collectCalendarEvents(entries, freshUser, { fromMs, toMs })
  const events = mergeGoogleCalendarEvents(crmEvents, freshUser, { fromMs, toMs })
  const reminders = collectUpcomingReminders(entries, freshUser, { withinHours: 168 })
  const googleCalendar = getGoogleCalendarSyncStatus(freshUser)

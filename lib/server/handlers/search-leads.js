...
export default async function handler(req, res) {
  ...
  if (!result?.leads?.length && isPerplexityConfigured() && discoveryFiltersReady(searchFilters, naturalQuery)) {
    const discovery = await discoverLeadsWithPerplexity(searchFilters, Math.min(count, 10), discoveryOpts)

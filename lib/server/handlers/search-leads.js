...
const FREE_NOTICE =
  'Results include only contacts with both email and phone. Saved discoveries are reused from your database. Pipeline leads are excluded.'
...
export default async function handler(req, res) {
  ...
  return sendJson(res, 200, {
    ...
    netNew: 0,
    provider: 'none',
    notice: `No contacts with both email and phone matched your filters. Try keyword "exporter" plus one state, or import contacts in Admin.${pipelineHint}${hints.length ? ` ${hints.join('. ')}.` : ''}`,
    discoveryError,
    user: quotaUser,
  })
}
...
function buildPrompt(filters, count) {
  ...
  return `Find ${Math.min(count, 5)} Indian B2B contacts matching:\n${criteria}\n\nReturn ONLY a JSON array. Each object MUST include firstName, lastName, title, company, email (business), phone (+91), city, state, industry. Omit rows missing email or phone.`
}

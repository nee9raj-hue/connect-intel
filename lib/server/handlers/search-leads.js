...
import { mergeParsedFilters, parseSearchQuery } from '../searchQueryParser.js'
...
async function persistAiLeadsAndSearch(
  ...
  const leads = filterUsableLeads(
    filterDiscoveryLeads(
      usable.slice(0, count).map((lead, index) =>
        shapeLeadForViewer(
          {
            ...lead,
  ...
  if (!leads.length && usable.length) {
    const fallback = usable.slice(0, count).map((lead, index) =>
      shapeLeadForViewer(
        {
          ...lead,
          id: lead.id || createId('lead'),
    ...
    return {
      leads: fallback,
      total: Math.max(usable.length, fallback.length),
      netNew: fallback.length,
      provider: source,
      notice:
  ...
  return {
    leads,
    total: Math.max(usable.length, leads.length),
    netNew: leads.length,
    provider: source,
...
export default async function handler(req, res) {
  ...
  const searchFilters = mergeParsedFilters({ ...filters, jobTitles: [] }, parsed)
  ...
  const canRunLiveAi =
    isPerplexityConfigured() && discoveryFiltersReady(searchFilters, naturalQuery)
  const useLivePerplexity = canRunLiveAi
  const grantFullPreview = aiDiscoveryLeft > 0
  ...
  if (useLivePerplexity) {
    ...
    if (toPersist?.length) {
      result = await persistAiLeadsAndSearch(
        ...
        'perplexity',
        {
          notice: grantFullPreview
            ? 'Live Perplexity AI search — full email and phone on the top 10 matches.'
            : 'Live Perplexity AI search — reveal email/phone at ₹1 each, or recharge for more free full previews.',
          discoveryMethod: discovery.method,
        },
        { fullContactPreview: grantFullPreview }
      )
      if (result?.leads?.length) {
        if (grantFullPreview) {
          try {
            ...
            discoveryError = quotaError.message
          }
        }
      }
      store = await readStore()
    } else if (discovery.error) {
      discoveryError = discovery.notice ? `${discovery.error} (${discovery.notice})` : discovery.error
    } else if (discovery.notice) {
      discoveryError = discovery.notice
    }
  } else if (!isPerplexityConfigured()) {
    discoveryError = 'Live AI search is not configured (PERPLEXITY_API_KEY missing on server).'
  }

  if (!result?.leads?.length) {
    const skipStaleDb = Boolean(result?.fromAiDiscovery)
    if (!skipStaleDb) {
      result = runLocalSearch(store, searchFilters, count, viewer, excludeIds, { fullContactPreview: false })
  ...
  if (!isPerplexityConfigured()) {
    hints.push('Set PERPLEXITY_API_KEY on the server for live Perplexity search')

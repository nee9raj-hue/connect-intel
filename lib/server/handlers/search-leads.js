...
const SEARCH_STORE_COLLECTIONS = [
...
const MIN_DB_RESULTS_BEFORE_AI = 3
const AI_DISCOVERY_TIMEOUT_MS = 45_000
...
function applyContactQuality(result) {
  ...
  const displayTotal = honestTotal
    ...
    : Math.max(AI_SEARCH_DISPLAY_TOTAL_MIN, result.total || 0, leads.length)
  ...
  return {
    ...
    total: displayTotal,
    netNew: honestTotal
      ? Math.max((result.total || leads.length) - (result.excludedPipelineCount || 0), leads.length)
      : Math.max(displayTotal - (result.excludedPipelineCount || 0), leads.length),
...
function tryLocalSearch(store, filters, count, viewer, excludeIds, shapeOptions = {}) {
  ...
  const hasRealDatabase = (store.companies?.length || 0) > 0
  if (hasRealDatabase) return null
  ...
  if (mockLeads.length) {
    return {
      ...
      provider: 'demo',
      notice: 'Sample prospects — import your exporter database in Platform backend → Data & imports.',
    }
  }
...
async function persistAiLeadsAndSearch(
  ...
  const freshStore = await readStore({ only: SEARCH_STORE_COLLECTIONS })
...
export default async function handler(req, res) {
  ...
  let { store, excludeIds } = await loadSearchContext(quotaUser)
  ...
  let result = runLocalSearch(store, searchFilters, count, viewer, excludeIds, { fullContactPreview: true })
  let discoveryError = null
  let liveAiAttempted = false
  let aiDiscoverySearchesLeft = aiDiscoveryLeft

  const dbHitCount = result?.leads?.length || 0
  const shouldTryLiveAi = canRunLiveAi && dbHitCount < MIN_DB_RESULTS_BEFORE_AI

  if (shouldTryLiveAi) {
    liveAiAttempted = true
    try {
      const discovery = await withTimeout(
        discoverLeadsWithPerplexity(searchFilters, Math.min(count, AI_SEARCH_FETCH_COUNT), discoveryOpts),
      ...
      if (toPersist?.length) {
        ...
        if (aiResult?.leads?.length) {
          result = aiResult
          if (aiDiscoveryLeft > 0) {
            try {
              aiDiscoverySearchesLeft = await consumeAiDiscoverySearch(viewer)
              const refreshedStore = await readStore({ only: SEARCH_STORE_COLLECTIONS })
          ...
          const reloaded = await loadSearchContext(viewer)
      ...
      discoveryError = error.message || 'Live AI search timed out — showing database matches.'
    }
  } else if (!liveAiAvailable && discoveryFiltersReady(searchFilters, naturalQuery) && dbHitCount < MIN_DB_RESULTS_BEFORE_AI) {
    discoveryError =

...
export default async function handler(req, res) {
  ...
  const useLivePerplexity = canRunLiveAi && aiDiscoveryLeft > 0
  ...
  if (useLivePerplexity) {
    ...
    if (toPersist?.length) {
      result = await persistAiLeadsAndSearch(
        ...
        {
          notice:
            'Live Perplexity AI search — full email and phone on the top 10 matches. Saved to your database.',
  ...
  } else if (canRunLiveAi && aiDiscoveryLeft <= 0) {
    discoveryError =
      'Your 3 free live AI searches are used. Recharge credits for more Perplexity searches, or reveal email/phone at ₹1 each below.'
  }

  if (!result?.leads?.length) {
    const skipStaleDb = naturalQuery.length >= 8 && canRunLiveAi
    if (!skipStaleDb) {
  ...
  if (!isPerplexityConfigured()) {
    ...
    hints.push('Describe your search in a short sentence to run AI discovery when the database is empty')
  }

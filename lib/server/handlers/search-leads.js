...
const FREE_NOTICE =
  'Results include only contacts with email or phone. Saved discoveries are reused from your database. Pipeline leads are excluded.'
...
export default async function handler(req, res) {
  ...
  return sendJson(res, 200, {
    ...
    netNew: 0,
    provider: 'none',
    notice: `No contacts with email or phone matched your filters. Try keyword "exporter" plus one state, or import contacts in Team.${pipelineHint}${hints.length ? ` ${hints.join('. ')}.` : ''}`,
    discoveryError,
    user: quotaUser,
  })
}

...
describe('extension page capture', () => {
  ...
  it('extracts city and state from Greater Ahmedabad Area profile text', () => {
    const extract = loadPageCapture({
      document: {
        title: 'Sulay Lavsi | LinkedIn',
        querySelector(sel) {
          if (sel === 'main') {
            return {
              innerText:
                'Sulay Lavsi\nFounder - Bummer | Forbes30U30\nGreater Ahmedabad Area\nContact info\n20,982 followers',
              querySelectorAll: () => [],
              querySelector: () => null,
            }
          }
    ...
    const capture = extract()
    assert.equal(capture.city, 'Ahmedabad')
    assert.match(capture.location, /Ahmedabad/i)
  })
})

...
export async function syncPipelineCompaniesForOrg(user, metaStoreHint = null) {
  ...
  const parentByCompanyId = new Map(
    hierarchy.map((row) => [String(row.id), row.parentCompanyId || null])
  )

  const rows = companies.map((c) =>
    companyRowToSql(organizationId, {
      ...c,
      parentCompanyId: parentByCompanyId.get(String(c.id)) ?? c.parentCompanyId ?? null,
    })
  )
...
export async function listPipelineCompaniesPage(
  organizationId,
  { search = '', limit = 50, offset = 0, rootsOnly = false } = {}
) {
  ...
  if (rootsOnly) {
    path += '&parent_company_id=is.null'
  }
...
export async function updatePipelineCompanyParent(organizationId, companyId, parentCompanyId) {
  ...
  const hierarchy = await listPipelineCompanyHierarchy(organizationId)
  const verdict = validateCompanyParentAssignment(hierarchy, companyId, parentCompanyId)
  if (!verdict.ok) return verdict

  const cid = verdict.companyId
  const parentId = verdict.parentCompanyId

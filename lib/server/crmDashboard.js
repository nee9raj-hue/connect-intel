...
export function buildTeamDashboard(
  store,
  user,
  { period = 'week', memberUserId = null, light = false, detailed = false } = {}
) {
  ...
  if (accountType !== 'company' || !orgId) {
    ...
    return {
      ...
      members: [],
      activityByDay: buildActivityByDay(activityEntries, since, dashboardPeriod),
  ...
  return {
    ...
    members: memberStats,
    activityByDay: buildActivityByDay(activityEntries, since, dashboardPeriod, scopedMemberUserId),
    recentActivities: listCrmActivities(store, user, activityEntries, {
      since,
      memberUserId: scopedMemberUserId,

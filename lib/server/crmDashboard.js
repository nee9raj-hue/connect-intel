...
import { collectTouchpoints } from './crmTouchpoints.js'
...
export function buildTeamDashboard(store, user, { period = 'week', memberUserId = null } = {}) {
  ...
  // Keep KPI row aligned with activity-by-day chart (same touchpoint source).
  if (teamIntelligence?.rollup && allDashboardEntries.length) {
    const chartEntries = scopedMemberUserId ? entries : allDashboardEntries
    const since = periodStart(period)
    let chartCalls = 0
    let chartEmails = 0
    let chartTasks = 0
    let chartContacts = new Set()
    for (const entry of chartEntries) {
      for (const point of collectTouchpoints(entry, since)) {
        chartContacts.add(entry.id || entry.lead?.id)
        const kind = String(point.type || '').toLowerCase()
        if (kind === 'call') chartCalls += 1
        else if (kind === 'email' || kind === 'email_inbound') chartEmails += 1
        else if (kind === 'task') chartTasks += 1
      }
    }
    teamIntelligence.rollup.calls = Math.max(teamIntelligence.rollup.calls || 0, chartCalls)
    teamIntelligence.rollup.emails = Math.max(teamIntelligence.rollup.emails || 0, chartEmails)
    teamIntelligence.rollup.tasksCreated = Math.max(
      teamIntelligence.rollup.tasksCreated || 0,
      chartTasks
    )
    teamIntelligence.rollup.contactsOpened = Math.max(
      teamIntelligence.rollup.contactsOpened || 0,
      chartContacts.size
    )
  }

import { CRM_STATUSES, normalizeCrmLeadStatus } from './crm.js'
...
import { buildIntelligenceV3 } from './teamIntelligenceV3.js'
import { buildDashboardIntelligenceV3 } from './dashboardIntelligenceV3.js'
import { buildTeamActivityTimeline, buildMemberUsageDetail } from './teamActivityTimeline.js'
import { isFreightDealOrg } from '../freightDeal.js'
import {
  collectTouchpoints,
  buildActivityRollup,
  buildActivityRollupsForPeriods,
...
function periodStartForDashboard(period, timeZone) {
  return periodStart(normalizeDashboardPeriod(period), timeZone)
}
...
function emptyBucket(date, label) {
  return {
    date,
    label,
    count: 0,
...
function buildLocalDayBuckets(period, timeZone) {
  ...
  while (cursor <= end) {
    ...
    buckets.push(emptyBucket(key, formatLocalDateLabel(new Date(cursor), tz)))
    if (cursor >= end) break
...
function entryAssigneeUserId(entry) {
  if (entry.assignedToUserId) return String(entry.assignedToUserId)
  return String(entry.savedByUserId || entry.userId || '')
}
...
function listTeamMembersFallback(store, orgId, user) {
  if (!orgId) {
    return [{ userId: user.id, name: user.name || user.email || 'You' }]
  }
  return listTeamMembers(store, orgId).filter((m) => (m.status || 'active') === 'active')
}

export function buildTeamDashboard(
  store,
  user,
  { period = 'week', memberUserId = null, light = false, detailed = false, timeZone = null, viewerScope = null } = {}
) {
  const tz = resolveTimeZone(user, timeZone)
  const dashboardPeriod = normalizeDashboardPeriod(period)
  const since = periodStartForDashboard(dashboardPeriod, tz)
  ...
  if (accountType !== 'company' || !orgId) {
    ...
    const activityEntries = entriesForActivityScanWindows(
      entries,
    ...
    const summary = orgSummary(entries, since, activityRollup.org)
    const activityByDay = buildActivityByDay(activityEntries, since, dashboardPeriod, null, tz)
    const intelMembers = teamIntelligence.members?.length
      ? teamIntelligence.members
      : [
          {
            userId: user.id,
            name: user.name || user.email || 'You',
            ...teamIntelligence.rollup,
          },
        ]
    const intelligenceV3 = buildIntelligenceV3({
      entries,
      members: intelMembers,
      rollup: teamIntelligence.rollup || {},
      comparison: teamIntelligence.comparison || {},
      summary,
      activityByDay,
      memberUserId: user.id,
      isAdmin: false,
      prevMemberProfiles: teamIntelligence.prevMemberProfiles || [],
    })
    const dashboardV3 = buildDashboardIntelligenceV3({
      intelligenceV3,
      summary,
      comparison: teamIntelligence.comparison || {},
      rollup: teamIntelligence.rollup || {},
    ...
    return {
      ...
      members: [],
      activityByDay,
      recentActivities: listCrmActivities(store, user, activityEntries, {
        since,
        memberUserId: user.id,
        responseLimit: 12,
      }),
      ...
      teamSnapshot: light ? null : buildTeamSnapshot(store, user, entries, since, period, tz),
      teamIntelligence,
      intelligenceV3,
      dashboardV3,
    }
  ...
  let scopedMemberUserId = viewerScope?.scopedMemberId ?? (memberUserId ? String(memberUserId) : null)
  const isManager = viewerScope?.isManager ?? false
  const members = viewerScope?.rosterMembers?.length
    ? viewerScope.rosterMembers
    : listTeamMembersFallback(store, orgId, user)

  if (!viewerScope && scopedMemberUserId && !isAdmin && !isManager && scopedMemberUserId !== String(user.id)) {
  ...
  const filterUserId = scopedMemberUserId || (isAdmin || isManager ? null : user.id)
  if (filterUserId) {
  ...
  const rollupMemberId = scopedMemberUserId || filterUserId
  const summaryActivity =
    rollupMemberId && orgActivityRollup.perUser.has(String(rollupMemberId))
      ? orgActivityRollup.perUser.get(String(rollupMemberId))
  ...
  let memberUsage = null
  if (detailed) {
    activityTimeline = buildTeamActivityTimeline(store, user, allDashboardEntries, {
      since,
      memberUserId: scopedMemberUserId,
      limit: 100,
      freightOrg,
    ...
    if (scopedMemberUserId) {
      const storeUser = (store.users || []).find((u) => String(u.id) === String(scopedMemberUserId))
      if (storeUser) memberUsage = buildMemberUsageDetail(storeUser, since)
    } else if (!isAdmin) {
      const storeUser = (store.users || []).find((u) => String(u.id) === String(user.id))
      if (storeUser) memberUsage = buildMemberUsageDetail(storeUser, since)
    }
  }

  const summary = orgSummary(summaryEntries, since, summaryActivity)
  const activityByDay = buildActivityByDay(
    activityEntries,
    since,
    dashboardPeriod,
    scopedMemberUserId || filterUserId,
    tz
  )

  const intelligenceV3 = buildIntelligenceV3({
    entries: allDashboardEntries,
    members: teamIntelligence.members || [],
    rollup: teamIntelligence.rollup || {},
    comparison: teamIntelligence.comparison || {},
    summary,
    activityByDay,
  ...
  const dashboardV3 = buildDashboardIntelligenceV3({
    ...
    comparison: teamIntelligence.comparison || {},
    rollup: teamIntelligence.rollup || {},
    personal: Boolean(scopedMemberUserId) || (!isAdmin && !isManager),
    isAdmin,
    isManager,
  })
  ...
  return {
    ...
    recentActivities: listCrmActivities(store, user, activityEntries, {
      since,
      memberUserId: scopedMemberUserId,
      responseLimit: detailed ? 40 : 12,
    }),
    activityTimeline,
    memberUsage,
    statusBreakdown: statusBreakdownFromEntries(entries),
    valueByStage: valueByStageFromEntries(entries),
    memberOptions: viewerScope?.memberOptions?.length
...
function valueByStageFromEntries(entries) {
  ...
  for (const entry of entries) {
    ...
    const st = normalizeCrmLeadStatus(raw.status)
    if (totals[st]) {
      totals[st].count += 1
...
function orgSummary(entries, since, activityOrg = null) {
  ...
  for (const entry of entries) {
    const raw = entry?.crm && typeof entry.crm === 'object' ? entry.crm : {}
    const status = normalizeCrmLeadStatus(raw.status)
    const deal = Number(raw.dealValue) || 0
    const weight = STAGE_WEIGHT[status] ?? 0.15
    const score = Number(raw.leadScore) || 0

    if (status !== 'unqualified') contacted += 1
    if (status === 'onboarding' || status === 'new_account' || status === 'active_trading') {
      won += 1
      wonValue += deal
    }
    if (raw.nextFollowUpAt && new Date(raw.nextFollowUpAt).getTime() <= now) needsFollowUp += 1
    ...
    if (last && now - new Date(last).getTime() > 7 * MS_DAY && status !== 'lost' && status !== 'churned') {
      staleLeads += 1
    }
...
function buildActivityByDay(entries, since, period, actorUserId = null, timeZone = null) {
  const tz = resolveTimeZone({}, timeZone)
  const buckets = buildLocalDayBuckets(period, tz)
  ...
  for (const entry of entries || []) {
    for (const point of collectTouchpoints(entry, since, Infinity, {
      ...
      const day = localDateKey(point.at, tz)
...
function buildTeamSnapshot(store, user, entries, since, period, timeZone = null) {
  const tz = resolveTimeZone(user, timeZone)
  const orgId = user.organizationId
  const now = Date.now()
  const days60 = 60 * MS_DAY
  const monthStartMs = periodStart('month', tz)
  const prevMonthStartMs = previousPeriodStart('month', tz)
  const monthStart = new Date(monthStartMs).toISOString()
  const prevMonthStart = new Date(prevMonthStartMs).toISOString()

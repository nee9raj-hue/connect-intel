...
import { collectTouchpoints, recentPipelineEntriesForActivity, buildActivityRollup } from './crmTouchpoints.js'
...
export function buildTeamDashboard(store, user, { period = 'week', memberUserId = null } = {}) {
  ...
  if (accountType !== 'company' || !orgId) {
    ...
    const activityRollup = buildActivityRollup(entries, since)
  ...
  if (
    scopedMemberUserId &&
    isAdmin &&
    !members.some((m) => String(m.userId) === String(scopedMemberUserId))
  ) {
    scopedMemberUserId = null
  }
  ...
  const orgActivityRollup = buildActivityRollup(allDashboardEntries, since)
  const prevOrgActivityRollup = buildActivityRollup(
    allDashboardEntries,
    since - periodDays * MS_DAY,
    since
  )

  const summaryEntries = scopedMemberUserId ? entries : allDashboardEntries
  const summaryActivity =
    scopedMemberUserId && orgActivityRollup.perUser.has(String(scopedMemberUserId))
      ? orgActivityRollup.perUser.get(String(scopedMemberUserId))
  ...
  return {
    ...
    members: memberStats,
    activityByDay: buildActivityByDay(allDashboardEntries, since, period, scopedMemberUserId),
    statusBreakdown: statusBreakdownFromEntries(entries),
    valueByStage: valueByStageFromEntries(entries),
...
function buildActivityByDay(entries, since, period, actorUserId = null) {
  ...
  const recent = recentPipelineEntriesForActivity(entries, 800)
  const actorId = actorUserId ? String(actorUserId) : null

  for (const entry of recent) {
    for (const point of collectTouchpoints(entry, since)) {
      if (actorId && String(point.actorUserId || '') !== actorId) continue

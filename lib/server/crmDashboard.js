...
export function collectTouchpoints(entry, since) {
  ...
  if (
    ...
    inPeriod(crm.lastCommunicationAt, since) &&
    crm.lastCommunicationSummary &&
    !points.some((p) => Math.abs(new Date(p.at).getTime() - new Date(crm.lastCommunicationAt).getTime()) < 120000)
  ) {
    points.push({ at: crm.lastCommunicationAt, type: crm.lastCommunicationType || 'note' })
  }

  for (const task of crm.tasks || []) {
    if (task.createdAt && inPeriod(task.createdAt, since)) {
      points.push({ at: task.createdAt, type: 'task' })
    }
  }

...
import { pipelineLeadSearchHaystack } from './pipelineQuery.js'
import {
  filterOrgRows,
  userCanViewNote,
  userCanViewTask,
...
export function searchPlatform(store, user, { q = '', limit = 50 } = {}) {
  ...
  if (results.length < cap && user.organizationId) {
    for (const task of filterOrgRows(store.teamTasks, user.organizationId)) {
      if (results.length >= cap) break
      if (!userCanViewTask(task, user)) continue
      const hay = `${task.title || ''} ${task.description || task.body || ''}`.toLowerCase()
      if (!hay.includes(query)) continue
      push({
        type: 'task',
        id: task.id,
        title: task.title || 'Task',
        subtitle: [task.status, task.dueAt].filter(Boolean).join(' · ') || 'Task',
        panel: 'chithi',
        tab: 'tasks',
      })
    }
  ...
  if (results.length < cap && user.organizationId) {
    for (const note of filterOrgRows(store.teamNotes, user.organizationId)) {
      ...
      const hay = `${note.title || ''} ${note.body || note.content || ''}`.toLowerCase()
      if (!hay.includes(query)) continue
      push({
        type: 'note',
        id: note.id,
        title: note.title || 'Team note',
        subtitle: 'Team note',
        panel: 'chithi',
      })
    }
  ...
  if (results.length < cap) {
    for (const template of filterMarketingRows(store.marketingTemplates || [], user)) {
      if (results.length >= cap) break
      const hay = `${template.name || ''} ${template.subject || ''}`.toLowerCase()
      if (!hay.includes(query)) continue
      push({
        type: 'template',
        id: template.id,
        title: template.name || 'Template',
        subtitle: template.subject || 'Email template',
        panel: 'marketing',
        tab: 'templates',
      })
    }

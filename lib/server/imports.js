...
function normalizeCompany(row, datasetType, importJobId) {
  const name = getValue(row, ['company', 'companyName', 'company_name', 'business_name', 'name'])
  if (!name) return null
...
export function importRowsIntoStore(store, datasetType, rows, actor) {
  ...
  const importJob = {
    id: importJobId,
    name: `${datasetType} import ${new Date(now).toLocaleString('en-IN')}`,
    datasetType,

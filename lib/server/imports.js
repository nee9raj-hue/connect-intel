...
function normalizeCompany(row, datasetType, importJobId, organizationId = null) {
...
function normalizeContact(row, companyId, importJobId, company, organizationId = null) {
...
function scopedCompanyMapKey(organizationId, companyKey) {
  return organizationId ? `${organizationId}::${companyKey}` : companyKey
}

function processImportRows(store, datasetType, rows, importJobId, companyKeyToId, contactIndex, organizationId = null) {
  ...
  for (const rawRow of rows) {
    const row = normalizeImportRow(rawRow)
    const company = normalizeCompany(row, datasetType, importJobId, organizationId)
    if (!company) {
      rejectedRows += 1
      continue
    }

    const companyKey = scopedCompanyMapKey(organizationId, getCompanyKey(company))
    ...
    const contact = normalizeContact(row, companyId, importJobId, targetCompany, organizationId)
    if (contact) {
...
export function importRowsChunkIntoStore(store, datasetType, rows, actor, options = {}) {
  ...
  const companyKeyToId = new Map(
    store.companies
      .filter((company) => !organizationId || !company.organizationId || company.organizationId === organizationId)
      .map((company) => [scopedCompanyMapKey(organizationId, getCompanyKey(company)), company.id])
  )
  const contactIndex = buildContactIndex(store)
  const stats = processImportRows(store, datasetType, rows, importJob.id, companyKeyToId, contactIndex, organizationId)

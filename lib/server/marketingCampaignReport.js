...
function deliveryStatusForEnrollment(enrollment, { channel = 'email' } = {}) {
  ...
  if (sentCount > 0) {
    const hasProof =
      channel === 'whatsapp'
        ? Boolean(enrollment.lastSentAt)
        : Boolean(enrollment.lastSendMessageId || enrollment.lastSentAt)
    return hasProof ? 'delivered' : 'pending'
  }
...
export function buildCampaignReport(store, user, campaignId, enrollmentRows = null) {
  ...
  const enrollments = filterMarketingEnrollmentsForUser(store, user, source)

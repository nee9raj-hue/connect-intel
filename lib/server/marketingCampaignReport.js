...
function leadPhone(lead, enrollment) {
  return (
...
function leadFromEntry(entry, fallbackEmail, enrollment = null) {
  ...
  return {
    ...
    title: lead.title || '',
    city: lead.city || lead.metro || lead.location?.city || '',
    state: lead.state || lead.region || lead.location?.state || '',
    phone: leadPhone(lead, enrollment),
  }
}
...
export function buildCampaignReport(store, user, campaignId, enrollmentRows = null) {
  ...
  for (const enrollment of enrollments) {
    ...
    const contact = leadFromEntry(entry, enrollment.contactEmail, enrollment)
    ...
    let phone = contact.phone || enrollment.contactPhone || ''

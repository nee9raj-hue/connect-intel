...
import { startBackgroundCampaignSend } from './email/campaignSendOrchestrator.js'
...
export async function queuePipelineBulkCampaign(user, metaStore, body) {
  ...
  let pendingSends = enrollments.length
  let queuedSends = enrollments.length
  if (!enrollments.length) {
    pendingSends = await countPendingCampaignSends(campaign.id)
    queuedSends = await countQueuedCampaignEnrollments(campaign.id)
  }

  let sendBurst = null
  if (pendingSends > 0) {
    sendBurst = await processCampaignSendBurst(user, campaign.id, {
      limit: Math.min(pendingSends, 50),
      maxMs: 240_000,
    })
    if ((sendBurst.pendingSends ?? 0) <= 0) {
      invalidatePipelineIndex(pipelineShardNameForUser(user))
    }
  }

  const pendingAfter = sendBurst?.pendingSends ?? pendingSends
  const sent = sendBurst?.sent ?? 0
  const failed = sendBurst?.failed ?? 0
  const firstError = sendBurst?.firstError || null

  let background = null
  if (pendingAfter > 0 && isBackgroundEmailEnabled()) {
    background = await startBackgroundCampaignSend(campaign.id, user.id, {
      source: 'pipeline_bulk',
    })
  }

  let sendStatus = 'queued'
  if (pendingAfter <= 0 && sent > 0) sendStatus = 'completed'
  else if (pendingAfter <= 0 && failed > 0 && sent === 0) sendStatus = 'failed'
  else if (sent > 0) sendStatus = 'sending'
  else if (background?.started) sendStatus = 'preparing'
  ...
  return {
    ...
    background: pendingAfter > 0 && background?.started ? true : false,

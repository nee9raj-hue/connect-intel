...
export async function queuePipelineBulkCampaign(user, metaStore, body) {
  ...
  // Seed enrollments synchronously so the audience is durable even if a deferred
  // send task is dropped by the platform.
  ...
  // Small batches (≤25) send inline within this request — reliable within the
  // function budget and returns real sent/failed counts (and any error) to the
  // user immediately. Only larger batches are handed to the background worker.
  if (resolveEmailSendMode(recipientCount) === EMAIL_SEND_MODE.INLINE) {

...
import { collectPipelineOrganizationIds, pipelineEntryInOrganization } from './pipelineOrgIdentity.js'
...
export async function filterPipelineEntriesVisibleAsync(user, entries, metaStore) {
  ...
  const visible = listPipelineSavedEntries(store, user, {
    visibleOwnerIds,
    trustedOrgScope: user.accountType === 'company',
  })
...
export function isPipelineEntryInUserOrg(user, entry) {
  ...
  return pipelineEntryInOrganization(entry, collectPipelineOrganizationIds(user))
}

/** Delete: org admins only in company workspaces; solo users may delete their own rows. */
export function canUserDeletePipelineEntry(user, entry, metaStore) {
  if (!user || !entry) return false
  const { accountType, orgRole } = resolveOrgRole(user, metaStore || {})
  if (accountType === 'individual' || !user.organizationId) {
    const uid = user.id
    return entry.userId === uid || entry.savedByUserId === uid || entry.assignedToUserId === uid
  }
  if (user.isPlatformAdmin) return true
  return Boolean(user.isOrgAdmin || orgRole === 'org_admin')
}

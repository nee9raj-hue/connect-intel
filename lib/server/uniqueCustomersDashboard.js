...
import { dashboardSalesOwnerId, dashboardSalesOwnerLabel } from '../erpOwner.js'
...
function ownerIdOf(entry, ownerIndex) {
  return dashboardSalesOwnerId(entry, ownerIndex)
}
...
function lastOrderMs(entry) {
  const iso = resolveLeadLastOrderCreatedAt({
    crm: entry?.crm,
    crm_payload: entry?.crm_payload,
    erp: entry?.erp || entry?.lead?.erp,
    lead: entry?.lead,
    tradingProfile: entry?.tradingProfile || entry?.lead?.tradingProfile,
  })
  if (!iso) return null
...
export function matchUniqueCustomerFilters(entry, row, filters = {}, ownerIndex) {
  ...
  if (ownerIds.length) {
    const oid = ownerIdOf(entry, ownerIndex)
 ...
 */
export function buildUniqueCustomersReport(rows, { year, month, weeks = [], timeZone, ownerNames = {}, ownerIndex } = {}) {
  ...
  for (const row of rows || []) {
    ...
    mergeCustomer(customers, {
      ...
      ownerId: ownerIdOf(entry, ownerIndex),
      lastOrderMs: lastMs,
  ...
  for (const c of inColumns.sort((a, b) => a.label.localeCompare(b.label))) {
    ...
    const list = groupsMap.get(gid) || {
      ownerId: gid,
      ownerName: dashboardSalesOwnerLabel(gid, null, ownerNames),
      customers: [],
      uniqueCustomers: 0,
      revenue: 0,
    }

...
/** public.leads.lead_status check constraint (enterprise_crm_schema). */
const ENTERPRISE_LEAD_STATUSES = new Set([
  'new',
  'contacted',
  'follow_up',
  'replied',
...
const ACCOUNT_STATUS_TO_ENTERPRISE = {
  unqualified: 'new',
  qualified: 'contacted',
...
function normStatus(status) {
  const canonical = normalizeCrmLeadStatus(status)
  if (ENTERPRISE_LEAD_STATUSES.has(canonical)) return canonical
  if (ACCOUNT_STATUS_TO_ENTERPRISE[canonical]) return ACCOUNT_STATUS_TO_ENTERPRISE[canonical]
  const raw = String(status || '').trim().toLowerCase()
  if (ENTERPRISE_LEAD_STATUSES.has(raw)) return raw
  return 'new'
}
...
export function buildEnterpriseLeadRow(entry, { organizationUuid, assignedProfileId }) {
  ...
  return {
    ...
    crm_payload: buildCrmPayload(entry),
...
export async function resolveOrgUuid(legacyOrgId, cache, options) {
  return resolveOrganizationUuid(legacyOrgId, cache, options)
}

export async function resolveProfileId(legacyUserId, cache, options) {
  return resolveProfileUuid(legacyUserId, cache, options)
}

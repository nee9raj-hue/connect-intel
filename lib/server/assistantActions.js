...
export function sanitizeAssistantActions(actions = []) {
  ...
  for (const raw of actions.slice(0, 5)) {
    ...
    if (raw.type === 'apply_rfq' && raw.payload?.freight && typeof raw.payload.freight === 'object') {
      out.push({
        type: 'apply_rfq',
        leadId: raw.leadId ? String(raw.leadId) : null,
        label: String(raw.label || 'Apply to deal RFQ').slice(0, 80),
        payload: { freight: raw.payload.freight },
      })
      continue
    }

...
export function gmailScopesForKind(kind) {
  if (kind === 'user_crm') {
    return [GMAIL_SEND_SCOPE, GMAIL_READONLY_SCOPE, EMAIL_SCOPE].join(' ')
  }
  return `${GMAIL_SEND_SCOPE} ${EMAIL_SCOPE}`
}

export function buildGmailOAuthStartUrl({ organizationId, userId, kind = 'invite', loginHint } = {}) {
  ...
  const params = new URLSearchParams({
    ...
    access_type: 'offline',
    prompt: 'consent',
    state: signState({ kind, organizationId, userId, n: crypto.randomBytes(8).toString('hex') }),
  })
...
async function exchangeTokensAndEmail(code, existingRefreshToken) {
  ...
  return { refreshToken, email, connectedAt: new Date().toISOString() }
}
...
export async function completeCrmUserGmailOAuth(code, { existingRefreshToken = null } = {}) {
  ...
  return {
    ...
    connectedAt,
    purpose: 'crm',
    scope: gmailScopesForKind('user_crm'),
  }
}
...
function buildRawMime({ from, to, subject, html, text, replyTo, cc }) {
  const boundary = `ci_${Date.now()}`
  const ccLine =
    cc?.length > 0 ? `Cc: ${cc.map((e) => e.trim()).join(', ')}` : null
  const lines = [
    `From: ${from}`,
    `To: ${to}`,
    ccLine,
...
export async function sendViaGmailOAuth({
  ...
  try {
    ...
    const raw = buildRawMime({ from, to, subject, html, text, replyTo, cc })

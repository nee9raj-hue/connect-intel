...
export function accessTokenFromRefreshResult(result) {
...
/** RFC 2045 body part — never use encoded-word (=?UTF-8?B?...) in the body. */
function encodeMimeBodyPart(content) {
  const str = String(content || '')
  if (/^[\x09\x0A\x0D\x20-\x7E]*$/.test(str)) {
    return { transferEncoding: '7bit', body: str }
  }
  const wrapped = Buffer.from(str, 'utf8')
    .toString('base64')
    .replace(/(.{76})/g, '$1\r\n')
    .trim()
  return { transferEncoding: 'base64', body: wrapped }
}

function buildMimeAlternativeParts(innerBoundary, text, html) {
  const parts = []
...
function buildRawMime({ from, to, subject, html, text, replyTo, cc, attachments = [], messageId = null }) {
  ...
  const alternativeParts = buildMimeAlternativeParts(innerBoundary, text, html)
...
export async function sendViaGmailOAuth({
  ...
  try {
    ...
    if (!accessToken) {
      return { sent: false, error: 'Could not refresh Gmail access token' }
    }
    const raw = buildRawMime({ from, to, subject, html, text, replyTo, cc, attachments, messageId })
    ...
    return {
      ...
      rfc822MessageId: messageId || null,

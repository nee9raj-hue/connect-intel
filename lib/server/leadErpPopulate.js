...
import { getLatestWorkspaceImport } from './orgWorkspaceImport.js'
...
function indexForOrg(store, orgId) {
  ...
  const built = {
    importId,
    builtAt: Date.now(),
    ...buildWorkspaceShipmentIndex(latest?.rows || []),
  }
...
function codesFromLeadText(entry) {
  ...
  const trading = entry?.tradingProfile || {}
  const codes = new Set()
  const add = (value) => {
...
export function populateLeadErp(store, user, entry) {
  ...
  return buildErpFromFacts({
    stored,
    trading: entry?.tradingProfile || null,
    shipments,

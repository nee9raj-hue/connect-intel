...
function mapRowToFlatten(row, { freightOrg = false } = {}) {
  ...
  const deal = freightOrg ? { ...rawDeal, stage: combinedStage } : rawDeal
...
export async function listPipelineDealsPage(
  ...
  const rows = await supabaseRest(
    `${TABLE}?${baseQuery}` +
      ...
      `&order=updated_at.desc&limit=8000`,
    {},
    { timeoutMs: 30_000 }
  )
  ...
  deals = deals.slice(off, off + lim).map(({ _stageNorm, stage, ...rest }) => rest)
...
export async function countScopedPipelineDealsByStage(
  ...
  try {
    rows = await supabaseRest(
      `${TABLE}?${parts.join('&')}&select=stage,payload&limit=8000`,
      {},

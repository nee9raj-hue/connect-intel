...
import { suggestCrmStatusFromTradingProfile } from '../crmLeadStatuses.js'
...
import { mergeLeadErp, buildErpFromFacts } from '../leadErp.js'
...
function buildPhoneIndex(store, user) {
  const index = new Map()
...
export function importActiveTradingRows(
  ...
  const phoneIndex = buildPhoneIndex(store, actor)
  ...
  for (const rawRow of rows) {
    ...
    for (const entry of entries) {
      ...
      const erpOverlay = buildErpFromFacts({
        stored: entry.erp || entry.lead?.erp,
        trading: entry.tradingProfile,
        deals: entry.crm?.deals || [],
      })
      entry.erp = mergeLeadErp(entry.erp, erpOverlay)
      if (entry.lead) entry.lead.erp = mergeLeadErp(entry.lead.erp, erpOverlay)

      if (promoteToActive && parsed.firstShipmentAt) {
        const crm = normalizeExtendedCrm(entry.crm || defaultCrm())
        const suggested = suggestCrmStatusFromTradingProfile(entry.tradingProfile, crm.status)
        if (suggested) {
          crm.status = suggested
          entry.crm = crm
        } else if (crm.status !== 'lost') {
          crm.status = 'active_trading'
          entry.crm = crm
        }
        entry.crm = appendActivity(entry.crm, {
          type: 'note',
          summary: `Trading profile updated: first shipment ${parsed.firstShipmentAt.slice(0, 10)}`,
          userId: actor?.id,

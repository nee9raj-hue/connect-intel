...
export const PERMISSION_ACTIONS = [
  ...
  { id: 'delete_leads', label: 'Delete leads & deals' },
...
export async function getRolePermissionMatrix(legacyOrgId) {
  ...
  for (const role of PERMISSION_ROLES) {
    if (role !== 'admin' && matrix[role]) matrix[role].delete_leads = false
  }
...
export async function setRolePermission(legacyOrgId, { role, action, allowed }) {
  ...
  if (action === 'delete_leads' && role !== 'admin') {
    throw new Error('Only Org Admin can delete leads and deals')
  }

import { normalizeDealCustomerTypeFilter, normalizePipelineTrack } from '../crmPipelineFlow.js'
...
export function parsePipelineQueryParams(url) {
  ...
  const dealCustomerType = normalizeDealCustomerTypeFilter(
    url.searchParams.get('dealCustomerType') || url.searchParams.get('customerType') || ''
  )
  return {
    status,
    q,
    ...
    dealCustomerType: dealCustomerType || undefined,
  }
}
...
export function serializePipelineFilters(filters = {}) {
  ...
  const dealCustomerType = String(filters.dealCustomerType || '').trim()
  if (dealCustomerType) params.set('dealCustomerType', dealCustomerType)
  return params
}

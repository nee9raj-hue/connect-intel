...
import { getMarketingCampaign, enrollCampaign } from './marketingCampaigns.js'
...
export async function processDueAutomationRuns({ limit = MAX_RUNS_PER_CRON } = {}) {
  ...
  for (const run of due) {
    ...
    try {
      const result = await executeAutomationAction(store, automation, run)
...
async function executeAutomationAction(store, automation, run) {
  const campaignId = automation.campaignId
  if (!campaignId) return { sent: 0 }
  ...
  if (automation.listId) {
    list = getMarketingList(store, owner, automation.listId)
  } else if (automation.segmentId) {
    const segment = getMarketingSegment(store, owner, automation.segmentId)
    const leadIds = await resolveSegmentLeadIds(store, owner, segment)
    list = { id: `seg_${segment.id}`, leadIds, channel: segment?.channel || 'email' }
  }

  if (!list?.leadIds?.length) {
    list = { id: 'auto_single', leadIds: [run.leadId], channel: campaign.channel || 'email' }
  } else if (!list.leadIds.includes(run.leadId)) {
    list = { ...list, leadIds: [run.leadId] }
  }

  const { pipelineStore } = await loadPipelineStoreForLeadIds(owner, list.leadIds)
  const fullStore = { ...store, savedLeads: pipelineStore.savedLeads }
  const enrolled = await enrollCampaign(fullStore, owner, campaign, list)
  await writeCampaignSendShard(fullStore, owner, { ...campaign, status: 'active' })

import { CRM_STATUSES, foldCrmStatusCounts, isCrmLeadStatusFilter } from './crm.js'
import { crmLeadStatusPostgrestIn, crmStatusSqlAliases } from '../crmLeadStatuses.js'
import { isSupabaseEnabled, supabaseRest } from './supabaseClient.js'
import { isPipelineHierarchyRbacEnabled } from './infra/config.js'
import { filtersNeedExactPostgrestCount, filtersUseDenormalizedSql, filtersUseEntryLocationFilter } from './pipelineFilterSql.js'
...
function scopeNeedsPostgrestCount(scoped) {
  return Boolean(
    scoped.scope?.repOwnAndUnassigned ||
      ...
      scoped.rpc?.p_owner_id == null && scoped.role && scoped.role !== 'admin'
  )
}
...
async function countScopedLeadsViaPostgrest(scoped, status) {
  ...
  if (status) {
    ...
    if (inn) parts.push(`lead_status=${inn}`)
  }
...
export async function loadScopedPipelineStatusCounts(user, metaStore, filters = {}) {
  ...
  const byStatus = foldCrmStatusCounts(rows.map((r) => ({ status: r.status, count: Number(r.cnt) || 0 })))
  ...
  return {
    ...
    console.warn('pipeline status counts:', error?.message || error)
    return null
  }
}
...
async function readAssigneeIndexCount(shardName, assignee, status) {
  ...
  if (status && status !== 'all' && isCrmLeadStatusFilter(status)) {
    const aliases = new Set(crmStatusSqlAliases(status))
    return (bucket.byStatus || []).reduce((sum, row) => {
      if (!aliases.has(row.status) && !aliases.has(String(row.status || '').toLowerCase())) return sum
...
export async function countScopedPipelineLeads(user, metaStore, filters = {}) {
  ...
  try {
    if (filtersUseEntryLocationFilter(filters)) {
      const { countEntryLocationFilteredLeads } = await import('./pipelineListLoad.js')
      return countEntryLocationFilteredLeads(user, metaStore, filters)
    }
    ...
    const statusAliases = status ? crmStatusSqlAliases(status) : []
    const needsPostgrestCount =
      scopeNeedsPostgrestCount(scoped) ||
      (assignee && assignee !== '__unassigned__') ||
      hasFollowUpFilter ||
      filtersNeedExactPostgrestCount(filters) ||
      statusAliases.length > 1

    if (needsPostgrestCount) {
      const postgrestCount = await countScopedLeadsViaPostgrest(scoped, statusAliases.length ? status : null)
      if (postgrestCount != null) return postgrestCount
    }

    if (assignee && assignee !== '__unassigned__' && !filtersUseDenormalizedSql(filters) && !isCompany) {
    ...
    if (!assignee || assignee === '__unassigned__' || sqlCount > 0 || isCompany) return sqlCount

    const fallback = await readAssigneeIndexCount(shardName, assignee, status)
    return fallback ?? 0
  } catch (error) {
    console.warn('pipeline lead count:', error?.message || error)
    return null
  }
}

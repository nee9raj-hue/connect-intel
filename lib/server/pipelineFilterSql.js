...
export function appendPipelineFilterSqlParts(parts, filters = {}) {
  ...
  return appendPipelineTagSqlParts(appendLastShipmentSqlParts(next, filters), filters)
}
...
export function filtersNeedExactPostgrestCount(filters = {}) {
  return (
    ...
    (Array.isArray(filters.teamIds) && filters.teamIds.length > 0) ||
    Boolean(String(filters.teamId || '').trim())
  )
}

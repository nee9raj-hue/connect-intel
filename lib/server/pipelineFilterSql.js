...
import { erpStageStatusAliases, collectCrmStageFilterIds } from '../crmPipelineFlow.js'
import { mergeErpExclusiveSqlParts } from '../pipelineCrmErpUnion.js'
import { notesPresenceSqlParts, normalizeNotesPresence } from '../pipelineColumnFilters.js'
...
export function appendPipelineFilterSqlParts(parts, filters = {}) {
  ...
  const erpExclusive = appendPipelineErpTagSqlParts(appendLastShipmentSqlParts([], filters), filters)
  next.push(...mergeErpExclusiveSqlParts(filters, erpExclusive))
  next.push(...notesPresenceSqlParts(filters))

  return appendPipelineTeamSqlParts(appendPipelineTagSqlParts(next, filters), filters)
}
...
function erpTagOrStageParts(name) {
  const parts = erpTagContainsParts(name)
  const aliases = erpStageStatusAliases(name)
  if (aliases.length) {
    const inn = aliases.join(',')
    parts.push(`entry->crm->>status.in.(${inn})`)
    parts.push(`lead_status.in.(${inn})`)
  }
  return parts
}
...
export function appendPipelineErpTagSqlParts(parts, filters = {}) {
  ...
  if (modeAll) {
    next.push(`and=(${names.map((name) => `or(${erpTagOrStageParts(name).join(',')})`).join(',')})`)
    return next
  }

  const anyParts = names.flatMap((name) => erpTagOrStageParts(name))
...
export function filtersNeedExactPostgrestCount(filters = {}) {
  return (
    ...
    Boolean(normalizeNotesPresence(filters)) ||
    collectCrmStageFilterIds(filters).length > 0 ||

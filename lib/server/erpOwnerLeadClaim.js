...
import {
  ...
  canonicalErpOwner,
  erpOwnerIdentityPostgrestParts,
...
function claimOrFilter(user, stubUserIds = []) {
  const orParts = [...erpOwnerIdentityPostgrestParts(user)]
...
async function loadClaimCandidateRows(organizationId, orFilter) {
  ...
  try {
    for (;;) {
      ...
      const page = await supabaseRest(path, {}, { timeoutMs: 12_000 })
...
/** Admin/manager opening a teammate's Owner filter — claim that member's ERP book. */
export async function claimErpLeadsForFilteredMember(viewer, assigneeUserId) {
  const id = String(assigneeUserId || '').trim()
  if (!id || id === '__unassigned__' || !viewer?.organizationId) return null
  if (String(viewer.id) === id) return claimErpLeadsForMemberOnce(viewer)

  let metaStore = await readStore({ only: META })
  metaStore = await patchStoreWithFreshOrgRoster(metaStore)
  const { resolveOrgRole, getMembership } = await import('./organizations.js')
  const { orgRole } = resolveOrgRole(viewer, metaStore)
  const member = (metaStore.users || []).find((row) => String(row.id) === id)
  if (!member || !getMembership(metaStore, member.id, viewer.organizationId)) return null

  if (orgRole !== 'org_admin') {
    const { resolveManagerVisibleOwnerIds } = await import('./pipelineManagerScope.js')
    const visible = (await resolveManagerVisibleOwnerIds(viewer, metaStore)) || []
    if (!visible.map(String).includes(id)) return null
  }

  return claimErpLeadsForMemberOnce({
    ...member,

...
function stubUserIdsForClaim(store, organizationId, claimUser) {
  ...
  const email = String(claimUser.email || '')
    .trim()
    .toLowerCase()
  const first = ownerFirstNameKey(claimUser.name)
  const ids = []
  for (const user of store?.users || []) {
    if (String(user.id) === String(claimUser.id)) continue
    const membership = (store.organizationMemberships || []).find(
      (m) => m.userId === user.id && m.organizationId === organizationId && m.status !== 'inactive'
    )
    if (!membership) continue
    const otherEmail = String(user.email || '')
      .trim()
      .toLowerCase()
    const sameEmail = email && otherEmail && email === otherEmail
    const stubTwin =
      user.source === 'erp-owner' && first && ownerFirstNameKey(user.name) === first
    if (sameEmail || stubTwin) ids.push(user.id)
  }
  return ids
}

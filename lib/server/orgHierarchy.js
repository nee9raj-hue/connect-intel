...
const HIERARCHY_SQL_READ = { timeoutMs: 8_000, attempts: 1, bypassCircuit: true }
...
async function requireOrgUuid(legacyOrgId, store) {
  ...
  if (!orgUuid) {
    throw new Error('Team structure is still syncing. Refresh in a moment.')
  }
  return orgUuid
}

export async function listOrgHierarchy(legacyOrgId, { skipLeadCounts = true } = {}) {

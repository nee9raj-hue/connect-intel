...
import { resolveOrganizationUuid, resolveProfileUuid } from './orgSqlResolve.js'
import { syncOrgMembershipToSql } from './orgSqlSync.js'
import { AUTH_STORE_COLLECTIONS, readStore } from './store.js'
import { looksLikeUuid, resolvePipelineOrgIdentity } from './pipelineOrgIdentity.js'
...
async function resolveManagerProfileId(managerLegacyUserId) {
  if (!managerLegacyUserId) return null
  const store = await readStore({ only: AUTH_STORE_COLLECTIONS })
  const profileId = await resolveProfileUuid(managerLegacyUserId, orgCache(), {
    store,
    autoSync: true,
  })
  if (!profileId) {
    throw new Error('That teammate is not synced to Teams yet. Ask them to sign in once, then try again.')
  }
  return profileId
}

async function requireOrgUuid(legacyOrgId, store) {
  if (!legacyOrgId) throw new Error('Organization required')
  if (looksLikeUuid(legacyOrgId)) return legacyOrgId
  try {
    const identity = await resolvePipelineOrgIdentity(legacyOrgId)
    const uuid = identity.ids.find((id) => looksLikeUuid(id))
    if (uuid) return uuid
  } catch (error) {
    console.warn('org uuid identity:', error?.message || error)
  }
  const loaded = store || null
  const orgUuid = await resolveOrganizationUuid(legacyOrgId, orgCache(), {
    store: loaded,
    autoSync: Boolean(loaded),
  })
  if (!orgUuid) {
    throw new Error('Organization not synced to SQL — run npm run org:sql-backfill')
  }
  return orgUuid
}
...
export async function createTeam(legacyOrgId, { departmentId, name, managerLegacyUserId }) {
  ...
  if (managerLegacyUserId) {
    managerProfileId = await resolveManagerProfileId(managerLegacyUserId)
  }
...
export async function updateTeam(legacyOrgId, teamId, patch = {}) {
  ...
  if (patch.managerLegacyUserId !== undefined) {
    ...
    body.manager_profile_id = patch.managerLegacyUserId
      ? await resolveManagerProfileId(patch.managerLegacyUserId)
      : null
  }
...
export async function ensureMemberProfile(legacyOrgId, userId) {
  ...
  if (!sync.profileUuid) {
    throw new Error('Member profile missing in SQL. Ask your admin to run npm run profiles:sql-backfill')
  }
  return { id: sync.profileUuid }
}

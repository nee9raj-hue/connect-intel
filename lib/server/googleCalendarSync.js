...
/** Best-effort Google Calendar sync after CRM save — do not block the HTTP response. */
export async function syncCrmScheduleToGoogleAfterSave({
  user,
  storeUser,
  leadId,
  leadForGoogle,
  lastAddedMeeting = null,
  lastAddedTask = null,
  completedTaskForGoogle = null,
}) {
  const { updatePipelineStore } = await import('./pipelineShard.js')
  const { findPipelineEntry } = await import('./pipelineAccess.js')
  const { normalizeExtendedCrm } = await import('./crmWorkflow.js')
  const { repairPipelineEntryCrm } = await import('./tenantIsolation.js')

  if (lastAddedMeeting) {
    const googleCalendar = await pushCrmMeetingToGoogle(storeUser, lastAddedMeeting, leadForGoogle)
    if (googleCalendar?.pushed && googleCalendar.googleEventId) {
      await updatePipelineStore(user, async (draft) => {
        const row = findPipelineEntry(draft, user, leadId)
        if (!row) return draft
        const crm = normalizeExtendedCrm(row.crm)
        const meeting = crm.meetings.find((m) => m.id === lastAddedMeeting.id)
        if (meeting) {
          meeting.googleEventId = googleCalendar.googleEventId
          meeting.googleHtmlLink = googleCalendar.htmlLink || null
        }
        row.crm = crm
        repairPipelineEntryCrm(draft, user, row)
        return draft
      })
    }
  }
  ...
  if (lastAddedTask) {
    const taskGoogle = await pushCrmTaskToGoogle(storeUser, lastAddedTask, leadForGoogle)
    if (taskGoogle?.pushed && taskGoogle.googleEventId) {
      await updatePipelineStore(user, async (draft) => {
        const row = findPipelineEntry(draft, user, leadId)
        if (!row) return draft
        const crm = normalizeExtendedCrm(row.crm)
        const task = crm.tasks.find((t) => t.id === lastAddedTask.id)
        if (task) {
          task.googleEventId = taskGoogle.googleEventId
          task.googleHtmlLink = taskGoogle.htmlLink || null
        }
        row.crm = crm
        repairPipelineEntryCrm(draft, user, row)
        return draft
  ...
  if (completedTaskForGoogle?.googleEventId) {
    await removeCrmItemFromGoogle(storeUser, completedTaskForGoogle.googleEventId)
  }
}

/** Push one saved CRM meeting after the rep grants Google Calendar access. */
export async function pushSavedCrmMeetingToGoogle(sessionUser, { leadId, meetingId } = {}) {
  if (!leadId || !meetingId) return { pushed: false, error: 'leadId and meetingId are required' }

  const { readStore } = await import('./store.js')
  const { updatePipelineStore } = await import('./pipelineShard.js')
  const { loadPipelineLeadForMutation } = await import('./pipelineShard.js')
  const { findPipelineEntry } = await import('./pipelineAccess.js')
  const { normalizeExtendedCrm } = await import('./crmWorkflow.js')
  const { repairPipelineEntryCrm } = await import('./tenantIsolation.js')

  const store = await readStore({ only: ['users'] })
  const dbUser = store.users.find((u) => u.id === sessionUser.id) || sessionUser
  const loaded = await loadPipelineLeadForMutation(sessionUser, leadId)
  if (!loaded?.entry) return { pushed: false, error: 'Lead not found' }

  const meeting = normalizeExtendedCrm(loaded.entry.crm).meetings.find((m) => m.id === meetingId)
  if (!meeting) return { pushed: false, error: 'Meeting not found' }

  const result = await pushCrmMeetingToGoogle(dbUser, meeting, loaded.entry.lead || loaded.entry)
  if (result?.pushed && result.googleEventId) {
    await updatePipelineStore(sessionUser, async (draft) => {
      const row = findPipelineEntry(draft, sessionUser, leadId)
      if (!row) return draft
      const crm = normalizeExtendedCrm(row.crm)
      const hit = crm.meetings.find((m) => m.id === meetingId)
      if (hit) {
        hit.googleEventId = result.googleEventId
        hit.googleHtmlLink = result.htmlLink || null
      }
      row.crm = crm
      repairPipelineEntryCrm(draft, sessionUser, row)
      return draft
    })
  }
  return result

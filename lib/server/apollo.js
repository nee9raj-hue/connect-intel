...
/** Find people with LinkedIn URLs via Apollo keyword search (contact enrichment). */
export async function searchApolloLinkedinProfiles(contact = {}) {
  ...
  const params = new URLSearchParams()
  params.set('q_keywords', [name, company, title].filter(Boolean).join(' '))
  ...
  try {
    ...
    for (const person of people) {
      ...
      if (!linkedin || !/linkedin\.com\/in\//i.test(linkedin)) continue
      const org = person.organization || {}
      matches.push({
        id: `apollo-${person.id}`,
        linkedin,
        firstName: person.first_name || firstName,
        lastName: person.last_name || person.last_name_obfuscated || lastName,
        fullName: [person.first_name, person.last_name].filter(Boolean).join(' ') || name,
        ...
        phone: '',
        confidence: 'high',
        reason: 'Matched via Apollo people database',
      })
    }

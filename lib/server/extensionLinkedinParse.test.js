...
describe('extension LinkedIn parse helpers', () => {
  ...
  it('parses employer and location from LinkedIn JSON-LD', () => {
    const json = JSON.stringify({
      '@graph': [
        {
          '@type': 'Person',
          name: 'Vasanth Tamilselvan',
          jobTitle: ['Founder'],
          worksFor: [{ '@type': 'Organization', name: 'Ariro Toys' }],
          address: {
            '@type': 'PostalAddress',
    ...
    const person = parseLinkedInJsonLd(json)
    assert.equal(person.name, 'Vasanth Tamilselvan')
    assert.equal(person.company, 'Ariro Toys')
    assert.equal(person.title, 'Founder')
    assert.equal(person.city, 'Chennai')
    assert.equal(person.state, 'Tamil Nadu')
  })

  it('JSON-LD company wins over a followed/sidebar company', () => {
    const company = pickCompanyName({
      jsonLdCompany: 'Ariro Toys',
      experienceCompany: 'Skillmatics',
    })
    assert.equal(company, 'Ariro Toys')
  })

  it('returns null for JSON-LD without a Person node', () => {
    assert.equal(parseLinkedInJsonLd('{"@type":"WebPage"}'), null)
    assert.equal(parseLinkedInJsonLd('not json'), null)
  })
})

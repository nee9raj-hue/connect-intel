...
function normalizeOriginClearance(raw) {
  const base = emptyOriginClearance()
  if (!raw || typeof raw !== 'object') return base
  return {
    broker: String(raw.broker || '').trim().slice(0, 200),
    chargesInr: numOrNull(raw.chargesInr),
    notes: String(raw.notes || '').slice(0, 1000),
  }
}

function normalizeDestinationClearance(raw) {
  const base = emptyDestinationClearance()
  if (!raw || typeof raw !== 'object') return base
  return {
...
function clearanceHasData(clearance, { requireApplicable = false } = {}) {
  if (!clearance) return false
  if (requireApplicable && !clearance.applicable) return false
  return Boolean(clearance.broker || clearance.chargesInr != null || clearance.notes)
}
...
export function normalizeFreightRfq(raw) {
  ...
  const out = {
    ...
    boxes,
    volumetricDivisor: normalizeVolumetricDivisor(raw.volumetricDivisor),
    originClearance: normalizeOriginClearance(raw.originClearance),
    destinationClearance: normalizeDestinationClearance(raw.destinationClearance),
  ...
  const hasData =
    ...
    clearanceHasData(out.originClearance) ||
    clearanceHasData(out.destinationClearance, { requireApplicable: true }) ||
...
export function mergeFreightRfq(existing, patch) {
  ...
  if (patch.originClearance && typeof patch.originClearance === 'object') {
    merged.originClearance = { ...(existing?.originClearance || emptyOriginClearance()), ...patch.originClearance }
  }
  if (patch.destinationClearance && typeof patch.destinationClearance === 'object') {
    merged.destinationClearance = {
      ...(existing?.destinationClearance || emptyDestinationClearance()),
      ...patch.destinationClearance,
    }
  }

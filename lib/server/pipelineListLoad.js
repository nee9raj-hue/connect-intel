...
export async function loadScopedLeadsListView(
  ...
  const buildFromRows = async (sqlRows) => {
    ...
    const pageEntries = filterPipelineEntries(visible, {
      ...filters,
      skipStatusFilter: Boolean(scoped.status && scoped.status !== 'all'),
    })
...
export async function loadPipelineListPage(
  ...
  const pageEntries = filterPipelineEntries(visible, {
    ...filters,
    skipStatusFilter: statusFilter !== 'all' && CRM_STATUSES.includes(statusFilter),
  })
  const { leads } = listPipelinePage(pipelineStore, user, {
    light,
    limit: lim,
...
export async function loadPipelineBoardView(
  ...
  await Promise.all(
    statusesToLoad.map(async (status) => {
      ...
      const filtered = filterPipelineEntries(visible, {
        ...filters,
        status,

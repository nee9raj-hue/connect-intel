...
const PIPELINE_SQL_READ = { timeoutMs: 12_000, attempts: 1, bypassCircuit: true }
...
export async function countEntryLocationFilteredLeads(user, metaStore, filters = {}) {
  ...
  while (attempts < maxAttempts) {
    ...
    const rows = await supabaseRest(scopedLeadsListUrl(scoped), {}, PIPELINE_SQL_READ)
...
export async function loadScopedLeadsListView(
  ...
  const [rows, indexTotal] = await Promise.all([
    supabaseRest(scopedLeadsListUrl(scoped), {}, PIPELINE_SQL_READ).catch((err) => {
  ...
  if (!leads.length && (total == null || total > 0)) {
    ...
    if (healed) {
      const retryRows = await supabaseRest(scopedLeadsListUrl(scoped), {}, PIPELINE_SQL_READ)

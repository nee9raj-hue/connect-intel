import { CRM_STATUSES, isCrmLeadStatusFilter, normalizeCrmLeadStatus } from './crm.js'
...
import { filtersUseDenormalizedSql, filtersUseEntryLocationFilter, filtersUseTagSql, stripLocationSqlFilters } from './pipelineFilterSql.js'
...
export async function resolveListTotalFromIndex(user, metaStore, shardName, filters = {}) {
  ...
  if (filtersUseDenormalizedSql(filters)) {
...
async function classifyAndPersistErpStages(entries, shardName) {
  try {
    const { persistErpAccountStagesForEntries } = await import('./erpAccountStageApply.js')
    await persistErpAccountStagesForEntries(entries, { shardName })
  } catch (err) {
    console.warn('erp stage classify on pipeline list:', err?.message || err)
  }
}
...
export async function countEntryLocationFilteredLeads(user, metaStore, filters = {}) {
  ...
  while (attempts < maxAttempts) {
    ...
    const rows = await supabaseRest(scopedLeadsListUrl(scoped), {}, { timeoutMs: 8_000, attempts: 2 })
...
export async function loadScopedLeadsListView(
  ...
  if (filtersUseTagSql(filters) || filtersUseDenormalizedSql(filters)) {
  ...
  if (!leads.length && (total == null || total > 0)) {
    ...
    if (healed) {
      const retryRows = await supabaseRest(scopedLeadsListUrl(scoped), {}, { timeoutMs: 8_000, attempts: 2 })
...
export async function loadPipelineBoardView(
  ...
  const statusesToLoad =
    statusFilter !== 'all' && isCrmLeadStatusFilter(statusFilter)
      ? [normalizeCrmLeadStatus(statusFilter)]
      : CRM_STATUSES
  ...
  await Promise.all(
    statusesToLoad.map(async (status) => {
      ...
      const filtered = filterPipelineEntries(visible, { ...filters, status })
  ...
  for (const entry of loaded) {
    const nextStatus = normalizeCrmLeadStatus(entry?.crm?.status)
...
export async function loadPipelineDealsPage(
  ...
  if (
    pipelineDealsTableActive() &&
    user?.organizationId &&
    (await orgHasPipelineDeals(user.organizationId))
  ) {

...
import { resolvePipelineTableScope, visiblePipelineFromEntries } from './pipelineTableScope.js'
import { readStore } from './store.js'

export { resolvePipelineTableScope } from './pipelineTableScope.js'
...
/**
 * Paginated pipeline list — table page + index totals when possible; table/shard full walk when filters need a scan.
 */
...
/**
 * Deals view — paginated flatten from table-backed visible entries (no shard blob).
 */
...
export async function loadPipelineSummaryWithDeals(user, { freightOrg = false } = {}) {
  ...
  const doc = await readPipelineIndexDoc(shardName)
  if (doc?.cities) payload.cities = doc.cities
  if (doc?.states) payload.states = doc.states

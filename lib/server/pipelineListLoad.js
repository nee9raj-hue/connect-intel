...
export async function loadScopedLeadsListView(
  ...
  const scoped = await getScopedLeadsQuery(user, { ...filters, offset: off, limit: lim }, metaStore)
  ...
  return {
    ...
    hasMore: scoped.pagination.offset + leads.length < total,
    pipelineTotal: total,
    fromTable: true,
    fromSqlScope: true,
...
export async function loadPipelineListPage(user, { offset = 0, limit = 50, filters = {}, light = true } = {}) {
  const scopedView = await loadScopedLeadsListView(user, { offset, limit, filters, light })

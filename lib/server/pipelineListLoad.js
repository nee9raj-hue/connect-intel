...
import { filterPipelineEntries, pipelineEntryMatchesSearch } from './pipelineQuery.js'
import {
...
async function loadPipelineListFromLeadIds(
  ...
  const filteredBase = filterPipelineEntries(visible, { ...filters, q: '' })
  const query = String(filters.q || '').trim()
  const tokens = query.split(/\s+/).filter((token) => token.length >= 2)
  const precise = query
    ? filteredBase.filter((entry) => {
        if (pipelineEntryMatchesSearch(entry, query)) return true
        return tokens.length > 1 && tokens.every((token) => pipelineEntryMatchesSearch(entry, token))
      })
    : filteredBase
...
/** SQL table search first (source of truth). Meili/platform only if the table path is unavailable. */
async function loadPipelineTextSearchView(
  user,
  { offset = 0, limit = 50, filters = {}, light = true } = {}
) {
  if (String(filters.q || '').trim().length < 2) return null
  ...
  const meiliView = await loadPipelineListFromMeili(user, {
    ...
    console.warn('pipeline meili list:', err?.message || err)
    return null
  })
  if (meiliView) return meiliView

  return loadPipelineListFromPlatformSearch(user, { offset, limit, filters, light }).catch((err) => {
    console.warn('pipeline platform search:', err?.message || err)
    return null
  })
}

/** Shard / platform search when pipeline_leads SQL search is unavailable. */
...
async function loadPipelineListFromMeili(user, { offset = 0, limit = 50, filters = {}, light = true } = {}) {
  ...
  if (!Array.isArray(leadIds) || leadIds.length === 0) return null
...
export async function loadPipelineListPage(
  ...
  const searchQ = String(incomingFilters.q || '').trim()
  if (searchQ.length >= 2) {
    const searchView = await loadPipelineTextSearchView(user, {
      offset: off,
      limit: lim,
      filters: incomingFilters,
      light,
    ...
    if (!isCompanyPipeline(user)) {
      return loadPipelineListFromShard(user, {
        offset: off,
        limit: lim,
        filters: incomingFilters,
        light,
    ...
    throw new Error('Pipeline search temporarily unavailable')
  }

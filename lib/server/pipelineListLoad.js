...
import { filtersUseTagSql } from './pipelineFilterSql.js'
...
export async function loadScopedLeadsListView(
  ...
  if (filtersUseTagSql(filters)) {
    const tagTotal = await countScopedPipelineLeads(user, metaStore, filters)
    if (tagTotal != null) total = tagTotal
  }
  ...
  const buildFromRows = async (sqlRows) => {
    ...
    const visible = listPipelineSavedEntries(pipelineStore, user, {
      visibleOwnerIds,
      unassignedPool: assignee === '__unassigned__',
    })

...
function sanitizeDealServerFilters(filters) {
  ...
  return {
    ...
    year: f.year ? String(f.year).slice(0, 4) : null,
    month: f.month ? String(f.month).slice(0, 2) : null,
    weeks: Array.isArray(f.weeks)
      ? f.weeks.map((w) => String(w).replace(/^week/i, '').trim()).filter(Boolean).slice(0, 12)
      : String(f.weeks || '')
          .split(/[,\s]+/)
          .map((w) => w.replace(/^week/i, '').trim())
          .filter(Boolean)
          .slice(0, 12),
  }
}

import { createId, readStore, updateStore, updateStorePartial, INVITE_STORE_COLLECTIONS } from './store.js'
...
function memberCanSeePipelineEntry(entry, user, { orgRole, visibleOwnerIds, unassignedPool = false }) {
  if (!entry) return false
  if (unassignedPool) return isPipelineLeadUnassigned(entry)
  const owner = pipelineOwnerUserId(entry)
  if (orgRole === 'org_admin') return !isPipelineLeadUnassigned(entry) || unassignedPool
  if (isPipelineLeadUnassigned(entry)) return false
...
export function listPipelineSavedEntries(store, user, options = {}) {
  ...
  const unassignedPool = Boolean(options.unassignedPool)
  ...
  let entries
  if (unassignedPool) {
    entries = savedLeads.filter((e) => inOrg(e) && isPipelineLeadUnassigned(e))
    ...
    entries = savedLeads.filter(
      (e) => inOrg(e) && !isPipelineLeadUnassigned(e) && repPipelineEntryVisible(e, user.id)
    )
  }

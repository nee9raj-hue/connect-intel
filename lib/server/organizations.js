...
import { DEFAULT_TRIAL_CREDITS_PAISE } from './config.js'
import { mergeLeadForClient, defaultCrm, normalizeCrm } from './crm.js'
import { buildInviteUrl, createInviteToken, sendTeamInviteEmail } from './email.js'
...
export function buildOrgUserResponse(user, store) {
  ...
  return {
    ...
    accountType,
    orgRole,
    isOrgAdmin: orgRole === 'org_admin' || isPlatformAdmin,
    canSearch: accountType === 'individual' || orgRole === 'org_admin' || Boolean(membership?.canSearch),
    organizationName: org?.name || user.company,
    organizationLogoUrl: org?.logoUrl || null,
    pipelineRole: membership?.pipelineRole || (orgRole === 'org_admin' ? 'org_admin' : 'member'),
    onboardingComplete: Boolean(user.onboardingComplete ?? org?.onboardingComplete),
    assignedLeadCount:
...
export function listPipelineEntries(store, user) {
  const { orgRole, accountType } = resolveOrgRole(user, store)
  let entries = []

  if (accountType === 'individual' || !user.organizationId) {
    entries = store.savedLeads.filter((e) => e.userId === user.id)
  } else if (orgRole === 'org_admin') {
    entries = store.savedLeads.filter((e) => e.organizationId === user.organizationId)
  } else {
    entries = store.savedLeads.filter(
      (e) =>
        e.organizationId === user.organizationId &&
        (e.assignedToUserId === user.id || e.savedByUserId === user.id)
    )
  }
...
export function getPipelineLeadIds(store, user) {
  return new Set(listPipelineEntries(store, user).map((e) => e.id))
}
  ...
  if (invite?.token) {
    ...
    try {
      emailResult = await sendTeamInviteEmail({
        ...
        inviteUrl,
        roleLabel: roleLabel(invite.pipelineRole),
      })
    } catch (error) {
      emailResult = { sent: false, error: error.message }
    }

    return { store, inviteUrl, email: emailResult }
  }

  return { store, inviteUrl: null, email: emailResult, joinedImmediately: true }
}
...
export async function updateMemberPermissions(organizationId, memberUserId, { canSearch, pipelineRole }) {
  return updateStore((store) => {

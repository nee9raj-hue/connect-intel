...
export function buildOrgUserResponse(user, store) {
  ...
  const base = {
    ...
    crmFreeMode: true,
    plan: CRM_SOLO_FREE_TIER ? 'free' : publicUser.plan || 'free',
    subscriptionPlan: CRM_SOLO_FREE_TIER ? 'Free' : publicUser.subscriptionPlan || publicUser.plan || 'free',
    ...
    seatLimit: CRM_SOLO_FREE_TIER ? 25 : (user.seatLimit ?? org?.seatLimit ?? 20),
...
export async function completeOnboarding(userId, payload) {
  return updateStore((store) => {
    ...
    if (accountType === 'company') {
      ...
      if (domain) {
        const existingOrg = store.organizations.find(
          (o) =>

...
/** Admin, assignee, or any teammate claiming an unassigned lead (self-assign). */
export function canAssignLead(user, entry, assignToUserId = undefined) {
  ...
  if (assignToUserId !== undefined && String(assignToUserId) !== String(user.id)) {
    return false
  }
...
function entryOwnerUserId(entry) {
  return String(entry.assignedToUserId || entry.savedByUserId || entry.userId || '')
}

function isUnassignedPipelineEntry(entry) {
...
export function listPipelineSavedEntries(store, user, options = {}) {
  ...
  if (orgRole === 'org_admin') {
    ...
    entries = savedLeads.filter(
      (e) =>
        inOrg(e) && (isUnassignedPipelineEntry(e) || allowed.has(entryOwnerUserId(e)))
    )
  } else {
    entries = savedLeads.filter(
      (e) =>
        inOrg(e) && (isUnassignedPipelineEntry(e) || e.assignedToUserId === user.id)
    )
  }

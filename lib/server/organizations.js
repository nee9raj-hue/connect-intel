...
import { buildInviteUrl, getAppBaseUrl, sendTeamInviteEmail } from './email.js'
...
export function resolveOrgRole(user, store) {
  if (!user?.organizationId) {
    return { orgRole: 'individual', accountType: user?.accountType || 'individual' }
  }
  const org = getOrganization(store, user.organizationId)
  const membership = getMembership(store, user.id, user.organizationId)
  const accountType = org?.accountType || user.accountType || 'individual'
  let orgRole = membership?.role || user.orgRole || 'member'
  if (orgRole === 'admin') orgRole = 'org_admin'
  if (user.isOrgAdmin || user.orgRole === 'org_admin') orgRole = 'org_admin'
...
export function buildOrgUserResponse(user, store) {
  ...
  const base = {
    ...
    pipelineActorIds: listPipelineActorIds(store, user.organizationId, user),
  }

  return base
}

function memberCanSeePipelineEntry(entry, user, { orgRole, visibleOwnerIds, actorIds }) {
  if (!entry) return false
  if (orgRole === 'org_admin') return true
  const extra = [...new Set([...(visibleOwnerIds || []), ...(actorIds || []), user.id].map(String).filter(Boolean))]
  return repPipelineEntryVisible(entry, user.id, extra)
}
...
export function listPipelineSavedEntries(store, user, options = {}) {
  ...
  const visibleOwnerIds = [
    ...new Set([...(options.visibleOwnerIds || []), ...actorIds].map(String).filter(Boolean)),
  ]
  ...
  const orgIds = new Set(collectPipelineOrganizationIds(user, store))
  const inOrg = (e) =>
    options.trustedOrgScope || pipelineEntryInOrganization(e, orgIds)

  let entries
  if (orgRole === 'org_admin' || options.skipOwnerFilter) {
    entries = savedLeads.filter(inOrg)
  } else if (visibleOwnerIds.length) {
    entries = savedLeads.filter(
      (e) =>
        inOrg(e) &&
        memberCanSeePipelineEntry(e, user, { orgRole, visibleOwnerIds, actorIds })
    )
  } else {
    entries = savedLeads.filter((e) => inOrg(e) && repPipelineEntryVisible(e, user.id, actorIds))
  }
...
export function listCalendarPipelineEntries(store, user) {
  ...
  const orgIds = new Set(collectPipelineOrganizationIds(user, store))
  if (orgRole === 'org_admin') {
    return savedLeads.filter((e) => pipelineEntryInOrganization(e, orgIds))
  }
  return savedLeads.filter(
    (e) =>
      pipelineEntryInOrganization(e, orgIds) &&
...
export async function inviteTeamMember(organizationId, inviterUserId, options = {}) {
  ...
  const store = await updateStorePartial(INVITE_STORE_COLLECTIONS, (draft) => {
    ...
    const existingUser = draft.users.find(
    ...
    if (existingUser) {
      ...
      if (!already) {
        ...
        if (name && (!existingUser.name || existingUser.name === existingUser.email)) {
  ...
  const member = store.users.find(
    (u) => String(u.email || '').trim().toLowerCase() === normalizedEmail
  )

  const inviteUrl = invite?.token ? buildInviteUrl(invite.token) : null
  const joinedImmediately = Boolean(member && getMembership(store, member.id, organizationId))
  ...
  if (member?.organizationId === organizationId) {
    ...
    void import('./erpOwnerLeadClaim.js')
      .then((mod) =>
        mod.reassignErpOwnedLeads({ organizationId, onlyUserId: member.id })
      )
      .catch((err) => console.warn('erp owner claim after invite:', err?.message || err))
  }
...
export function applyPendingInvites(store, user) {
  ...
  const invites = (store.organizationInvites || []).filter(
    (i) => String(i.email || '').trim().toLowerCase() === email && i.status === 'pending'
  )
  for (const invite of invites) {
    if (!getMembership(store, user.id, invite.organizationId)) {
      store.organizationMemberships.push({
        id: createId('membership'),
        userId: user.id,
        organizationId: invite.organizationId,
...
export async function updateMemberPermissions(
  ...
  return updateStorePartial(['organizationMemberships'], (store) => {

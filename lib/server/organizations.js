...
export function listPipelineSavedEntries(store, user) {
  ...
  if (accountType === 'individual' || !user.organizationId) {
    return store.savedLeads.filter((e) => e.userId === user.id)
  }
  if (orgRole === 'org_admin') {
    return store.savedLeads.filter((e) => e.organizationId === user.organizationId)
  }
  return store.savedLeads.filter(
    (e) =>
      e.organizationId === user.organizationId &&
...
export function listPipelineEntries(store, user, { light = false } = {}) {
  const entries = listPipelineSavedEntries(store, user)
  const merge = light ? mergeLeadForTenantLight : mergeLeadForTenant

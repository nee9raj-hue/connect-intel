...
async function readEnrollmentMeta(campaignId) {
...
async function writeEnrollmentChunk(campaignId, chunkIndex, entries) {
  const name = enrollmentChunkName(campaignId, chunkIndex)
  const list = (Array.isArray(entries) ? entries : []).map(slimEnrollmentRow)
  await writeStoreCollections({ [name]: list }, [name])
}

async function writeEnrollmentMeta(campaignId, meta) {

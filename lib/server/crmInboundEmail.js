...
import { updatePipelineStore, readPipelineShardEntries, pipelineOrgShardName, pipelineUserShardName } from './pipelineShard.js'
...
export async function ingestCrmInboundEmailEvent(eventData) {
  ...
  await updatePipelineStore(matchedUser, async (draft) => {
    const row = findPipelineEntry(draft, matchedUser, parsed.leadId)
    if (!row) return draft
...
async function ingestCrmInboundBounce({ leadId, fromRaw, subject, emailId }) {
  ...
  await updatePipelineStore(matchedUser, async (draft) => {
    const row = findPipelineEntry(draft, matchedUser, leadId)
    if (!row) return draft
...
async function findLeadEntryForInbound(leadId) {
  ...
  const tryStore = (savedLeads, user) => {
    const store = { ...meta, savedLeads }
    const entry = findPipelineEntry(store, user, target)
    if (entry) return { entry, user, store }
    return null
  }
  ...
  for (const entry of meta.savedLeads || []) {
    ...
    if (!user && orgId) {
      user = (meta.users || []).find((u) => u.organizationId === orgId && u.isOrgAdmin)
    }
    if (user) {
      const hit = tryStore(meta.savedLeads, user)
      if (hit) return hit
    }
  }
  ...
  for (const org of meta.organizations || []) {
    if (org?.id) shardNames.add(pipelineOrgShardName(org.id))
  }
  for (const user of meta.users || []) {
    if (user?.id) shardNames.add(pipelineUserShardName(user.id))
  }

  for (const shardName of shardNames) {
    const entries = (await readPipelineShardEntries(shardName)) || []
    ...
    if (user && entry.organizationId) {
      user = (meta.users || []).find(
        (u) => u.organizationId === entry.organizationId && (u.isOrgAdmin || u.orgRole === 'org_admin')
      )
      if (!user) {
    ...
    if (!user) continue
    const hit = tryStore(entries, user)
    if (hit) return hit
  }

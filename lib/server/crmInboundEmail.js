...
import { formatMailboxFrom } from './gmailOAuth.js'
...
function extractEmailAddressesFromHeaderValue(value) {
  const raw = String(value || '')
  if (!raw.trim()) return []
  const found = raw.match(/[\w.+-]+@[\w.-]+\.\w+/gi) || []
  return [...new Set(found.map((addr) => addr.trim().toLowerCase()))]
}
...
export function extractInboundAddressFromRecipients(recipients) {
  ...
  for (const raw of list) {
    for (const addr of extractEmailAddressesFromHeaderValue(raw)) {
...
function extractInboundAddressFromEmailPayload(eventData, email) {
  const hdrs = headerMap(email?.headers)
  const candidates = [
    ...(Array.isArray(eventData?.to) ? eventData.to : eventData?.to ? [eventData.to] : []),
    ...(Array.isArray(eventData?.cc) ? eventData.cc : eventData?.cc ? [eventData.cc] : []),
    ...(Array.isArray(eventData?.bcc) ? eventData.bcc : eventData?.bcc ? [eventData.bcc] : []),
    hdrs.to,
    hdrs['delivered-to'],
    hdrs['x-original-to'],
    hdrs['envelope-to'],
...
export async function ingestCrmInboundEmailEvent(eventData) {
  ...
  let parsed =
    extractInboundAddressFromRecipients(toList) ||
    extractInboundAddressFromRecipients(eventData?.cc) ||
    extractInboundAddressFromRecipients(eventData?.bcc)

  let received = emailId ? await fetchResendReceivingEmail(emailId) : { ok: false }
  const email = received.ok ? received.email || {} : {}

  if (!parsed) {
    parsed = extractInboundAddressFromEmailPayload(eventData, email)
  }
  if (!parsed) {
  ...
  const resolvedLeadId = entryLeadId(matchedEntry)
  ...
  if (duplicate) {
    return { ok: true, duplicate: true, leadId: resolvedLeadId }
  }
  ...
  await updatePipelineStoreForEntry(matchedEntry, async (draft) => {
    const row = findPipelineEntryRaw(draft, resolvedLeadId)
    if (!row) return draft
...
async function ingestCrmInboundBounce({ leadId, fromRaw, subject, emailId }) {
  ...
  return { ok: true, bounce: true, leadId: resolvedLeadId, emailId }
}
...
function inboundLeadIdMatches(target, row) {
  const lid = entryLeadId(row)
  if (!lid) return false
  const needle = String(target || '')
  if (lid === needle) return true
  const sanitizedRow = sanitizeLeadIdForAddress(lid)
  const sanitizedTarget = sanitizeLeadIdForAddress(needle)
  return sanitizedRow === needle || sanitizedRow === sanitizedTarget || lid === sanitizedTarget
}

function resolveUserForPipelineEntry(meta, entry) {
  const assigneeId = entry.assignedToUserId || entry.savedByUserId || entry.userId
  let user = (meta.users || []).find((u) => u.id === assigneeId)
  ...
  if (!user && entry.organizationId) {
    ...
    if (user) {
      user = { ...user, organizationId: entry.organizationId, accountType: user.accountType || 'company' }
    }
  }
  return user || null
}
...
async function findLeadEntryForInbound(leadId) {
  ...
  for (const org of meta.organizations || []) {
    ...
    const entry = entries.find((row) => inboundLeadIdMatches(target, row))
    if (!entry) continue
  ...
  for (const entry of meta.savedLeads || []) {
    if (!inboundLeadIdMatches(target, entry)) continue
    const user = resolveUserForPipelineEntry(meta, entry)
    if (user) return { entry, user }
  }
...
/**
 * Reply-To header for outbound CRM sends.
 * Shows the rep's work email to the lead; routing address stays on our inbound domain.
 */
export function formatCrmInboundReplyToHeader(leadId, { repEmail, repName } = {}) {
  const routing = buildCrmInboundReplyAddress(leadId)
  if (!routing) return null
  const email = String(repEmail || '')
    .trim()
    .toLowerCase()
  if (!email.includes('@')) return routing
  const name = String(repName || '')
    .trim()
    .replace(/[\r\n<>"]/g, ' ')
    .slice(0, 60)
  const display = name && !name.includes('@') ? `${name} (${email})` : email
  return formatMailboxFrom(display, routing)
}

/** Reply-To for outbound CRM sends — lead replies hit inbound; we forward to rep. */
export function crmOutboundReplyTo(leadId, rep = {}) {
  if (!isCrmInboundEmailEnabled()) return null
  return formatCrmInboundReplyToHeader(leadId, rep)
}

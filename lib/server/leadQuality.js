/** Leads must have reachable email + phone to appear in search results. */
...
export function filterUsableLeads(leads = []) {
  return (leads || []).map(normalizeLeadContact).filter(hasCompleteContact)
}

/** Rows worth saving to master DB even if not shown in search (missing phone or email). */

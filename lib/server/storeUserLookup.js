...
export function userRecordFromProfileRow(profile, organization) {
  ...
  return {
    ...
    pipelineRole: p.pipeline_role || (orgAdmin ? 'org_admin' : 'member'),
...
async function findUserViaProfiles(email) {
  ...
  try {
    const profiles = await supabaseRest(
      `profiles?email=ilike.${encodeURIComponent(email)}&select=legacy_user_id,organization_id,email,full_name,role,pipeline_role,can_search,metadata&limit=1`,
      {},
    ...
    const user = userRecordFromProfileRow(profile, organization)
...
/** Point-lookup one organization JSON row (lead tags, access requests) without the users blob. */
export async function findOrganizationByLegacyId(orgId) {
  ...
  try {
    ...
    return asObject(rows[0]?.org)
  } catch (error) {
    console.warn('findOrganizationByLegacyId:', error?.message || error)
    return null
  }
}

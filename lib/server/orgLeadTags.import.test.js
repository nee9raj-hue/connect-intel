...
test('normalizeLeadTagIds keeps ids after listing stamps missing tag ids', () => {
  const orgId = 'org-1'
  const store = miniStore(orgId, [{ name: 'Hot' }])
  const listed = listOrgLeadTagDefinitions(store, orgId)
  assert.equal(listed.length, 1)
  assert.ok(listed[0].id)
  const kept = normalizeLeadTagIds([listed[0].id], store, orgId)
  assert.deepEqual(kept, [listed[0].id])
})

...
export function applyErpOverlayToLeadEntry(
  ...
  const dumpPerson = canonicalErpOwner(hit.overlay)
  const dumpOwnerUserId = matchErpOwnerUserId(dumpPerson, ownerIndex)
  ...
  if (!ownerAuthoritative) {
    ...
    if (dumpOwnerUserId && String(entry.assignedToUserId || '') === String(dumpOwnerUserId)) {
      entry.assignedToUserId = null
      entry.teamId = null
      entry.departmentId = null
    }
  ...
  if (!ownerAuthoritative) {
    return { matched: true, assigned: false, reclaimed: false, changed: true }
  }

...
function shouldPreferCrmFeed(feedActivities, memberId, off) {
  ...
  return actors.size <= 1 && feedActivities.length >= 10
}
...
export async function readActivityLog(
  ...
  const { rosterMembers: members, memberOptions } = await loadViewerRoster(user, meta, {
    visibleOwnerIds,
  })
  ...
  const shouldTryCrmFallback =
    ...
    ((rollup.activitiesTotal || 0) === 0 || shouldPreferCrmFeed(activities, scopedMemberId, offset))

  if (shouldTryCrmFallback) {
    try {
      const { readActivityLogFromCrmEntries } = await import('./activityLogCrmFallback.js')
      const crm = await readActivityLogFromCrmEntries(user, {
        scopedMemberId,
        leadIds,
        since,
        until,

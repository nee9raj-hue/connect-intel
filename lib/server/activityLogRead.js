...
export async function readActivityLog(
  ...
  if (rollupOnly) {
    return buildResult([], rollup, prevRollup, source, warming, 0)
  }
  ...
  const shouldTryCrmFallback =
    ...
    (feedStale ||
      ...
      (scopedMemberId && !activities.length) ||
...
export async function readActivityLogCached(user, options = {}) {
  const { fresh = false, rollupOnly = false, ...rest } = options
  if (!fresh) {
    const cacheKey = activityLogCacheKey(user, { ...rest, limit: rollupOnly ? 0 : rest.limit })
  ...
  if (!fresh) {
    const cacheKey = activityLogCacheKey(user, {
      ...rest,
      limit: rollupOnly ? 0 : rest.limit,
    })

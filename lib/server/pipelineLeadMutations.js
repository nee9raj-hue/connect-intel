...
export async function persistManualPipelineLead(user, organizationId, fields) {
  ...
  if (createdEntry && organizationId) {
    try {
      const { relinkOrphanDealsToEntry } = await import('./relinkOrphanDeals.js')
      const { entry: linked } = await relinkOrphanDealsToEntry(user, createdEntry)
      if (linked) createdEntry = linked
    } catch (err) {
      console.warn('deal relink on create:', err?.message || err)
    }
  }
...
export async function persistMasterContactUpdate(user, contactId, patch = {}) {
  ...
  const tableActive = pipelineLeadsTableActive()
  ...
  const store = await withStoreLock(async () => {
    ...
    // When the pipeline_leads table is the source of truth, the JSON shard is a
    // mirror that can lag behind. Pull the canonical entries that reference this
    // contact from the table so the access check and snapshot sync operate on the
    // same rows the pipeline list renders — otherwise the edit never lands.
    let seedEntries = shardEntries
    if (tableActive) {
      const tableEntries = (await readPipelineLeadsByIds(shardName, [id])) || []
      if (tableEntries.length) {
        const byLeadId = new Map(
          shardEntries.map((entry) => [String(entry.lead?.id || entry.id), entry])
        )
        for (const entry of tableEntries) {
          byLeadId.set(String(entry.lead?.id || entry.id), entry)
        }
        seedEntries = [...byLeadId.values()]
      }
    }

    const draft = attachPipelineEntriesToStore(masterStore, seedEntries)
...
/**
 * Merge duplicate master contacts and persist pipeline + store updates.
 */
export async function persistMasterContactMerge(user, primaryContactId, mergeContactIds = []) {
  ...
  const { mergeMasterContacts } = await import('./contactDedup.js')
  const { pipelineLeadsTableActive, readPipelineLeadsByIds } = await import('./pipelineLeadsTable.js')
  const tableActive = pipelineLeadsTableActive()
  ...
  const store = await withStoreLock(async () => {
    const shardName = pipelineShardNameForUser(user)
    const masterStore = await readStore({ only: MASTER_COLLECTIONS })
    let shardEntries = (await readPipelineShardEntries(shardName, { bypassCache: true })) || []

    const allIds = [primaryId, ...mergeIds]
    if (tableActive) {
      const tableEntries = (await readPipelineLeadsByIds(shardName, allIds)) || []
      if (tableEntries.length) {
        const byLeadId = new Map(
          shardEntries.map((entry) => [String(entry.lead?.id || entry.id), entry])
        )
        ...
        shardEntries = [...byLeadId.values()]
      }
    }

    const draft = attachPipelineEntriesToStore(masterStore, shardEntries)
    result = mergeMasterContacts(draft, user, primaryId, mergeIds)

    const touchedIds = new Set(allIds)
    const updatedEntries = (draft.savedLeads || []).filter((entry) =>
      touchedIds.has(entry.contactId) || touchedIds.has(entry.lead?.id)
    )
    for (const entry of updatedEntries) {
      touchPipelineEntry(entry)
    }

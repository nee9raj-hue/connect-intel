...
import { PIPELINE_COLLABORATOR_IDS_BOOTSTRAP_SQL } from './migrations/pipelineCollaboratorIdsBootstrap.js'
...
export async function resolvePgConnectionString() {
  ...
  if (password) {
    ...
    const hosts = poolerProbeHosts(region)
    // Prefer hosts that answered (not tenant-not-found) — aws-1 often correct when aws-0 fails.
    const ordered = [...hosts].sort((a, b) => {
      const score = (h) => (h.includes('aws-1-') ? 0 : h.includes('aws-0-') ? 1 : 2)
...
export async function applyPipelineCollaboratorIdsBootstrap() {
  return applySupabaseSql(PIPELINE_COLLABORATOR_IDS_BOOTSTRAP_SQL, {
    file: 'pipelineCollaboratorIdsBootstrap',
  })
}
...
export async function applySupabaseSql(sql, { file = null } = {}) {
  ...
  return {
    ...
    hint:
      'Set SUPABASE_DB_PASSWORD to your Supabase database password (Settings → Database → reset if unsure). ' +
      'Set SUPABASE_DB_HOST to your session pooler host (often aws-1-REGION.pooler.supabase.com, not aws-0). ' +
      'Or set SUPABASE_ACCESS_TOKEN for Management API migrate.',
  }
}

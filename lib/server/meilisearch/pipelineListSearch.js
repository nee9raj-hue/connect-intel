...
export async function searchVisiblePipelineLeadIds(user, metaStore, q, { limit = 500, assigneeUserId = null } = {}) {
  ...
  let hits
  try {
    hits = await meiliSearch(MEILI_CRM_INDEX, {
      q: query,
      limit: fetchLimit,
      filter: filterParts.join(' AND '),
    })
  } catch (err) {
    console.warn('pipeline meili search:', err?.message || err)
    return null
  }
  // Circuit open / timeout / disabled mid-call must fall through to SQL — not “zero hits”.

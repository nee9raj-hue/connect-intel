...
async function filterMeiliHitsForUser(user, metaStore, hits, limit) {
  ...
  for (const hit of hits || []) {
    ...
    if (doc.type === 'lead' || doc.type === 'deal') {
      ...
      const visible = await isPipelineEntryVisibleAsync(user, meiliDocToEntryStub(doc), metaStore)
      if (!visible) continue
    } else if (user.organizationId && doc.organizationId && doc.organizationId !== user.organizationId) {
      continue
    } else if (!user.organizationId && doc.organizationId) {
      continue
    }

    results.push(hitToResult(doc))

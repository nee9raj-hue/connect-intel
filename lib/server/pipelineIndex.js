...
export function buildPipelineIndexDoc(entries, { freightOrg = false, organizationId = null } = {}) {
  ...
  const doc = {
    ...
    total: summary.total,
    unassignedTotal: list.filter((entry) => isPipelineLeadUnassigned(entry)).length,
...
export function applyPipelineSummaryForUser(doc, user, store) {
  ...
  if (!isOrgMember) {
    const unassignedTotal = doc.unassignedTotal ?? 0
    return {
      total: Math.max(0, (doc.total || 0) - unassignedTotal),
      unassignedTotal,
...
export async function attachPipelineIndexLocationFacets(summary, user, store) {
  ...
  if (summary.cities?.length && summary.states?.length) return summary
...
export async function loadPipelineSummaryFast(user, store, options = {}) {
  ...
  if (!summary) {
    ...
    const doc = await readPipelineIndexDoc(shardName)
    if (!doc) return null
    summary = applyPipelineSummaryForUser(doc, user, store)
    if (summary && doc.unassignedTotal != null && summary.unassignedTotal == null) {
      summary = { ...summary, unassignedTotal: doc.unassignedTotal }
    }
  }

  if (summary && summary.unassignedTotal == null && isPipelineHierarchyRbacEnabled()) {
    const unassignedTotal = await countScopedPipelineLeads(user, store, {
      assigneeUserId: '__unassigned__',
    })
    if (unassignedTotal != null) {
      summary = { ...summary, unassignedTotal }
    }
  }

  return attachPipelineIndexLocationFacets(summary, user, store)
}

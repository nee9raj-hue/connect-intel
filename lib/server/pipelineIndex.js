...
import { isSupabaseEnabled, supabaseRest, upsertCollection } from './supabaseClient.js'
  ...
  if (isSupabaseEnabled()) {
    try {
      const rows = await supabaseRest(
        ...
        {},
        { attempts: 1, timeoutMs: 4_000, bypassCircuit: true }
      )
      doc = rows?.[0]?.json?.[0] && typeof rows[0].json[0] === 'object' ? rows[0].json[0] : null
      if (!doc && rows?.[0]?.json && typeof rows[0].json === 'object' && !Array.isArray(rows[0].json)) {
        doc = rows[0].json
      }
    } catch (error) {
      console.warn('pipeline index skip:', error?.message || error)
      return null
    }
  } else {
...
export async function loadPipelineSummaryFast(user, store, options = {}) {
  ...
  if (isCompany && summary) {
    return attachSqlDealCountsToSummary(summary, user, store, options)
  }
...
async function attachSqlDealCountsToSummary(summary, user, store, options = {}) {
  if (!summary || options.skipDealCounts) return summary

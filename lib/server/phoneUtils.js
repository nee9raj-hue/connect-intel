...
/** Empty is allowed (login / signup). Non-empty values must still be a valid number. */
export function validateOptionalMobileInput(input) {
  if (!String(input || '').trim()) {
    return { ok: true, mobileE164: null, display: null }
  }
  return validateMobileInput(input)
}

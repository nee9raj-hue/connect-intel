...
describe('leadErp', () => {
  ...
  it('loosely matches company keys with extra legal suffixes already stripped', () => {
    const a = normalizeCompanyMatchKey('Shree Enterprises JPR')
    const b = normalizeCompanyMatchKey('Shree Enterprises')
    assert.equal(companyKeysLooselyMatch(a, b), true)
    assert.equal(companyKeysLooselyMatch('ab', 'abcde'), false)
    assert.equal(companyKeysLooselyMatch('xlp', normalizeCompanyMatchKey('XLP Logistics Pvt Ltd')), false)
    assert.equal(companyKeysLooselyMatch('xlp engineers', normalizeCompanyMatchKey('XLP ENGINEERS PVT LTD')), true)
    assert.equal(
      companyKeysLooselyMatch(
        normalizeCompanyMatchKey('Express'),
        normalizeCompanyMatchKey('NAGINA EXPRESS')
      ),
      false
    )
  })

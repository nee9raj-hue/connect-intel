...
test('combined board columns keep CRM and ERP leads in their own stages', () => {
  const cols = freightBoardColumnIds()
  assert.deepEqual(cols.slice(0, 4), ['fresh', 'qualified', 'unqualified', 'account_created_erp'])
  assert.ok(cols.includes('erp_active'))
  assert.equal(boardBucketForLeadStatus('fresh', cols), 'fresh')
  assert.equal(boardBucketForLeadStatus('active_trading', cols), 'erp_active')
  assert.equal(boardBucketForLeadStatus('new_account', cols), 'erp_early')
  assert.equal(boardBucketForLeadStatus('contacted', cols), 'qualified')
  assert.deepEqual(freightBoardColumnIds({ pipelineTrack: 'crm' }), [
    'fresh',
    'qualified',
    'unqualified',
    'account_created_erp',
  ])
  assert.ok(!freightBoardColumnIds({ pipelineTrack: 'erp' }).includes('fresh'))
})

test('ERP stages map to display tags', () => {
  assert.equal(erpStageTagFromLead({ crm: { status: 'new_account', tagIds: ['crm-1'] } }).name, 'Early')
  assert.equal(erpStageTagFromLead({ crm: { status: 'active_trading' } }).name, 'Active')
  assert.equal(erpStageTagFromLead({ crm: { status: 'churned' } }).name, 'Churn')
  assert.equal(erpStageTagFromLead({ crm: { status: 'at_risk' } }).name, 'Dormant')
  assert.equal(erpStageTagFromLead({ crm: { status: 'unqualified' } }), null)
  assert.equal(erpStageTagFromLead({ crm: { status: 'fresh' } }), null)
  assert.equal(erpStageTagFromLead({ crm: { status: 'follow_up' } }), null)
})

...
export function consumeTeamScopedLeadTags(tagIds, tags = [], memberTeamIds = [], { isOrgAdmin = false } = {}) {
  const remaining = []
  const teamIds = []
  const byId = new Map((tags || []).map((tag) => [String(tag.id), tag]))
  const allowedTeams = new Set((memberTeamIds || []).map(String).filter(Boolean))
  for (const id of tagIds || []) {
    const tag = byId.get(String(id))
    const teamId = String(tag?.teamId || '').trim()
    const isTeamTag = Boolean(teamId) || String(tag?.source || '') === 'org_team'
    if (
      isTeamTag &&
      teamId &&
      (isOrgAdmin || allowedTeams.size === 0 || allowedTeams.has(teamId))
    ) {
      teamIds.push(teamId)
      continue
    }

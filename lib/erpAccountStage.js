...
export function suggestCrmStatusFromErp(entry, now = Date.now()) {
  ...
  if (!traded && lastMs == null && firstMs == null) return null
  ...
  if (effectiveLast == null) {
    if (Number.isFinite(shipmentCount) && shipmentCount > 0) return 'active_trading'
    return null
  }
...
export function applyErpAccountStage(entry, now = Date.now()) {
  ...
  if (!suggested || MANUAL_CRM_ONLY_STATUSES.has(suggested)) return entry

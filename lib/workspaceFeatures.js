...
import { TEAM_INTELLIGENCE_IN_CRM_ENABLED } from './crmUiFlags.js'
...
export const WORKSPACE_PRESETS = {
  general_crm: {
    ...
    features: {
      ...
      homeTeamMetrics: false,
      activeTradingImport: false,
      fieldVisitExpenses: false,
    },
  },
  logistics_trading: {
...
export function listWorkspaceFeatureDefinitions() {
  const all = [
    ...
    {
      key: 'fieldVisitExpenses',
      label: 'Field visit & travel claims',
      description:
        'Log field visits with travel mode and km in the pipeline, plus a Field expenses page for monthly totals and CSV export.',
      group: 'CRM',
    },
    {
      key: 'freightDealRfq',
      label: 'Freight RFQ on deals',
      description:
        'Shipping RFQ fields on deals — pickup/delivery ZIP, weight, box dimensions, cargo readiness. Enabled automatically for Xindus.',
      group: 'CRM',
    },
  ]
  ...
  if (!TEAM_INTELLIGENCE_IN_CRM_ENABLED) {
    return all.filter(
      (def) => def.key !== 'homeTeamMetrics' && def.key !== 'dashboardTeamIntelligence'
    )
  }
  return all
}
...
export function resolveWorkspaceFeatures(org) {
  ...
  if (!TEAM_INTELLIGENCE_IN_CRM_ENABLED) {
    features.homeTeamMetrics = false
    features.dashboardTeamIntelligence = false
  }
  return {
    presetId,

...
- **Sidebar pipeline stage counts** are org-wide totals from SQL (`pipeline_leads` status counts), not filtered list views. After any CRM status change, counts must update immediately (optimistic shift + fresh summary fetch) and server bootstrap cache must invalidate — never show filtered table counts in the sidebar.

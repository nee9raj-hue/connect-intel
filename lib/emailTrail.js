...
export function isTrailGmailMessage(msg, ctx) {
  ...
  if (!ctx.hasCrmOutbound) {
    if (fromEmails.includes(userEmail) && toEmails.includes(leadEmail)) return true
    // Inbound reply before CRM has logged the original outbound (e.g. sent from Gmail directly).
    if (fromEmails.includes(leadEmail) && toEmails.includes(userEmail)) return true
    return false
  }
...
export function isTrailCrmEmail(record, ctx) {
  ...
  if (record.resendInboundId) return true
  if (!record.gmailMessageId) return true
  const provider = String(record.provider || '')
  if (provider === 'inbound_sync' || provider === 'manual' || provider === 'gmail_bounce') return true
  if (record.direction === 'outbound' && provider !== 'gmail_sync') return true
  // gmail_sync and legacy inbox imports with a Gmail id must pass trail validation

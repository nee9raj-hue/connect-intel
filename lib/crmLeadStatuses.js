...
export const CRM_LEAD_STATUS_DEFS = [
  ...
  },
  {
    ...
    label: 'Churned',
    color: 'stone',
    hint: 'No shipments booked in 90+ days.',
  },
  {
    id: 'lost',
    label: 'Lost',
...
export function normalizeCrmLeadStatus(status) {
  ...
  if (CRM_STATUS_IDS.includes(raw)) return raw
...
export function isCrmLeadStatusFilter(status) {
  ...
  return CRM_STATUS_IDS.includes(raw) || Boolean(LEGACY_CRM_STATUS_MAP[raw])
}
...
export function leadStatusToStandardDealStage(status) {
  ...
  if (id === 'onboarding' || id === 'active_trading') return 'won'
...
export function suggestCrmStatusFromTradingProfile(profile, currentStatus, now = Date.now()) {
  ...
  if (previous >= 2 && recent < previous * 0.7) {
    if (['active_trading', 'at_risk', 'onboarding', 'churned'].includes(canonical)) return 'at_risk'
  }

  const count = Number(profile.shipmentCount) || times.length
  if (count >= 1 && last && now - last < 90 * MS_DAY) {
    if (['onboarding', 'at_risk', 'churned', 'active_trading'].includes(canonical)) return 'active_trading'
  }

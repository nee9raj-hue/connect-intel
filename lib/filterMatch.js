...
function tokenMatchesHaystack(token, haystack) {
  ...
  // export / exporter / exports — common B2B India searches
  if (/^export/i.test(token) || /^export/i.test(stem)) {
    if (/\bexport/.test(haystack)) return true
  }

  // textile / textiles
  if (/^textile/i.test(token) || /^textile/i.test(stem)) {
    if (/\btextile/.test(haystack)) return true
  }
